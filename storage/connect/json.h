...
class JSON : public BLOCK {
 public:
  ...
  virtual void   InitArray(PGLOBAL) {X}
//virtual PJVAL  AddValue(PGLOBAL g, PJVAL jvp = NULL, int *x = NULL) {X return NULL;}
  virtual PJPR   AddPair(PGLOBAL, PSZ) {X return NULL;}
	virtual PJAR   GetKeyList(PGLOBAL) {X return NULL;}
	virtual PJVAL  GetValue(const char *) {X return NULL;}
  virtual PJOB   GetObject(void) {return NULL;}
  virtual PJAR   GetArray(void) {return NULL;}
	virtual PJVAL  GetValue(int) {X return NULL;}
  ...
  virtual PSZ    GetText(PGLOBAL, PSZ) {X return NULL;}
	virtual bool   Merge(PGLOBAL, PJSON) { X return true; }
	virtual bool   SetValue(PGLOBAL, PJVAL, int) { X return true; }
  virtual void   SetValue(PGLOBAL, PJVAL, PSZ) {X}
  virtual void   SetValue(PVAL) {X}
  virtual void   SetValue(PJSON) {X}
  virtual void   SetString(PGLOBAL, PSZ, short) {X}
  virtual void   SetInteger(PGLOBAL, int) {X}
  virtual void   SetFloat(PGLOBAL, double) {X}
	virtual void   DeleteKey(char *) {X}
	virtual bool   DeleteValue(int) {X return true;}
