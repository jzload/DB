  ...
  virtual bool   FormatValue(PVAL vp, char *fmt) = 0;
	...
	*/
	...
	void SetValueNonAligned(const char *p)
	{
		...
		SetValue(*(reinterpret_cast<TYPE*>((const_cast<char*>(p))))); // x86 can cast non-aligned memory directly
#else
...
//virtual PSZ    GetCharValue(void) {return VALUE::GetCharValue();}
  ...
  virtual bool   GetBinValue(void *buf, int buflen MY_ATTRIBUTE((unused)), bool go);
  ...
  virtual void   Print(PGLOBAL g MY_ATTRIBUTE((unused)), FILE *, uint);
  virtual void   Print(PGLOBAL g MY_ATTRIBUTE((unused)), char *, uint);
