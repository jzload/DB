...
/***********************************************************************/
static void global_open_error_msg(GLOBAL_CONNECT *g, int msgid, const char *path, 
                                                        const char *mode)
{
...
FILE *global_fopen(GLOBAL_CONNECT *g, int msgid, const char *path, const char *mode)
{
  FILE *f;
...
int global_open(GLOBAL_CONNECT *g, int msgid, const char *path, int flags)
{
  int h;
...
int global_open(GLOBAL_CONNECT *g, int msgid, const char *path, int flags, int mode)
{
  int h;
...
bool EvalLikePattern(LPCSTR sp, LPCSTR tp)
  ...
  if ((p = const_cast<LPSTR>(strpbrk(tp, "%_")))) /* Get position of next % or _   */
    n = p - tp;
  else
...
//  b = mp.Sub;
  ...
  if (trace > 1)
    htrc("PlgDBalloc: in %p size=%d used=%d free=%d sub=%d\n",
          arp, static_cast<int>(mp.Size), pph->To_Free, pph->FreeBlk, mp.Sub);
...
//  assert (mp.Memp != NULL);
  ...
  if (trace > 1)
    htrc("PlgDBrealloc: %p size=%d sub=%d\n", mp.Memp, static_cast<int>(mp.Size), mp.Sub);
  ...
  if (trace)
    htrc(" newsize=%d newp=%p sub=%d\n", static_cast<int>(mp.Size), mp.Memp, mp.Sub);
...
void PlgDBfree(MBLOCK& mp)
  {
  if (trace > 1)
    htrc("PlgDBfree: %p sub=%d size=%d\n", mp.Memp, mp.Sub, static_cast<int>(mp.Size));
...
//size = ((size + 3) / 4) * 4;       /* Round up size to multiple of 4 */
  ...
  if (trace > 1)
    htrc("PlgDBSubAlloc: memp=%p size=%d used=%d free=%d\n",
         memp, static_cast<int>(size), pph->To_Free, pph->FreeBlk);
