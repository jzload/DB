...
class DllExport TDBVIR : public TDBASE {
 public:
  ...
  virtual bool SetRecpos(PGLOBAL, int recpos)
               {N = recpos - 2; return false;}
	virtual int  RowNumber(PGLOBAL, bool MY_ATTRIBUTE((unused)) b = false) {return N + 1;}
          int  TestFilter(PFIL filp, bool nop);

  // Database routines
	virtual PCOL MakeCol(PGLOBAL g, PCOLDEF cdp, PCOL cprec, int n);
  virtual int  Cardinality(PGLOBAL g) {return (g) ? Size : 1;}
  virtual int  GetMaxSize(PGLOBAL) {return Size;}
  virtual bool OpenDB(PGLOBAL g);
  virtual int  ReadDB(PGLOBAL g);
  virtual int  WriteDB(PGLOBAL g);
  virtual int  DeleteDB(PGLOBAL g, int irc);
	virtual void CloseDB(PGLOBAL) {}
...
class TDBVICL : public TDBCAT {
 public:
  ...
  // Methods
  virtual int  Cardinality(PGLOBAL) {return 2;} // Avoid DBUG_ASSERT
