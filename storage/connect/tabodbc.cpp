...
/***********************************************************************/
PTDB ODBCDEF::GetTable(PGLOBAL g, MODE m MY_ATTRIBUTE((unused)))
  {
  PTDB tdbp = NULL;
...
bool TDBODBC::MakeInsert(PGLOBAL g)
  ...
  for (colp = Columns; colp; colp = colp->GetNext())
    if (colp->IsSpecial()) {
      ...
      if(zsql_is_reserved_field(colp->GetName()))
        continue;
			// Column name can be encoded in UTF-8
			Decode(colp->GetName(), buf, sizeof(buf));
...
int TDBODBC::GetCount(PGLOBAL g) {

  if (!g) return 0;

  int result;
  char   tbn[FN_REFLEN];
  std::string query_str("SELECT COUNT(*) FROM ");
  ...
  if (Quote) {
    query_str.append(Quote).append(tbn).append(Quote);
  }
  else {
    query_str.append(tbn);
  }

  // Allocate a Count(*) column (must not use the default constructor)
  Cnp = new(g) ODBCCOL;
  Cnp->InitValue(g);
  result = ocp->GetResultSize(const_cast<char*>(query_str.c_str()), Cnp);
...
/***********************************************************************/
int TDBODBC::WriteDB(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
...
/***********************************************************************/
void TDBODBC::CloseDB(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
//if (To_Kindex) {
//  To_Kindex->Close();
//  To_Kindex = NULL;
...
/***********************************************************************/
void ODBCCOL::ReadColumn(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
...
/***********************************************************************/
void ODBCCOL::WriteColumn(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
  /*********************************************************************/
...
/***********************************************************************/
int TDBXDBC::GetMaxSize(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
  if (MaxSize < 0)
...
/*  Data Base delete line routine for ODBC access method.              */
/***********************************************************************/
int TDBXDBC::DeleteDB(PGLOBAL g, int irc MY_ATTRIBUTE((unused)))
  {
...
/***********************************************************************/
void XSRCCOL::WriteColumn(PGLOBAL g MY_ATTRIBUTE((unused)))
  {
