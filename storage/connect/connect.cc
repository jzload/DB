...
RCODE CntIndexRead(PGLOBAL g, PTDB ptdb, OPVAL op,
  ...
  if (!x) {
    ...
    if (kr)
      ((PTDBASE)ptdb)->SetRecpos(g, *reinterpret_cast<int*>(const_cast<uchar*>(kr->key)));
...
//  if (!tdbp->To_Xdp) {
  ...
  if (kr) {
		char   *kp= reinterpret_cast<char*>(const_cast<uchar*>(kr->key));
		int     len= kr->length;
		short   lg;
		bool    rcb;
		PVAL    valp;
    ...
    for (n= 0; n < tdbp->Knum; n++) {
      ...
      if (len == kp - reinterpret_cast<char*>(const_cast<uchar*>(kr->key))) {
        n++;
        break;
      } else if (len < kp - reinterpret_cast<char*>(const_cast<uchar*>(kr->key))) {
...
int CntIndexRange(PGLOBAL g, PTDB ptdb, const uchar* *key, uint *len,
  ...
  if (!x) {
    ...
    for (i= 0; i < 2; i++)
      if (key[i])
        k[i] = *reinterpret_cast<int*>(const_cast<uchar*>(key[i])) + (incl[i] ? 0 : 1 - 2 * i);
      else
        ...
        if (kmap[i] & (key_part_map)(1 << n)) {
          ...
          if (!valp->IsTypeNum()) {
            if (colp->GetColUse(U_VAR)) {
              ...
              ((char*)&lg)[1]= ((char*)p)[0];
#else   // !WORDS_BIGENDIAN
              lg= *reinterpret_cast<short*>(const_cast<uchar*>(p));
#endif   //!WORDS_BIGENDIAN
              p+= sizeof(short);
              rcb= valp->SetValue_char(reinterpret_cast<char*>(const_cast<uchar*>(p)), (int)lg);
            } else
              rcb= valp->SetValue_char(reinterpret_cast<char*>(const_cast<uchar*>(p)), valp->GetClen());
...
#else   // !WORDS_BIGENDIAN
            valp->SetBinValue(reinterpret_cast<void*>(const_cast<uchar*>(p)));
#endif  // !WORDS_BIGENDIAN
