  ...
  for (field = 0; field < fields; field++) {
    ...
    if (trace)
      htrc("%-11s %c  %6ld  %3d   %2d  %3d  %3d\n",
           thisfield.Name, thisfield.Type, static_cast<long>(thisfield.Offset), len,
...
/****************************************************************************/
int DBFBASE::ScanHeader(PGLOBAL g, PSZ fn, int lrecl MY_ATTRIBUTE((unused)),
                        int *rln, char *defpath)
  {
  int       rc;
...
/*  Deleted lines are just flagged in the first buffer character.           */
/****************************************************************************/
int DBMFAM::DeleteRecords(PGLOBAL g MY_ATTRIBUTE((unused)), int irc)
  {
  if (irc == RC_OK)
    *Fpos = '*';
