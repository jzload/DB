...
/***********************************************************************/
bool EXTDEF::DefineAM(PGLOBAL g, LPCSTR, int)
{
	Desc = NULL;
...
bool TDBEXT::MakeCommand(PGLOBAL g)
{
	...
	do {
		qrystr[i] = (Qrystr[i] == '`') ? ' ' : tolower(Qrystr[i]);
	} while (Qrystr[i++]);
	...
	if ((p = strstr(qrystr, name))&&
		(Mode == MODE_DELETE|Mode == MODE_UPDATE)) 
	{

	         if(Mode == MODE_DELETE)
		    sprintf(stmt, "DELETE FROM %s %s ", TableName, p+strlen(name));
		else if (Mode == MODE_UPDATE)
		    sprintf(stmt, "UPDATE %s %s ", TableName, p+strlen(name));
/*
		for (i = 0; i < p - qrystr; i++)
			stmt[i] = (Qrystr[i] == '`') ? ' ' : Qrystr[i];

		stmt[i] = 0;
		k += i + (int)strlen(Name);

		if (qtd && *(p - 1) == ' ')
			strcat(strcat(strcat(stmt, "\""), TableName), "\"");
		else
