...
void COLUMN::Print(PGLOBAL g, FILE *f, uint n)
  ...
  if (Name)
    fprintf(f, "%sCOLUMN: %s.%s\n", m,
      ((!Qualifier) ? const_cast<PSZ>("?") : Qualifier), Name);
  else         // LNA
    fprintf(f, "%sC%d\n", m, (!Qualifier) ? 0 : *(int *)(const_cast<char*>(Qualifier)));
...
void COLUMN::Print(PGLOBAL, char *ps, uint z)
  ...
  if (Name)
    sprintf(buf, "COLUMN: %s.%s table=%p col=%p",
      ((!Qualifier) ? const_cast<PSZ>("?") : Qualifier), Name, To_Table, To_Col);
  else         // LNA
    sprintf(buf, "C%d", (!Qualifier) ? 0 : *(int *)(const_cast<char*>(Qualifier)));
