...
// Due to this macro is removed from 5.7 to 8.0. So we define it.
#ifndef  WITH_PARTITION_STORAGE_ENGINE
#define WITH_PARTITION_STORAGE_ENGINE
#endif
...
static handler *connect_create_handler(handlerton *hton,
                                       TABLE_SHARE *table,
                                       bool partitioned MY_ATTRIBUTE((unused)),
                                       MEM_ROOT *mem_root);
...
/***********************************************************************/
/* zsql: Compare the input field_name is the reserved field for GDB.   */
/* zsql: The reserved field name is "GTID" and "GDB_BID".              */
/***********************************************************************/
bool zsql_is_reserved_field(const char* field_name)
{
  if(0==strcmp(field_name, "GTID") ||
     0==strcmp(field_name, "GDB_BID"))
    return true;
  return false;
}
...
bool ExactInfo(void) {return /*THDVAR(current_thd, exact_info)*/true;}
USETEMP UseTemp(void) {return (USETEMP)/*THDVAR(current_thd, use_tempfile)*/TMP_AUTO;}
int GetConvSize(void) {return /*THDVAR(current_thd, conv_size)*/SZCONV;}
TYPCONV GetTypeConv(void) {return (TYPCONV)/*THDVAR(current_thd, type_conv)*/TPC_NO;}
uint GetJsonGrpSize(void)
  {return /*connect_hton ? THDVAR(current_thd, json_grp_size) : */10;}
uint GetWorkSize(void) {return /*THDVAR(current_thd, work_size)*/SZWORK;}
void SetWorkSize(uint) 
{
  // Changing the session variable value seems to be impossible here
  // and should be done in a check function 
  push_warning(current_thd, Sql_condition::SL_WARNING, 1, 
...
static int check_msg_path (MYSQL_THD thd, SYS_VAR *var,
...
void PushWarning(PGLOBAL g, THD *thd, int level)
  {
  if (thd) {
    Sql_condition::enum_severity_level wlvl;

    wlvl= (Sql_condition::enum_severity_level)level;
    push_warning(thd, wlvl, 1, g->Message);
  } else
    htrc("%s\n", g->Message);
...
{
  { &con_key_mutex_CONNECT_SHARE_mutex, "CONNECT_SHARE::mutex", 
    0, 0, PSI_DOCUMENT_ME}
};
...
static void init_connect_psi_keys()
{
  ...
  if (psi_mutex_service == NULL)
    return;

  count= array_elements(all_connect_mutexes);
  mysql_mutex_register(category, all_connect_mutexes, count);
#endif
}
#else
static void init_connect_psi_keys() {}
                                   ...
                                   MEM_ROOT *mem_root)
{
  ...
  if (trace)
    htrc("New CONNECT %p, table: %.*s\n", h,
          static_cast<int>(table ? table->table_name.length : 6),
...
ha_connect::~ha_connect(void)
{
  if (trace)
    htrc("Delete CONNECT %p, table: %.*s, xp=%p count=%d\n", this,
                         static_cast<int>(table ? table->s->table_name.length : 6),
 ...
 */
const char *ha_connect::index_type(uint) 
{ 
#if 0
...
ulong ha_connect::index_flags(uint, uint, bool) const
{
  ...
  ha_connect *hp= const_cast<ha_connect*>(this);
...
{
  ulonglong   flags= HA_CAN_VIRTUAL_COLUMNS | 
//                   HA_REC_NOT_IN_SEQ | without in 8.0
                               ...
                               const char *oplist, const char *def)
{
  ...
  char  key[16], val[256];
  char *pk, *pv, *pn;
  char *opval= const_cast<char*>(def);
  int   n;

  for (pk= const_cast<char*>(oplist); pk; pk= ++pn) {
    pn= strchr(pk, ',');
    pv= strchr(pk, '=');
...
char *GetStringTableOption(PGLOBAL g, PTOS options, char *opname, char *sdef)
{
  ...
  return opval ? const_cast<char*>(opval) : sdef;
...
int GetIntegerTableOption(PGLOBAL g, PTOS options, char *opname, int idef)
{
  ...
  if ((ulonglong) opval == (ulonglong)NO_IVAL) {
    ...
    if ((pv= GetListOption(g, opname, options->oplist)))
      opval= CharToNumber(pv, strlen(pv), INT_MAX64, true);
    else
      return idef;
...
char *ha_connect::GetStringOption(char *opname, char *sdef)
{
  ...
  if (!stricmp(opname, "Connect")) {
    ...
    if (chif)
      opval= const_cast<char*>(chif->csname);
  ...
  if (!opval) {
    if (sdef && !strcmp(sdef, "*")) {
      ...
      if (!stricmp(opname, "Dbname") || !stricmp(opname, "Database"))
        opval= const_cast<char*>(GetDBName(NULL));    // Current database
      else if (!stricmp(opname, "Type"))  // Default type
        opval= (!options) ? NULL :
               (options->srcdef)  ? const_cast<char*>("MYSQL") :
               (options->tabname) ? const_cast<char*>("PROXY") : const_cast<char*>("DOS");
      else if (!stricmp(opname, "User"))  // Connected user
        opval= const_cast<char*>("root");
      else if (!stricmp(opname, "Host"))  // Connected user host
        opval= const_cast<char*>("localhost");
      else
...
void *ha_connect::GetColumnOption(PGLOBAL g, void *field, PCOLINFO pcf)
{
  ...
  pcf->Name= const_cast<char*>(fp->field_name);
  ...
  if (fop) {
    ...
    pcf->Datefmt= const_cast<char*>(fop->dateformat);
    pcf->Fieldfmt= const_cast<char*>(fop->fieldformat);
  } else {
    pcf->Offset= -1;
    pcf->Freq= 0;
    pcf->Datefmt= NULL;
  ...
  chset = const_cast<char*>(fp->charset()->name);
  ...
  {
    LEX_STRING comment_str = to_lex_string(fp->comment);
    pcf->Remark= strz(g, comment_str);
  }
  else
    pcf->Remark= NULL;
...
bool ha_connect::GetIndexOption(KEY *kp, char *opname)
{
  ...
  if (options) {
    ...
    LEX_STRING comment_str = to_lex_string(kp->comment);
    char *pv, *oplist= strz(xp->g, comment_str);
...
PIXDEF ha_connect::GetIndexInfo(TABLE_SHARE *s)
{
  ...
  for (int n= 0; (unsigned)n < s->keynames.count; n++) {
    ...
    pn= const_cast<char*>(s->keynames.type_names[n]);
    name= PlugDup(g, pn);
    unique= (kp.flags & 1) != 0;
    pkp= NULL;
    ...
    for (int k= 0; (unsigned)k < kp.user_defined_key_parts; k++) {
      pn= const_cast<char*>(kp.key_part[k].field->field_name);
      name= PlugDup(g, pn);
...
{
  return (IsPartitioned()) ? partname : const_cast<char*>(GetTableName());
} // end of GetTableName
...
int ha_connect::OpenTable(PGLOBAL g, bool del)
{
  ...
  if (xmod != MODE_INSERT || tdbp->GetAmType() == TYPE_AM_MYSQL
    ...
    for (field= table->field; fp= *field; field++) {
      if(zsql_is_reserved_field(fp->field_name))
          continue;
    ...
    if (k1) {
      ...
      for (field= table->field; fp= *field; field++){
        if(zsql_is_reserved_field(fp->field_name))
          continue;
        if (bitmap_is_set(map, fp->field_index)) {
          strcpy(p, const_cast<char*>(fp->field_name));
          p+= (strlen(p) + 1);
          } // endif used field
      }
...
bool ha_connect::CheckColumnList(PGLOBAL g)
{
  ...
  if ((rc= setjmp(g->jumper[++g->jump_level])) == 0) {
    for (field= table->field; fp= *field; field++)
      if (bitmap_is_set(map, fp->field_index)) {
        if (!(colp= tdbp->ColDB(g, const_cast<PSZ>(fp->field_name), 0))) {
  ...
  for (field= table->field; *field && !rc; field++) {
    ...
    if (bitmap_is_set(map, fp->field_index) || alter) {
      ...
      for (colp= tdbp->GetColumns(); colp; colp= colp->GetNext())
        if ((!mrr || colp->GetKcol()) &&
            !stricmp(colp->GetName(), const_cast<char*>(fp->field_name)))
          break;
      ...
      if (!value->IsNull()) {
        ...
        if (rc > 0) {
          ...
          push_warning(thd, Sql_condition::SL_WARNING, 1, buf);
...
/***********************************************************************/
bool ha_connect::MakeKeyWhere(PGLOBAL g, PSTRG qry, OPVAL, char q,
	                            const key_range *kr)
{
	const uchar     *ptr;
...
//	klen= len= ranges[i]->length;
		...
		for (kpart= kfp->key_part; rem; rem--, kpart++) {
			...
			if (q) {
				oom|= qry->Append(q);
				oom|= qry->Append(const_cast<PSZ>(fp->field_name));
				oom|= qry->Append(q);
			}	else
				oom|= qry->Append(const_cast<PSZ>(fp->field_name));
...
//				? OP_EQ : OP_LIKE;
			...
			oom|= qry->Append(const_cast<PSZ>(GetValStr(op, false)));
			...
			if (kpart->key_part_flag & HA_VAR_LENGTH_PART) {
				...
				varchar.set_quick(reinterpret_cast<char*>(const_cast<uchar*>(ptr + HA_KEY_BLOB_LENGTH)),
				...
				res= fp->val_str(&str, const_cast<uchar*>(ptr));
    ...
    for (i= 0; i < condf->argument_count(); i++) {
      ...
      if ((iscol= args[i]->type() == COND::FIELD_ITEM)) {
        ...
        if (pField->field->table != table ||
            !(colp[i]= tdbp->ColDB(g, const_cast<PSZ>(pField->field->field_name), 0)))
...
PCFIL ha_connect::CheckCond(PGLOBAL g, PCFIL filp, const Item *cond)
{
  ...
  if (cond->type() == COND::COND_ITEM) {
    char   *pb0=NULL, *pb1=NULL, *pb2=NULL, *ph0=NULL, *ph1=NULL, *ph2=NULL;
    bool    bb = false, bh = false;
    Item_cond *cond_item= dynamic_cast<Item_cond*>(const_cast<Item*>(cond));
    ...
    Item_func *condf= dynamic_cast<Item_func*>(const_cast<Item*>(cond));
    Item*     *args= condf->arguments();
 ...
 */
const COND *ha_connect::cond_push(const COND *cond, bool other_tbls_ok MY_ATTRIBUTE((unused)))
{
  ...
  if (tdbp) {
    ...
    if (b) {
      ...
      tdbp->SetFilter(CondFilter(g, const_cast<Item*>(cond)));
...
*/
int ha_connect::records(ha_rows* num_rows)
{
  if (!valid_info)
  ...
  if (tdbp){
    ...
    int ret = tdbp->GetCount(xp->g);
    if (ret < 0) {
      *num_rows = HA_POS_ERROR;
      return 1;
    }else{
      *num_rows = (ha_rows)ret;
    }
    return 0;
  }
  else{
...
int ha_connect::optimize(THD* thd, HA_CHECK_OPT*)
{
  ...
  if (tdbp && !tdbp->IsRemote()) {
    ...
    if ((rc= ((PTDBASE)tdbp)->ResetTableOpt(g, dop, dox))) {
      if (rc == RC_INFO) {
        push_warning(thd, Sql_condition::SL_WARNING, 1, g->Message);
        rc= 0;
      } else
        rc= HA_ERR_INTERNAL_ERROR;
...
//statistic_increment(ha_read_key_count, &LOCK_status);
  ...
  table->set_row_status_from_handler(!(rc == RC_OK));
  return rc;
} // end of ReadIndexed
...
//statistic_increment(ha_read_rnd_next_count, &LOCK_status);
  ...
  switch (CntReadNext(xp->g, tdbp)) {
    ...
    default:            // Read error
      ...
      ha_rows num_rows;
      records(&num_rows);
      rc= (num_rows) ? HA_ERR_INTERNAL_ERROR : HA_ERR_END_OF_FILE;
      break;
    } // endswitch RC
  ...
  table->set_row_status_from_handler(rc);
  DBUG_RETURN(rc);
} // end of rnd_next
...
//  case TAB_CATLG:
    ...
    case TAB_NIY:
      my_printf_error(ER_UNKNOWN_ERROR,
                      "Unsupported table type %s", MYF(0), options ? options->type:"or None options");
      return true;
    ...
    case TAB_WMI:
      ...
      */
      ...
      if ((!quick && thd->lex->requires_prelocking()) || 
          (table->pos_in_table_list && table->pos_in_table_list->grant.privilege & FILE_ACL))
        return false;
      status_var_increment(thd->status_var.access_denied_errors);
      my_error(access_denied_error_code(thd->password), MYF(0),
               thd->m_security_ctx->m_priv_user, thd->m_security_ctx->m_priv_host,
               (thd->password ?  ER_DEFAULT(ER_YES) : ER_DEFAULT(ER_NO)));
      return true;
...
int ha_connect::start_stmt(THD *thd, thr_lock_type lock_type)
{
  ...
  if (check_privileges(thd, GetTableOptionStruct(), const_cast<char*>(table->s->db.str), true))
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
...
//        DBUG_RETURN(HA_ERR_INTERNAL_ERROR);  causes assert error
  ...
  if (check_privileges(thd, options, const_cast<char*>(table->s->db.str))) {
...
bool CheckSelf(PGLOBAL g, TABLE_SHARE *s, const char *host,
                      const char *db, char *tab, const char *src, int)
{
  if (src)
    return false;
                       ...
                       dd::Table *table_def MY_ATTRIBUTE((unused)))
{
  ...
  */
  ...
  if (!options) 
  {
    my_message(ER_UNKNOWN_ERROR, "The options is empty", MYF(0));
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
...
//      fprintf(stderr, "Unsupported type column %s\n", fp->field_name);
  ...
  if ((sqlcom == SQLCOM_CREATE_TABLE || *GetTableName() == '#') && inward) {
    ...
    if ((type == TAB_FMT || options->readonly) && sqlcom == SQLCOM_CREATE_TABLE)
      push_warning(thd, Sql_condition::SL_WARNING, 1,
  ...
  if (table) {
    ...
    if (n < 0) {
      if (errno != ENOENT) {
        ...
        push_warning(table->in_use, Sql_condition::SL_WARNING, 1, buf);
        return true;
      } else
        return false;
...
/**
  ...
  */
bool ha_connect::NoFieldOptionChange(TABLE *tab MY_ATTRIBUTE((unused)))
{
  bool rc= true;
#if 0  
...
ha_connect::check_if_supported_inplace_alter(TABLE *altered_table MY_ATTRIBUTE((unused)),
...
bool ha_connect::check_if_incompatible_data(HA_CREATE_INFO *, uint)
{
  ...
  push_warning(ha_thd(), Sql_condition::SL_WARNING, 1,
...
int ha_connect::multi_range_read_next(char** range_info)
{
                                         ...
                                         uint *flags, Cost_estimate *cost)
{
  ...
  ha_rows rows= ds_mrr.dsmrr_info(keyno, n_ranges, keys, bufsz,
                                  flags, cost);
  xp->g->Mrr= !(*flags & HA_MRR_USE_DEFAULT_IMPL);
  return rows;
...
static SYS_VAR* connect_system_variables[]= {
  MYSQL_SYSVAR(xtrace),
