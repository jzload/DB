...
/***********************************************************************/
template <class TYPE>
bool TYPVAL<TYPE>::SetConstFormat(PGLOBAL g MY_ATTRIBUTE((unused)), FORMAT& fmt)
  {
  char c[32];
...
/***********************************************************************/
template <class TYPE>
void TYPVAL<TYPE>::Print(PGLOBAL g, char *ps, uint z MY_ATTRIBUTE((unused)))
  {
  if (Null)
    strcpy(ps, "<null>");
  else
...
ulonglong TYPVAL<PSZ>::GetUBigintValue(void)
  {
  return CharToNumber(Strp, strlen(Strp), INT_MAX64, true);
