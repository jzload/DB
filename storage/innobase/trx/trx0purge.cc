...
const page_size_t TrxUndoRsegsIterator::set_next() {
  ...
  if (m_iter != m_trx_undo_rsegs.end()) {
    ...
    m_purge_sys->iter.gtmgtid = (*m_iter)->last_gtmgtid;
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  m_purge_sys->iter.gtmgtid = m_purge_sys->rseg->last_gtmgtid;
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  new (&purge_sys->gtm_view) gtmgtid_ReadView();
  purge_sys->last_clone_time = ut_time_monotonic_us();
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  purge_sys->gtm_view.~gtmgtid_ReadView();
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    /*undo中的gtmgtid记录不见得是递增的*/
    rseg->last_gtmgtid = undo->gtmgtid;
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  auto gtmgtid = trx_undo_gtmgtid_read(log_hdr);
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  if (purge_sys->iter.gtmgtid && purge_sys->view.get_gtmgtid_readview() &&
      purge_sys->iter.gtmgtid >=
          purge_sys->view.get_gtmgtid_readview()->get_next_gtmgtid()) {
    return (NULL);
  }
#endif
...
/** get the trx purge done gtmgtid. */
static inline void trx_purge_set_done_gtmgtid() {
  rw_lock_x_lock(&purge_sys->latch);
  purge_sys->done_gtmgtid = purge_sys->iter.gtmgtid > 0
                                ? purge_sys->iter.gtmgtid
                                : purge_sys->done_gtmgtid;
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  DBUG_EXECUTE_IF(
    "trx_purge_before_purge_really",
    ut_ad(!debug_sync_set_action(
        current_thd, STRING_WITH_LEN("before_purge_really "
                                      "SIGNAL in_purge "
                                      "WAIT_FOR select_Done"))););

  ib_time_monotonic_us_t current_time = ut_time_monotonic_us();

  trx_sys->mvcc->clone_oldest_gtmview(
      &purge_sys->gtm_view, &purge_sys->view,
      (current_time - purge_sys->last_clone_time) >
          static_cast<ib_time_monotonic_us_t>(srv_distributed_mvcc_purge_timeout) *
              1000000);

  purge_sys->last_clone_time = current_time;
#endif
  purge_sys->view_active = true;

  rw_lock_x_unlock(&purge_sys->latch);
 
  DEBUG_SYNC_C("before_purge_really");
