                                               ...
                                               strings */
{
  ...
  if (trx->mysql_thd == NULL) {
    ...
    row->trx_query = NULL;
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
    row->trx_serial_num = NULL;
    row->trx_gtm_gtid = NULL;
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT
    goto thd_done;
  }
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
  char tsn_buf[MAX_TSN_LEN + 1];
  serial_len = thd_get_serial_num_safe(trx->mysql_thd, tsn_buf, sizeof(tsn_buf));
  if (serial_len > 0)
  {
      row->trx_serial_num = static_cast<const char*>(
                  ha_storage_put_memlim(
                      cache->storage, tsn_buf, serial_len + 1,
                      MAX_ALLOWED_FOR_STORAGE(cache)));
      if (row->trx_serial_num == NULL)
          return(FALSE);
  }
  else
  {
  ...
  char gtid_buf[MAX_GTID_LEN + 1];
  gtid_len = thd_get_gtm_gtid_safe(trx->mysql_thd, gtid_buf, sizeof(gtid_buf));
  if (gtid_len > 0)
  {
      row->trx_gtm_gtid = static_cast<const char*>(
                  ha_storage_put_memlim(
                      cache->storage, gtid_buf, gtid_len + 1,
                      MAX_ALLOWED_FOR_STORAGE(cache)));
      if (row->trx_gtm_gtid == NULL)
          return(FALSE);
  }
  else
  {
