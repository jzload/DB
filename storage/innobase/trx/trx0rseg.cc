...
/** read the gtmgtid from undo log header.
@param[in]	undo_log_hdr	last_gtmgtid read from
@param[in]	rseg	pointer to an rseg to set last_gtmgtid */
static inline void get_gtmgtid_from_undolog_header(page_t *undo_log_hdr,
                                                   trx_rseg_t *rseg) {
  ut_ad(undo_log_hdr);
  ut_ad(rseg);

  auto flag = mach_read_ulint(undo_log_hdr + TRX_UNDO_FLAGS, MLOG_1BYTE);

  if ((flag & TRX_UNDO_FLAG_GTID) != 0) {  // exist GTMGTID
    rseg->last_gtmgtid = mach_read_from_8(undo_log_hdr + TRX_UNDO_LOG_GTMGTID);
  } else {
    rseg->last_gtmgtid = 0;
  }
}
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    get_gtmgtid_from_undolog_header(undo_log_hdr, rseg);
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
#ifdef UNIV_DEBUG
