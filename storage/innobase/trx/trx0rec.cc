...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  missing_history = trxid_gtmgtid_visable_gtid(
      trx_id, name, &purge_sys->view, purge_sys->view.get_gtmgtid_readview(),
      gtmgtid);
#else
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  gtmgtid = row_get_rec_gtmgtid(rec, index, offsets);
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
                            index->table->name, &undo_rec, gtmgtid)) {
#else
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
      missing_extern = trxid_gtmgtid_visable_gtid(
          trx_id, index->table->name, &purge_sys->view,
          purge_sys->view.get_gtmgtid_readview(), gtmgtid);
#else
      missing_extern =
