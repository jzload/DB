...
static ulint trx_undo_header_create(
    page_t *undo_page, /*!< in/out: undo log segment
                       ...
                       TRX_UNDO_LOG_GTID_HDR_SIZE bytes
                       free space on it */
    trx_id_t trx_id,   /*!< in: transaction id */
    mtr_t *mtr)        /*!< in: mtr */
{
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
/** Read and gtm GTID from undo header during recovery.
@param[in]	undo_log	undo log header */
ulonglong trx_undo_gtmgtid_read(trx_ulogf_t *undo_log_header) {
  auto flag = mach_read_ulint(undo_log_header + TRX_UNDO_FLAGS, MLOG_1BYTE);
  if ((flag & TRX_UNDO_FLAG_GTMGTID) == 0) {
    return 0;
  }

  return mach_read_from_8(undo_log_header + TRX_UNDO_LOG_GTMGTID);
}
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
static void trx_undo_header_add_space_for_xid(page_t *undo_page,
  ...
  ulint new_limit = add_gtid ? TRX_UNDO_LOG_GTID_HDR_SIZE : TRX_UNDO_LOG_XA_HDR_SIZE;
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  new_limit = add_gtmgtid ? TRX_UNDO_LOG_HDR_SIZE : new_limit;
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  gtmgtid = trx_undo_gtmgtid_read(undo_header);
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
...
static void trx_undo_mem_init_for_reuse(
    ...
    const XID *xid,   /*!< in: X/Open XA transaction identification*/
    ulint offset,
    ulonglong gtmgtid_para = 0) /*!< in: undo log header byte offset on page */
{
  ut_ad(mutex_own(&((undo->rseg)->mutex)));
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
  ...
  if (trx->mysql_thd) {
    gtmgtid = thd_get_gtmgtid(trx->mysql_thd);
  }
...
static trx_undo_t *trx_undo_reuse_cached(trx_t *trx, trx_rseg_t *rseg,
  ...
  trx_undo_mem_init_for_reuse(undo, trx_id, xid, offset, gtmgtid);
  undo->gtid_allocated = add_space_gtid;
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  if (undo->gtmgtid) {
    undo->flag |= TRX_UNDO_FLAG_GTMGTID;
    mlog_write_ulint(undo_header + TRX_UNDO_FLAGS, undo->flag, MLOG_1BYTE, mtr);
    mlog_write_ull(undo_header + TRX_UNDO_LOG_GTMGTID, undo->gtmgtid, mtr);
  }
#endif // HAVE_ZSQL_DISTRIBUTE_MVCC
