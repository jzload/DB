...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  return (!trxid_gtmgtid_visable_gtid(trx_id, name, &purge_sys->view,
                                      purge_sys->view.get_gtmgtid_readview(),
                                      gtmgtid));
#else
...
dberr_t row_vers_build_for_consistent_read(
  ...
  ut_ad(!trxid_gtmgtid_visable(trx_id, view, p_gtm_view, rec, index, *offsets));
#else
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    if (trxid_gtmgtid_visable(trx_id, view, p_gtm_view, prev_version, index,
                              *offsets)) {
#else
