...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
          !trxid_gtmgtid_visable(rec_trx_id, view, view->get_gtmgtid_readview(), rec, m_config.m_index, offsets)) {
#else
        !view->changes_visible(rec_trx_id, table_name)) {
#endif //HAVE_ZSQL_DISTRIBUTE_MVCC
        rec_t *old_vers;

        auto ret = row_vers_build_for_consistent_read(rec, mtr, m_config.m_index, &offsets,
                                           view, &heap, heap, &old_vers,
                                           nullptr, nullptr);
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
        if (ret != DB_SUCCESS) {
          this->m_err.store(ret);
        }
#endif //HAVE_ZSQL_DISTRIBUTE_MVCC
        rec = old_vers;
...
dberr_t Parallel_reader::Ctx::traverse() {
  ...
  for (;;) {
    ...
    /* Note: if history is missed under distributed_mvcc, skip is true and err info 
    will never be passed outside, mysqld will be stucked */
    if (skip && m_scan_ctx->m_err == DB_MISSING_HISTORY) {
      err = m_scan_ctx->m_err;
    }
