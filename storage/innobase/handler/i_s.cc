...
#define IDX_TRX_SERIAL_NUM      24
    {STRUCT_FLD(field_name,     "trx_serial_num"),
     STRUCT_FLD(field_length,   MAX_TSN_LEN),
...
#define IDX_TRX_GTM_GTID      25
    {STRUCT_FLD(field_name,     "trx_gtm_gtid"),
     STRUCT_FLD(field_length,   MAX_GTID_LEN),
     STRUCT_FLD(field_type,     MYSQL_TYPE_STRING),
     STRUCT_FLD(value,          0),
     STRUCT_FLD(field_flags,    MY_I_S_MAYBE_NULL),
     STRUCT_FLD(old_name,       ""),
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
    /* trx_serial_num */
    OK(field_store_string(fields[IDX_TRX_SERIAL_NUM], row->trx_serial_num));

    /* trx_gtm_gtid */
    OK(field_store_string(fields[IDX_TRX_GTM_GTID], row->trx_gtm_gtid));
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT
...
#define INNODB_TABLES_FLAGS2 9
    {STRUCT_FLD(field_name, "FLAG2"),
     STRUCT_FLD(field_length, MY_INT32_NUM_DECIMAL_DIGITS),
     STRUCT_FLD(field_type, MYSQL_TYPE_LONG), STRUCT_FLD(value, 0),
     STRUCT_FLD(field_flags, 0), STRUCT_FLD(old_name, ""),
     STRUCT_FLD(open_method, 0)},
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  OK(fields[INNODB_TABLES_FLAGS2]->store(table->flags2));
#endif //HAVE_ZSQL_DISTRIBUTE_MVCC
  OK(schema_table_store_record(thd, table_to_fill));
