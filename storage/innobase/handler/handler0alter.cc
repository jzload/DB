...
/** Set the gtid flags after alter_table
@param[in]  altered_table     table after alter*/
inline void ha_innobase::alter_table_set_gtid_flags2(TABLE *altered_table, dict_table_t* table) {
  DICT_TF2_FLAG_UNSET(table, DICT_TF2_HAS_GTID);
  if (altered_table->s->fields > 1 && table_share->tmp_table == NO_TMP_TABLE) {
    auto & field_name = altered_table->s->field[altered_table->s->fields - 1]->field_name;
    auto type = altered_table->s->field[altered_table->s->fields - 1]->type();
    if (strncasecmp(field_name, GTMGTID_COL, 4) == 0 && type == MYSQL_TYPE_LONGLONG) {
      DICT_TF2_FLAG_SET(table, DICT_TF2_HAS_GTID);
    }
  }
}
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    // here check the altered_table has gtid 
    alter_table_set_gtid_flags2(altered_table, m_prebuilt->table);
#endif //HAVE_ZSQL_DISTRIBUTE_MVCC
    return false;
  }
...
#endif
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    //modify sucess , check the gtid flag  
    alter_table_set_gtid_flags2(altered_table, ctx->new_table);
#endif
  }
