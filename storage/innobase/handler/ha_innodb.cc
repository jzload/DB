...
static PSI_thread_info all_innodb_threads[] = {
    ...
    PSI_KEY(lock_wait_snapshort_write_thread, 0, 0, PSI_DOCUMENT_ME),
...
static SHOW_VAR innodb_status_variables[] = {
    ...
    {"distributed_mvcc_purge_at", (char *)&export_vars.innodb_distributed_mvcc_purge_at_gtmgtid,
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
#endif
#ifdef UNIV_DEBUG
                                      ...
                                      the global innodb_lock_wait_timeout */
{
  ...
  ulong timeout = thd_spec_lock_wait_time(thd);
  return (timeout > 0 ? timeout : THDVAR(thd, lock_wait_timeout));
}
...
int convert_error_code_to_mysql(dberr_t error, uint32_t flags, THD *thd) {
  switch (error) {
    ...
    case DB_TOO_MANY_LOCKS:
      return (HA_ERR_TOO_MANY_LOCKS);
#endif  // HAVE_ZSQL_LOCK_DEPTH_LIMIT
  }
}
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
  /* innodb lock wait log file */
  if (srv_lock_wait_log_dir == nullptr) {
    srv_lock_wait_log_dir = default_path;
  }
  Fil_path::normalize(srv_lock_wait_log_dir);

  if (strchr(srv_lock_wait_log_dir, ';')) {
    //"syntax error in innodb_lock_wait_dir"
    log_errlog(ERROR_LEVEL, ER_INNODB_INVALID_INNODB_LOCK_WAIT_DIRECTORY);
    return HA_ERR_INITIALIZATION;
  }
#endif  // HAVE_ZSQL_TSN_AND_LOCK_WAIT
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
void create_table_info_t::set_flags2_of_gtid() {
  if (m_form->s->fields > 1 && !is_temp_table()) {
    auto &field_name = m_form->s->field[m_form->s->fields - 1]->field_name;
    auto type = m_form->s->field[m_form->s->fields - 1]->type();
    if (strncasecmp(field_name, GTMGTID_COL, 4) == 0 &&
        type == MYSQL_TYPE_LONGLONG) {
      m_flags2 |= DICT_TF2_HAS_GTID;
    }
  }
}
...
/** Check if a table has FOREIGN KEY.
@return > 0 if has FOREIGN KEY */

uint ha_innobase::has_foreign_key(void) {
                               ...
                               int lock_type) /*!< in: lock type */
{
  ...
  switch (m_prebuilt->table->quiesce) {
    case QUIESCE_START:
      ...
      if (!srv_read_only_mode &&
#ifdef HAVE_ZSQL_COPY_TABLE
          (sql_command == SQLCOM_FLUSH || sql_command == SQLCOM_COPY_TABLE) &&
#else
...
static void purge_run_now_set(THD *thd MY_ATTRIBUTE((unused)),
  ...
  if (*(bool *)save && trx_purge_state() != PURGE_STATE_DISABLED) {
    trx_purge_run();
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    innodb_purge_run_now = TRUE;
    innodb_purge_stop_now = FALSE;
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
  }
}
...
static void purge_stop_now_set(THD *thd MY_ATTRIBUTE((unused)),
  ...
  if (srv_is_shutting.load()) return;

  if (*(bool *)save && trx_purge_state() != PURGE_STATE_DISABLED) {
    trx_purge_stop();

#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    innodb_purge_run_now = FALSE;
    innodb_purge_stop_now = TRUE;
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
  }
}
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
/** Update innodb_mvcc_purge_gtmgtid,
which control InnoDB gtmgtid to purge.
@param[out]	var_ptr   current value
@param[in]	save      to-be-assigned value */
static void innodb_distributed_mvcc_purge_gtmgtid_update(MYSQL_THD thd, SYS_VAR *var,
                                             void *var_ptr, const void *save) {
  ulonglong check = *static_cast<ulonglong *>(var_ptr);
  ulonglong to_save = *static_cast<const ulonglong *>(save);

  if (to_save == 0) srv_distributed_mvcc_purge_gtmgtid = 0;

  if (check < to_save) {
    srv_distributed_mvcc_purge_gtmgtid = to_save;
    srv_wake_purge_thread_if_not_active();
  }
}
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
static MYSQL_SYSVAR_BOOL(distributed_mvcc_purge, srv_distributed_mvcc_purge_flag, PLUGIN_VAR_NOCMDARG,
                         "distribute mvcc purge flag", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONGLONG(distributed_mvcc_purge_gtmgtid, srv_distributed_mvcc_purge_gtmgtid,
                              PLUGIN_VAR_NOCMDARG,
                              "lower than the gtmgtid should be purge.", NULL,
                              innodb_distributed_mvcc_purge_gtmgtid_update, 0, 0,
                              ULLONG_MAX, 0);

static MYSQL_SYSVAR_ULONG(distributed_mvcc_purge_timeout, srv_distributed_mvcc_purge_timeout,
                          PLUGIN_VAR_NOCMDARG,
                          "timeout used when getting the new gtmgtid to purge",
                          NULL, NULL, 3600, 1, ULONG_MAX, 0);
#endif
...
#ifdef HAVE_ZSQL_LOCK_DEPTH_LIMIT
static MYSQL_SYSVAR_UINT(lock_wait_max_depth, srv_lock_wait_max_depth,
                         PLUGIN_VAR_OPCMDARG,
                         "if the count of lock wait is bigger than "
                         "lock_wait_max_depth, reports error.",
                         NULL, NULL, 0, 0, 5000, 0);
#endif  // HAVE_ZSQL_LOCK_DEPTH_LIMIT
...
/* lock_wait_log */
static MYSQL_SYSVAR_BOOL(
    lock_wait_log, srv_lock_wait_log, PLUGIN_VAR_OPCMDARG,
    "Enable or Disable tracing innodb lock wait log since innodb started.",
    NULL, NULL, true);

static MYSQL_SYSVAR_STR(lock_wait_log_dir, srv_lock_wait_log_dir,
                        PLUGIN_VAR_READONLY, "The lock_wait_log dir of innodb.",
                        NULL, NULL, NULL);

static MYSQL_SYSVAR_UINT(
    lock_wait_collect_time, srv_lock_wait_collect_time, PLUGIN_VAR_OPCMDARG,
    "Lock wait snapshot is collected every this time millseconds.", NULL, NULL,
    500, 0, 2000, 0);

static MYSQL_SYSVAR_UINT(lock_wait_log_size, srv_lock_wait_log_size,
                         PLUGIN_VAR_OPCMDARG,
                         "Lock wait log file max size, MB.", NULL, NULL, 512,
                         64, (uint)(~0), 0);
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    MYSQL_SYSVAR(distributed_mvcc_purge),
    MYSQL_SYSVAR(distributed_mvcc_purge_gtmgtid),
    MYSQL_SYSVAR(distributed_mvcc_purge_timeout),
#endif
...
#ifdef HAVE_ZSQL_LOCK_DEPTH_LIMIT
    MYSQL_SYSVAR(lock_wait_max_depth),
#endif  // HAVE_ZSQL_LOCK_DEPTH_LIMIT

#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
    MYSQL_SYSVAR(lock_wait_log),
    MYSQL_SYSVAR(lock_wait_log_dir),
    MYSQL_SYSVAR(lock_wait_collect_time),
    MYSQL_SYSVAR(lock_wait_log_size),
#endif  // HAVE_ZSQL_TSN_AND_LOCK_WAIT
    NULL};
