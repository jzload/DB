...
/** check wether contain gtid col and it is the last col with bigint type
@param[in] n_mysql_cols   number of all the column 
@param[in] gtid_col   dd::column of gtid, can be null
@return true    if match otherwise doesn't match */
static inline bool check_has_correct_gtmgtid(unsigned n_mysql_cols, const dd::Column *gtid_col) {
  auto ret = false;
  if (n_mysql_cols > 1 && gtid_col && 
      gtid_col->type() == dd::enum_column_types::LONGLONG &&
      gtid_col->ordinal_position() == n_mysql_cols) {
    ret = true;
  } 
  return ret;
}
/** set the DICT_TF2_HAS_GTID flag
@param[in] has   true if has gtid column 
@param[in] m_table   dict_table to set, cannot be null*/
static inline void set_gtmgtid_flags2(bool has, dict_table_t *m_table) {
  DBUG_ASSERT(m_table);
  if (has) {
    DICT_TF2_FLAG_SET(m_table, DICT_TF2_HAS_GTID);
  }else {
    DICT_TF2_FLAG_UNSET(m_table, DICT_TF2_HAS_GTID);
  }
}
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  //check the last column weath is gtid
  bool has_last_gtid = false;
  const dd::Column *gtid_col;
  gtid_col = dd_find_column(&dd_tab->table(), GTMGTID_COL);
  has_last_gtid =check_has_correct_gtmgtid(n_mysql_cols, gtid_col);
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  set_gtmgtid_flags2(has_last_gtid, m_table);
#endif
