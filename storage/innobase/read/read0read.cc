...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  // for mvcc pool : m_gtmgtid_views and m_gtmgtid_free
  UT_LIST_INIT(m_gtmgtid_views, &gtmgtid_ReadView::m_view_list);
  UT_LIST_INIT(m_gtmgtid_free, &gtmgtid_ReadView::m_view_list);
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    gtmgtid_ReadView *gtm_view = UT_NEW_NOKEY(gtmgtid_ReadView());
    if (gtm_view) UT_LIST_ADD_FIRST(m_gtmgtid_free, gtm_view);
#endif
  }
}
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  // MVCC POOL : release
  for (gtmgtid_ReadView *gtm_view = UT_LIST_GET_FIRST(m_gtmgtid_free);
       gtm_view != NULL; gtm_view = UT_LIST_GET_FIRST(m_gtmgtid_free)) {
    UT_LIST_REMOVE(m_gtmgtid_free, gtm_view);

    UT_DELETE(gtm_view);
  }

  ut_a(UT_LIST_GET_LEN(m_gtmgtid_free) == 0);
}
#endif
/**
...
void MVCC::view_open(ReadView *&view, trx_t *trx) {
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
        add_gtmview_to_view(trx->mysql_thd, view, true);
#endif
        return;
      } else {
        view->m_closed = true;
  ...
  // check has gtid read_view , and put it in view
  if (view != NULL) {
    view->prepare(trx->id);
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    add_gtmview_to_view(trx->mysql_thd, view, false);
#endif
...
void MVCC::clone_oldest_gtmview(gtmgtid_ReadView *gtmView, ReadView *view,
                                bool is_timeout) {
  // MVCC: make the gtmgtid view
  if (srv_distributed_mvcc_purge_flag && !is_timeout &&
      srv_distributed_mvcc_purge_gtmgtid != 0) {
    gtmView->set_next_gtmgtid(srv_distributed_mvcc_purge_gtmgtid);
    view->gtmgtid_readview_ptr = gtmView;
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    // remove the gtmview from view and mvcc
    remove_gtmview_from_view(ptr, true);
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    remove_gtmview_from_view(view, false);
#endif
