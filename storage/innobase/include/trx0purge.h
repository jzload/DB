...
struct purge_iter_t {
  ...
  trx_id_t trx_no; /*!< Purge has advanced past all
                   ...
                   than this */
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  ulonglong gtmgtid; /*!< Purge has advanced past all
                    gtm transactions whose number is less
                    than this */
#endif
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  gtmgtid_ReadView gtm_view; /*!< The purge will not remove undo logs
                            which are >= this view (purge gtm view) */

  ib_time_monotonic_us_t last_clone_time; /*!< The last time make the clone */
  ulonglong done_gtmgtid;                 /*!< The gtmgtid done just for purge */
#endif
