...
class MVCC {
 public:
  ...
  void clone_oldest_gtmview(gtmgtid_ReadView *gtmView, ReadView *view,
                            bool is_timeout);
 ...
 private:
  ...
  gtmgtid_ReadView *get_oldest_gtmview() const {
    return UT_LIST_GET_LAST(m_gtmgtid_views);
  };

  inline gtmgtid_ReadView *get_gtmgtid_view();
  void add_gtmview_to_view(THD *thd, ReadView *view, bool need_mutex);
  void remove_gtmview_from_view(ReadView *view, bool need_mutex);
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
 ...
 private:
  ...
  view_list_t m_views;
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  // when create add the gtid_view_list_t, where commit   remove it and free it
  typedef UT_LIST_BASE_NODE_T(gtmgtid_ReadView) gtid_view_list_t;

  gtid_view_list_t m_gtmgtid_views;

  // free gtmview for reuse
  gtid_view_list_t m_gtmgtid_free;
#endif  // HAVE_ZSQL_DISTRIBUTE_MVCC
};
