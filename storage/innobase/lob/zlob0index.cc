...
void z_index_entry_t::purge(dict_index_t *index, z_first_page_t &first) {
  set_data_len(0);
  ...
  while (true) {
    ...
    switch (type) {
      ...
      case FIL_PAGE_TYPE_ZLOB_DATA:
        btr_page_free_low(index, block, ULINT_UNDEFINED, m_mtr);
        break;
...
void z_index_entry_t::read(z_index_entry_mem_t &entry_mem) const {
...
void z_index_entry_t::move_version_base_node(z_index_entry_t &entry) {
  ut_ad(m_mtr != nullptr);
...
void z_index_entry_t::set_old_version(z_index_entry_t &entry) {
  flst_base_node_t *version_list = get_versions_list();
...
std::ostream &z_index_entry_mem_t::print(std::ostream &out) const {
  out << "[z_index_entry_mem_t: self=" << m_self << ", prev=" << m_prev
      << ", next=" << m_next << ", versions=" << m_versions
      << ", m_trx_id=" << m_trx_id
