    ...
    ReadView *view)       /*!< in: consistent read view */
{
  ...
  auto *p_gtm_view = view->get_gtmgtid_readview();
  return trxid_gtmgtid_visable(trx_id, view, p_gtm_view, rec, index, offsets);
    ...
    const ReadView *view)      /*!< in: consistent read view */
{
  ...
  #ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  //all the second rec can not see
  if (index->table && 
      DICT_TF2_FLAG_IS_SET(index->table, DICT_TF2_HAS_GTID) &&
      view->get_gtmgtid_readview()) {
    return false;
  }
  #endif
  ut_ad(max_trx_id > 0);
...
DB_SKIP_LOCKED, or DB_LOCK_NOWAIT, DB_TOO_MANY_LOCKS */
  ...
  if (wait_for != nullptr) {
    switch (sel_mode) {
      ...
      case SELECT_ORDINARY:
        ...
        if (srv_lock_wait_max_depth &&
            get_wait_lock_count(mode, block, heap_no, trx) >= srv_lock_wait_max_depth)
        {
          return DB_TOO_MANY_LOCKS;
        }
...
dberr_t lock_sec_rec_read_check_and_lock(
  ...
  ut_ad(err == DB_SUCCESS || err == DB_SUCCESS_LOCKED_REC ||
        ...
        err == DB_LOCK_NOWAIT || err == DB_TOO_MANY_LOCKS);
  return (err);
}
...
/** Checks if locks of other transactions prevent an immediate read, or passing
 ...
 "offsets".
 @return DB_SUCCESS, DB_LOCK_WAIT, or DB_DEADLOCK, DB_TOO_MANY_LOCKS */
...
#ifdef HAVE_ZSQL_LOCK_DEPTH_LIMIT
/**
get lock count on the recod heap_no
@return lock or NULL */
uint get_wait_lock_count(
   ulint               mode,    /*!< in: LOCK_S or LOCK_X,
                                  possibly ORed to LOCK_GAP or
                                  LOC_REC_NOT_GAP,
                                  LOCK_INSERT_INTENTION */
