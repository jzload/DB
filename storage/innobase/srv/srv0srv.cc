...
/** the switch for mvcc purge*/
bool srv_distributed_mvcc_purge_flag;

/** lower than this value must be purged*/
unsigned long long srv_distributed_mvcc_purge_gtmgtid;

/** timeout when there are none gtimgtid purge get*/
ulong srv_distributed_mvcc_purge_timeout;
...
/** innodb lock wait max depth */
uint srv_lock_wait_max_depth = 0;
#endif  // HAVE_ZSQL_LOCK_DEPTH_LIMIT

#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
/** Boolean config, where open lock wait trace log */
bool srv_lock_wait_log = true;

/** lock wait cache collect thread wirte cache to disk every this time */
uint srv_lock_wait_collect_time = 500;

/** Str config, where to place lock wait trace log */
char *srv_lock_wait_log_dir = nullptr;

/** innodb lock wait log file size */
uint srv_lock_wait_log_size = 512;
...
void srv_export_innodb_status(void) {
  ...
  ulonglong done_gtmgtid = purge_sys->done_gtmgtid;
  rw_lock_s_unlock(&purge_sys->latch);

  export_vars.innodb_distributed_mvcc_purge_at_gtmgtid = done_gtmgtid;
#endif
#ifdef UNIV_DEBUG
  rw_lock_s_lock(&purge_sys->latch);
