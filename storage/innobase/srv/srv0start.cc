...
std::atomic<bool> srv_is_shutting{false};
...
mysql_pfs_key_t lock_wait_snapshort_write_thread_key;
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
    if (lock_wait_snapshot_thread_active && lock_wait_cache_initialized)
      os_event_set(lw_slot_cache->start_collect);
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT
...
/** before Shut down start purge when it had been stop. */
static inline void start_purge_for_shutdown() {
  srv_is_shutting.store(true);
  while (trx_purge_state() == PURGE_STATE_STOP) {
    trx_purge_run();
  }
  ut_ad(purge_sys->n_stop == 0);
