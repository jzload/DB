...
/** 
 * BUG by zsql: The end_thread should deallocator after tls_tables.
 * If first deallocator end_thread, then deallocator tls_tables, the shared_block
 * may be double freed. So we put the end_thread before tls_tables in one .cc file,
 * at thread exit will deallocator tls_tables first and then deallocator end_thread.
 * The end_thread is copy from allocator.cc file
 */
...
int Handler::delete_row(const uchar *mysql_row) {
  ...
  DBUG_ASSERT(m_rnd_iterator_is_positioned || m_index_cursor.is_positioned());

  if (!m_rnd_iterator_is_positioned) {
    DBUG_ASSERT(m_index_cursor.is_positioned());

    this->position(mysql_row);

    /* ref to Handler::rnd_pos() */
    uchar *position = this->ref;
    Storage::Element *row = *reinterpret_cast<Storage::Element **>(position);
    m_rnd_iterator = Storage::Iterator(&m_opened_table->rows(), row);
  }
#else
