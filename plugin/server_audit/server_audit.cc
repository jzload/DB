...
#else
#define mysql_mutex_real_mutex(A) (reinterpret_cast<pthread_mutex_t*>(&(A)->m_mutex))
#endif
...
static char *default_home= const_cast<char*>(".");
...
static ulong syslog_facility_audit, syslog_priority;
...
static void update_syslog_priority(MYSQL_THD thd, struct SYS_VAR *var,
                                   void *var_ptr, const void *save);
static void update_mode(MYSQL_THD thd, struct SYS_VAR *var,
                        void *var_ptr, const void *save);
...
static MYSQL_SYSVAR_ENUM(syslog_facility, syslog_facility_audit, PLUGIN_VAR_RQCMDARG,
                          ...
                          bool nu __attribute__((unused)) )
{
  ...
  while (*e && *e != ' ' && *e != ',')
    ++e;
  *length= e - entry;
  return reinterpret_cast<uchar*>(const_cast<char*>(entry));
}
...
static int coll_insert(struct user_coll *c, char *n, int len)
{
  if (c->n_users >= c->n_alloced)
  {
    c->n_alloced+= 128;
    if (c->users == NULL)
      c->users= reinterpret_cast<user_name*>(malloc(c->n_alloced * sizeof(c->users[0])));
    else
      c->users= reinterpret_cast<user_name*>(realloc(c->users, c->n_alloced * sizeof(c->users[0])));
...
static int start_logging()
{
  ...
  {
    openlog(syslog_ident, LOG_NOWAIT, syslog_facility_codes[syslog_facility_audit]);
    error_header();
...
static int write_log(const char *message, int len)
{
  ...
  {
    syslog(syslog_facility_codes[syslog_facility_audit] |
...
do_log_query:
  ...
  if (query_len > (message_size - csize)/2)
  {
    ...
    if (big_buffer_alloced < (query_len * 2 + csize))
    {
      ...
      big_buffer= reinterpret_cast<char*>(realloc(big_buffer, big_buffer_alloced));
      if (big_buffer == NULL)
      {
...
static void auditing_v8(MYSQL_THD thd, struct mysql_event_general_v8 *ev_v8) __attribute__((__unused__));
...
static void auditing_v13(MYSQL_THD thd, unsigned int *ev_v0) __attribute__((__unused__));
...
static int server_audit_init(void *p __attribute__((unused)))
{
  ...
  if(!(int_mysql_data_home= reinterpret_cast<char**>(dlsym(RTLD_DEFAULT, "mysql_data_home"))))
  {
    if(!(int_mysql_data_home= reinterpret_cast<char**>(dlsym(RTLD_DEFAULT, "?mysql_data_home@@3PADA"))))
...
{
  bool new_var = *(reinterpret_cast<bool*>(const_cast<void*>(save)));
  if (output_type == OUTPUT_FILE && logfile && new_var)
...
{
  char *new_var = *(reinterpret_cast<char**>(const_cast<void*>(save)));
  char *new_name= new_var ? new_var : empty_str;
...
static void update_file_rotations(MYSQL_THD thd  __attribute__((unused)),
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  rotations= *(reinterpret_cast<unsigned int*>(const_cast<void*>(save)));
  error_header();
...
static void update_file_rotate_size(MYSQL_THD thd  __attribute__((unused)),
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  file_rotate_size= *(reinterpret_cast<unsigned long long*>(const_cast<void*>(save)));
  error_header();
...
{
  char *new_var = *(reinterpret_cast<char**>(const_cast<void*>(save)));
  char *new_users= new_var ? new_var : empty_str;
  if (!maria_55_started || !debug_server_started)
    flogger_mutex_lock(&lock_operations);
...
static void update_output_type(MYSQL_THD thd,
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  ulong new_output_type= *(reinterpret_cast<ulong*>(const_cast<void*>(save)));
...
static void update_syslog_facility(MYSQL_THD thd  __attribute__((unused)),
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  ulong new_facility= *(reinterpret_cast<ulong*>(const_cast<void*>(save)));
  if (syslog_facility_audit == new_facility)
    return;

  mark_always_logged(thd);
  error_header();
  fprintf(stderr, "SysLog facility was changed from '%s' to '%s'.\n",
          syslog_facility_names[syslog_facility_audit],
          syslog_facility_names[new_facility]);
  syslog_facility_audit= new_facility;
}
...
static void update_syslog_priority(MYSQL_THD thd  __attribute__((unused)),
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  ulong new_priority= *(reinterpret_cast<ulong*>(const_cast<void*>(save)));
...
static void update_logging(MYSQL_THD thd,
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  char new_logging= *(reinterpret_cast<char*>(const_cast<void*>(save)));
...
static void update_mode(MYSQL_THD thd  __attribute__((unused)),
              ...
              void *var_ptr  __attribute__((unused)), const void *save)
{
  unsigned int new_mode= *(reinterpret_cast<unsigned int*>(const_cast<void*>(save)));
...
{
  char *new_var = *(reinterpret_cast<char**>(const_cast<void*>(save)));
  char *new_ident= new_var ? new_var : empty_str;
  ...
  else
  {
    ...
    if (serv_ver[0] == '5' && serv_ver[2] == '5')
    {
      ...
      if (sc <= 10)
      {
        ...
        mysql_descriptor.event_notify= nullptr;
      }
      else if (sc < 14)
      {
        mysql_descriptor.interface_version= 0x0200;
