...
const int
    Query_tables_list::binlog_stmt_unsafe_errcode[BINLOG_STMT_UNSAFE_COUNT] = {
        ...
        ER_BINLOG_STMT_UNSAFE_MERGE_UPDATE,
...
const char *
    SELECT_LEX::type_str[static_cast<int>(enum_explain_type::EXPLAIN_total)] = {
        ...
        "MINUS", "MINUS RESULT", "UNION MINUS RESULT"
...
void LEX::reset() {
  ...
  option_list_last = nullptr;
#endif // HAVE_ZSQL_CONNECT_ENGINE

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  //Used for closing implicit cursor after 'for loop' statememt
  need_close_cur = false;
  cursor_name_to_close = {NULL, 0};
  trunc_part_type = trunc_partition_type::NONE;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  m_has_internal_tmp_table = false;

  clear_privileges();
  grant_as.cleanup();

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  // bugfix: parse_place is not reset, which make the next create_func_comm crash
  parse_place = IN_PARSE_INVALID;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
}
...
/**
  ...
  @param linkage_type union, minus(except) or other type
...
SELECT_LEX *LEX::new_union_query(SELECT_LEX *curr_select, bool distinct
  ...
  */
  if (outer_most && result) {
    if (EXCEPT_TYPE == linkage_type) {
      my_error(ER_WRONG_USAGE, MYF(0), "MINUS", "INTO");
      return NULL;
    }
...
#ifdef HAVE_ZSQL_MINUS
  DBUG_ASSERT(linkage_type == UNION_TYPE || linkage_type == EXCEPT_TYPE);
  select->linkage = linkage_type;
#else
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
// oracle mode special, mysql mode normal
static bool is_normal_ident_in_mysql_mode(THD* thd, const SYMBOL *symbol) {
  if (!symbol)
    return true;
  ...
  if (0 == my_strcasecmp(system_charset_info, symbol->name, "EXCEPTION"))
    return true;

  if (symbol->tok == SQLCODE_SYM || symbol->tok == EXCEPTION_SYM)
    return true;
...
static int find_keyword(THD* thd, Lex_input_stream *lip, uint len, bool function) {
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (is_normal_ident_in_mysql_mode(thd, symbol))
      return 0;
#endif
...
/**
 * This func is used to get the TSN or GTID info from the special comment,
 * the special comment format is:
 * +TSN=input_string.
 * +GTID=input_string.
 **/
bool get_tsn_or_gtid_str(Lex_input_stream* lip, size_t begin_pos,
                         char* buf, size_t buflen)
{
  size_t count = 0;

  if (NULL == lip || NULL == buf || buflen == 0 ) {
    return false;
  }

  while (lip->yyPeekn(begin_pos) != '\0') {
    if (count >= buflen) {
      buf[count-1] = '\0';
      break;
    }

    if (lip->yyPeekn(begin_pos) == '*' && lip->yyPeekn(begin_pos+1) == '/') {
      buf[count] = '\0';
      break;
    }

    buf[count] = lip->yyPeekn(begin_pos);
    begin_pos++;
    count++;
  }

  if (lip->yyPeekn(begin_pos) == '\0') {
    return false;
  }
 ...
 * +GTID=input_string.
 **/
void get_tsn_or_gtid_info(Lex_input_stream* lip, THD* thd)
{
  char buff[MAX_TSN_LEN+1] = {0};
  char buff1[MAX_GTID_LEN+1] = {0};

  if (lip->yyPeekn(3) == 'T' &&
      lip->yyPeekn(4) == 'S' &&
      lip->yyPeekn(5) == 'N' &&
      lip->yyPeekn(6) == '=') {
    if (get_tsn_or_gtid_str(lip, 7, buff, sizeof(buff))) {
      thd->set_stmt_serial_num(buff, strlen(buff));
    }
  }
  ...
  if (lip->yyPeekn(3) == 'G' &&
      ...
      lip->yyPeekn(6) == 'D' &&
      lip->yyPeekn(7) == '=') {
    if (get_tsn_or_gtid_str(lip, 8, buff1, sizeof(buff1))) {
      thd->set_stmt_gtm_gtid(buff1, strlen(buff1));
    }
  }
    ...
    switch (state) {
      ...
      case MY_LEX_IDENT:
        ...
        if (start == lip->get_ptr() && c == '.' && ident_map[lip->yyPeek()])
          ...
          lip->yyUnget();
          if ((tokval = find_keyword(thd, lip, length, c == '('))) {
      ...
      case MY_LEX_INT_OR_REAL:  // Complete int or incomplete real
        if (c != '.' ||   /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
            (lip->yyPeek() == '.' && thd->lex->parse_place == IN_FOR_LOOP)) {
      ...
      case MY_LEX_CMP_OP:  // Incomplete comparison operator
        if (state_map[lip->yyPeek()] == MY_LEX_CMP_OP ||
          ...
          lip->yySkip();
        if ((tokval = find_keyword(thd, lip, lip->yyLength() + 1, 0))) {
          lip->next_state = MY_LEX_START;  // Allow signed numbers
          return (tokval);
        }
      ...
      case MY_LEX_BOOL:
        ...
        lip->yySkip();
        tokval = find_keyword(thd, lip, 2, 0);  // Is a bool operator
        lip->next_state = MY_LEX_START;    // Allow signed numbers
        return (tokval);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
          if ('.' == lip->yyPeek() && thd->lex->parse_place == IN_FOR_LOOP) {
            lip->yyGet();
            return MY_LEX_FOR_LOOP_SEP;
          }
#endif
      ...
      case MY_LEX_IDENT_OR_KEYWORD:
        ...
        */
        ...
        if ((tokval = find_keyword(thd, lip, length, 0))) {
...
SELECT_LEX_UNIT::SELECT_LEX_UNIT(enum_parsing_context parsing_context)
  ...
  switch (parsing_context) {
    ...
    case CTX_WHERE:
#ifdef HAVE_ZSQL_MERGE_INTO
    case CTX_MERGE_INTO_UPDATE_WHERE:
    case CTX_MERGE_INTO_INSERT_WHERE:
#endif /* HAVE_ZSQL_MERGE_INTO */
    case CTX_DERIVED:
...
SELECT_LEX::SELECT_LEX(Item *where, Item *having)
    : next(NULL),
      ...
      uncacheable(0),
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      row_item_info(nullptr),
#endif

#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
      if_need_replace_schema_table(false),
      prior_expr_flag(false),
      can_fix_outer_field(true),
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
...
void SELECT_LEX::print_insert(const THD *thd, String *str,
  ...
  if (!print_insert_all(thd, str, query_type)) return;
...
void SELECT_LEX::print_hints(const THD *thd, String *str,
  ...
  if (thd->lex->opt_hints_global) {
    ...
    String hint_str(buff, sizeof(buff), system_charset_info);
    hint_str.length(0);
    opt_hints_qb->append_full_join_anti_hint(&hint_str);
    if (hint_str.length() > 0) {
      str->append(STRING_WITH_LEN("/*+ "));
      str->append(hint_str.ptr(), hint_str.length());
      str->append(STRING_WITH_LEN("*/ "));
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  Item *const rownum_where = join ? join->where_cond_rownum : nullptr;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  if (cur_where || cond_value != Item::COND_UNDEF) {
    ...
    if (cur_where) {
      ...
      if (rownum_where) {
        ...
        rownum_where->print(thd, str, query_type);
      }
    }
    else if (rownum_where) {
      rownum_where->print(thd, str, query_type);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
    }
    else
...
LEX::LEX()
    : result(0),
      ...
      oracle_options(0),
      zsql_options(0),
      need_close_cur(false),  //HAVE_ZSQL_ORACLE_COMPATIBILITY
      trunc_part_type(trunc_partition_type::NONE), //HAVE_ZSQL_ORACLE_COMPATIBILITY
      m_has_internal_tmp_table(false)
{
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if ((master && master->with_rownum) || first_select()->with_rownum)
  {
    return false;
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  */
  ...
  switch (sql_command) {
    ...
    */
    ...
    case SQLCOM_MERGE_INTO:
    {
      if (oracle_options & ORA_MERGE_INTO_WITH_INSERT)
        new_trg_event_map |=
          static_cast<uint8>(1 << static_cast<int>(TRG_EVENT_INSERT));
      if (oracle_options & ORA_MERGE_INTO_WITH_UPDATE)
        new_trg_event_map |=
          static_cast<uint8>(1 << static_cast<int>(TRG_EVENT_UPDATE));
      break;
    }
...
#ifdef HAVE_ZSQL_MINUS
    Query_Blocks_Linkage_Type linkage_type =
      get_query_blocks_linkage_type(master_unit()->first_select());

    if (Query_Blocks_Linkage_Type::ALL_ARE_UNION == linkage_type)
      return enum_explain_type::EXPLAIN_UNION_RESULT;
    else if (Query_Blocks_Linkage_Type::ALL_ARE_MINUS == linkage_type)
      return enum_explain_type::EXPLAIN_MINUS_RESULT;
    else
      return enum_explain_type::EXPLAIN_UNION_MINUS_RESULT;
#else
    return enum_explain_type::EXPLAIN_UNION_RESULT;
#endif // HAVE_ZSQL_MINUS
  }
...
#ifdef HAVE_ZSQL_MINUS
    if (linkage == EXCEPT_TYPE)
      return enum_explain_type::EXPLAIN_MINUS;
#endif // HAVE_ZSQL_MINUS
