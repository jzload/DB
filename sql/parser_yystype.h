...
struct table_values_pair;
struct table_fields_pair;
...
enum class trunc_partition_type;
...
enum PT_joined_table_type {
  ...
  ,
  /* FULL JOIN */
  JTT_NATURAL_FULL = JTT_NATURAL | JTT_FULL
#endif
};
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  /* Below value used for distincting DECIMAL, NUMERIC(...) in yacc */
  TEMPORAY_NUMERIC_START = 10000,
  NUMBER,
  NUMERIC,
  FIXED,
#endif
...
union YYSTYPE {
  ...
  */
  ...
  class PT_hierarchical *hierarchical; //HAVE_ZSQL_START_WITH_CONNECT_BY
...
#ifdef HAVE_ZSQL_MERGE_INTO
  class PT_joined_table_on *join_table_on;
#endif /* HAVE_ZSQL_MERGE_INTO */
  ...
  } table_fields_values_list;
  struct {
    table_values_pair *values;
    table_fields_pair *fields;
  } table_fields_values;
  Semantics_type length_qualifier;
  struct {
    const char *c_str;
    Semantics_type qualifier;
  } varchar2_data_type;
#endif /* HAVE_ZSQL_INSERT_ALL */
#ifdef HAVE_ZSQL_MERGE_INTO
  struct {
    Merge_fields_values *insert_fields_values;
    Merge_fields_values *update_fields_values;
    ulonglong merge_option;
  } merge_clauses;

  Merge_fields_values *merge_fields_values;
#endif /* HAVE_ZSQL_MERGE_INTO */
};
