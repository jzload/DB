...
class Item_func : public Item_result_field {
 ...
 public:
  ...
  ResetDecType reset_decimal_flag = UNKNOWN_RESET;
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  ResetDecType get_reset_decimal_flag() { return reset_decimal_flag; }
  void set_reset_decimal_flag(ResetDecType flag) { reset_decimal_flag = flag; }
  bool check_and_reset_decimal();
  void set_no_reset_dec_flag();
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  // ROWNUM
  bool find_rownum_item(THD *thd, SELECT_LEX *select_lex, uint el,
                        bool &change_row_to_ref) override;

  bool find_extractable_rownum_cond();
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
private:
  bool set_datetime_data_type(Item_result &r0, Item_result &r1);
...
#ifdef HAVE_ZSQL_TO_DATE
 public:
  bool date_op(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date) override;
  enum Item_result numeric_context_result_type() const override;

 private:
  bool date_arithmetic(MYSQL_TIME *ltime, Item *item1, Item *item2, bool neg);
#endif /* HAVE_ZSQL_TO_DATE */
};
...
class Item_func_plus final : public Item_func_additive_op {
 public:
  ...
  void *clone(Parse_tree_transformer *ptt) const override;  //HAVE_ZSQL_ORACLE_COMPATIBILITY begin
};
...
/* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
class Item_func_cursor_attribute final : public Item_int_func {
 public:
  enum enum_cursor_attribute_type {
    UNKNOWN = -1,
    ISOPEN,
    FOUND,
    NOT_FOUND,
  ...
  sp_instr *m_i;// My push instruction
  enum_cursor_attribute_type m_cursor_attribute_type;
  sp_cursor *m_runtime_cursor;

  Item_func_cursor_attribute(THD *thd, LEX_STRING* name, sp_instr *i,
  ...
  const char *func_name() const override { return "cursor"; }
};
/* #endif */
