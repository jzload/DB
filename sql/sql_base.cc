  ...
  */
  ...
  if ((flags & (MYSQL_OPEN_HAS_MDL_LOCK | MYSQL_OPEN_SECONDARY_ENGINE)) == 0) {
    ...
    */
    ...
    mdl_start_time = thd->current_utime();
    if (open_table_get_mdl_lock(thd, ot_ctx, table_list, flags, &mdl_ticket) ||
        mdl_ticket == NULL) {
      DEBUG_SYNC(thd, "before_open_table_wait_refresh");
      return true;
    }
    thd->mdl_req_time = thd->current_utime() - mdl_start_time;
...
int run_before_dml_hook(THD *thd) {
  int out_value = 0;
  (void)RUN_HOOK_INLINE(transaction, before_dml, (thd, out_value));
...
bool lock_table_names(THD *thd, TABLE_LIST *tables_start,
  ...
  */
  ...
  for (table = tables_start; table && table != tables_end;
    ...
    if (SQLCOM_COPY_TABLE == thd->lex->sql_command &&
        table != tables_start && flags != 0) {
      /* 在Copy table功能中，执行discard时要求dest table的schema增加IX lock.
       * 在这里修改flags来将此table加入到schema_set，供下面Phase 2中加锁处理 */
      flags = 0;
    }
...
static Field *find_field_in_natural_join(THD *thd, TABLE_LIST *table_ref,
  ...
  // It's a full join in natural join
  if (nj_col->full_join_field) {
    DBUG_ASSERT(nj_col->full_join_field->field);
    found_field = nj_col->full_join_field->field;
  }
  else
#endif
...
/*
 * FULL JOIN
 * For full join converted derived table, we have is_full_join_derived
 * set, and full_join_fields_list not null. Search full_join_fields_list
 * to find given name.
 *
 * Param
 * found[out]  if has non_unique error, point to the first match field.
 */
Field *find_field_in_table_ref(THD *thd, TABLE_LIST *table_list,
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  if (table_list->is_full_join_derived) {
    if (!table_list->full_join_contains_table(db_name, table_name))
      return 0;
  }
  /* NOT full join derived table. */
  else
#endif
  ...
  *actual_table = NULL;
  ...
  if (table_list->is_full_join_derived)
  {
    DBUG_ASSERT(table_list->full_join_fields_list);
    DBUG_ASSERT(table_list->full_join_contains_table(db_name, table_name));
    if ((fld = find_field_in_full_join(table_list, db_name,
                                       table_name, name, found)))
  ...
  if (table_list->field_translation) {
    ...
    */
    if (table_name && table_name[0]) {
      ...
      *
      * SELECT test.t1.a1
      * FROM test.t1 full join test2.t1 on test.t1.a=test2.t1.a join t5 using(a)
      * GROUP BY t1.a1
      *
      * That is when two db have the same table with same columns are full joined,
      * Then as an operand, natural joined to another table.
      */
                 , found
#endif
                 )))
  ...
  *resolution = NOT_RESOLVED;
  ...
  for (uint i = 0; (item = li++); i++) {
    if (field_name && item->real_item()->type() == Item::FIELD_ITEM) {
      ...
      /* Refactored by full join. */
      /* All the item_field->***_name's in this if clause are replaced. */
      const char *item_field_name = item_field->field_name,
                 *item_table_name = item_field->table_name,
                 *item_db_name = item_field->db_name;
#ifdef HAVE_ZSQL_FULL_JOIN
      get_full_join_field_name_from_item(item_field, &item_field_name,
                                         &item_table_name, &item_db_name);
#endif
      ...
      */
      ...
      if (table_name) {
        ...
        */
        if (item_field_name && item_table_name &&
            !my_strcasecmp(system_charset_info, item_field_name,
                           field_name) &&
            !my_strcasecmp(table_alias_charset, item_table_name,
                           table_name) &&
            (!db_name ||
             (item_db_name && !strcmp(item_db_name, db_name)))) {
          if (found_unaliased) {
...
/*
 * FULL JOIN
 * Additional comment for full join:
 *
 * @param is_natural_right_join_in_full
 *              if this is a natural right join in full join nests.
 * @Note
 * We change this to keep the same order for fields output and tables.
 */
...
#ifdef HAVE_ZSQL_FULL_JOIN
                                             /* FULL JOIN */
                                             , bool is_natural_right_join_in_full
#endif
                                             ) {
  Field_iterator_table_ref it_1, it_2;
  Natural_join_column *nj_col_1, *nj_col_2;
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN*/
  List<Natural_join_column> *non_join_columns2 = nullptr;
  if (is_natural_right_join_in_full &&
      !(non_join_columns2 = new (thd->mem_root)
                            List<Natural_join_column>))
    return true;
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
      /* FULL JOIN*/
      // Mark this column as natural join / using column
      nj_col_1->is_natural_join_field = true;
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
      if (is_natural_right_join_in_full)
        non_join_columns2->push_back(nj_col_2);
      else
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
  if (is_natural_right_join_in_full && non_join_columns2->elements > 0)
    natural_using_join->join_columns->concat(non_join_columns2);
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
    /* FULL JOIN */
    // table_ref_1 and table_ref_2 has been swapped
    bool right_natural_in_full = table_ref_2->in_full_join_nest();
#endif
    if (store_natural_using_join_columns(thd, table_ref, table_ref_1,
                                         table_ref_2, using_fields,
                                         found_using_fields,
#ifdef HAVE_ZSQL_FULL_JOIN
                                         right_natural_in_full
#endif
                                         ))
      return true;
...
bool setup_fields(THD *thd, Ref_item_array ref_item_array, List<Item> &fields,
  ...
  */
  ...
  while ((item = it++)) {
    ...
    bool cols_uncorrect = false;
    if (row_infos) { // it is in update statement
      DBUG_ASSERT(thd->lex->sql_command == SQLCOM_UPDATE||
              thd->lex->sql_command == SQLCOM_UPDATE_MULTI);
      switch (item->type()) {
        case Item::ROW_ITEM : {
           //   all item in row item must  field_item
          if (check_row_item(item)) return true;
          row_infos->insert(std::make_pair(count, item));
          // row_item skip the check_cols
        }
        break;
        case Item::SUBSELECT_ITEM : {
            item_row = (*row_infos)[count];
            cols_uncorrect = (item_row ? (*(it.ref()))->check_cols(item_row->cols()): (*(it.ref()))->check_cols(1));
        }
        break;
        default:
          // when item_field in view, the ref may be replaced, so item should assign from it.ref()
          cols_uncorrect = (item = *(it.ref()))->check_cols(1);
          break;
      }
    }else {
        cols_uncorrect = (item = *(it.ref()))->check_cols(1);
...
bool insert_fields(THD *thd, Name_resolution_context *context,
  ...
  //HAVE_ZSQL_START_WITH_CONNECT_BY
  Item *wild_field = *it->ref();
  ...
  for (TABLE_LIST *tables = (table_name ? context->table_list
...
#ifdef HAVE_ZSQL_FULL_JOIN
    /* FULL JOIN */
    // for full join derived table, we must search for its children.
    ...
    */
    ...
    for (; !field_iterator.end_of_fields(); field_iterator.next()) {
      ...
      //HAVE_ZSQL_START_WITH_CONNECT_BY
      if(pass_item_replace_info(item, wild_field) || item->check_replace_schema_table_valid(thd)) {
        return true;
      }
...
#ifdef HAVE_ZSQL_FULL_JOIN
      // if tb.*, only output columns from tb
      if (!is_table_name_match_full_join_field(db_name, table_name, tables, field_iterator))
        continue;
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
        /* FULL JOIN */
        // For full join derived table, we record fields info from inner tables.
        if (gen_full_join_info &&
            generate_full_join_field(thd, item,
                                     field_iterator,
                                     fj_fields_list))
          return true;
#endif
...
bool fill_record(THD *thd, TABLE *table, List<Item> &fields, List<Item> &values,
  ...
  */
  ...
  if (fill_fields(thd, table, fields, values, bitmap,
