...
static int open_binary_frm(THD *thd, TABLE_SHARE *share,
  ...
  */
  ...
  if ((n_length = uint4korr(head + 55))) {
    ...
    //ODBC options
    if(1 == head[45])
    {//skip reading options, do it later
      options_len = uint2korr(next_chunk);
      next_chunk += 2;
      options_buff = next_chunk;
...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
  //CONNECT ODBC engine
  //restore table options and field options from frm
  if(head[45] == 1 && options_buff && options_len){
      if (engine_table_options_frm_read(options_buff, options_len, share))
        goto err;
...
int open_table_from_share(THD *thd, TABLE_SHARE *share, const char *alias,
  ...
  */
  ...
  bitmap_init(&outparam->pack_row_tmp_set,
              ...
              false);
  bitmap_init(&outparam->def_fields_set_insert_saved,
...
bool TABLE::init_tmp_table(THD *thd, TABLE_SHARE *share, MEM_ROOT *m_root,
  ...
  if (!is_virtual) {
    ...
    /* Used for quick_sync check, temp_table not support quick_sync. */
    thd->lex->m_has_internal_tmp_table = true;
  }
  s = share;
  in_use = thd;
...
#ifdef HAVE_ZSQL_FULL_JOIN
Natural_join_column::Natural_join_column(Full_join_field *field_param,
                                         TABLE_LIST *tab) {
  DBUG_ASSERT(tab->full_join_fields_list);
  table_field = NULL;
  view_field = NULL;
  full_join_field = field_param;
  table_ref = tab;
  is_common = false;
}
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
  if (full_join_field) {
    DBUG_ASSERT(table_field == NULL && view_field == NULL);
    return full_join_field->real_alias ? full_join_field->real_alias :
                                         full_join_field->real_field_name;
  }
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
  else if (full_join_field) {
    return full_join_field->create_item(thd);
  }
#endif
  return table_field;
}
...
#ifdef HAVE_ZSQL_FULL_JOIN
  if (full_join_field) {
    return full_join_field->real_table_name;
  }
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
  if (full_join_field) {
    return full_join_field->real_db_name;
  }
#endif
  /*
    ...
    DBUG_ASSERT(
        ...
        (table_ref->join_columns &&
         ...
         ((table_ref->field_translation &&
          ...
          /* Full join: If this is a full join derived table, this also applies. */
          (!table_ref->field_translation &&
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  /* This is a full join derived table. */
  else if (table_ref->is_full_join_derived)
  {
    DBUG_ASSERT(table_ref->full_join_fields_list);
    field_it = &full_join_it;
    DBUG_PRINT("info",
               ("field_it for '%s' is Field_iterator_full_join", table_ref->alias));
  }
#endif
...
Natural_join_column *Field_iterator_table_ref::get_or_create_column_ref(
  ...
  {
    Full_join_field *tmp_field = full_join_it.field_ref();
    nj_col = new (thd->mem_root) Natural_join_column(tmp_field, table_ref);
    field_count = table_ref->full_join_fields_list->elements;
  }
  else
#endif
...
void TABLE::clear_column_bitmaps() {
  ...
  */
  ...
  bitmap_clear_all(&def_fields_set_insert_saved);
