...
Field *Field_new_decimal::create_from_item(Item *item) {
  ...
  */
  ...
  return new (*THR_MALLOC) Field_new_decimal(
      len, item->maybe_null, item->item_name.ptr(), dec,
      item->unsigned_flag
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      , item->item_options
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (item_options & HAVE_TO_NUMBER_ITEM) {
    uint8 actual_frac = get_actual_fraction_dec_val(decimal_value);
    my_decimal_round(E_DEC_FATAL_ERROR, decimal_value, actual_frac, false, decimal_value);
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
bool Field::set_warning(Sql_condition::enum_severity_level level, uint code,
  ...
  */
  ...
  if (thd->lex->sql_command != SQLCOM_INSERT &&
      ...
      thd->lex->sql_command != SQLCOM_MERGE_INTO &&
