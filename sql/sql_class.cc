...
THD::THD(bool enable_plugins)
  ...
  init_sql_alloc(key_memory_thd_main_sp_mem_root, &main_sp_mem_root,
                 global_system_variables.query_alloc_block_size,
                 global_system_variables.query_prealloc_size);
  ...
  utime_after_lock = 0L;
  sp_exists_endmark = false;  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  current_linfo = 0;
  slave_thread = 0;
  ...
  *scramble = '\0';
  ...
  m_trx_serial_num[0]= '\0';
  m_trx_gtm_gtid[0]= '\0';
  m_stmt_serial_num[0]= '\0';
  m_stmt_gtm_gtid[0]= '\0';
  ...
  init();
  init_slow_log_time();/*initial slow log variable*/
#if defined(ENABLED_PROFILING)
  ...
  */
  ...
  m_quick_parser_state = new Parser_state();
  m_quick_parser_state->m_lip.found_semicolon = NULL;

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  set_sp_mem_root(&main_sp_mem_root);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
}
...
THD::~THD() {
  ...
  delete m_quick_parser_state;
  m_quick_parser_state = nullptr;

  free_root(&main_mem_root, MYF(0));

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  free_root(&main_sp_mem_root, MYF(0));
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
void THD::awake(THD::killed_state state_to_set) {
  ...
  */
  ...
  if (state_to_set != THD::KILL_QUERY && state_to_set != THD::KILL_TIMEOUT) {
    ...
    if (!slave_thread)
      MYSQL_CALLBACK(this->func_scheduler,
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
void Query_arena::free_items2() {
  Item *next;
  DBUG_TRACE;
  /* This works because items are allocated with (*THR_MALLOC)->Alloc() */
  for (; sp_item_list; sp_item_list = next) {
    next = sp_item_list->next_free;
    DBUG_ASSERT(sp_item_list != next);
    sp_item_list->delete_self();
  }
  /* Postcondition: free_list is 0 */
}
#endif
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
void THD::set_sp_mem_root(MEM_ROOT *src_mem_root) {
  sp_mem_root = src_mem_root;
}
#endif
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
void THD::set_stmt_serial_num(const char* serial_num_str, size_t len)
{
    mysql_mutex_lock(&LOCK_thd_query);

    if(NULL == serial_num_str || len == 0 )
    {
        m_stmt_serial_num[0] = '\0';
    }
    else if(len > MAX_TSN_LEN)
    {
        memcpy(m_stmt_serial_num, serial_num_str, MAX_TSN_LEN);
        m_stmt_serial_num[MAX_TSN_LEN] = '\0';
    }
    else
    {
        memcpy(m_stmt_serial_num, serial_num_str, len);
        m_stmt_serial_num[len] = '\0';
    }
...
void THD::set_stmt_gtm_gtid(const char* gtm_gtid_str, size_t len)
{
    mysql_mutex_lock(&LOCK_thd_query);

    if(NULL == gtm_gtid_str || len == 0 )
    {
        m_stmt_gtm_gtid[0] = '\0';
    }
    else if(len > MAX_GTID_LEN)
    {
        memcpy(m_stmt_gtm_gtid, gtm_gtid_str, MAX_GTID_LEN);
        m_stmt_gtm_gtid[MAX_GTID_LEN] = '\0';
    }
    else
    {
        memcpy(m_stmt_gtm_gtid, gtm_gtid_str, len);
        m_stmt_gtm_gtid[len] = '\0';
    }
...
const char* THD::get_trx_serial_num()
{
    if(m_trx_serial_num[0] != '\0')
        return m_trx_serial_num;
    else
        return NULL;
}
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
const char* THD::get_trx_gtm_gtid() const
#endif // HAVE_ZSQL_DISTRIBUTE_MVCC
{
  if(m_trx_gtm_gtid[0] != '\0')
    return m_trx_gtm_gtid;
  else
    return NULL;
}

void *THD::get_active_gtmgtids(int64 *len) const {
  *len = active_gtmgtids.first;

  return active_gtmgtids.second;
}

void THD::reset_stmt_gdb_hint_info()
{
    if(m_stmt_serial_num[0] == '\0' && m_stmt_gtm_gtid[0] == '\0')
        return;
...
void THD::reset_trx_gdb_hint_info()
{
    if(m_trx_serial_num[0] == '\0' && m_trx_gtm_gtid[0] == '\0')
        return;
...
void THD::copy_gdb_hint_from_stmt_to_trx()
{
    if(m_stmt_serial_num[0] == '\0' && m_stmt_gtm_gtid[0] == '\0')
        return;

    mysql_mutex_lock(&LOCK_thd_query);
    if(m_stmt_serial_num[0] != '\0')
    {
        memcpy(m_trx_serial_num, m_stmt_serial_num, sizeof(m_trx_serial_num));
    }
    if(m_stmt_gtm_gtid[0] != '\0')
    {
        memcpy(m_trx_gtm_gtid, m_stmt_gtm_gtid, sizeof(m_trx_gtm_gtid));
    }
    mysql_mutex_unlock(&LOCK_thd_query);
    return;
}
...
void THD::set_open_table_time(bool set_start_time)
{
  if (set_start_time)
  {
    open_table_time = ulonglong2uint64_t(current_utime());
    return;
  }

  open_table_time = ulonglong2uint64_t(current_utime()) - open_table_time;
  return;
}
