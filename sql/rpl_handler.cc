...
int Trans_delegate::before_commit(THD *thd, bool all,
  ...
  int ret = 0;
  ret = repl_semi_report_before_commit(&param);
...
int Trans_delegate::before_dml(THD *thd, int &result) {
  ...
  /* Avoid dict_sys->mutex in prepare_table_info for GoldenDB.
   * This only used in Group Replication, but no support in GoldenDB.
   * */
  // prepare_table_info(thd, param.tables_info, param.number_of_tables);
  // prepare_transaction_context(thd, param.trans_ctx_info);

  int ret = 0;
  ret = repl_semi_report_before_dml(&param, result);
...
int Trans_delegate::before_rollback(THD *thd, bool all) {
  ...
  int ret = 0;
  ret = repl_semi_report_before_rollback(&param);
...
int Trans_delegate::after_commit(THD *thd, bool all) {
  ...
  int ret = 0;
  ret = repl_semi_report_commit(&param);
...
int Trans_delegate::after_rollback(THD *thd, bool all) {
  ...
  int ret = 0;
  ret = repl_semi_report_rollback(&param);
...
int Trans_delegate::trans_begin(THD *thd, int &out) {
  ...
  int ret = 0;
  ret = repl_semi_report_begin(&param, out);
...
int Binlog_storage_delegate::after_flush(THD *thd, const char *log_file,
  ...
  int ret = 0;
  ret = repl_semi_report_binlog_update(&param, log_file, log_pos);
...
int Binlog_storage_delegate::after_sync(THD *thd, const char *log_file,
  ...
  int ret = 0;
  ret = repl_semi_report_binlog_sync(&param, log_file, log_pos);
...
int Binlog_transmit_delegate::transmit_start(THD *thd, ushort flags,
  ...
  int ret = 0;
  ret = repl_semi_binlog_dump_start(&param, log_file, log_pos);
...
int Binlog_transmit_delegate::transmit_stop(THD *thd, ushort flags) {
  ...
  int ret = 0;
  ret = repl_semi_binlog_dump_end(&param);
...
#define RESERVE_HEADER_SIZE 32
  ...
  int ret = 0;
  ret = repl_semi_reserve_header(&param,header,RESERVE_HEADER_SIZE,&hlen);
...
int Binlog_transmit_delegate::before_send_event(THD *thd, ushort flags,
  ...
  int ret = 0;
  ret = repl_semi_before_send_event(&param,(uchar *)packet->ptr(),packet->length(),
...
int Binlog_transmit_delegate::after_send_event(THD *thd, ushort flags,
  ...
  int ret = 0;
  ret = repl_semi_after_send_event(&param, packet->ptr(), packet->length(),
...

{
  ...
  int ret = 0;
  ret = repl_semi_reset_master(&param);
...
int Binlog_relay_IO_delegate::thread_start(THD *thd, Master_info *mi) {
  ...
  int ret = 0;
  ret = repl_semi_slave_io_start(&param);
...
int Binlog_relay_IO_delegate::thread_stop(THD *thd, Master_info *mi) {
  ...
  int ret = 0;
  ret = repl_semi_slave_io_end(&param);
...
int Binlog_relay_IO_delegate::applier_start(THD *thd, Master_info *mi) {
  ...
  int ret = 0;
  ret = repl_semi_slave_sql_start(&param);
...
int Binlog_relay_IO_delegate::applier_stop(THD *thd, Master_info *mi,
  ...
  int ret = 0;
  ret = repl_semi_slave_sql_stop(&param, aborted);
...
int Binlog_relay_IO_delegate::before_request_transmit(THD *thd, Master_info *mi,
  ...
  int ret = 0;
  ret = repl_semi_slave_request_dump(&param, (uint32)flags);
...
int Binlog_relay_IO_delegate::after_read_event(THD *thd, Master_info *mi,
  ...
  int ret = 0;
  ret = repl_semi_slave_read_event(&param, packet, len, event_buf, event_len);
...
int Binlog_relay_IO_delegate::after_queue_event(THD *thd, Master_info *mi,
  ...
  int ret = 0;
  ret = repl_semi_slave_queue_event(&param, event_buf, event_len, flags);
...

{
  ...
  int ret = 0;
  ret = repl_semi_reset_slave(&param);
...
int Binlog_relay_IO_delegate::applier_log_event(THD *thd, int &out) {
  ...
  int ret = 0;
  ret = repl_semi_apply_slave(&param, &trans_param, out);
...
int launch_hook_trans_begin(THD *thd, TABLE_LIST *all_tables) {
  ...
  if (hold_command) {
    ...
    thd->enter_stage(&stage_hook_begin_trans, &old_stage, __func__, __FILE__,
                     __LINE__);
    RUN_HOOK_INLINE(transaction, trans_begin, (thd, ret));
...
bool replication_plugin_satisfied_quick_sync()
{
  // semi_sync_enabled = false, dosent support quick sync
  // after commit dosent support quick sync
  int is_quick_sync_enabled = g_is_quick_sync_enabled.load();
  if(is_quick_sync_enabled)
    return true;
