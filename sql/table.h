...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
  engine_option_value *option_list{nullptr};  /* text options for table */
  ha_table_option_struct *option_struct{nullptr}; /* structure with parsed options */
#endif // HAVE_ZSQL_CONNECT_ENGINE
...
struct TABLE {
  ...
  */
  ...
  /**
    Bitmap of table fields (columns), which are explicitly set in prepare of
    INSERT INTO/ MERGE INTO statement.
    It is declared here to avoid memory allocation on MEM_ROOT).
  */
  MY_BITMAP def_fields_set_insert_saved;

  MY_BITMAP *read_set{nullptr}, *write_set{nullptr}; /* Active column sets */

#ifdef HAVE_ZSQL_CONNECT_ENGINE
  /* Set if using virtual fields */
  MY_BITMAP *vcol_set{nullptr}, *def_vcol_set{nullptr};
#endif // HAVE_ZSQL_CONNECT_ENGINE
 ...
 public:
  ...
  void mark_columns_needed_for_merge(THD *thd, uint merge_option);
...
class Natural_join_column {
 public:
  ...
  /* FULL JOIN */
  /*
   * Mark for natural join / join ... using fields.
   * Typically, these fields are not accessible via qualified name, i.e.
   * tab.col, form.
   * For ordinary join type, we support for now. However, with current
   * implementation of full join, we cannot support this feature.
   */
  bool is_natural_join_field{false};
  Full_join_field *full_join_field;
#endif
  /*
...
#ifdef HAVE_ZSQL_FULL_JOIN
  Natural_join_column(Full_join_field *field_param, TABLE_LIST *tab);
#endif
  const char *name();
...
struct TABLE_LIST {
...
#ifdef HAVE_ZSQL_MERGE_INTO
  /// Prepare check option for a view used as tartget table in merge into
  /// because the not matched rows will never match the top join cond
  /// (top join is a right join)
  bool prepare_check_option_for_merge_into(THD *thd, bool is_cascaded = false);
#endif /* HAVE_ZSQL_MERGE_INTO */
...
#ifdef HAVE_ZSQL_MERGE_INTO
  void set_merge_into_target_table();
#endif /* HAVE_ZSQL_MERGE_INTO */
...
#ifdef HAVE_ZSQL_MERGE_INTO
  bool is_merge_into_target{false};
#endif /* HAVE_ZSQL_MERGE_INTO */
  ...
  */
  ...
  /* FULL JOIN */
 public:
  // Mark full join converted derived table
  bool is_full_join_derived{false};
  // Field list for full join name resolution
  List<Full_join_field> *full_join_fields_list{nullptr};
  // Full join derived table must know about its children tables
  bool full_join_contains_table(const char *db_name, const char *table_name);
  // If this table is inside a full join nest.
  bool in_full_join_nest();

 private:
  // Set full_join_fields_list for table, map Field's to Full_join_field's.
  void setup_full_join_fields_list();
  // Generate Fields name for full join derived table.
  bool make_full_join_table_column_names(THD *thd);
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  virtual bool is_natural_join_field() { return false; };
#endif
};

#ifdef HAVE_ZSQL_FULL_JOIN
/* FULL JOIN */
class Field_iterator_full_join : public Field_iterator
{
  List_iterator_fast<Full_join_field> fj_fields_it;
  Full_join_field *cur_field_ref;

public:
  Field_iterator_full_join() : cur_field_ref(NULL){}
  void set(TABLE_LIST *table);
  void next();
  bool end_of_fields() { return !cur_field_ref; }
  const char *name() { return cur_field_ref->real_field_name; }
  Item *create_item(THD *thd);
  Field *field() { return cur_field_ref->field; }
  Full_join_field *field_ref() { return cur_field_ref; }
  bool is_natural_join_field()
  {
    return cur_field_ref->is_natural_join_field;
  }
};
#endif /* HAVE_ZSQL_FULL_JOIN */
...
class Field_iterator_natural_join : public Field_iterator {
 ...
 public:
  ...
  bool is_natural_join_field()
  {
    return cur_column_ref->is_natural_join_field;
  };
#endif
};
...
class Field_iterator_table_ref : public Field_iterator {
...
#ifdef HAVE_ZSQL_FULL_JOIN
  Field_iterator_full_join full_join_it;
#endif
 ...
 public:
  ...
  bool is_natural_join_field()
  {
    return field_it->is_natural_join_field();
  }
  Full_join_field *create_full_join_field(THD *thd, Item_ident *item);
  Full_join_field *get_full_join_field();
#endif
};
