...
/*
  common info of name replacement in `start with connect by`.
  Cause of there are many `start with connect by` in one sql stemenet,
    info below are common for each query-expression but can ONLY be accessd
    duaring contextualizement.
*/
class replace_global_info {
 public:

  /* the dst schema&table names to replace, when if_need_replace_schema_table is true
     and prior_expr_flag is true;
  */
  const char *prior_schema = nullptr;
  const char *prior_table = nullptr;

  /* the dst schema&table names to replace, when if_need_replace_schema_table is true
     and prior_expr_flag is false;
  */
  const char *cur_schema = nullptr;
  const char *cur_table = nullptr;
};
...
enum enum_lex_parse_place {
  IN_PARAM_LIST,            // param_list
  IN_AS_STATEMENT,          // as block
  IN_DECLARE_STATEMENT,     // declare block
  IN_FOR_LOOP,              // for loop
  IN_SF_DEFINITION_RETURN,
  IN_PARSE_INVALID          // init val
};
/* #endif */
...
class Table_ident {
  ...
  */
  ...
  void *clone(Parse_tree_transformer *ptt) const;
};
...
class Index_hint {
  ...
  */
  ...
  void *clone(Parse_tree_transformer *) const;
};
...
class SELECT_LEX_UNIT {
  ...
  */
  ...
  Query_result_union *union_result; // Used for UNION, MINUS
  ...
  */
  ...
  /* FULL JOIN */
  // holds full join child table column info, passed from first_select
  List<Full_join_field> *full_join_fields_list{nullptr};
  bool setup_full_join_fields_list(THD *thd, SELECT_LEX *select);
#endif
...
enum class enum_explain_type {
  ...
  EXPLAIN_MINUS,
  EXPLAIN_MINUS_RESULT,
  EXPLAIN_UNION_MINUS_RESULT,
...
class SELECT_LEX {
  ...
  */
  ...
  /**
    record the row item position and row item,
    this just use in setup_fields temporary
  */
  void *row_item_info;
  #endif
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  /**
   For schema.table name replacement for `start with connect by`.
   while if_need_replace_schema_table is true, if prior_expr_flag is false we replace \
     schema and table with cur_schema and cur_table; else prior_schema and prior_table;
  */
  bool if_need_replace_schema_table; //if need to replace name of schema and table
  bool prior_expr_flag; //true means we are in PTI_prior

  bool can_fix_outer_field; //true means able to do fix_outer_field when fix_fields, default true
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  // Store full join fields list
  List<Full_join_field> *full_join_fields_list{nullptr};
  // Parse time full join exists mark.
  bool has_full_join{false};
  /*
   * Test if this is the anti join select of a full join derived table.
   */
  bool is_full_join_first_select()
  {
    return master && master->first_select() == this && master->derived_table
         && master->derived_table->is_full_join_derived;
  }
  // check duplicate table alias for full join derived tables.
  bool check_duplicate_table_alias_for_full_join(THD *thd, TABLE_LIST *tbl);
private:
  // table_list including tables in full join
  List<TABLE_LIST> *table_list_full{nullptr};
  bool alias_duplicate_with_full_join_inner_tables(TABLE_LIST *tbl);
  /*
   * Test if this query block has and needs apply the FULL_JOIN_ANTI hint.
   */
  bool need_apply_full_join_anti_hint(THD *thd);
  bool can_apply_full_join_anti_hint();
  void apply_full_join_anti_hint(THD *thd);
  bool check_and_create_full_join_fields_list(THD *thd,
                                              bool &is_fj_first_select);

public:
#endif
  /**
    List of columns that is in the OF clause:
    select ... from ... for update of m_lock_item_list
  */
  List<Item> m_lock_item_list;
  PT_table_locking_clause *m_table_locking;
  ...
  */
  ...
  bool print_insert_all(const THD *thd, String *str, enum_query_type query_type);
  void print_insert_all_values(const THD *thd, String *str,
          enum_query_type query_type, Sql_cmd_insert_values *tmp_sql_cmd);
  void print_insert_all_select(const THD *thd, String *str,
          enum_query_type query_type, Sql_cmd_insert_select *tmp_sql_cmd);
  ...
  */
  ...
  /**
    Print detail of the MERGE INTO statement.

    @param      thd          Thread handler
    @param[out] str          String of output
    @param      query_type   Options to print out string output
  */
  void print_merge(const THD *thd, String *str, enum_query_type query_type);

  /**
    Print list of items from the input param. Used in MERGE INTO ...

    @param      thd          Thread handle
    @param[out] str          String of output
    @param      query_type   Options to print out string output
    @param[in]  fields       List of items to be print.
  */
  void print_item_list(const THD *thd, String *str, enum_query_type query_type,
                       List<Item> fields);

  /**
    Print "on (condition)". Used in MERGE INTO ...

    @param      thd          Thread handle
    @param[out] str          String of output
    @param      query_type   Options to print out string output
  */
  void print_on_condition(const THD *thd, String *str, enum_query_type query_type);
...
#ifdef HAVE_ZSQL_MERGE_INTO
  /**
    Specially used in MERGE INTO, materialize the source table
    if source table contain the target table.
    Called in SELECT_LEX::resolve_placeholder_tables()
  */
  bool materialize_for_merge_into(THD *thd, TABLE_LIST * table);
#endif /* HAVE_ZSQL_MERGE_INTO */
 ...
 public:
  bool with_rownum;
  Rownum_controller m_Rownum_controller;
  ulonglong m_rownum_option;
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  bool setup_of_clause(THD* thd);
  bool acquire_tables_sw_lock(THD *thd);
  bool check_mdl_lock(TABLE_LIST* cur_table);
  bool get_mdl_lock(THD* thd, TABLE_LIST* cur_table, PT_table_locking_clause* table_locking);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
/* not limited to UNION. It's linkage like UNION, MINUS... */
inline bool SELECT_LEX_UNIT::is_union() const {
  return first_select()->next_select() &&
         (first_select()->next_select()->linkage == UNION_TYPE ||
          first_select()->next_select()->linkage == EXCEPT_TYPE);
}
...
#ifdef HAVE_ZSQL_MERGE_INTO
    /**
       MERGE INTO update clause is unsafe because when there is no stable set,
       choose the first value to make the update.
       For example:
       source table  matches  target table
       record_s               record_t_1
       record_s               record_t_2
       Use record_t_1 to update record_s. This is unsafe.
    */
    BINLOG_STMT_UNSAFE_MERGE_UPDATE,
#endif /* HAVE_ZSQL_MERGE_INTO */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    /**
       Using ROWNUM is unsafe.
    */
    BINLOG_STMT_UNSAFE_ROWNUM,
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
    /**
...
struct LEX : public Query_tables_list {
  ...
  */
  ...
  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  enum_lex_parse_place parse_place;
  ...
  */
  ...
  /**
    Helper pointer to the end of the list when parsing options for
      ...
      LEX::create_field->option_list (for fields)
      LEX::option_list             (for indexes)
  */
  engine_option_value *option_list_last {nullptr};
  ...
  */
  ...
  void push_rgi(replace_global_info rgi) {
    rgis.push_back(rgi);
  }

  void pop_rgi() {
    DBUG_ASSERT(!rgis.empty());
    if(!rgis.empty()) {
      rgis.pop_back();
    }
  }

  replace_global_info current_rgi() {
    if(!rgis.empty()) {
      return rgis.back();
    } else {
      replace_global_info def_rgi;
      return def_rgi;
    }
  }
 ...
 public:
  ...
  /* used for flags introduced in zsql */
  ulonglong zsql_options;

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  /**
    Whether need to close an implicit cursor after 'for loop' statement in SP
  */
  bool need_close_cur;
  LEX_STRING cursor_name_to_close;
  // truncate partition type
  trunc_partition_type trunc_part_type;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */

#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  ulonglong full_join_count = 0;
#endif

#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  /* true if current sql has `start with connect by` */
  bool  is_swcb = false;

  // used in `start with connect by`, save rgi info of subquerys in a query
  std::list<replace_global_info> rgis;
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */

  /* Used for quick_sync, true means this sql has internal_tmp table. */
  bool m_has_internal_tmp_table;
...
class Parser_state {
  ...
  */
  ...
  void quick_sync_reset()
  {
    m_yacc.reset();
    m_comment = false;
    m_input.m_compute_digest = false;
    m_lip.found_semicolon = NULL;
    m_digest_psi = NULL;
  }
