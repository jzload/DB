...
class Query_arena {
 ...
 public:
  ...
  MEM_ROOT *sp_mem_root;  // Pointer to current memroot
  Item *sp_item_list;
 ...
 public:
  Query_arena(MEM_ROOT *mem_root_arg, enum enum_state state_arg)
      : m_item_list(nullptr), mem_root(mem_root_arg), sp_mem_root(NULL),
        sp_item_list(NULL), state(state_arg) {}
...
class THD : public MDL_context_owner,
  ...
  */
  ...
  std::atomic<int32> quick_sync_refcount;

 public:
  void set_quick_sync()
  {
    quick_sync_refcount.store(2);
  }

  bool is_quick_sync()
  {
    bool ret = false;
    if (quick_sync_refcount.load())
      ret = true;
    return ret;
  }

  int dec_refcount_for_quicksync()
  {
    int refcount = 0;
    if (quick_sync_refcount.load())
      refcount = --quick_sync_refcount;
    return refcount;
  }

  int get_quick_sync_refcount()
  {
    return quick_sync_refcount.load();
  }

  void reset_quick_sync_refcount()
  {
    quick_sync_refcount.store(0);
  }
  ...
  */
  ...
  THD_event_functions *func_scheduler;
 ...
 public:
  ...
  /*the time value is start time*/
  uint64_t  req_wait_time, pre_dispatch_time, sql_dispatch_time;
  uint64_t  sql_parse_time, sql_execute_time;
  /*the time value is time interval*/
  uint64_t  exec_prep_time, mdl_req_time, innodblock_req_time, open_table_time;
  uint64_t  order_commit_time, flush_time, sync_time, commit_time;
  uint64_t  ack_wait_time, commit_queue_wait_time;

  bool sp_exists_endmark;
  ...
  */
  ...
  /**
    ...
    group commit.
    The org_value is set NULL in the begin of order_commit, but we should set NULL after
    finish_commit, otherwish the show quicksync status will coredump.
  */
  THD *next_to_commit;
  ...
  */
  ...
  void set_prev_trans_end_pos(my_off_t binlog_pos)
  {
    m_prev_trans_end_pos = binlog_pos;
  }
  
  my_off_t get_prev_trans_end_pos() { return m_prev_trans_end_pos; }
  ...
  */
  ...
  Parser_state *m_quick_parser_state;
  ...
  */
  ...
  void init_slow_log_time()
  {
    ...
    exec_prep_time = 0;
    mdl_req_time = 0;
    innodblock_req_time = 0;
    ack_wait_time = 0;
    order_commit_time = 0;
    commit_queue_wait_time = 0;
    open_table_time = 0;
    flush_time = 0;
    sync_time = 0;
    commit_time=0;
  ...
  */
  void update_slow_query_status() {
    ...
    if(req_wait_time != 0) /*if equal 0, it is not pushback queue*/
    {
      if(end_utime_of_query > req_wait_time + variables.long_query_time)
      {
          server_status|= SERVER_QUERY_WAS_SLOW;
      }
    }
    else if(sql_parse_time != 0)/*some time is 0,like administrator command*/
    {
      if(end_utime_of_query > sql_parse_time + variables.long_query_time)
      {
          server_status|= SERVER_QUERY_WAS_SLOW;
      }
    }
    else if(end_utime_of_query > utime_after_lock + variables.long_query_time)
    {
      server_status|= SERVER_QUERY_WAS_SLOW;
    }
  }
  ...
  */
  ...
  void set_sp_mem_root(MEM_ROOT *tmp_mem_root);
#endif
  ...
  */
  ...
  void set_curtrans_quick_sync() {m_quick_sync_mode = true;}
  void set_maybe_quick_sync() {m_mabye_quick_sync_mode = true;}
  void set_trx_chain_and_release(bool tx_chain, bool tx_release)
  {
    m_tx_chain = tx_chain;
    m_tx_release = tx_release;
  }

  bool is_maybe_quick_sync()
  {
      return m_mabye_quick_sync_mode;
  }

  bool is_curtrans_quick_sync()
  {
      if (!m_mabye_quick_sync_mode)
          return false;
      return m_quick_sync_mode;
  }

  bool is_real_quick_sync()
  {
      if (is_curtrans_quick_sync() && is_quick_sync())
         return true;

      return false;
  }

  // not multi statements then return false
  // middle of multi_statements then return false
  // just first and end of multi_statements return true
  bool middle_of_multi_statements()
  {
      if (m_multi_statements & SERVER_MULTI_STATEMENTS_MIDDLE)
        return true;

      return false;
  }

  void reset_maybe_quick_sync()
  {
    m_mabye_quick_sync_mode = false;
  }

  void reset_quick_sync()
  {
      // dont rest original group_quick_sync
      m_mabye_quick_sync_mode = false;
      m_quick_sync_mode = false;
      /* the no_quicksync leader will be set false after finish_commit.*/
      m_leader_thread = false;
      reset_quick_sync_refcount();
  }

  void reset_quick_sync_for_next_stmt()
  {
      m_quick_sync_commit_queue = NULL;
      m_next_leader = NULL;
      next_to_commit = NULL;
      m_do_rotate = false;
      m_need_clear_owned_gtid = false;
      m_quick_run_slave_post_commit = false;
      m_quick_mgr_switched = false;
      m_quick_mgr_ticket = nullptr;
      m_quick_mgr_cur_ticket = nullptr;
      m_quick_mgr_ptr = nullptr;
      m_quick_mgr_src_res_grp = nullptr;
      m_quick_mgr_dest_res_grp = nullptr;
      m_quick_orig_query = NULL_CSTR;
      m_quick_query_str = nullptr;
      m_quick_query_len = 0;
      m_quick_sync_ticket = NULL;
      m_trx_type = Transaction_ctx::STMT;
      m_tx_chain = 0;
      m_tx_release = 0;
      m_first_level = false;
      m_mabye_quick_sync_mode = false;
      ...
      m_quick_sync_err = 0;
      m_leader_thread = false;
      m_need_execute = false;
      m_session_stage = 0;
      m_group_quick_sync = 0;
      added_to_super_queue = 0;
      removed_from_super_queue = 0;
      server_status &= ~(SERVER_STATUS_GROUP_BELOW_LWM | SERVER_STATUS_GROUP_BELOW_HWM);
      m_ordered_commit = 0;
  }

  void reset_all_quick_sync_flags();
...
// add by zte team
public:
  THD *m_quick_sync_commit_queue;
  THD *m_next_leader;  // several leader thread share one tranxnode
  bool m_do_rotate;
  bool m_need_clear_owned_gtid; // use in clearup_after_ha_commit_trans
  bool m_quick_run_slave_post_commit; // use in clearup_after_ha_commit_trans
  bool m_quick_mgr_switched;
  MDL_ticket *m_quick_mgr_ticket;
  MDL_ticket *m_quick_mgr_cur_ticket;
  resourcegroups::Resource_group_mgr *m_quick_mgr_ptr;
  resourcegroups::Resource_group *m_quick_mgr_src_res_grp;
  resourcegroups::Resource_group *m_quick_mgr_dest_res_grp;
  Secondary_engine_optimization m_quick_saved_secondary_engine;
  LEX_CSTRING m_quick_orig_query;
  const char *m_quick_query_str;
  size_t m_quick_query_len;

  MDL_ticket  *m_quick_sync_ticket;
  Transaction_ctx::enum_trx_scope m_trx_type;

  // mysql_execute_command--trans_commit
  bool m_tx_chain;
  bool m_tx_release;
  bool m_first_level;
  bool m_released_thread;
  int m_quick_sync_err;
  char m_multi_statements;
  bool m_leader_thread;
  bool m_need_execute;
  int   m_session_stage;
  const char *m_multi_statements_end_pos;
  bool  m_group_quick_sync;
  bool added_to_super_queue;
  bool removed_from_super_queue;
  bool executing_prepare_stmt;
  bool m_ordered_commit;

#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
  char m_trx_serial_num[MAX_TSN_LEN+1];
  char m_trx_gtm_gtid[MAX_GTID_LEN+1];
  char m_stmt_serial_num[MAX_TSN_LEN+1];
  char m_stmt_gtm_gtid[MAX_GTID_LEN+1];

  void set_stmt_serial_num(const char* serial_num_str, size_t len);
  void set_stmt_gtm_gtid(const char* gtm_gtid_str, size_t len);
  const char* get_trx_serial_num();
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
  const char* get_trx_gtm_gtid() const;
  //for readview open

  void *get_active_gtmgtids(int64 *len) const;
 #endif
  void reset_stmt_gdb_hint_info();
  void reset_trx_gdb_hint_info();
  void copy_gdb_hint_from_stmt_to_trx();
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT

private:
  bool m_mabye_quick_sync_mode;
  bool m_quick_sync_mode;
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
public:
//save the active gtmgtids
using gtmgtid_slice_t = std::pair<int64, void*>;
gtmgtid_slice_t active_gtmgtids;
#endif
public:
  void set_open_table_time(bool set_start_time);
