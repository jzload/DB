...
class Unique_on_insert {
  ...
  */
  ...
  /**
    Check whether a row id which has been stored
    This happens in MERGE INTO command update procedure when the pk is updated.

    @param ptr pointer to the new rowid

    @returns
      false  update success
      true   duplicate or error
  */
  bool unique_check(void *ptr);
