...
static const char K_MERGE_INTO_UPDATE_WHERE_SUBQUERIES[] = "update_clause_where_subqueries";
static const char K_MERGE_INTO_INSERT_WHERE_SUBQUERIES[] = "insert_clause_where_subqueries";
...
#ifdef HAVE_ZSQL_MERGE_INTO
  SQ_MERGE_INTO_UPDATE_WHERE,  ///< MERGE INTO ... UPDATE WHERE clause subqueries
  SQ_MERGE_INTO_INSERT_WHERE,  ///< MERGE INTO ... INSERT WHERE clause subqueries
#endif /* HAVE_ZSQL_MERGE_INTO */
...
static const char *list_names[SQ_total] = {
    ...
    K_HAVING_SUBQUERIES,
#ifdef HAVE_ZSQL_MERGE_INTO
    K_MERGE_INTO_UPDATE_WHERE_SUBQUERIES,
    K_MERGE_INTO_INSERT_WHERE_SUBQUERIES,
#endif /* HAVE_ZSQL_MERGE_INTO */
...
bool Explain_format_JSON::begin_context(enum_parsing_context ctx,
  ...
  switch (ctx) {
...
#ifdef HAVE_ZSQL_MERGE_INTO
                  current_context->type == CTX_MERGE_INTO_UPDATE_WHERE ||
                  current_context->type == CTX_MERGE_INTO_INSERT_WHERE ||
#endif /* HAVE_ZSQL_MERGE_INTO */
    ...
    case CTX_MERGE_INTO_UPDATE_WHERE: {
      subquery_ctx *ctx =
          new (*THR_MALLOC) subquery_ctx(CTX_MERGE_INTO_UPDATE_WHERE, NULL, current_context);
      if (ctx == NULL || current_context->add_subquery(SQ_MERGE_INTO_UPDATE_WHERE, ctx))
        return true;
      current_context = ctx;
      break;
    }
    case CTX_MERGE_INTO_INSERT_WHERE: {
      subquery_ctx *ctx =
          new (*THR_MALLOC) subquery_ctx(CTX_MERGE_INTO_INSERT_WHERE, NULL, current_context);
      if (ctx == NULL || current_context->add_subquery(SQ_MERGE_INTO_INSERT_WHERE, ctx))
        return true;
      current_context = ctx;
      break;
    }
