...
#include "sql/zsql_features/minus/minus.h"    // is_minus_query_block, delete_row_for_minus
...
bool MaterializeIterator::Init() {
  ...
  if (m_unit != nullptr && m_unit->is_recursive()) {
    ...
    bool has_minus = get_last_minus_query_block(m_query_blocks_to_materialize,
                                                last_minus_qb_index);
    if (has_minus) {
      if (MaterializeQueryBlocksWithMinus(last_minus_qb_index)) {
        return true;
      }
    } else
...
#ifdef HAVE_ZSQL_MINUS
  bool is_minus = is_minus_query_block(query_block);
#endif // HAVE_ZSQL_MINUS

  PFSBatchMode pfs_batch_mode(query_block.subquery_iterator.get());
  while (*stored_rows < m_limit_rows) {
#ifdef HAVE_ZSQL_MINUS
    if (is_minus && *stored_rows == 0) {
      /* empty set minus this query block, return early. */
      return false;
    }
#endif // HAVE_ZSQL_MINUS
...
#ifdef HAVE_ZSQL_MINUS
    if (is_minus) {
      error = delete_row_for_minus(table(), stored_rows);
      if (error != 0) {
        /* error, unlikely */
...
#ifdef HAVE_ZSQL_TEMPTABLE
      if (m_ondisk_tmp_table &&
          thd()->variables.tmp_table_max_rows &&
          *stored_rows > thd()->variables.tmp_table_max_rows)
      {
...
#ifdef HAVE_ZSQL_TEMPTABLE
      //check if rowcount exceed the limit of temp_table_max_rows
      if (m_ondisk_tmp_table &&
          thd()->variables.tmp_table_max_rows &&
          *stored_rows > thd()->variables.tmp_table_max_rows)
      {
...
vector<string> MaterializeIterator::DebugString() const {
  ...
  if (m_cte != nullptr && m_cte->recursive) {
    ...
    Query_Blocks_Linkage_Type linkage_type =
      get_linkage_type_by_array(m_query_blocks_to_materialize);

    if (linkage_type == Query_Blocks_Linkage_Type::ALL_ARE_MINUS)
      str = "Minus materialize";
    else if (linkage_type == Query_Blocks_Linkage_Type::ALL_ARE_MISC)
      str = "Union/Minus materialize";
    else
#endif // HAVE_ZSQL_MINUS
...
bool TemptableAggregateIterator::Init() {
  ...
  Opt_trace_array trace_steps(trace, "steps");
#ifdef HAVE_ZSQL_TEMPTABLE
  uint64_t m_rows_in_table{0};
#endif /* HAVE_ZSQL_ERROR_ON_BIG_TEMPTABLE */
...
#ifdef HAVE_ZSQL_TEMPTABLE
    ++m_rows_in_table;
    //check if the count of row exeed the temp_table_max_rows limit
    if (m_ondisk_tmp_table &&
        thd()->variables.tmp_table_max_rows &&
