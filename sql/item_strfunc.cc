...
String *Item_func_error_code::val_str(String *) {
  null_value = 1;
  if (init) {
    null_value = 0;
    str_value.set_int(m_sql_err_no, true, system_charset_info);
  }
  return &str_value;
}

String *Item_func_error_message::val_str(String *) {
  null_value = 1;
  if (init) {
    null_value = 0;
    str_value.length(0);
    str_value.append(m_message,strlen(m_message));
  }
  return &str_value;
}
/* #endif */
