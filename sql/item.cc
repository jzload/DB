...
String *Item::val_string_from_decimal(String *str) {
  ...
  if (null_value || NULL == dec) return error_str();

  uint8 fraction = decimals;
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (check_item_option(HAVE_TO_NUMBER_ITEM))
    fraction = get_actual_fraction_dec_val(dec);
#endif/* HAVE_ZSQL_ORACLE_COMPATIBILITY */

  my_decimal_round(E_DEC_FATAL_ERROR, dec, fraction, false, &dec_buf);
...
/* used only in Field_iterator_full_join::create_item() */
Item_field::Item_field(THD *thd, Name_resolution_context *context_arg, Field *f,
                        const char *alias)
    : Item_ident(context_arg, f->table->s->db.str, *f->table_name,
                 f->field_name),
  ...
  */
  {
    ...
    /*
      ...
      i.e. Field_iterator_full_join::create_item()
    */
    item_name.set(alias);
  }
  set_field(f);
...
static Item **find_field_in_group_list(Item *find_item, ORDER *group_list) {
  ...
  for (ORDER *cur_group = group_list; cur_group; cur_group = cur_group->next) {
    if ((*(cur_group->item))->real_item()->type() == Item::FIELD_ITEM) {
      ...
      const char *cur_field_name = cur_field->field_name,
                 *cur_table_name = cur_field->table_name,
                 *cur_db_name = cur_field->db_name;
#ifdef HAVE_ZSQL_FULL_JOIN
      get_full_join_field_name_from_item(cur_field, &cur_field_name,
                                         &cur_table_name, &cur_db_name);
#endif

      DBUG_ASSERT(cur_field_name != 0);

      if (!my_strcasecmp(system_charset_info, cur_field_name,
                         field_name))
        ++cur_match_degree;
      else
        continue;
      ...
      if (cur_table_name && table_name) {
        ...
        if (my_strcasecmp(table_alias_charset, cur_table_name,
                          table_name))
          /* Same field names, different tables. */
          return NULL;

        ++cur_match_degree;
        if (cur_db_name && db_name) {
          /* If field_name is also qualified by a database name. */
          if (strcmp(cur_db_name, db_name))
...
int Item_field::fix_outer_field(THD *thd, Field **from_field,
                                Item **reference) {
  if(!thd->lex->current_select()->can_fix_outer_field) {
    my_error(ER_BAD_FIELD_ERROR, MYF(0), this->full_name(), thd->where);
    return -1; // don't find in outer select
  }
  ...
  if (!field)  // If field is not checked
  {
    ...
    */
    ...
    /*
      ...
      or, this is an fake ident justed used to validate names
    */
    DBUG_ASSERT(
        from_field != view_ref_found ||
        context->select_lex ==
            dynamic_cast<Item_ident *>(*reference)->context->select_lex ||
        name_replace_info.fake_ident_flag);
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
      if(this->check_replace_schema_table_valid(thd)) {
        goto error;
      }
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  if(this->check_replace_schema_table_valid(thd)) {
    my_error(ER_START_WITH_CONNECT_BY, MYF(0), "check schema/table of field failed");
    goto error;
  }
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (check_item_option(HAVE_TO_NUMBER_ITEM))
    fraction = get_actual_fraction_dec_val(&decimal_value);
#endif/* HAVE_ZSQL_ORACLE_COMPATIBILITY */

  my_decimal_round(E_DEC_FATAL_ERROR, &decimal_value, fraction, false,
                   &decimal_value);
...
/* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
Item_sp_notation_param::Item_sp_notation_param(const POS &pos,
                                               THD  *thd,
                                               Item *param_value,
                                               const char *param_name,
                                               uint  param_name_length)
                        :Item(pos), m_thd(thd),m_value(param_value) {
  m_name.str = strmake_root(thd->mem_root, param_name, param_name_length);
  m_name.length = param_name_length;
}

bool Item_sp_notation_param::fix_fields(THD *thd, Item **) {
  m_value->fix_fields(thd, &m_value);
  fixed = 1;
  return false;
}
/* #endif */
