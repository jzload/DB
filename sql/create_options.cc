...
LEX_CSTRING table_type_str = {STRING_WITH_LEN("table_type")};
LEX_CSTRING connect_flag_str = {STRING_WITH_LEN("FLAG")};
                               ...
                               bool suppress_warning)
{
  ...
  push_warning_printf(thd, Sql_condition::SL_WARNING, ER_BAD_OPTION_VALUE,
      ...
      *val= (uint) opt->def_value;
      ...
      while (*start)
      {
        ...
        if (!my_strnncoll(system_charset_info,
                          reinterpret_cast<uchar*>(const_cast<char*>(start)), end-start,
                       ...
                       bool suppress_warning, MEM_ROOT *root)
{
  ...
  if (rules)
  {
    ...
    *option_struct= root->Alloc(option_struct_size);
  }
  ...
  for (opt= rules; rules && opt->name; opt++)
  {
    ...
    for (val= *option_list; val; val= val->next)
    {
      last= val;
      if (my_strnncoll(system_charset_info,
                       reinterpret_cast<uchar*>(const_cast<char*>(opt->name)), opt->name_length,
      ...
      */
          ...
          (thd->lex->sql_command == SQLCOM_CREATE_TABLE || seen))
      {
        ...
        if (!sysvar->session_is_default(thd))
        {
          ...
          if ((str= sysvar->val_str(&sbuf, thd, OPT_SESSION, const_cast<LEX_STRING*>(&null_lex_str))))
          {
...
*/
static bool resolve_sysvars(handlerton *hton MY_ATTRIBUTE((unused)), ha_create_table_option *rules)
{
  for (ha_create_table_option *opt= rules; rules && opt->name; opt++)
  {
    if (opt->type == HA_OPTION_TYPE_SYSVAR)
    {
      ...
      case GET_ENUM:
      {
        ...
        opt->values= my_strndup(0, str.ptr(), str.length()-1, MYF(MY_WME));
        if (!opt->values)
          return 1;
        break;
      }
...
*/
static void free_sysvars(handlerton *hton MY_ATTRIBUTE((unused)), ha_create_table_option *rules)
{
  for (ha_create_table_option *opt= rules; rules && opt->name; opt++)
  {
    if (opt->var)
...
uint option_list_frm_length(engine_option_value *opt)
{
  uint res= 0;
...
*/
uchar *option_list_frm_image(uchar *buff, engine_option_value *opt)
{
  for (; opt; opt= opt->next)
...
*/
  if (buff < buff_end)
    sql_print_warning("Table '%s' was created in a later version - "
...
engine_option_value *merge_engine_table_options(engine_option_value *first,
                                                ...
                                                MEM_ROOT *root)
{
  engine_option_value *end=nullptr, *opt;
                            ...
                            ha_create_table_option *rules)
{
  ...
  {
      if (!my_strnncoll(system_charset_info,
                        reinterpret_cast<uchar*>(const_cast<char*>(rules->name)), rules->name_length,
                        (uchar*)opt->name.str, opt->name.length))
        return true;
  }
  return false;
