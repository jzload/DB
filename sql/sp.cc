...
//used to generate the spname index of sp
std::atomic_int generate_spname_index(0);
...
Stored_routine_creation_ctx *
Stored_routine_creation_ctx::create_routine_creation_ctx(
    const dd::Object_id client_collation_id,
    const dd::Object_id connection_collation_id,
    const dd::Object_id schema_collation_id) {
  /* Load character set/collation attributes. */
  const CHARSET_INFO *client_cs =
      dd_get_mysql_charset(client_collation_id);
  const CHARSET_INFO *connection_cl =
      dd_get_mysql_charset(connection_collation_id);
  const CHARSET_INFO *db_cl =
      dd_get_mysql_charset(schema_collation_id);
...
static enum_sp_return_code db_find_routine(THD *thd, enum_sp_type type,
  ...
  *sphp = NULL;  // In case of errors
  ...
  if (thd->variables.m_opt_parse_mode == ORACLE_PARSE_MODE)
    params_str = routine->parameter_str();
  else
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
/*
 * Added for SQLCOM_DECL_BLOCK, representing 'declare begin end' block,
 * duaring create_routine, some values of sp are changed.
 * so, do the same thing before execute sp
 */
bool sp_adjust_info_for_execute(THD *thd, sp_head *sp) {
  if(sp->m_chistics->daccess == SP_DEFAULT_ACCESS)
    sp->m_chistics->daccess = SP_DEFAULT_ACCESS_MAPPING;
  if(sp->m_chistics->suid == SP_IS_DEFAULT_SUID)
    sp->m_chistics->suid = SP_DEFAULT_SUID_MAPPING;

  if(!thd->lex->definer) {
    thd->lex->definer = create_default_definer(thd);
    if(!thd->lex->definer)
      return true;
  }
  sp->set_definer(thd->lex->definer->user, thd->lex->definer->host);
...
/**
  ...
  @param thd    Thread context.
  @param type   Stored routine type  (enum_sp_type::PROCEDURE or
...
*/
static enum_sp_return_code sp_drop_routine_internal(THD *thd,
                                                  enum_sp_type type,
                                                  sp_name *name,
                                                  const dd::Routine *routine) {
  DBUG_TRACE;
...
static bool create_routine_precheck(THD *thd, sp_head *sp) {
  ...
  if (sr != nullptr) {
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (thd->lex->oracle_options & ORA_CREATE_OR_REPLACE) {
      if (sp_drop_routine_internal(thd, sp->m_type, thd->lex->spname, sr))
        return true;
    }
    else {
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
sp_head *sp_setup_routine_from_oldsp(THD *thd, enum_sp_type type, sp_head *sp) {
  DBUG_ASSERT(type == enum_sp_type::PROCEDURE);
  ulong level = sp->m_last_cached_sp->m_recursion_level + 1;
  ...
  Stored_program_creation_ctx *creation_ctx =
    ...
    thd->collation()->number);

  const char *returns = "";
  sp_head *new_sp = NULL;
  if (db_load_routine(thd, type, sp->m_db.str, sp->m_db.length,
                      sp->m_name.str, sp->m_name.length, &new_sp,
                      sp->m_sql_mode, sp->m_params.str, returns, sp->m_body.str,
                      sp->m_chistics, sp->m_definer_user.str,
                      sp->m_definer_host.str, sp->m_created, sp->m_modified,
                      creation_ctx))
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  /* standalone Declare block in Oracle, append 'as' */
  if(0 == strncmp(name, INNER_SP_NAME_HEADER, strlen(INNER_SP_NAME_HEADER)))
  {
    buf->append(STRING_WITH_LEN(" AS \n"));
    buf->append(STRING_WITH_LEN(" BEGIN \n"));
    buf->append(body, bodylen);
    buf->append(STRING_WITH_LEN(";\n"));
    buf->append(STRING_WITH_LEN(" END"));
  } else
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  {
    buf->append(body, bodylen);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
/*
 * for declare begin end block at the head of statements, make it an anonymous sp,
 * of which name is generated here.
 */
sp_name *sp_generate_inner_name(THD *thd) {
  sp_name *result = NULL;
  LEX *lex= thd->lex;
  LEX_STRING db;
  LEX_STRING preuse_sp_name;
  char preuse_sp_strname[64] = {0};
  size_t preuse_spname_length = 0;
  char inner_sp_default_db[16] = "mysql";

  //won't be so long
  int cur_spname_index = generate_spname_index.fetch_add(1, std::memory_order_relaxed);
  snprintf(preuse_sp_strname, sizeof(preuse_sp_strname), "%s%d", INNER_SP_NAME_HEADER, cur_spname_index);

  preuse_spname_length = strlen(preuse_sp_strname);
  preuse_sp_name.str = \
    strmake_root(thd->mem_root, preuse_sp_strname, preuse_spname_length);
  preuse_sp_name.length = preuse_spname_length;
  if (unlikely(sp_check_name(&preuse_sp_name)))
  {
    return NULL;
  }
  if (lex->copy_db_to(&db.str, &db.length)) {
    db.str = strmake_root(thd->mem_root, inner_sp_default_db, strlen(inner_sp_default_db));
    if(!db.str)
      return NULL;
    db.length = strlen(db.str);
  }
  result= new(thd->mem_root) sp_name(to_lex_cstring(db), preuse_sp_name, false);
  if(NULL != result)
    result->init_qname(thd);
  return result;
}

/**
 * Generate a name for 'declare block' and start parsing the procedure
*/
sp_head *sp_start_parsing_noname(THD *thd, enum_sp_type sp_type, sp_name **out_sp_name) {
  if(NULL == out_sp_name)
    return NULL;
  *out_sp_name = sp_generate_inner_name(thd);
  if(NULL == *out_sp_name)
    return NULL;
  return sp_start_parsing(thd, sp_type, *out_sp_name);
}
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
uint sp_get_flags_for_command(LEX *lex) {
  ...
  switch (lex->sql_command) {
    ...
    case SQLCOM_REPAIR:
    case SQLCOM_EXECUTE_IMMEDIATE:  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
