...
class Item_func_cursor_attribute;
class Item_func_error_code;
class Item_func_error_message;

#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
// cursor_name and its sp_instr_cpush
typedef struct {
...
class sp_pcontext {
 ...
 public:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  const char *m_cursor_value_name; // just for loop
  /* #endif */
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  const Mem_root_array<sp_variable *> *get_vars()  {
    return (const Mem_root_array<sp_variable *>*)&m_vars;
  }

  uint get_handlers_cursors_number(uint *handlers, uint *cursors);

  void set_exception_flag(bool with_exception) {
    m_with_exception = with_exception;
  }

  bool is_with_exception() const { return m_with_exception; }

  /// find variable and return index
  sp_variable *find_variable(LEX_STRING name,
                             uint *idx,
                             bool current_scope_only) const;

  sp_label *find_label(LEX_STRING name,sp_label::enum_type lab_type);

  int push_cursor_instr(LEX_STRING *cursor_name, sp_instr *i, uint offset);

  void push_cursor_attribute(Item_func_cursor_attribute *cursor_attr) {
    m_cursor_attr_list.push_back(cursor_attr);
  }

  sp_instr_cpush *get_cpush_by_cursor_name(LEX_STRING cursor_name,
                                           bool current_scope);

  List<Item_func_cursor_attribute> *get_cursor_attr_list() {
    return &m_cursor_attr_list;
  }

  Mem_root_array<sp_pcontext *> *get_children() { return &m_children; }

  void push_item_error_code(Item_func_error_code *item_error_code) {
    // if (m_scope == HANDLER_SCOPE)
    m_error_code_list.push_back(item_error_code);
  }

  void push_item_error_message(Item_func_error_message *item_error_message) {
    //if (m_scope == HANDLER_SCOPE)
    m_error_message_list.push_back(item_error_message);
  }

  bool is_handler_scope() {
    return (m_scope == HANDLER_SCOPE);
  }

  List<Item_func_error_code> *get_error_code_list() {
    return &m_error_code_list;
  }

  List<Item_func_error_message> *get_error_message_list() {
    return &m_error_message_list;
  }

  sp_label *find_label_by_index(uint dest_index, uint *cur_index,
                                sp_label::enum_type lab_type);
  sp_label *get_recent_lable_accord_type(sp_label::enum_type lab_type);

  uint GetConditionsSize() const { return m_conditions.size(); }
  /* #endif */
 ...
 private:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  List<cursor_instr_t> m_cursor_instr_list;
  List<Item_func_cursor_attribute> m_cursor_attr_list;

  /// compatible oracle exception
  bool m_with_exception;
  List<Item_func_error_code> m_error_code_list;
  List<Item_func_error_message> m_error_message_list;
  /* #endif */
};
