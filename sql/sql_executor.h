...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  inline bool match_rownum_condition(THD *thd, bool &match_rownum_cond) {
    match_rownum_cond = rownum_condition() ? 
                         rownum_condition()->val_int() == true : true;
    return thd->is_error();
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
