...
handlerton *ha_checktype(THD *thd, enum legacy_db_type database_type,
  ...
  (void)RUN_HOOK_INLINE(transaction, after_rollback, (thd, false));
...
// where to get all param
bool cleanup_after_ha_commit_trans(THD *thd, bool error)
{
  DBUG_ENTER("cleanup_after_ha_commit_trans");
...
*/
  ...
  if (thd->m_quick_sync_ticket)
  {
    ...
    */
    ...
    thd->mdl_context.release_lock(thd->m_quick_sync_ticket);
  }
  ...
  if (thd->m_need_clear_owned_gtid)
  {
    thd->server_status&= ~SERVER_STATUS_IN_TRANS;
    /*
  ...
  if (thd->m_quick_run_slave_post_commit) {
...
int ha_commit_trans(THD *thd, bool all, bool ignore_global_read_lock) {
  ...
  */
  ...
  thd->m_need_clear_owned_gtid = need_clear_owned_gtid;
  ...
  */
  if (is_real_trans && is_atomic_ddl_commit_on_slave(thd)) {
    ...
    */
    ...
    thd->m_quick_run_slave_post_commit = run_slave_post_commit;
    ...
    */
    ...
    if (rw_trans && !ignore_global_read_lock) {
      ...
      */
      ...
      thd->m_quick_sync_ticket = mdl_request.ticket;
      release_mdl = true;
...
int ha_commit_low(THD *thd, bool all, bool run_after_commit) {
  ...
  */
  ...
  if (run_after_commit && thd->get_transaction()->m_flags.run_hooks) {
    ...
    */
    if (!error) (void)RUN_HOOK_INLINE(transaction, after_commit, (thd, all));
...
int ha_rollback_low(THD *thd, bool all) {
  ...
  (void)RUN_HOOK_INLINE(transaction, before_rollback, (thd, all));
  ...
  */
  ...
  (void)RUN_HOOK_INLINE(transaction, after_rollback, (thd, all));
  return error;
}
...
void handler::print_error(int error, myf errflag) {
  ...
  switch (error) {
    ...
    case HA_ERR_TOO_MANY_LOCKS:
      textno = ER_TOO_MANY_LOCKS;
      break;
#endif // HAVE_ZSQL_LOCK_DEPTH_LIMIT
    case HA_ERR_BIG_TRANSACTION:
      textno = ER_EXISTS_BIG_TRANSACION;
      break;
    default: {
    ...
    */
    if (likely(!(error = write_locked_table_maps(thd)))) {
      ...
      */
      ...
      if (!error
          && thd->variables.transaction_max_binlog_size
          && !is_slave_reapply_thread(thd)
          && thd->has_big_transaction())
      {
        return HA_ERR_BIG_TRANSACTION;
      }
    }
  }
...
int handler::ha_write_row(uchar *buf) {
  ...
  if (
      ...
      NULL == table_share->option_list &&
#endif // HAVE_ZSQL_CONNECT_ENGINE
      unlikely((error = binlog_log_row(table, 0, buf, log_func))))
...
int handler::ha_update_row(const uchar *old_data, uchar *new_data) {
  ...
  if (
      ...
      NULL == table_share->option_list &&
#endif // HAVE_ZSQL_CONNECT_ENGINE
      unlikely((error = binlog_log_row(table, old_data, new_data, log_func))))
    return error;
  return 0;
}
...
int handler::ha_delete_row(const uchar *buf) {
  ...
  */
  ...
  if (
      ...
      NULL == table_share->option_list &&
#endif // HAVE_ZSQL_CONNECT_ENGINE
      unlikely((error = binlog_log_row(table, buf, 0, log_func))))
    return error;
  return 0;
}
