...
static constexpr int num_prepared_stmt_limit = 20 * 1024 * 1024;
...
static Sys_var_ulonglong Sys_transaction_max_binlog_size(
    "transaction_max_binlog_size",
    "Max binlog size per transaction. If exceeded, abort the transaction",
    SESSION_VAR(transaction_max_binlog_size), CMD_LINE(REQUIRED_ARG),
...
static char default_list[FN_REFLEN]= {""};
static Sys_var_gtmgtid_activelist Sys_active_list(
    "active_gtm_gtid_list", "active list of transaction",
    HINT_UPDATEABLE SESSION_VAR(gtmgtid_active_list), CMD_LINE(NO_ARG),
    IN_SYSTEM_CHARSET, DEFAULT(default_list));

static Sys_var_ulonglong Sys_lowwater_mark(
    "next_gtm_gtid",
    "lowwater_mark for next gtid_max",
     HINT_UPDATEABLE SESSION_ONLY(next_gtmgtid), CMD_LINE(NO_ARG),
     VALID_RANGE(0, ULLONG_MAX), DEFAULT(0), BLOCK_SIZE(1));
#endif
...
static Sys_var_ulong Sys_lock_wait_timeout(
    ...
    VALID_RANGE(0, LONG_TIMEOUT), DEFAULT(LONG_TIMEOUT), BLOCK_SIZE(1));
...
static bool fix_kill_idle_transaction(sys_var *, THD *,
                                      enum_var_type)
{
...
static Sys_var_ulong Sys_parse_mode(
    "parse_mode",
    "How db parse query statment",
    SESSION_VAR(m_opt_parse_mode), CMD_LINE(REQUIRED_ARG),
...
static Sys_var_bool Sys_export_null_as_space(
    ...
    SESSION_VAR(export_null_as_space),
    CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_import_space_as_null(
    "import_space_as_null",
    "When import data, replace 0 length space as null",
    SESSION_VAR(import_space_as_null),
    CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(0), ON_UPDATE(0));
...
static Sys_var_uint Sys_threadpool_size(
 ...
 "executing threads (threads in a waiting state do not count as executing).",
  ...
  VALID_RANGE(1, MAX_THREAD_GROUPS), DEFAULT(64), BLOCK_SIZE(1),
  NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
...
static Sys_var_ulonglong Sys_tmp_table_max_rows(
    "tmp_table_max_rows",
    "If an internal in-memory temporary table exceeds this size, MySQL "
    "abort the current query",
    SESSION_VAR(tmp_table_max_rows), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, (uint64_t)~(intptr)0), DEFAULT(0),
    BLOCK_SIZE(1));
#endif /* HAVE_ZSQL_TEMPTABLE */

/* variable for goldendb */
static char *goldendb_server_version_ptr;
static Sys_var_version Sys_version_goldendb(
       "goldendb_version", "Goldendb server version",
       READ_ONLY GLOBAL_VAR(goldendb_server_version_ptr), NO_CMD_LINE,
       IN_SYSTEM_CHARSET, DEFAULT(goldendb_server_version));

/* gtid_idx mechanism switch*/
static Sys_var_bool Sys_binlog_gtmgtid_index_enabled(
       "enable_binlog_gtmgtid_index",
       "Enable the ${binlog_name}.gtid_idx mechanism.",
       GLOBAL_VAR(opt_gtmgtid_idx),
       CMD_LINE(OPT_ARG), DEFAULT(true));
/************************/
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
static Sys_var_ulong Sys_listagg_max_len(
    "listagg_max_len",
    "The maximum length of the result of function  LISTAGG()",
    HINT_UPDATEABLE SESSION_VAR(m_listagg_max_len), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(4, ULONG_MAX), DEFAULT(4000), BLOCK_SIZE(1));
#endif
...
/******      add for semisync inline optimize      ******/

static bool fix_rpl_semi_sync_master_enabled(sys_var *,
                                             THD *, enum_var_type)
{
...
static Sys_var_bool Sys_rpl_semi_sync_master_enabled(
       "rpl_semi_sync_master_enabled",
       "Enable semi-synchronous replication master (disabled by default). ",
        GLOBAL_VAR(rpl_semi_sync_master_enabled),
        CMD_LINE(OPT_ARG),
        DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(0), ON_UPDATE(fix_rpl_semi_sync_master_enabled));

static bool fix_rpl_semi_sync_master_timeout(sys_var *,
                                             THD *, enum_var_type)
{
  repl_semisync_master->setWaitTimeout(rpl_semi_sync_master_timeout);
  return false;
}
static Sys_var_ulong Sys_rpl_semi_sync_master_timeout(
       "rpl_semi_sync_master_timeout",
       "The timeout value (in ms) for semi-synchronous replication in the master",
        GLOBAL_VAR(rpl_semi_sync_master_timeout),
        CMD_LINE(OPT_ARG),VALID_RANGE(0, ~0L),
        DEFAULT(10000), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(0), ON_UPDATE(fix_rpl_semi_sync_master_timeout));


static bool check_rpl_semi_sync_master_wait_no_slave(sys_var *,
                                                     THD *, set_var *var)
{
  if (!var->value)
    return false; // DEFAULT is ok

  repl_semisync_master->set_wait_no_slave((char)var->save_result.ulonglong_value);
  return false;
}
static Sys_var_bool Sys_rpl_semi_sync_master_wait_no_slave(
       ...
       "Wait until timeout when no semi-synchronous replication slave available (enabled by default). ",
        ...
        CMD_LINE(OPT_ARG),
        DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(check_rpl_semi_sync_master_wait_no_slave),
        ON_UPDATE(0));

static Sys_var_bool Sys_service_available_even_bellow_lwm(
    "service_available_even_bellow_lwm",
    "Whether to allow writing when groups is less than the lwm cond (disabled by default). ",
    GLOBAL_VAR(service_available_even_bellow_lwm),
    CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(0), ON_UPDATE(0));


static bool fix_rpl_semi_sync_master_trace_level(sys_var *,
                                                 THD *, enum_var_type)
{
...
static Sys_var_ulong Sys_rpl_semi_sync_master_trace_level(
       "rpl_semi_sync_master_trace_level",
       "The tracing level for semi-sync replication.",
        GLOBAL_VAR(rpl_semi_sync_master_trace_level),
        CMD_LINE(OPT_ARG),VALID_RANGE(0, ~0L),
        DEFAULT(32), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(0), ON_UPDATE(fix_rpl_semi_sync_master_trace_level));


static const char *wait_point_names[]= {"AFTER_SYNC", "AFTER_COMMIT", NullS};
static bool fix_rpl_semi_sync_master_wait_point(sys_var *,
                                                THD *, enum_var_type)
{
  set_is_quick_sync_enabled();
  return false;
}
static Sys_var_enum Sys_rpl_semi_sync_master_wait_point(
       "rpl_semi_sync_master_wait_point",
       "Semisync can wait for slave ACKs at one of two points,",
       GLOBAL_VAR(rpl_semi_sync_master_wait_point),
       CMD_LINE(OPT_ARG), wait_point_names,
       DEFAULT(WAIT_AFTER_SYNC), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
       ON_UPDATE(fix_rpl_semi_sync_master_wait_point));

static bool check_rpl_semi_sync_master_wait_for_slave_count(sys_var *,
                                                   THD *, set_var *var)
{
  if (!var->value)
    return false; // DEFAULT is ok

  repl_semisync_master->setWaitSlaveCount(var->save_result.ulonglong_value);
  return false;
}
static bool fix_rpl_semi_sync_master_wait_for_slave_count(sys_var *,
                                           THD *, enum_var_type)
{
  rpl_semi_sync_master_wait_for_slave_count = repl_semisync_master->get_wait_slave_count_inline();
  return false;
}
static Sys_var_uint Sys_rpl_semi_sync_master_wait_for_slave_count(
       ...
       "replicated to at least rpl_semi_sync_master_wait_for_slave_count slaves",
        ...
        CMD_LINE(OPT_ARG),VALID_RANGE(1, 65535),
        DEFAULT(1), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(check_rpl_semi_sync_master_wait_for_slave_count),
        ON_UPDATE(fix_rpl_semi_sync_master_wait_for_slave_count));


static bool check_rpl_semi_sync_master_quick_sync_enabled(sys_var *,
                                                   THD *, set_var *var)
{
  if (!var->value)
    return false; // DEFAULT is ok

  if(repl_semisync_master->set_quick_sync_enabled(
                                     var->save_result.ulonglong_value))
    return true;
...
static Sys_var_bool Sys_rpl_semi_sync_master_quick_sync_enabled(
       "rpl_semi_sync_master_quick_sync_enabled",
       "Enable the quick-sync mode(enabled by default, disabled means semi-sync mode). ",
        GLOBAL_VAR(rpl_semi_sync_master_quick_sync_enabled),
        CMD_LINE(OPT_ARG),
        DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(check_rpl_semi_sync_master_quick_sync_enabled),
        ON_UPDATE(0));


static bool fix_rpl_semi_sync_master_inspection_time(sys_var *,
                                             THD *, enum_var_type)
{
  time_inspection->set_inspection_time(rpl_semi_sync_master_inspection_time);
  return false;
}
static Sys_var_ulong Sys_rpl_semi_sync_master_inspection_timeout(
       "rpl_semi_sync_master_inspection_time",
       "The timeout inspection time (in ms) for semi-sync replication in the master",
        GLOBAL_VAR(rpl_semi_sync_master_inspection_time),
        CMD_LINE(OPT_ARG),VALID_RANGE(1, ~0L),
        DEFAULT(1000), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(0), ON_UPDATE(fix_rpl_semi_sync_master_inspection_time));


static bool check_rpl_semi_sync_master_wait_cond_hwm(sys_var *,
                                                    THD *, set_var *var)
{
  if (!var->value)
    return false; // DEFAULT is ok

  if(false == repl_semisync_master->set_wait_cond_hwm(
                           (char*)var->save_result.string_value.str))
    return true;

  return false;
}
static Sys_var_charptr Sys_rpl_semi_sync_master_wait_cond_hwm(
       "rpl_semi_sync_master_wait_cond_hwm",
       "How many slave_groups the events should be replicated to. Semisynchronous "
       "replication master will wait until all events of the transaction are "
       "replicated to at least rpl_semi_sync_master_wait_cond_hwm slaves",
       GLOBAL_VAR(rpl_semi_sync_master_wait_cond_hwm),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_wait_cond_hwm),
       ON_UPDATE(0));


static bool check_rpl_semi_sync_master_wait_cond_lwm(sys_var *,
                                                    THD *, set_var *var)
{
  if (!var->value)
    return false; // DEFAULT is ok

  if(false == repl_semisync_master->set_wait_cond_lwm(
                           (char*)var->save_result.string_value.str))
    return true;

  return false;
}
static Sys_var_charptr Sys_rpl_semi_sync_master_wait_cond_lwm(
       "rpl_semi_sync_master_wait_cond_lwm",
       "How many slave_groups the events should be replicated to. Semisynchronous "
       "replication master will wait until all events of the transaction are "
       "replicated to at least rpl_semi_sync_master_wait_cond_lwm slaves",
       GLOBAL_VAR(rpl_semi_sync_master_wait_cond_lwm),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_wait_cond_lwm),
       ON_UPDATE(0));


static bool check_rpl_semi_sync_master_group_all(sys_var *, THD *,
                                        set_var *var, unsigned int group_id)
{
  if (!var->value)
    return false; // DEFAULT is ok

  if(false == repl_semisync_master->set_ack_group_info(
                         (char*)var->save_result.string_value.str, group_id))
    return true;
  return false;
}
static bool check_rpl_semi_sync_master_group1(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 1);
}
static bool check_rpl_semi_sync_master_group2(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 2);
}
static bool check_rpl_semi_sync_master_group3(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 3);
}
static bool check_rpl_semi_sync_master_group4(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 4);
}
static bool check_rpl_semi_sync_master_group5(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 5);
}
static bool check_rpl_semi_sync_master_group6(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 6);
}
static bool check_rpl_semi_sync_master_group7(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 7);
}
static bool check_rpl_semi_sync_master_group8(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 8);
}
static bool check_rpl_semi_sync_master_group9(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 9);
}
static bool check_rpl_semi_sync_master_group10(sys_var *self,
                                              THD *thd, set_var *var)
{
  return check_rpl_semi_sync_master_group_all(self, thd, var, 10);
}

static Sys_var_charptr Sys_rpl_semi_sync_master_group1(
       "rpl_semi_sync_master_group1",
       "The group_1 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[0]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group1), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group2(
       "rpl_semi_sync_master_group2",
       "The group_2 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[1]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group2), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group3(
       "rpl_semi_sync_master_group3",
       "The group_3 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[2]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group3), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group4(
       "rpl_semi_sync_master_group4",
       "The group_4 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[3]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group4), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group5(
       "rpl_semi_sync_master_group5",
       "The group_5 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[4]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group5), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group6(
       "rpl_semi_sync_master_group6",
       "The group_6 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[5]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group6), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group7(
       "rpl_semi_sync_master_group7",
       "The group_7 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[6]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group7), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group8(
       "rpl_semi_sync_master_group8",
       "The group_8 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[7]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group8), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group9(
       "rpl_semi_sync_master_group9",
       "The group_9 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[8]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group9), ON_UPDATE(0));

static Sys_var_charptr Sys_rpl_semi_sync_master_group10(
       "rpl_semi_sync_master_group10",
       "The group_10 info for quick semi-sync replication.",
       GLOBAL_VAR(rpl_semi_sync_master_group[9]),
       CMD_LINE(OPT_ARG),
       IN_FS_CHARSET, DEFAULT(NULL),
       NO_MUTEX_GUARD, NOT_IN_BINLOG,
       ON_CHECK(check_rpl_semi_sync_master_group10), ON_UPDATE(0));


static bool fix_rpl_semi_sync_slave_enabled(sys_var *,
                                            THD *, enum_var_type)
{
  repl_semisync_slave->setSlaveEnabled(rpl_semi_sync_slave_enabled != 0);
  return false;
}
static Sys_var_bool Sys_rpl_semi_sync_slave_enabled(
       "rpl_semi_sync_slave_enabled",
       "Enable semi-synchronous replication slave (disabled by default). ",
        GLOBAL_VAR(rpl_semi_sync_slave_enabled),
        CMD_LINE(OPT_ARG),
        DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
        ON_CHECK(0), ON_UPDATE(fix_rpl_semi_sync_slave_enabled));


static bool fix_rpl_semi_sync_trace_level(sys_var *,
                                          THD *, enum_var_type)
{
  repl_semisync_slave->setTraceLevel(rpl_semi_sync_slave_trace_level);
  return false;
}
static Sys_var_ulong Sys_rpl_semi_sync_slave_trace_level(
...
static Sys_var_charptr Sys_default_date_format(
    ...
    SESSION_VAR(default_date_format), CMD_LINE(OPT_ARG), IN_FS_CHARSET,
    DEFAULT("YYYY-MM-DD HH24:MI:SS"),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
    ON_UPDATE(0));

static Sys_var_charptr Sys_default_timestamp_format(
    "default_timestamp_format", "Default timestamp format for TO_TIMESTAMP().",
    SESSION_VAR(default_timestamp_format), CMD_LINE(OPT_ARG), IN_FS_CHARSET,
    DEFAULT("YYYY-MM-DD HH24:MI:SS.FF"),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
    ON_UPDATE(0));

static const char *my_nls_length_semantics[] = {"BYTE", "CHAR", NullS};
static Sys_var_enum Sys_nls_length_semantics(
    "nls_length_semantics",
    "Default length semantics to use for VARCHAR2 table columns.",
    SESSION_VAR(nls_length_semantics), CMD_LINE(OPT_ARG),
    my_nls_length_semantics, DEFAULT(SEMANTICS_BYTE));
