  ...
  if (!tables_list &&
    ...
    */
    if (select_lex->cond_value != Item::COND_FALSE &&
      ...
      query_result->set_minus_op(select_lex->linkage == EXCEPT_TYPE);
  ...
  */
  ...
  if (group_list && simple_group) {
    ...
    if (!group_list && !table->is_distinct && order && simple_order &&
        ...
        !(select_lex->m_rownum_option & ROWNUM_IN_SELECT_LIST_OR_HAVING) &&
...
#ifdef HAVE_ZSQL_MINUS
  select_lex->query_result()->set_minus_op(select_lex->linkage == EXCEPT_TYPE);
#endif // HAVE_ZSQL_MINUS
  ...
  if (!end_of_records) {
    ...
    if ((error = table->file->ha_write_row(table->record[0]))) {
      ...
      if (!table->file->is_ignorable_error(error)) {
        ...
        qep_tab[-1].m_tmp_table_on_disk = true;
#endif /* HAVE_ZSQL_TEMPTABLE */
      }
    }
#ifdef HAVE_ZSQL_TEMPTABLE
    qep_tab[-1].m_rows_in_table++;
    if (qep_tab[-1].m_tmp_table_on_disk &&
        thd->variables.tmp_table_max_rows &&
        qep_tab[-1].m_rows_in_table > thd->variables.tmp_table_max_rows) {
      my_error(ER_EXISTS_BIG_TMP_TABLE, MYF(0));
...
#ifdef HAVE_ZSQL_MINUS
        select->query_result()->set_minus_op(select->linkage == EXCEPT_TYPE);
#endif // HAVE_ZSQL_MINUS
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      if (select_lex->with_rownum  && (primary_tables - const_tables) == 1 &&
          !m_rownum_is_pushed_to_sort)
      {

        iterator = NewIterator<RownumIterator>(thd, move(iterator),
                                               where_cond_rownum, select_lex);

        m_rownum_is_pushed_to_sort = true;
      }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (select_lex->with_rownum && !m_rownum_is_pushed_to_sort)
  {
...
#ifdef HAVE_ZSQL_MINUS
    join->select_lex->query_result()->set_minus_op(
                              join->select_lex->linkage == EXCEPT_TYPE);
#endif // HAVE_ZSQL_MINUS
...
static enum_nested_loop_state evaluate_join_record(JOIN *join,
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  reset_rownum_controller_flag(join->select_lex);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  if (condition) {
  ...
  if (found) {
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (join->select_lex->with_rownum && join->plan_is_single_table() &&
        join->qep_tab == qep_tab) {
      if (qep_tab->match_rownum_condition(join->thd, match_rownum_cond))
        return NESTED_LOOP_ERROR;
      if (!match_rownum_cond)
        return NESTED_LOOP_ROWNUM_END;
    }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
    /*
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    increase_rownum_controller(join->select_lex);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
    ...
    if (having_is_true(qep_tab->having)) {
      ...
      if ((error = table->file->ha_write_row(table->record[0]))) {
        ...
        if (create_ondisk_from_heap(join->thd, table, error, true, NULL))
          return NESTED_LOOP_ERROR;  // Not a table_is_full error
#ifdef HAVE_ZSQL_TEMPTABLE
        qep_tab->m_tmp_table_on_disk = true;
#endif /* HAVE_ZSQL_TEMPTABLE */
      }

#ifdef HAVE_ZSQL_TEMPTABLE
       ++qep_tab->m_rows_in_table;
#endif /* HAVE_ZSQL_TEMPTABLE */
...
enum_nested_loop_state QEP_tmp_table::put_record(bool end_of_records) {
  ...
  {
    THD *thd = qep_tab->join()->thd;
    if (qep_tab->m_tmp_table_on_disk &&
        thd && thd->variables.tmp_table_max_rows &&
        qep_tab->m_rows_in_table > thd->variables.tmp_table_max_rows)
    {
      my_error(ER_EXISTS_BIG_TMP_TABLE, MYF(0));
      rc = NESTED_LOOP_ERROR;
    }
