...
*/
double accumulate_statement_cost(const LEX *lex) {
...
*/
bool retry_with_secondary_engine(const THD *thd) {
...
bool Sql_cmd_dml::execute_inner(THD *thd) {
  ...
  Sql_cmd_insert_select *tmp_sql_cmd = dynamic_cast<Sql_cmd_insert_select*>(thd->lex->m_sql_cmd);
  if (NULL != tmp_sql_cmd
      && tmp_sql_cmd->is_insert_all_global)
  {
    return execute_inner_for_insert_all(thd);
  }
...
bool make_join_readinfo(JOIN *join, uint no_jbuf_after) {
  ...
  if (join->where_cond_rownum && join->primary_tables >= 1)
  {
    QEP_TAB *const qep_tab = &join->qep_tab[join->primary_tables - 1];
    qep_tab->set_rownum_condition(join->where_cond_rownum);
  }
  ...
  */
  if (need_tmp_before_win) {
    ...
    */
    ...
    /*
      When ROWNUM in select list or having cond and window funcs set up tmp tables,
      instead of stream read, a materialization is needed,
      because the having_cond is always pushed to the first win table.
    */
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (m_windowing_steps && !qep_tab[curr_tmp_table].temporary_table_deduplicates())
    {
      qep_tab[curr_tmp_table].set_temporary_table_deduplicates(
