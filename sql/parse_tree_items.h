...
extern bool pass_item_replace_info(Item *dst, Item *src);
/*
extern void get_replaced_schema_table_name(const char *prev_schema_name,
...
private:
  void fix_db_table_name(Parse_context *pc); //HAVE_ZSQL_START_WITH_CONNECT_BY
};
...
class PTI_simple_ident_ident : public Parse_tree_item {
 ...
 public: /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  LEX_CSTRING ident;
  Symbol_location raw;
 ...
 public:
  ...
  bool b_fielditem_simple() const override { return true; }
  /* #endif */
  void *clone(Parse_tree_transformer *ptt) const override; //HAVE_ZSQL_ORACLE_COMPATIBILITY
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
class PTI_prior_item : public Parse_tree_item {
  typedef Parse_tree_item super;

  Item *item;

 public:
  PTI_prior_item(const POS &pos, Item *item_arg)
      : super(pos), item(item_arg) {}

  bool itemize(Parse_context *pc, Item **res) override {
    //if_need_replace_schema_table should be set previously!!
    if(!pc->select->if_need_replace_schema_table) {
      my_error(ER_START_WITH_CONNECT_BY, MYF(0), "replace flag has not been set");
      return true;
    }
    pc->select->prior_expr_flag = true;
    bool itemize_res = item->itemize(pc, res);
    pc->select->prior_expr_flag = false;
    return itemize_res;
  }
  void *clone(Parse_tree_transformer *ptt) const override;
};
...
class PTI_udf_expr : public Parse_tree_item {
 ...
 public:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  Item *get_item() const { return expr; }
  /* #endif */
  void *clone(Parse_tree_transformer *ptt) const override; //HAVE_ZSQL_ORACLE_COMPATIBILITY
};
...
class PTI_text_literal : public Item_string {
 ...
 protected:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
 public:
  LEX_STRING get_literal() const { return literal; }
  void set_literal(LEX_STRING &lstr) { literal = lstr; }
  /* #endif */
};
...
class PTI_context : public Parse_tree_item {
 ...
 public:
  ...
  bool itemize(Parse_context *pc, Item **res) override {
    ...
    if (pc->select->parsing_place != CTX_MERGE_INTO_UPDATE_WHERE &&
      pc->select->parsing_place != CTX_MERGE_INTO_INSERT_WHERE)
