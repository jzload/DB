...
/*
  @Desc: Check constraints about gdb_format, called at the end of prepare()
  @Return: true means check failed, false means ok
*/
bool Query_result_export::check_constraint() {
  //If `ecsv` is used, enclosed cannot be empty!
  if(exchange->datapump.pump_type == PUMP_EXPORT_ECSV &&
      (!exchange->field.enclosed || !exchange->field.enclosed->length())) {
    my_error(ER_WRONG_USAGE_GDB_FORMAT, MYF(0), \
        "field enclosement cannot be empty when `ECSV` format is used");
    return true;
  }

  if(exchange->datapump.pump_type == PUMP_EXPORT_NOSPACE_CSV &&
      (field_term_length > 0 || exchange->field.enclosed->length() > 0)) {
    //PUMP_EXPORT_NOSPACE_CSV can only be used when field enclosement and terminator are all empty string
    my_error(ER_WRONG_USAGE_GDB_FORMAT, MYF(0), \
        "nospace_filling can only used when field enclosement and terminator are empty string");
    return true;
  }

  //If `jcsv` is used, enclosed must be empty!
  if(exchange->datapump.pump_type == PUMP_EXPORT_JCSV &&
      exchange->field.enclosed && exchange->field.enclosed->length() ) {
    my_error(ER_WRONG_USAGE_GDB_FORMAT, MYF(0), \
        "field enclosement must be empty when `JCSV` format is used");
    return true;
  }
...
bool Query_result_export::prepare(THD *thd, List<Item> &list,
  ...
  //datapump_csv should be set together with pump_type
  DBUG_ASSERT((thd->lex->datapump_csv && exchange->datapump.pump_type > PUMP_UNKNOWN) ||
      (!thd->lex->datapump_csv && exchange->datapump.pump_type == PUMP_UNKNOWN));
...
#define NEED_ESCAPING(x)                               \
  ...
  (NEED_ESCAPING_ECSV(x)) :                            \
    ((exchange->datapump.pump_type == PUMP_EXPORT_JCSV) ?     \
    (NEED_ESCAPING_JCSV(x)) : (NEED_ESCAPING_MYSQL(x)) ))


#define NEED_ESCAPING_ECSV(x)                          \
  ((int)(uchar)(x) == escape_char ||                   \
   (int)(uchar)(x) == field_sep_char)

// JCSV escapes nothing
#define NEED_ESCAPING_JCSV(x) (false)

#define NEED_ESCAPING_MYSQL(x)                         \
  ((int)(uchar)(x) == escape_char ||                   \
...
/*
  ...
  fields enclosement and terminater are all empty
 */
#define NEED_FIX_ROW_SIZE \
  (fixed_row_size && exchange->datapump.pump_type != PUMP_EXPORT_NOSPACE_CSV && \
  exchange->datapump.pump_type != PUMP_EXPORT_JCSV)
...
bool Query_result_export::send_data(THD *thd, List<Item> &items) {
  ...
  while ((item = li++)) {
    ...
    if (NEED_FIX_ROW_SIZE) {  // Fill with space
