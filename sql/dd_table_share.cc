...
static bool prepare_share(THD *thd, TABLE_SHARE *share,
  ...
  if(share->db_create_options & HA_OPTION_CONNECT_ENGINE){
...
static bool fill_share_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  LEX_STRING value{nullptr, 0};
  engine_option_value *end = nullptr;
  if (table_options.exists("tabname"))
  {
    name = to_lex_string(tabname_str);
    table_options.get("tabname", &value, &share->mem_root);
    new (&share->mem_root)
      engine_option_value(name, value, true, 
                          &share->option_list, &end);
    share->db_create_options |= HA_OPTION_CONNECT_ENGINE;
  }
  // Read option_list string
  if (table_options.exists("option_list"))
  {
    name = to_lex_string(option_list_str);
    table_options.get("option_list", &value, &share->mem_root);
    new (&share->mem_root)
      engine_option_value(name, value, true, 
                          &share->option_list, &end);
    share->db_create_options |= HA_OPTION_CONNECT_ENGINE;
  }
  // Read table_type string
  if (table_options.exists("table_type"))
  {
    name = to_lex_string(table_type_str);
    table_options.get("table_type", &value, &share->mem_root);
    new (&share->mem_root)
      engine_option_value(name, value, false, 
                          &share->option_list, &end);
    share->db_create_options |= HA_OPTION_CONNECT_ENGINE;
  }
#endif // HAVE_ZSQL_CONNECT_ENGINE
...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
  // connect engine column option.
  if (column_options->exists("FLAG"))
  {
    uint32 connect_flag = 0;
    LEX_STRING name{nullptr, 0};
    engine_option_value *end = nullptr;

    name = to_lex_string(connect_flag_str);
    column_options->get("FLAG", &connect_flag);
    new (&share->mem_root)
      engine_option_value(name, connect_flag, &reg_field->option_list, 
                          &end, &share->mem_root);
    reg_field->flags |= FIELD_HAS_CONNECT_FLAG;
    reg_field->connect_flag = connect_flag;
  }
#endif // HAVE_ZSQL_CONNECT_ENGINE
  ...
  if (part_info->part_type == partition_type::RANGE) {
    if (part_info->column_list) {
      ...
      if (part_value->max_value()) {
        ...
        if (part_obj->options().exists("is_range_str"))
        {
          part_info->is_range_str = true;
          DBUG_ASSERT(!part_info->column_list);
          part_elem->range_str = strdup_root(mem_root, part_value->value_utf8().c_str());
          if (!part_elem->range_str) return true;
        } else {
#endif
