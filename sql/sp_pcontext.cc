...
sp_pcontext::sp_pcontext(THD *thd)
    : m_cursor_value_name(NULL),  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
      m_level(0),
      m_max_var_index(0),
...
uint sp_pcontext::get_handlers_cursors_number(uint* handlers , uint* cursors) {
  if (NULL == handlers || NULL == cursors)
    return 1;

  *handlers = m_handlers.size();

  *cursors  = m_cursors.size();

  return 0;
}

/**
  add for compatible with oracle
  compare name and type
*/
sp_label *sp_pcontext::find_label(LEX_STRING name,
                                  sp_label::enum_type lab_type) {
  List_iterator_fast<sp_label> li(m_labels);
  sp_label *lab;
  ...
  return (m_parent && (m_scope == REGULAR_SCOPE)) ?
             m_parent->find_label(name,lab_type) :
             NULL;
}

int sp_pcontext::push_cursor_instr(LEX_STRING* cursor_name,
                                   sp_instr *i, uint offset) {
  cursor_instr_t *cursor_instr =
      (cursor_instr_t *)(*THR_MALLOC)->Alloc(sizeof(cursor_instr_t));

  if (!cursor_instr)
    return 1;

  cursor_instr->cursor_name = cursor_name;
  cursor_instr->instr = i;
  cursor_instr->cursor_offset = offset;
  m_cursor_instr_list.push_back(cursor_instr);

  return 0;
}

sp_instr_cpush *sp_pcontext::get_cpush_by_cursor_name(LEX_STRING cursor_name,
                                                      bool current_scope) {
  cursor_instr_t *cursor_instr = NULL;
  List_iterator<cursor_instr_t> iter(m_cursor_instr_list);

  while ((cursor_instr = iter++) != NULL) {
    if (my_strnncoll(system_charset_info,
                     (const uchar *)cursor_name.str,
                     cursor_name.length,
                     (const uchar *)cursor_instr->cursor_name->str,
                     cursor_instr->cursor_name->length) == 0) {
      return (sp_instr_cpush*)cursor_instr->instr;
    }
  }

  if (!current_scope) {
    return m_parent
              ? m_parent->get_cpush_by_cursor_name(cursor_name, current_scope)
              : NULL;
  }
...
/*
  ...
  compare name and type
*/
sp_label *sp_pcontext::find_label_by_index(uint dest_index,
                                           uint *cur_index,
                                           sp_label::enum_type lab_type) {
  List_iterator_fast<sp_label> li(m_labels);
  sp_label *lab;
  ...
  return (m_parent && (m_scope == REGULAR_SCOPE))
             ? m_parent->find_label_by_index(dest_index, cur_index, lab_type)
             : NULL;
}

sp_label *sp_pcontext::get_recent_lable_accord_type(
                           sp_label::enum_type lab_type) {
  List_iterator_fast<sp_label> li(m_labels);
  sp_label *lab;
  ...
  return (m_parent && (m_scope == REGULAR_SCOPE))
             ? m_parent->get_recent_lable_accord_type(lab_type)
             : NULL;
}
...
sp_variable *sp_pcontext::find_variable(LEX_STRING name,
  ...
  return (!current_scope_only && m_parent)
             ? m_parent->find_variable(name, idx, false)
             : NULL;
}
/* #endif */
