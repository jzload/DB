...
/**
 * An enum type to indicate what kind of `truncate partition/subpartition` 
 * is to be executed. If it's not `truncate partition/subpartition` command,
 * NONE will be used.
 */
enum class trunc_partition_type { NONE = 0, PARTITION, SUBPARTITION };
...
/*
  ...
  in the VALUES IN struct, only for range(substr())
*/

typedef struct p_range_str_list_val {
  const char *value_str;
  bool max_value;
} part_range_str_list_val;
#endif
...
#ifdef HAVE_ZSQL_RANGE_STR
  // just for RANGE (substr()).. values less then ('xx')
