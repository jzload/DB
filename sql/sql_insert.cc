...
bool check_valid_table_refs(const TABLE_LIST *view, List<Item> &values,
                            table_map map) {
...
bool validate_default_values_of_unset_fields(THD *thd, TABLE *table) {
  MY_BITMAP *write_set = &table->def_fields_set_insert_saved;
...
bool Sql_cmd_insert_values::execute_inner(THD *thd) {
  ...
  if (is_insert_all_global && is_insert_all_local) {
  ...
  */
  ...
  bitmap_copy(&insert_table->def_fields_set_insert_saved, insert_table->write_set);
...
#ifdef HAVE_ZSQL_INSERT_ALL
    if (is_insert_all_global) {
      statistic_insert_all_affected_rows(thd, info.stats.copied + info.stats.deleted +
                      (thd->get_protocol()->has_client_capability(CLIENT_FOUND_ROWS)
                        ? info.stats.touched
                        : info.stats.updated), id);
     } else {
      my_ok(thd,
...
#ifdef HAVE_ZSQL_INSERT_ALL
    if (is_insert_all_global) {
      statistic_insert_all_affected_rows(thd, info.stats.copied + info.stats.deleted + updated, id);
    } else {
...
bool check_view_insertability(THD *thd, TABLE_LIST *view,
...
bool Sql_cmd_insert_base::prepare_inner(THD *thd) {
...
#ifdef HAVE_ZSQL_INSERT_ALL
  if (is_insert_all_global && is_insert_all_local) {
    return prepare_inner_for_insert_all(thd);
  }
#endif /* HAVE_ZSQL_INSERT_ALL */
  ...
  */
  ...
  // if support INSERT ALL INTO SELECT syntax, need to skip the table inserted into,
  // then assign the first select table
