...
ulong thd_spec_lock_wait_time(const THD *thd){
  if (thd && thd->lex->sql_command == SQLCOM_SELECT)
    return thd->lex->spec_lock_wait_time;
...
/**
  Return the trx serial num string from special comment in goldendb.
  @param thd user thread
  @param buf copy the string from thd to the buf
  @param buflen max len of the buf
  @return length of trx_serial_num string
*/
extern "C" size_t thd_get_serial_num_safe(MYSQL_THD thd, char *buf, size_t buflen)
{
  size_t res_len= 0;
  mysql_mutex_lock(&thd->LOCK_thd_query);
  const char* serial_num = thd->get_trx_serial_num();
  if (NULL == serial_num || !serial_num[0])
  {
    res_len = 0;
  }
  else
  {
    res_len= MY_MIN(buflen - 1, strlen(serial_num));
    strncpy(buf, serial_num, res_len);
  }
  buf[res_len]= '\0';
  mysql_mutex_unlock(&thd->LOCK_thd_query);
...
/**
  Return the trx gtm gtid string from special comment in goldendb.
  @param thd user thread
  @param buf copy the string from thd to the buf
  @param buflen max len of the buf
  @return length of trx_gtm_gtid string
*/
extern "C" size_t thd_get_gtm_gtid_safe(MYSQL_THD thd, char *buf, size_t buflen)
{
  size_t res_len= 0;
  mysql_mutex_lock(&thd->LOCK_thd_query);
  const char* gtm_gtid = thd->get_trx_gtm_gtid();
  if (NULL == gtm_gtid || !gtm_gtid[0])
  {
    res_len = 0;
  }
  else
  {
    res_len= MY_MIN(buflen - 1, strlen(gtm_gtid));
    strncpy(buf, gtm_gtid, res_len);
  }
  buf[res_len]= '\0';
  mysql_mutex_unlock(&thd->LOCK_thd_query);
...
void thd_wait_begin(MYSQL_THD thd, int wait_type) {
  ...
  MYSQL_CALLBACK(thd->func_scheduler, thd_wait_begin,
                 (thd, wait_type));
}
...
void thd_wait_end(MYSQL_THD thd) {
  ...
  MYSQL_CALLBACK(thd->func_scheduler, thd_wait_end,
                 (thd));
}
