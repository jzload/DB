...
static int get_partition_id_range_str(partition_info *part_info,
                                      uint32 *part_id, longlong *func_value);
static int range_str_cmp(const String *str,
                         const part_range_str_list_val *part_range_str,
...
static void set_up_partition_func_pointers(partition_info *part_info) {
  ...
  if (part_info->is_sub_partitioned()) {
    ...
    if (part_info->part_type == partition_type::RANGE) {
      ...
      else if (part_info->is_range_str) /* #ifdef HAVE_ZSQL_RANGE_STR */
        part_info->get_part_partition_id = get_partition_id_range_str;
      else
  ...
  } else /* No subpartitioning */
  {
    ...
    if (part_info->part_type == partition_type::RANGE) {
      if (part_info->column_list)
        ...
        part_info->get_partition_id = get_partition_id_range_str;
      else
...
/*
  Check if a particular field is in need of character set
  handling for range substring partition functions.

  SYNOPSIS
    field_is_partition_charset_range_str()
    field              The field to check
  ...
  RETURN VALUES
    ...
    true               In need of character set handling
*/
bool field_is_partition_charset_range_str(Field *field) {
  if (field->type() == MYSQL_TYPE_STRING ||
      field->type() == MYSQL_TYPE_VARCHAR)
    return false;
...
/*
  ...
  SYNOPSIS
    ...
    ok_with_charsets                    Will we report allowed charset
                                        fields as ok
    is_range_str                        Marking range substr Scenarios
  RETURN VALUES
    false                               Success
    true                                Error
...
bool check_part_func_fields(Field **ptr, bool ok_with_charsets,
  ...
  while ((field = *(ptr++))) {
    ...
    /* just for range substr(), just check first field */
    if (is_range_str) /* #ifdef HAVE_ZSQL_RANGE_STR */
      return field_is_partition_charset_range_str(field);
  ...
  */
  if (part_info->part_type == partition_type::HASH) {
    ...
    if (unlikely(!part_info->column_list)) {
      if (part_info->is_range_str) { /*  #ifdef HAVE_ZSQL_RANGE_STR */
        if (part_info->part_expr->result_type() != STRING_RESULT) {
          part_info->report_part_expr_error(false);
          goto end;
        }
      } else {
  ...
  if (((part_info->part_type != partition_type::HASH ||
      ...
      check_part_func_fields(part_info->part_field_array, true,
                  part_info->is_range_str /* #ifdef HAVE_ZSQL_RANGE_STR */ )) ||
...
/**
  ...
  @param[in]    part_obj    partition object in dd.
  @param[in]    part_elem   partition element.
  @param[in]    field       Field pointer.
  @param[in]    part_info   Partition info.
  @param[out]   output_str  expr to string

  @return 1 ERROR,  0 SUCC
*/

int expr_convert_str(partition_element *part_elem, partition_info *part_info,
                     const HA_CREATE_INFO *create_info,
                     const List<Create_field> *create_fields,
                     String *output_str) {
  DBUG_ASSERT(part_elem != NULL);
  DBUG_ASSERT(part_info != NULL);
  DBUG_ASSERT(output_str != NULL);
  DBUG_ASSERT(output_str->is_empty());

  List_iterator<part_elem_value> list_it(part_elem->list_val_list);
  part_elem_value *list_value = list_it++;
  if (list_value == NULL) {
    output_str->append(part_elem->range_str);
    return 0;
  }

  part_column_list_val *col_val = &list_value->col_val_array[0];
  DBUG_ASSERT(!col_val->item_expression->null_value);

  const Item_func *item_func = down_cast<const Item_func *>(part_info->part_expr);
  DBUG_ASSERT(item_func != NULL);

  Item *Item_field = item_func->arguments()[0];
  DBUG_ASSERT(Item_field != NULL);
  const char *field_name = (Item_field->item_name).ptr();

  Field *field = (part_info->part_field_array != NULL &&
                  part_info->part_field_array[0] != NULL)
                     ? part_info->part_field_array[0]
                     : NULL;

  if (create_info == NULL && field == NULL) return 1;

  return expr_to_string(output_str, col_val->item_expression, field, field_name,
                        create_info,
  ...
  if (part_info->part_type == partition_type::RANGE) {
    ...
    if (part_info->column_list) {
      ...
      if (!p_elem->max_value) {
        ...
        if (part_info->is_range_str) { /* #ifdef HAVE_ZSQL_RANGE_STR */
          String range_str;
          int res = expr_convert_str(p_elem, part_info, NULL, NULL, &range_str);
          if (res != 0) return 1;
          err += add_string_object(fptr, &range_str);
          err += add_end_parenthesis(fptr);
        } else {
...
#ifdef HAVE_ZSQL_RANGE_STR
  if (part_info->is_range_str)
    return part_info->get_part_partition_id_charset(part_info, part_id, func_value);
#endif
...
#ifdef HAVE_ZSQL_RANGE_STR
int range_str_cmp(const String *str,
                  const part_range_str_list_val *part_range_str,
                  partition_info *part_info) {

  if (part_range_str->max_value)
    return -1;
  Field **field = part_info->part_field_array;
  size_t func_res_str_len = str->length();
  set_if_smaller(func_res_str_len, (uint)~0L);

  size_t range_str_len = my_strlen(part_range_str->value_str);
  set_if_smaller(range_str_len, (uint)~0L);

  //   A negative number if first_arg < second_arg
  //   A positive number if first_arg > second_arg
  //   0 if strings are equal
  int res = (*field)->charset()->coll->strnncollsp((*field)->charset(),
                                  (const uchar *)str->ptr(),
                                  func_res_str_len,
                                  (const uchar *)part_range_str->value_str,
                                  range_str_len);
  return res;
}

int get_partition_id_range_str(partition_info *part_info, uint32 *part_id,
                                  longlong *) {
  part_range_str_list_val *range_str_array = part_info->range_str_array;
  uint min_part_id = 0;
  uint max_partition = part_info->num_parts - 1;
  uint max_part_id = max_partition;
  uint loc_part_id = 0;
  ...
  String str;
  Item *item_expr = part_info->part_expr;
  ...
  while (max_part_id > min_part_id)
  {
    /* equal to "loc_part_id = (max_part_id + min_part_id) / 2", Prevent overflow*/
    loc_part_id = (max_part_id & min_part_id) + ((max_part_id ^ min_part_id) >> 1);
    if (range_str_cmp(res, &range_str_array[loc_part_id],part_info) >= 0)
      min_part_id = loc_part_id + 1;
    else
  ...
  *part_id = (uint32)loc_part_id;
  ...
  if (loc_part_id == max_partition &&
      range_str_cmp(res, &range_str_array[loc_part_id],part_info) >= 0 &&
...
uint prep_alter_part_table(THD *thd, TABLE *table, Alter_info *alter_info,
  ...
  if (alter_info->flags &
    ...
    const partition_element *tab_max_range_str = NULL;
    const partition_element *alt_max_range_str = NULL;
    Partition_handler *part_handler = table->file->get_partition_handler();
...
#ifdef HAVE_ZSQL_RANGE_STR
      if (tab_part_info->check_support_range_str())
        alt_part_info->is_range_str = true;
#endif
        ...
        do {
          ...
          if (is_name_in_list(part_elem->partition_name,
            ...
            if (tab_part_info->column_list) {
              ...
              tab_max_elem_val = p++;
            } else if (tab_part_info->is_range_str) { /* #ifdef HAVE_ZSQL_RANGE_STR */
              tab_max_range_str = part_elem;
            } else
            ...
            if (!found_first) {
              ...
              do {
                ...
                if (tab_part_info->column_list) {
                  ...
                  alt_max_range_str = alt_part_elem;
                } else
      ...
      if (alter_info->flags == Alter_info::ALTER_REORGANIZE_PARTITION &&
        ...
        if (is_last_partition_reorged) {
          if (tab_part_info->column_list) {
            ...
            is_error = tab_part_info->cmp_str_for_range_str(alt_max_range_str,
                                                              tab_max_range_str);
          } else {
          ...
          if (tab_part_info->column_list) {
            ...
            is_error = tab_part_info->cmp_str_for_range_str(
                                      alt_max_range_str,
                                      tab_max_range_str) ||
                       tab_part_info->cmp_str_for_range_str(
                                      tab_max_range_str,
                                      alt_max_range_str);  // a != b
          } else {
            is_error = alt_max_range != tab_max_range;
          }
        }
