...
static PSI_cond_info cond_list[] = {
    ...
    { &key_rotate_cond, "rotate_cond", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME}};

static PSI_thread_key key_worker_thread;
static PSI_thread_key key_timer_thread;
static PSI_thread_key key_rotate_thread;
static PSI_thread_info thread_list[] = {
    ...
    {&key_timer_thread, "timer_thread", PSI_FLAG_SINGLETON, 0,
     PSI_DOCUMENT_ME},
    {&key_rotate_thread, "rotate_thread", PSI_FLAG_SINGLETON, 0,
     PSI_DOCUMENT_ME}};
...
struct alignas(128) thread_group_t {
  ...
  connection_queue_t super_prio_queue;
  worker_list_t waiting_threads;
  worker_list_t super_waiting_threads;
  worker_thread_t *listener;
  pthread_attr_t *pthread_attr;
  my_thread_t     super_thread_id;
  int pollfd;
  int thread_count;
  int super_event_count;
...
static int wake_super_thread(thread_group_t *thread_group);
...
// static void *super_worker_main(void *param);
static void check_stall(thread_group_t *thread_group);
static void connection_abort(connection_t *connection, int quicksync_refcount);
...
static int io_poll_associate_fd(int pollfd, int fd, void *data) noexcept {
  if(data && ((connection_t*)data)->thd)
    ((connection_t*)data)->thd->m_session_stage = ENUM_EPOLL_ADD;
...
static int io_poll_start_read(int pollfd, int fd, void *data) noexcept {
  if(data && ((connection_t*)data)->thd)
    ((connection_t*)data)->thd->m_session_stage = ENUM_EPOLL_MOD;
  struct epoll_event ev;
  ev.data.u64 = 0; /* Keep valgrind happy */
  ev.data.ptr = data;
  ev.events = EPOLLIN | EPOLLET | EPOLLERR | EPOLLRDHUP | EPOLLONESHOT;
...
static connection_t *queue_get(thread_group_t *thread_group) noexcept {
  ...
  if ((c= thread_group->super_prio_queue.front()))
  {
    c->thd->removed_from_super_queue = true;
    thread_group->super_event_count--;
    thread_group->super_prio_queue.remove(c);
  }
  else if ((c= thread_group->high_prio_queue.front()))
  {
...
static void* rotate_thread(void *)
{
  my_thread_init();
  
  // wait for server start and initialize thd
  ...
  for(;;)
  {
    ...
    mysql_cond_wait(&COND_rotate_binlog, &LOCK_rotate_binlog);

    if (rotate_thread_stopped)
    {
    ...
    /*
      Fix a bug: 
      Due to the rotate event and format evnet and previous_gtid event 
      use thd->start_time as the timestamp in binlog. So the rotate
      thread need set the thd start_time before call rotate_wraper().
    */
    thd->set_time();

    if (tc_log)
    {
        ((MYSQL_BIN_LOG*)tc_log)->rotate_wraper(NULL);
    }
    mysql_mutex_unlock(&LOCK_rotate_binlog);    
  }

  mysql_mutex_destroy(&LOCK_rotate_binlog);
  delete thd;
  my_thread_end();
  return NULL;    
}
...
static bool queues_are_empty(const thread_group_t &tg) noexcept {
  return (tg.super_prio_queue.is_empty() &&
          tg.high_prio_queue.is_empty() &&
...
{
  my_thread_handle thread_id;
  DBUG_ENTER("start_rotate");
  mysql_mutex_init(key_rotate_mutex, &LOCK_rotate_binlog, NULL);
  mysql_cond_init(key_rotate_cond, &COND_rotate_binlog);
  mysql_thread_create(key_rotate_thread,&thread_id, NULL, rotate_thread, NULL);
  DBUG_VOID_RETURN;
}
  ...
  for (;;) {
    ...
    */
    for (int i = (listener_picks_event) ? 1 : 0; i < cnt; i++) {
      ...
      if(c && c->thd)
        c->thd->req_wait_time = my_micro_time();
...
int threadpool_append_commit_to_high_prio_queue(THD *thd)
{
    DBUG_ENTER("threadpool_append_commit_to_high_prio_queue");
    
    connection_t *c = (connection_t *)thd->scheduler.data;
    thread_group_t *thread_group = c->thread_group;
    ...
    /*get slave ack or enq commit start time*/
    thd->commit_queue_wait_time = thd->current_utime();
    thd->added_to_super_queue = true;
    thread_group->super_event_count++;
    thread_group->super_prio_queue.push_back(c);
    if(thread_group->super_event_count == 1)
    {
        if(wake_super_thread(thread_group))
        {}
    }
    // else if(thread_group->active_thread_count == 0)
    else if (thread_group->thread_count > thread_group->active_thread_count)
    {
        if(wake_thread(thread_group))
        {
            if(thread_group->thread_count == 1)
...
#if 0
static int create_super_worker(thread_group_t *thread_group)
{
  my_thread_handle thread_id;
  bool max_threads_reached= false;
  int err;
  
  DBUG_ENTER("create_super_worker");
  
  err= mysql_thread_create(key_worker_thread, &thread_id, 
         thread_group->pthread_attr, super_worker_main, thread_group);
  if (!err)
  {
    thread_group->last_thread_creation_time = my_microsecond_getsystime();
    //Global_THD_manager::get_instance()->inc_thread_created();
...
static int thread_group_init(thread_group_t *thread_group,
  ...
  thread_group->super_event_count = 0;
  thread_group->super_thread_id = 0;
  thread_group->no_super_thread = true;
  DBUG_RETURN(0);
}
...
static int wake_super_thread(thread_group_t *thread_group)
{
  DBUG_ENTER("wake_super_thread");
  worker_thread_t *thread = thread_group->super_waiting_threads.front();
  if(thread)
  {
    mysql_cond_signal(&thread->cond);
    DBUG_RETURN(0);
...
static void thread_group_close(thread_group_t *thread_group) noexcept {
  ...
  /* Wake all super workers.  */
  wake_super_thread(thread_group);

  if (thread_group->thread_count == 0 && thread_group->no_super_thread) {
    mysql_mutex_unlock(&thread_group->mutex);
  ...
  mysql_mutex_lock(&thread_group->mutex);
  while (thread_group->thread_count > 0 || !thread_group->no_super_thread) {
    mysql_mutex_unlock(&thread_group->mutex);
    my_sleep(10000);
    mysql_mutex_lock(&thread_group->mutex);
  }
...
static void queue_put(thread_group_t *thread_group, connection_t *connection) {
  ...
  connection->enqueue_time = my_micro_time();
  connection->thd->req_wait_time = connection->enqueue_time;
...
void append_event_to_high_prio_queue(THD* thd)
{
    connection_t *c = (connection_t *)thd->scheduler.data;
    thread_group_t *thread_group = c->thread_group;
    ...
    if (!oversubscribed) {
      ...
      if (io_poll_wait(thread_group->pollfd, &nev, 1, 0) == 1) {
        ...
        */
        if (connection_is_high_prio(*connection))
          ...
          */
          ...
          if(connection && connection->thd)
            connection->thd->req_wait_time = my_micro_time();
        ...
        if (connection) {
          thread_group->queue_event_count++;
          connection->thd->req_wait_time = 0;
          break;
        }
      }
    }
...
#if 0
static connection_t *get_super_event(worker_thread_t *current_thread, thread_group_t *thread_group)
{ 
  DBUG_ENTER("get_super_event");
  ...
  for(;;) 
  {
    ...
    //thread_group->queue_event_count++;   
    if ((connection = thread_group->super_prio_queue.front()))
    {
      connection->thd->removed_from_super_queue = true;
      thread_group->super_event_count--;
      thread_group->super_prio_queue.remove(connection);
      break;
    }
...
static void wait_begin(thread_group_t *thread_group) noexcept {
  ...
  my_thread_t real_id = my_thread_self();  
  
  mysql_mutex_lock(&thread_group->mutex);

  if (thread_group->super_thread_id != real_id)
  {
    thread_group->active_thread_count--;
...
bool Thread_pool_connection_handler::add_connection(
  ...
  thd->func_scheduler = &tp_event_functions;
...
int thd_start_io(THD *thd)
{
    connection_t *connection = (connection_t *)thd->scheduler.data;
    set_wait_timeout(connection);
    return start_io(connection);
}

static void handle_event(connection_t *connection) {
  DBUG_ENTER("handle_event");
  int err;
  int quicksync_refcount = 0;
  ...
  if (!connection->logged_in) {
    ...
    if (!err)
    {
      err = thd_start_io(connection->thd);
    }
      
  } else {
    /**
     * We should get the quicksync_refcount for the connection_abort, to avoid
     * two thread do the the connection_abort for one thd connection, and this
     * will cause crash.
    */
    err = threadpool_process_request(connection->thd, &quicksync_refcount);
  }

  if (err) connection_abort(connection, quicksync_refcount);
...
static void *super_worker_main(void *param)
{
  ...
  DBUG_ENTER("super_worker_main");
  
  thread_group_t *thread_group = (thread_group_t *)param;

  // only one super-thread, so don't need lock
  thread_group->no_super_thread = false;

  /* Init per-thread structure */
  mysql_cond_init(key_worker_cond, &this_thread.cond);
  this_thread.thread_group= thread_group;
  this_thread.event_count=0;
  thread_group->super_thread_id = my_thread_self(); 
  thread_group->super_waiting_threads.push_front(&this_thread);
  ...
  for(;;)
  {
    connection_t *connection = get_super_event(&this_thread, thread_group);
    if (!connection)
      break;
    this_thread.event_count++;
    handle_event(connection);
  ...
  */
  ...
  // only one super-thread, so don't need lock and threadgroup maybe closed
  thread_group->no_super_thread = true;
...
int tp_get_idle_thread_count() noexcept {
  ...
  for (uint i = 0;
    ...
    sum += (all_groups[i].thread_count - all_groups[i].active_thread_count - all_groups[i].waiting_thread_count);
  }
  return sum;
}

int tp_get_waiting_thread_count()
{
  int sum=0;
  for(uint i= 0;
...
int tp_get_active_thread_count()
{
  int sum=0;
  for(uint i= 0;
      i< array_elements(all_groups) && (all_groups[i].pollfd >= 0);
      i++)
  {
    sum+=all_groups[i].active_thread_count;
  }
  return sum;
}
...
void show_threadpool_status(THD *thd)
{
  connection_t *c = NULL; 
  int idle_thread_count = 0;
  ulonglong queue_time = 0;

  List<Item> field_list;
  Protocol *protocol= thd->get_protocol();
  DBUG_ENTER("show_threadpool_status");

  field_list.push_back(new Item_return_int("Group",4,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("Super_queue_events",10,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("High_queue_events",10,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("Low_queue_events",10,MYSQL_TYPE_LONG));

  field_list.push_back(new Item_return_int("Threadpool_active_threads",10,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("Threadpool_waiting_threads",10,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("Threadpool_idle_threads",10,MYSQL_TYPE_LONG));
  field_list.push_back(new Item_return_int("Wait_time",10, MYSQL_TYPE_LONGLONG));
  ...
  for (uint i=0; i< group_count ; i++)
  {
    ...
    idle_thread_count = thread_group->thread_count -
                        thread_group->active_thread_count -
                        thread_group->waiting_thread_count;
        
    protocol->store((int)i);
    protocol->store((uint32)thread_group->super_event_count);
    protocol->store((uint32)thread_group->high_prio_queue.elements());
    protocol->store((uint32)thread_group->queue.elements());
    protocol->store((uint32)thread_group->active_thread_count);
    protocol->store((uint32)thread_group->waiting_thread_count);
    protocol->store((uint32)idle_thread_count);

    queue_time = static_cast<ulonglong>(0);
    c = thread_group->queue.front();
    if (c && c->enqueue_time)
    {
      queue_time = (my_micro_time() - c->enqueue_time)/1000;
    }
    protocol->store(queue_time);
...
void wait_quick_sync_old_thread_exit(THD *thd)
{
  int wait_count = 0;
  while(2 == thd->get_quick_sync_refcount())
  {
    my_sleep(3 * 1000);
    wait_count++;
    if (wait_count % 10 == 0)
      sql_print_information("quick_sync_refcount is 2, has wait old thread exit times(%d).", wait_count);
  }
