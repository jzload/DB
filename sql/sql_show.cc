  ...
  */
  ...
  for (ptr = table->field; (field = *ptr); ptr++) {
    ...
    */
    ...
    if (flags & FIELD_HAS_CONNECT_FLAG) {
      packet->append(STRING_WITH_LEN(" FLAG="));
      packet->append_ulonglong(field->connect_flag);
    }
  ...
  */
  ...
  if (!foreign_db_mode) {
    ...
    */
    ...
    {//options for CONNECT engine, e.g. ODBC
      engine_option_value * opt = share->option_list;
      packet->append(STRING_WITH_LEN("\n "));
      for (; opt; opt= opt->next){
        packet->append(STRING_WITH_LEN(" "));     
        packet->append(opt->name.str, opt->name.length);
        packet->append(STRING_WITH_LEN("="));
        if(opt->quoted_value)
...
class thread_info {
 public:
  ...
  uint command;
  /* for quicksync status */
  uint   leader,released,refcount,session_stage,add_to_queue,need_execute,stmt_block,follower_num;
  const char *user, *host, *db, *proc_info, *state_info;
  CSET_STRING query_string;
  /* for quicksync status */
  CSET_STRING follower_threads;
};
...
class List_process_list : public Do_THD_Impl {
 ...
 public:
  ...
  void set_quick_sync_info(THD *inspect_thd, thread_info *thd_info);
  ...
  virtual void operator()(THD *inspect_thd) {
    ...
    /* for show quicksync status */
    if(m_client_thd->lex->sql_command == SQLCOM_SHOW_QUICKSYNC_STATUS)
    {
      set_quick_sync_info(inspect_thd, thd_info);
...
void show_quicksync_status(THD *thd,const char *user, bool verbose)
{
  Item *field;
  List<Item> field_list;
  Thread_info_array thread_quicksync_status(thd->mem_root);
  size_t max_query_length= (verbose ? thd->variables.max_allowed_packet :
                            QUICKSYNC_STATUS_WIDTH);
  Protocol *protocol= thd->get_protocol();
  DBUG_ENTER("show_quicksync_status");

  field_list.push_back(new Item_int(NAME_STRING("Id"), 0, MY_INT64_NUM_DECIMAL_DIGITS));
  field_list.push_back(field= new Item_empty_string("Leader",3));
  field_list.push_back(field= new Item_empty_string("Released",3));
  field_list.push_back(field= new Item_return_int("Refcount",4, MYSQL_TYPE_LONG));
  field_list.push_back(field= new Item_empty_string("Session_stage",max_query_length));
  field_list.push_back(field= new Item_empty_string("Add_to_queue",3));
  field_list.push_back(field= new Item_empty_string("Need_execute",3));
  /* Stmt_block value = "0 1 2 4" */
  field_list.push_back(field= new Item_return_int("Stmt_block",4, MYSQL_TYPE_LONG));
  field_list.push_back(field= new Item_return_int("Follower_num",4, MYSQL_TYPE_LONG));
  field_list.push_back(field= new Item_empty_string("Follower_list",max_query_length));
  ...
  if (!thd->killed)
  {
    thread_quicksync_status.reserve(Global_THD_manager::get_instance()->get_thd_count());
    List_process_list list_process_list(user, &thread_quicksync_status, thd, max_query_length);
    Global_THD_manager::get_instance()->do_for_all_thd_copy(&list_process_list);
  }

  // Return list sorted by thread_id.
  std::sort(thread_quicksync_status.begin(), thread_quicksync_status.end(), thread_info_compare());

  for (size_t ix= 0; ix < thread_quicksync_status.size(); ++ix)
  {
    thread_info *thd_quicksync_status = thread_quicksync_status.at(ix);
    protocol->start_row();
    protocol->store((ulonglong) thd_quicksync_status->thread_id);
    protocol->store(thd_quicksync_status->leader ? "Yes":"No", &my_charset_bin);
    protocol->store(thd_quicksync_status->released ? "Yes":"No", &my_charset_bin);
    protocol->store((uint32) thd_quicksync_status->refcount);
    protocol->store(session_stage[thd_quicksync_status->session_stage].str, &my_charset_bin);
    protocol->store(thd_quicksync_status->add_to_queue ? "Yes":"No", &my_charset_bin);
    protocol->store(thd_quicksync_status->need_execute ? "Yes":"No", &my_charset_bin);
    protocol->store((uint32) thd_quicksync_status->stmt_block);
    protocol->store((uint32) thd_quicksync_status->follower_num);
    protocol->store(thd_quicksync_status->follower_threads.str(),
                    thd_quicksync_status->follower_threads.charset());
...
void List_process_list::set_quick_sync_info(THD *inspect_thd, thread_info *thd_info)
{
  int tmp_len = 0;
  int follower_num = 0;
  char tmp_buf[QUICKSYNC_STATUS_WIDTH] = {0};
  
  thd_info->leader        = (uint)inspect_thd->m_leader_thread;
  thd_info->released      = (uint)inspect_thd->m_released_thread;
  thd_info->refcount      = (uint)inspect_thd->get_quick_sync_refcount();
  thd_info->session_stage = (uint)inspect_thd->m_session_stage;
  thd_info->add_to_queue  = (uint)inspect_thd->added_to_super_queue;
  thd_info->need_execute  = (uint)inspect_thd->m_need_execute;
  thd_info->stmt_block    = (uint)inspect_thd->m_multi_statements;
   
  if (inspect_thd->next_to_commit && inspect_thd->m_leader_thread)
  {
    tmp_len = sprintf((char*)tmp_buf,"[");            
    THD* tmp_thd = inspect_thd->next_to_commit;
    while (tmp_thd)
    {
        if (tmp_len < QUICKSYNC_STATUS_WIDTH-100) // reserver some space to save count of follower thread
          tmp_len += sprintf((char*)tmp_buf+tmp_len,"%d ", tmp_thd->thread_id());
    
        follower_num++;
        tmp_thd = tmp_thd->next_to_commit;
    }
    if (tmp_len < QUICKSYNC_STATUS_WIDTH-100)
      tmp_len += sprintf((char*)tmp_buf+tmp_len,"]");
    else
      tmp_len += sprintf((char*)tmp_buf+tmp_len,"...]");
    tmp_buf[tmp_len] = 0; 
  }

  thd_info->follower_num = (uint)follower_num;

  const size_t width = min<size_t>(m_max_query_length, tmp_len);
  const char *q = m_client_thd->strmake(tmp_buf, width);
  /* Safety: in case strmake failed, we set length to 0. */
  thd_info->follower_threads = CSET_STRING(q, q ? width : 0, inspect_thd->charset());
}
