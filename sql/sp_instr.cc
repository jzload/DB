...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  // execute immediate 'select xx' | v_strsql
  if (is_dynamic_stmt) {
    if (!m_dynamic_stmt) {
    ...
    LEX *old_lex = thd->lex;
    Item *old_item_list = thd->item_list();
    MEM_ROOT *old_root = thd->mem_root;
    Query_arena::enum_state old_state = thd->get_state();

    thd->mem_root  = thd->sp_mem_root;
    thd->set_item_list(thd->sp_item_list);

    LEX *sublex= new (thd->mem_root)st_lex_local;
    if (sublex == 0)
      return true;
    //sublex->set_current_select(sublex->select_lex);
    sublex->thd= thd;
    sublex->set_current_select(NULL);

    thd->lex = sublex;

    String str,*str2;
    if (m_dynamic_stmt->fix_fields(thd, (Item**)&m_dynamic_stmt))
      return true;
    str2 = m_dynamic_stmt->val_str(&str);
    m_query.str = (char*)str2->ptr();
    m_query.length = str2->length();
    thd->set_query((char*)str2->ptr(), str2->length());

    Parser_state parser_state ,*old_parser_state;
    old_parser_state = thd->m_parser_state;
    ...
    //m_lex_keeper.set_lex(sublex);

    this->m_lex = sublex;

    thd->m_parser_state = old_parser_state;
    thd->lex = old_lex;
    thd->mem_root = old_root;
    thd->set_item_list(old_item_list);
    thd->set_state(old_state);
  } else {
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
    /*
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (is_dynamic_stmt) {
    this->set_lex(old_sub_lex, true);
    set_first_execution(true);
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
static LEX_STRING make_string(THD *thd, const char *start_ptr,
                              const char *end_ptr) {
  LEX_STRING s;
...
#else
  bool ret = false;
  if (m_cursor_type == REF_CURSOR && !m_cursor_query_parsed) {
    LEX *old_lex = thd->lex;
    Query_arena::enum_state old_state = thd->get_state();
    Item *old_item_list = thd->item_list();
    MEM_ROOT *old_root = thd->mem_root;
    ...
    if(!m_splocal) {
      my_error(ER_PROCEDURE_EXECUTE_FAIL, MYF(0), "refcursor has not been opened!");
      return true;
    }

    String str, *res;
    if (m_splocal->fix_fields(thd, (Item**)&m_splocal))
      return true;

    res = m_splocal->val_str(&str);
    thd->set_query((char*)res->ptr(), res->length());
    ...
    cursor_query = to_lex_cstring(make_string(thd, thd->query().str,
                                      thd->query().str + thd->query().length));

    if (!cursor_query.str)
      return 1;

    this->set_query(&cursor_query);

    //m_lex_keeper.set_lex(sublex);
    //sp_pcontext *pctx= thd->lex->get_sp_current_parsing_ctx();
    ret = thd->sp_runtime_ctx->push_cursor(thd, this);
