...
/* Check order by has to_number function, if has to_number function
   should not reset to_number result decimal and precision. 
*/
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
inline static void set_func_item_no_reset_dec(Item* item) {
  if (Item::FUNC_ITEM == item->type()) {
    Item_func *arg_item = dynamic_cast <Item_func *>(item);
    if (NULL != arg_item) {
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      set_func_item_no_reset_dec(item);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
