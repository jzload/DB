...
bool check_fields(THD *thd, List<Item> &items) {
  ...
  while ((item = it++)) {
    ...
    */
    ...
    if (item->type() == Item::ROW_ITEM) continue;
#endif
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  Item *conds_rownum = nullptr;
  if (select_lex->with_rownum) {
    if (order) {
      my_error(ER_MISUSE_ROWNUM, MYF(0) ,"in UPDATE statement with ORDER BY");
      return true;
    }
    select_lex->m_Rownum_controller.init();
  }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (conds && (select_lex->m_rownum_option & ROWNUM_IN_WHERE_COND) &&
      extract_rownum_from_where_cond(thd, &conds, &conds_rownum))
    return true;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  qep_tab.set_rownum_condition(conds_rownum);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  {  // Start of scope for Modification_plan
    ...
    */
    ...
    while (true) {
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      reset_rownum_controller_flag(select_lex);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      if (0 != (error = deal_with_rownum_in_single_table_dml(thd, select_lex, qep_tab)))
      {
        break;
      }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
*/
bool prepare_partial_update(Opt_trace_context *trace, List<Item> *fields,
                                   List<Item> *values) {
  /*
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  RowItemInfos row_item_infos;
  select->row_item_info = (void*) &row_item_infos;
#endif
  if (setup_fields(thd, Ref_item_array(), *update_fields, UPDATE_ACL, NULL,
                   false, true )) {
    select->row_item_info = nullptr;
    return true;
  }
    ...
    if (table == table_to_update) {
      ...
      */
      ...
      if (error != HA_ERR_FOUND_DUPP_KEY && error != HA_ERR_FOUND_DUPP_UNIQUE) {
        if (error) {
          if (create_ondisk_from_heap(thd, tmp_table, error, true, NULL)) {
