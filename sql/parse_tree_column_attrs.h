...
class PT_column_attr_base : public Parse_tree_node_tmpl<Column_parse_context> {
 ...
 public:
  ...
  virtual void apply_connect_flag(uint *, bool *) const {}
...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
/// Node for the FLAG column attribute
class PT_flag_column_attr : public PT_column_attr_base {
  typedef PT_column_attr_base super;
  uint connect_flag;
  bool is_connect;

 public:
  explicit PT_flag_column_attr(uint value, bool flag) {
    connect_flag = value;
    is_connect = flag;
  }

  void apply_connect_flag(uint *value, bool *flag) const {
    *value = connect_flag;
    *flag = is_connect;
  }
  ...
  bool contextualize(Column_parse_context *pc) override {
    if (pc->is_generated) {
      my_error(ER_WRONG_USAGE, MYF(0), "FLAG", "generated column");
      return true;
    }
    return super::contextualize(pc);
  }
...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
        attr->apply_connect_flag(&connect_flag, &is_connect);
#endif // HAVE_ZSQL_CONNECT_ENGINE
      }
    }
    return false;
