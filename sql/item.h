...
enum enum_name_context_place_type {
  UNKNOWN_CONTEXT_PLACE,
  CURRENT_CONTEXT_EXCEPT_CTE,
  DERIVED_FIRST_SELECT,
};

/*
  keep info db/table name replacement, in `start with connect by`
*/
class Item_replace_info {
 public:
  /* used for db/table name replacement */
  void set_name_context(enum_name_context_place_type arg) {
    name_context = arg;
  }

  bool db_table_name_fixed = false;
  const char *replace_orig_db_name = nullptr;
  const char *replace_orig_table_name = nullptr;
  const char *replace_new_schema_name = nullptr;
  const char *replace_new_table_name = nullptr;
  enum_name_context_place_type name_context = CURRENT_CONTEXT_EXCEPT_CTE;

  const char *prior_table = nullptr; // name of generated `cte` table, for later check
  bool fake_ident_flag = false; // true if is a fake ident constructed to check names
};
...
/**
  used by item_options, represents the type of item;
  It is represented by bitmap, and the bit represents the corresponding type;
  used by set_item_option or check_item_option
*/

/* The expression is to_number type, or contains to_number type */
#define HAVE_TO_NUMBER_ITEM ((uint64)1 << 0)
...
class Mark_field {
 public:
  ...
  Mark_field(enum_mark_columns mark) : table(NULL), mark(mark) {}
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
public:
  virtual bool check_replace_schema_table_valid(THD * /*thd*/) {
    return false;
  }
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
  ...
  */
  ...
  inline void set_item_option(uint64 option) {
    item_options |= option;
  }

  inline bool check_item_option(uint64 option) {
    return (item_options & option) ? true : false;
  }
  ...
  */
  ...
  /**
    Item::walk function. Set bit in table->tmp_set for all fields in
    table 'arg' that are referred to by the Item.
    Specially used for MERGE INTO.
    In addition, set the bit for assiocated fields of generated column.
  */
  virtual bool add_field_to_set_processor_for_merge_into(uchar *) { return false; }
  ...
  */
  ...
  /**
    item_options:
    item option type, represents the type of item, like to_number/to_char and
    so on;
    It is represented by bitmap, and the bit represents the corresponding type;
  */
  uint64 item_options;
  ...
  */
  ...
  bool get_is_parser_item() const { return is_parser_item; }
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  /**
    Get str_value
  */
  virtual const String get_str_value() const { return str_value; }

  /**
    Get address of str_value
  */
  virtual const String *get_str_value_addr() const { return &str_value; }

  /**
    Find whether this item is a ROWNUM item.
  */
  virtual bool find_rownum_item(THD *, SELECT_LEX *, uint, bool &change_row_to_ref) {
    change_row_to_ref = false;
    return false;
  }

  bool search_array_for_rownum(THD *thd, SELECT_LEX *select_lex, uint el,
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  /* used for db/table name replacement */
  Item_replace_info name_replace_info;

  void set_schema_table(const char *db_arg, const char *table_arg);
...
class Item_field : public Item_ident {
  ...
  */
  ...
  /* Used in Field_iterator_full_join::create_item() */
  Item_field(THD *thd, Name_resolution_context *context_arg, Field *field,
              const char *alias);
#endif
  /*
...
#ifdef HAVE_ZSQL_MERGE_INTO
  bool add_field_to_set_processor_for_merge_into(uchar *arg) override;
#endif /* HAVE_ZSQL_MERGE_INTO */
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  void set_db_table_name(const char *db_arg, const char *table_arg);
  bool check_replace_schema_table_valid(THD *thd) override;
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */

private:
  void fix_db_table_name(Parse_context *pc);
...
class Item_param final : public Item, private Settable_routine_parameter {
  ...
  */
  ...
  bool basic_const_item() const override {
    /* Parse tree clone */
    /* Cloned Item_param in limit may be fixed multiple times */
  ...
  */
  ...
  /* ZSQL parse tree clone*/
  bool is_itemized{false};
 public:
  void *clone(Parse_tree_transformer *) const override;
};
...
class Item_hex_string : public Item_basic_constant {
 ...
 public:
  ...
  longlong val_int() override;
  Item *clone_item() const override {
    return new Item_hex_string(this);
  }
  String *val_str(String *) override {
    DBUG_ASSERT(fixed);
    return &str_value;
 ...
 protected:
  // fix bug for clone_item()
  void clone_inner(const Item_hex_string *item);

 private:
  Item_hex_string(const Item_hex_string *item);
  Item_hex_string(const POS &pos, const Item_hex_string *item);
...
class Item_bin_string final : public Item_hex_string {
 ...
 private:
  Item_bin_string(const Item_bin_string *item);
  Item_bin_string(const POS &pos, const Item_bin_string *item);
...
*/
  table_map used_tables() const override {
    ...
    return !(inner_map & ~INNER_TABLE_BIT) && first_inner_table != NULL
...
class Item_cache_row final : public Item_cache {
 ...
 public:
  Item_cache_row()
      : Item_cache(), values(0), item_count(2), save_array(false) {}
...
/* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
class Item_sp_notation_param final : public Item {
 protected:
  /*
    THD, which is stored in fix_fields() and is used in this_item() to avoid
    current_thd use.
 ...
 public:
  Item_sp_notation_param(const POS &pos,THD *thd, Item* value,
                         const char *param_name, uint param_name_length);

  enum Type type() const override { return NOTATION_PARAM_ITEM; }
  ...
  my_decimal *val_decimal(my_decimal *) override { return &dec; }

  bool get_date(MYSQL_TIME *, my_time_flags_t) override {
    return 0;
  }

  bool get_time(MYSQL_TIME *) override { return 0; }
};
/* #endif */
