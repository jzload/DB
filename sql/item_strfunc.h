...
class Item_func_error_code final : public Item_str_func {
  bool init;
  uint m_sql_err_no;

 public:
  Item_func_error_code() : Item_str_func(), init(false) {}

  void set_error_code(uint _err) {
    init = true;
    m_sql_err_no = _err;
  }
  ...
  bool resolve_type(THD *) override {
    set_data_type_string(uint32(MAX_INT_WIDTH));
    return false;
  }
...
class Item_func_error_message final : public Item_str_func {
  bool init;
  char m_message[MYSQL_ERRMSG_SIZE];

 public:
  // err_msg must end with '\0'
  void set_error_message(const char *err_msg) {
    init = true;
    strcpy(m_message,err_msg);
  }

  Item_func_error_message() : Item_str_func(), init(false) {}
  String *val_str(String *) override;

  bool resolve_type(THD *) override {
    set_data_type_string(uint32(MYSQL_ERRMSG_SIZE));
    return false;
  }
