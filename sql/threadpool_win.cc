...
#define WEAK_SYMBOL(return_type, function, ...)             \
  ...
  static pFN_##function my_##function =                     \
      (pFN_##function)(GetProcAddress(GetModuleHandle("kernel32"), #function))
...
WEAK_SYMBOL(PTP_TIMER, CreateThreadpoolTimer, PTP_TIMER_CALLBACK, PVOID pv,
...
int init_io(connection_t *connection, THD *thd) {
  ...
  switch (vio->type) {
    ...
    case VIO_TYPE_SHARED_MEMORY:
      connection->shm_read = CreateThreadpoolWait(
          shm_read_callback, connection, &connection->callback_environ);
  ...
  if (connection->handle) {
    ...
    connection->io =
        CreateThreadpoolIo(connection->handle, io_completion_callback,
                           connection, &connection->callback_environ);
    if (!connection->io) {
      tp_log_warning("Allocation failed", "CreateThreadpoolWait");
      return -1;
    }
...
int start_io(connection_t *connection, PTP_CALLBACK_INSTANCE instance) {
  ...
  DBUG_ASSERT(vio->type == VIO_TYPE_TCPIP || vio->type == VIO_TYPE_SSL ||
  ...
  if (vio->type == VIO_TYPE_TCPIP || vio->type == VIO_TYPE_SSL) {
    ...
    if (WSARecv(vio->sd, &buf, 1, &num_bytes, &flags, overlapped, NULL) == 0) {
      retval = last_error = 0;
    } else {
      retval = -1;
...
void set_wait_timeout(connection_t *connection, ulonglong old_timeout) {
  ulonglong new_timeout =
      now() + 10000000LL * connection->thd->get_wait_timeout();
...
static void check_thread_init() {
  if (FlsGetValue(fls) == NULL) {
    ...
    num_worker_thread.fetch_add(1, std::memory_order_relaxed);
  }
}
...
static VOID WINAPI thread_destructor(void *data) {
  if (data) {
    tp_stats.num_worker_threads.fetch_sub(1, std::memory_order_relaxed);
  }
}
...
void tp_post_kill_notification(THD *thd) {
  if (current_thd == thd) return; /* There is nothing to do.*/
...
static VOID CALLBACK io_completion_callback(PTP_CALLBACK_INSTANCE instance,
  ...
  if (io_result != ERROR_SUCCESS) goto error;
  ...
  if (threadpool_process_request(connection->thd, &quicksync_refcount)) goto error;

  set_wait_timeout(connection, old_timeout);
  if (start_io(connection, instance)) goto error;
...
static VOID CALLBACK timer_callback(PTP_CALLBACK_INSTANCE instance,
  ...
  if (timeout <= now()) {
    ...
    if (con->thd->net.vio) vio_shutdown(con->thd->net.vio, SD_BOTH);
  } else if (timeout != ULONGLONG_MAX) {
    /*
      Reset timer.
      There is a tiny possibility of a race condition, since the value of
      timeout could have changed to smaller value in the thread doing io
      callback.
...
bool Thread_pool_connection_handler::add_connection(
  ...
  PTP_WORK wrk =
      CreateThreadpoolWork(login_callback, con, &con->callback_environ);
  if (wrk) {
    SubmitThreadpoolWork(wrk);
    CloseThreadpoolWork(wrk);
  } else {
    /* Likely memory pressure */
    login_callback(NULL, con,
                   NULL); /* deletes connection if something goes wrong */
  }
  return true;
}

/**
  Sets the number of idle threads the thread pool maintains in anticipation of
  new requests.
*/
void tp_set_min_threads(uint val) {
  if (pool) SetThreadpoolThreadMinimum(pool, val);
}

void tp_set_max_threads(uint val) {
  if (pool) SetThreadpoolThreadMaximum(pool, val);
}
