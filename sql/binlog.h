...
#define BINLOG_WAIT_AFTER_SYNC    0
#define BINLOG_WAIT_AFTER_COMMIT  1
...
enum enum_gtmgtid_idx_write_state { 
  GTMGTID_IDX_WRITE_NORMAL, 
  GTMGTID_IDX_WRITE_ERROR, 
  GTMGTID_IDX_WRITE_NONE };
...
class MYSQL_BIN_LOG : public TC_LOG {
 ...
 private:
  ...
  /** The instrumentation key to use for opening the gtid_idx file. */
  PSI_file_key m_key_file_log_gtmgtid_idx;
  ...
  */
  ...
  void set_psi_keys(
      ...
      PSI_file_key key_file_log_gtmgtid_idx ) {
    ...
    m_key_file_log_gtmgtid_idx = key_file_log_gtmgtid_idx;
  }
 ...
 public:
  ...
  void rotate_wraper(THD *thd);
  bool is_quick_sync_trans(THD *thd);
  void purge();
  ...
  */
  ...
  bool quick_sync_commit(THD *thd);  
  int  quick_sync_leader_exectue(THD *thd);
  int  quick_sync_leader_ordered_commit(THD *thd);  
  int  quick_sync_leader_paralell_commit(THD *thd);
  int  quick_sync_follower_execute(THD *thd);
  
  int  quick_sync_clearup_and_reply(THD *thd);
  
  bool call_after_sync_hook_ext(THD *thd,
                                 THD *commit_queue,
                                 bool  do_rotate);

  void mark_xid_state_binlogged(THD *thd);
  
  void add_follower_thread_to_work_queue(THD *leader);

  inline bool is_group_quick_sync(THD *thd)
  {
     THD *tmp = thd;
     while (tmp)
     {
         if (!tmp->is_quick_sync())
            return false;
         tmp = tmp->next_to_commit;
     }

      return true;
  }

  void set_ordercommit_and_flush_start_time(THD *thd);  
  void set_flush_end_and_sync_start_time(THD *thd);
  void set_sync_end_and_commit_start_time(THD *thd);
  void set_commit_end_time(THD *thd);  
  void set_ordered_commit_time(THD *thd);

/* 
  about gtid_idx
*/
private:
  /* about 'gtm-gtid' 'binlog pos' idx file */
  bool gtmgtid_idx_switch;
  char gtmgtid_idx_file_name[FN_REFLEN + GTMGTID_IDX_FILE_SUFFIX_LEN];
  IO_CACHE gtmgtid_idx_file;
  enum_gtmgtid_idx_write_state gtmgtid_idx_write_state;
public:
  bool open_gtmgtid_idx_file_and_write_title();
  bool open_gtmgtid_idx_file();
  bool write_gtmgtid_idx_title();
  void flush_gtmgtid_idx(THD *thd);
  int flush_cache_to_gtmgtid_idx_file();
  void close_gtmgtid_idx_file();
  int register_purge_gtmgtid_idx_entry(const char *entry, size_t entry_len);
  bool del_gtmgtid_idx_file_allow_opened(const char *binlog_fname);
  void set_gtmgtid_idx_write_state(enum_gtmgtid_idx_write_state wstate);
  void mark_gtmgtid_idx_write_error();
};
...
enum enum_commit_stage
{
  ENUM_NOT_ENTER_COMMIT = 0,
  ENUM_DO_COMMAND,
  ENUM_BEFORE_FLUSH,
  ENUM_FLUSH_BINLOG,
  ENUM_BEFORE_SYNC,
  ENUM_SYNC_BINLOG,
  ENUM_BEFORE_COMMIT,
  ENUM_BEFORE_SYNC_HOOK1,
  ENUM_BEFORE_SYNC_HOOK2,
  ENUM_COMMIT_TRANS,
  ENUM_REPLY_TO_CLINET,
  ENUM_EPOLL_ADD,
  ENUM_EPOLL_MOD
};
