...
class Query_result_union : public Query_result_interceptor {
  ...
  bool m_tmp_table_on_disk;
#endif /* HAVE_ZSQL_TEMPTABLE */

#ifdef HAVE_ZSQL_MINUS
  bool m_is_minus_op; /* the operation in send_data() */
 ...
 public:
  ...
  Query_result_union()
      : Query_result_interceptor(), m_rows_in_table(0), 
#ifdef HAVE_ZSQL_TEMPTABLE
...
#ifdef HAVE_ZSQL_MINUS
  /* set minus op for send_data() */
  virtual void set_minus_op(bool is_minus_op) { m_is_minus_op = is_minus_op; }
#endif /* HAVE_ZSQL_MINUS */
};
