...
bool Item_func::fix_func_arg(THD *thd, Item **arg) {
  ...
  set_item_option(item->item_options);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
/**
 * Check and reset the reset_dec flag for to_number.
 * Return: true means need reset decimal, flase means not need reset.
 */
bool Item_func::check_and_reset_decimal() {
  bool res = false;
  ResetDecType flag = get_reset_decimal_flag();

  if (NEED_RESET_DEC == flag)
    return true;
  else if (NO_RESET_DEC == flag)
    return false;

  for (uint i = 0; i < arg_count; i++) {
    if (Item::FUNC_ITEM == args[i]->type()) {
      Item_func *arg_item = dynamic_cast <Item_func *>(args[i]);
      DBUG_ASSERT(NULL != arg_item);
      // Recursive calls
      if (arg_item->check_and_reset_decimal()) {
        res = true;
      }
    }
  }
  ...
  if (res) {
    resolve_type(current_thd);
    set_reset_decimal_flag(NEED_RESET_DEC);
  }
  else {
    set_reset_decimal_flag(NO_RESET_DEC);
  }
...
/**
 * reset the reset_dec flag no_reset_dec for to_number.
 */
void Item_func::set_no_reset_dec_flag() {
  for (uint i = 0; i < arg_count; i++) {
    if (Item::FUNC_ITEM == args[i]->type()) {
      Item_func *arg_item = dynamic_cast <Item_func *>(args[i]);
      if (NULL != arg_item)
        arg_item->set_no_reset_dec_flag();
    }
  }
...
#ifdef HAVE_ZSQL_TO_DATE
  if (set_datetime_data_type(r0, r1)) return;
#endif /* HAVE_ZSQL_TO_DATE */
...
#ifdef HAVE_ZSQL_TO_DATE
                || (r0 == INT_RESULT && r1 == DATETIME_RESULT)
                || (r0 == DATETIME_RESULT && r1 == INT_RESULT)
                || (r0 == DATETIME_RESULT && r1 == DATETIME_RESULT)
#endif /* HAVE_ZSQL_TO_DATE */
               );
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      // Reset the Item precision of to_number result to avoid loss precision.
      check_and_reset_decimal();
#endif/* HAVE_ZSQL_ORACLE_COMPATIBILITY */
      uint8 fraction = decimals;
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      if (check_item_option(HAVE_TO_NUMBER_ITEM))
        fraction = get_actual_fraction_dec_val(val);
#endif/* HAVE_ZSQL_ORACLE_COMPATIBILITY */
      my_decimal_round(E_DEC_FATAL_ERROR, val, fraction, false, val);
...
/* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
Item_func_cursor_attribute::Item_func_cursor_attribute(
                                THD *, LEX_STRING *name, sp_instr *i,
                                enum_cursor_attribute_type my_cursor_attr)
    :Item_int_func() {
  fixed = 1;
  m_cursor_name = name;
  m_cursor_attribute_type = my_cursor_attr;
  m_i = i;
  m_runtime_cursor = NULL;

  if(m_cursor_attribute_type == FOUND || \
      m_cursor_attribute_type == NOT_FOUND)
    maybe_null = true;
  else
    maybe_null = false;
}

/* null is only for %FOUND and %NOTFOUND attributes*/
bool Item_func_cursor_attribute::is_null() {
  return !m_runtime_cursor->has_fetched();
}



longlong Item_func_cursor_attribute::val_int() {
  if (!m_runtime_cursor) {
  ...
  if (m_cursor_attribute_type == FOUND) {
    if (m_runtime_cursor->has_fetched() && !m_runtime_cursor->no_data) {
      return 1;
    }
    return 0;
  }
  else if (m_cursor_attribute_type == NOT_FOUND) {
    if (m_runtime_cursor->has_fetched() && m_runtime_cursor->no_data) {
      return 1;
    }
    return 0;
  }
  else if (m_cursor_attribute_type == ISOPEN) {
    return m_runtime_cursor->is_open();
  }
  else if(m_cursor_attribute_type == ROWCOUNT) {
    return m_runtime_cursor->row_count;
  }
  else {
    return -1;
  }
