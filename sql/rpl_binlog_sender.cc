...
const unsigned char Binlog_sender::PACKET_FLAG_SYNC = 0x01;
const int Binlog_sender::PACKET_FLAG_SYNC_INDEX = 2;
const uint32 Binlog_sender::PACKET_MAX_COMBINE_SIZE= 16*1024*1024;
...
void Binlog_sender::cleanup() {
  ...
  if (m_transmit_started)
    (void)RUN_HOOK_INLINE(binlog_transmit, transmit_stop, (thd, m_flag));
...
int Binlog_sender::send_events(File_reader *reader, my_off_t end_pos) {
  ...
  bool need_send_packet_befor_return = false;
  ...
  while (likely(log_pos < end_pos) || end_pos == 0) {
    ...
    if (event_ptr == nullptr) {
      if (end_pos == 0)
      {
        /* need_send_packet_befor_return can be remove, 
           used by compare log_pos with end_pos.  */
        if (need_send_packet_befor_return)
        {
          if (unlikely(send_packet()))
            return 1;
        }
        return 0;  // Arrive the end of inactive file
      }
    ...
    need_send_packet_befor_return = true;
    ...
    */
    if (m_exclude_gtid &&
      ...
      /*
       * set m_need_reset_transmit to true because:
       * if this event is the last to skip, and now m_need_reset_transmit is false.
       * the events need to skip is stored to m_packet, and will be sent to slave with theses events
       * that doesn't to skip. then, some transasctions dump for many times
       */
      m_need_reset_transmit = true;
      need_send_packet_befor_return = false;      
      ...
      */
      ...
      // read all trxs in a flush group, then make a packet
      if (is_need_send_packet())
      {
        if (unlikely(send_packet()))
          return 1;
        need_send_packet_befor_return = false;
      ...
      {
        /*
         * a flush group trxs call send_packet() to make a net pakcet,
         * so, trxs in group dosen't reset m_packet.
         */
        m_need_reset_transmit = false;
      }
    }
  ...
  /* need_send_packet_befor_return can be remove, used by compare log_pos with end_pos.  */
  if (need_send_packet_befor_return)
  {
    if (unlikely(send_packet()))
      return 1;
...
inline int Binlog_sender::reset_transmit_packet(ushort flags,
  ...
  if (m_observe_transmission &&
      RUN_HOOK_INLINE(binlog_transmit, reserve_header, (m_thd, flags, &m_packet))) {
...
inline int Binlog_sender::read_event(File_reader *reader, uchar **event_ptr,
  ...
  if(m_need_reset_transmit)
  {
    if(reset_transmit_packet(0, 0))
      return 1;
  }
...
inline int Binlog_sender::before_send_hook(const char *log_file,
                                           my_off_t log_pos) {
  if (m_observe_transmission &&
      RUN_HOOK_INLINE(binlog_transmit, before_send_event,
...
inline int Binlog_sender::after_send_hook(const char *log_file,
                                          my_off_t log_pos) {
  if (m_observe_transmission &&
      RUN_HOOK_INLINE(binlog_transmit, after_send_event,
               (m_thd, m_flag, &m_packet, log_file, log_pos))) {
...
/*
 * Check is need to send packet.
 * If async, need send packet.
 * If mysqlbinlog dump, need send packet.
 * If semisync and m_packet include all trx for a flush group, need send packet.
 * If packet len > PACKET_MAX_COMBINE_SIZE, need send packet.
 */
inline bool Binlog_sender::is_need_send_packet()
{
  /* Mysqlbinlog maybe set the dump_thread m_flag true,
   * and this will cause the m_observe_transmission true.
   */
  if(m_flag & BINLOG_DUMP_NON_BLOCK)
    return true;

  /*
   * if slave disable semisync, it set m_observe_transmission false when dump start.
   * m_observe_transmission is changed for next dump start.
   * when slave is async, an event a packet
   */
  if (!m_observe_transmission)
    return true;

  /* if it is flush leader, need ack, so need send */
  if (PACKET_FLAG_SYNC == m_packet[PACKET_FLAG_SYNC_INDEX])
    return true;

  if (m_packet.length() > PACKET_MAX_COMBINE_SIZE)
    return true;
