    ...
    if (is_sum_func && !group && !save_sum_fields) { /* Can't calc group yet */
      ...
      */
      ...
      if (type ==  Item::FUNC_ITEM &&
          (down_cast<Item_func *>(item))->functype() == Item_func::ROWNUM_FUNC)
      {
        (down_cast<Item_func *>(item))->result_field = new_field;
      }
...
/**
  check if current command is preparedStmt and going to use cursor fetch 
  under threadpool mechanism. If true, we should assure that no thread_local 
  engine will be used, because this leads to memory problems under 
  threadpool scheduling.

  @param thd           thread handler
  @mem_engine          the mem_engine to use, may be modified in this func
*/
static void check_mem_engine_for_prepared_stmt(THD *thd, enum_internal_tmp_mem_storage_engine &mem_engine) {
  // if use_cursor_fetch == true, result will be stored in a temporary table, 
  // which should not be thread_local cause of threadpool mechanism.
  // so, we use memory engine instead of temptable engine
  if( (Connection_handler_manager::thread_handling == 
          Connection_handler_manager::SCHEDULER_THREAD_POOL)
      && (thd->lex->zsql_options & ZSQL_OPTION_USE_CURSOR_FETCH)
      && (mem_engine == TMP_TABLE_TEMPTABLE) ) {
...
static bool setup_tmp_table_handler(TABLE *table, ulonglong select_options,
  ...
  check_mem_engine_for_prepared_stmt(thd, mem_engine);
