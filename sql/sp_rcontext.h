...
class sp_rcontext {
 ...
 public:
  ...
  bool init_cursor_attr_list(const sp_pcontext *root_parsing_ctx);

  bool set_sp_cursor_for_attr_list(sp_instr *i, sp_cursor *runtime_cursor);
#endif
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  List<Item_func_cursor_attribute> m_cursor_attr_list;
#endif
};
...
class sp_cursor {
 ...
 public:
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  bool no_data;  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  bool fetched; /* has the cursor ever called by 'fetch into' ? */
  uint row_count; /* count of rows from cursor has been fetched until now */
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */

 public:
  explicit sp_cursor(sp_instr_cpush *i)
      : no_data(false), /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
        fetched(false), /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
        row_count(0), /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
        m_result(),
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  bool has_fetched() const { return fetched; }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
