...
{
  return reinterpret_cast<LEX_STRING*>(const_cast<LEX_CSTRING*>(&(thd->query())));
}
...
ulonglong thd_get_next_gtmgtid(const THD *thd) {
  DBUG_ASSERT(thd != nullptr);
  return thd->variables.next_gtmgtid;
}

void* thd_get_gtmgtids(const THD *thd, int64 *len) {
  return thd->get_active_gtmgtids(len);
}

ulonglong thd_get_gtmgtid(const THD *thd) {
  const char *p_trxgtmgtid = thd->get_trx_gtm_gtid();
  if (p_trxgtmgtid)
    return my_strtoull(p_trxgtmgtid, nullptr, 10);
  return 0ULL;
}
