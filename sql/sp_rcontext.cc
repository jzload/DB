...
sp_rcontext *sp_rcontext::create(THD *thd, const sp_pcontext *root_parsing_ctx,
  ...
  if (thd->variables.m_opt_parse_mode == ORACLE_PARSE_MODE && 
      ctx->init_cursor_attr_list(root_parsing_ctx)) {
    delete ctx;
    return NULL;
  }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (thd->variables.m_opt_parse_mode == ORACLE_PARSE_MODE) {
    set_sp_cursor_for_attr_list(i, c);
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (thd->variables.m_opt_parse_mode == ORACLE_PARSE_MODE &&
        cur_spi->get_parsing_ctx()->is_with_exception()) {
      List<Item_func_error_code> *item_error_code_list =
          cur_spi->get_parsing_ctx()->get_error_code_list();
      List<Item_func_error_message> *item_error_message_list =
          cur_spi->get_parsing_ctx()->get_error_message_list();

      if (item_error_code_list && item_error_code_list->elements) {
        Item_func_error_code *itm = NULL;
        List_iterator<Item_func_error_code> iter(*item_error_code_list);
        while ((itm = iter++)) {
          itm->set_error_code(da->mysql_errno());
        }
      }

      if (item_error_message_list && item_error_message_list->elements) {
        Item_func_error_message *itm = NULL;
        List_iterator<Item_func_error_message> iter(*item_error_message_list);
        while ((itm = iter++)) {
          itm->set_error_message(da->message_text());
        }
      }
    }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
bool sp_rcontext::init_cursor_attr_list(const sp_pcontext *root_parsing_ctx) {
  Item_func_cursor_attribute *pCsrAttr = NULL;
  List<Item_func_cursor_attribute>* cursor_attr_list =
      (const_cast<sp_pcontext*>(root_parsing_ctx))->get_cursor_attr_list();
  if (!cursor_attr_list)
    return true;

  uint elements = cursor_attr_list->elements;
  list_node *node = cursor_attr_list->first_node();
  for (uint i=0; i<elements; i++) {
    pCsrAttr = (Item_func_cursor_attribute*)(node->info);
    m_cursor_attr_list.push_back(pCsrAttr);
    node = node->next;
  }

  Mem_root_array<sp_pcontext *> *children =
      (const_cast<sp_pcontext*>(root_parsing_ctx))->get_children();
  if (!children)
    return true;

  for (uint i = 0; i < children->size(); i++) {
    sp_pcontext *ctx = children->at(i);
    init_cursor_attr_list(ctx);
  }

  return false;
}

bool sp_rcontext::set_sp_cursor_for_attr_list(sp_instr *i ,
                                              sp_cursor *runtime_cursor) {
  Item_func_cursor_attribute *pCsrAttr = NULL;
  List_iterator<Item_func_cursor_attribute> iter(m_cursor_attr_list);
  while ((pCsrAttr = iter++)) {
    if (pCsrAttr->m_i == i) {
      pCsrAttr->m_runtime_cursor = runtime_cursor;
    }
  }
...
bool sp_cursor::fetch(THD* thd, List<sp_variable> *vars) {
  ...
  */
  if (!m_server_side_cursor->is_open()) {
    ...
    //cursor is still opened, so the fetch successed. increase the rowcount.
    ++row_count;
  }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
