...
char goldendb_server_version[SERVER_VERSION_LENGTH];
bool opt_gtmgtid_idx;
...
std::atomic<int32> g_quick_sync_err_flag;      // set with variables rpl_semi_sync_master_err_flag
...
mysql_mutex_t LOCK_rotate_binlog;
mysql_cond_t COND_rotate_binlog;
bool rotate_thread_stopped = false;
  ...
  virtual void operator()(THD *killing_thd) {
    ...
    if (killing_thd->kill_immunizer) {
      ...
      */
      ...
      MYSQL_CALLBACK(killing_thd->func_scheduler,
...
static void clean_up(bool print_message) {
  ...
  */
  ...
  // wait for threadpool close completed
      ...
      case SIGQUIT:
        ...
        if (!connection_events_loop_aborted()) {
          ...
          */
          ...
          close_connections();
          /* fix bug: mysqld cannot shutdown when rotate binlog and rotate thread was closed */
          stop_rotate();
        }
        my_thread_end();
...
static void stop_rotate()
{
  DBUG_ENTER("stop_rotate");
  mysql_mutex_lock(&LOCK_rotate_binlog);
  rotate_thread_stopped = true;
...
SHOW_VAR com_status_vars[] = {
    ...
    {"merge_into",
     (char *)offsetof(System_status_var, com_stat[(uint)SQLCOM_MERGE_INTO]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    ...
    {"show_quicksync_status",
     (char*) offsetof(System_status_var, com_stat[(uint)SQLCOM_SHOW_QUICKSYNC_STATUS]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    {"show_threadpool_status",
     (char*) offsetof(System_status_var, com_stat[(uint)SQLCOM_SHOW_THREADPOOL_STATUS]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
#ifdef HAVE_ZSQL_COPY_TABLE
    {"copy_table",
     (char*) offsetof(System_status_var, com_stat[(uint) SQLCOM_COPY_TABLE]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
#endif // HAVE_ZSQL_COPY_TABLE
    {"show_slave_sync_replay_status",
     (char*) offsetof(System_status_var, com_stat[(uint) SQLCOM_SHOW_SLAVE_SYNCREPLAY_STATUS]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
    {"execute_immediate",
     (char*)offsetof(System_status_var, com_stat[(uint) SQLCOM_EXECUTE_IMMEDIATE]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    {"call_declare_block",
     (char *)offsetof(System_status_var, com_stat[(uint)SQLCOM_DECL_BLOCK]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    /* #endif */
...
int init_common_variables() {
  ...
  */
  mysql_bin_log.set_psi_keys(
      ...
      key_file_binlog_index_cache, key_file_binlog_gtid_idx);
#endif
...
#endif
{
  ...
  */
  ...
  /******** semisync inline optimize ************/
  if(semi_sync_master_plugin_init() || semi_sync_slave_plugin_init())
  {
    sql_print_error("Init semi_sync_master or semi_sync_slave failed.");
    unireg_abort(MYSQLD_ABORT_EXIT);
  }
...
struct my_option my_long_early_options[] = {
    ...
    {"goldendb_version", 'V', "Output version information and exit.", 0, 0, 0,
     GET_NO_ARG, NO_ARG, 0, 0, 0, 0, 0, 0},
...
static int show_binlog_current_timestamp(THD *, SHOW_VAR *var, char *buff)
{
  var->type= SHOW_LONG;
  var->value= buff;
  long *value= reinterpret_cast<long*>(buff);
  *value= static_cast<long>(binlog_current_timestamp.load());
  return 0;
}
...
*/
static int show_slave_current_relaylog_pos(THD *, SHOW_VAR *var, char *buff)
{
  channel_map.rdlock();
  ...
  if (mi)
  {
    var->type= SHOW_LONGLONG;
    var->value= buff;
    *((longlong *)buff)= mi->rli->get_event_relay_log_pos();
  }
  else
    var->type= SHOW_UNDEF;
...
int show_threadpool_idle_threads(THD *, SHOW_VAR *var, char *buff)
{
  var->type= SHOW_INT;
  var->value= buff;
...
int show_threadpool_waiting_threads(THD *, SHOW_VAR *var, char *buff)
{
  var->type= SHOW_INT;
  var->value= buff;
...
int show_threadpool_active_threads(THD *, SHOW_VAR *var, char *buff)
{
  var->type= SHOW_INT;
  var->value= buff;
...
SHOW_VAR status_vars[] = {
    ...
    {"Threadpool_active_threads", (char *)&show_threadpool_active_threads,
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Threadpool_threads", (char*)&tp_stats.num_worker_threads,
    ...
    {"Uptime_since_flush_status", (char *)&show_flushstatustime, SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
#endif
    {"Binlog_current_timestamp", (char *)&show_binlog_current_timestamp, SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Slave_current_relaylog_pos",(char *)&show_slave_current_relaylog_pos, SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_status", (char *)&SHOW_FNAME(status),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_clients", (char *)&SHOW_FNAME(clients),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_yes_tx", (char *)&rpl_semi_sync_master_yes_transactions,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_no_tx", (char *)&rpl_semi_sync_master_no_transactions,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_wait_sessions", (char *)&SHOW_FNAME(wait_sessions),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_net_avg_wait_time", (char *)&SHOW_FNAME(avg_net_wait_time),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    ...
    {"Rpl_semi_sync_master_no_times", (char *)&rpl_semi_sync_master_off_times,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_timefunc_failures", (char *)&rpl_semi_sync_master_timefunc_fails,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_wait_pos_backtraverse", (char *)&rpl_semi_sync_master_wait_pos_backtraverse,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    ...
    {"Rpl_semi_sync_master_tx_waits", (char *)&SHOW_FNAME(trx_wait_num),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_tx_avg_wait_time", (char *)&SHOW_FNAME(avg_trx_wait_time),
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_active_groups", (char *)&rpl_semi_sync_master_active_groups,
     SHOW_INT, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_current_wait_groups", (char *)&rpl_semi_sync_master_current_wait_groups,
     SHOW_INT, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_current_stat_master_flag", (char *)&rpl_semi_sync_master_current_stat_master_flag,
     SHOW_INT, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_err_flag", (char *)&rpl_semi_sync_master_err_flag,
     SHOW_INT, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group1_status", (char *)rpl_semi_sync_master_group_status[0],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group2_status", (char *)rpl_semi_sync_master_group_status[1],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group3_status", (char *)rpl_semi_sync_master_group_status[2],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group4_status", (char *)rpl_semi_sync_master_group_status[3],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group5_status", (char *)rpl_semi_sync_master_group_status[4],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group6_status", (char *)rpl_semi_sync_master_group_status[5],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group7_status", (char *)rpl_semi_sync_master_group_status[6],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group8_status", (char *)rpl_semi_sync_master_group_status[7],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group9_status", (char *)rpl_semi_sync_master_group_status[8],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"Rpl_semi_sync_master_group10_status", (char *)rpl_semi_sync_master_group_status[9],
     SHOW_CHAR, SHOW_SCOPE_GLOBAL},
...
static int mysql_init_variables() {
  ...
  my_stpcpy(goldendb_server_version, MYSQL_GOLDENDB_VERSION);
  key_caches.empty();
...
PSI_file_key key_file_binlog_gtid_idx;
...
static PSI_file_info all_server_files[]=
{
  ...
  { &key_file_binlog_gtid_idx, "binlog_gtid_idx", 0, 0, PSI_DOCUMENT_ME}
};
/* clang-format on */
#endif /* HAVE_PSI_INTERFACE */
...
PSI_stage_info stage_quick_sync_commit= { 0, "quick sync commit", 0};
PSI_stage_info stage_quick_sync_clear= { 0, "quick sync clear", 0};
/* clang-format on */
...
PSI_stage_info *all_server_stages[] = {
    ...
    &stage_quick_sync_commit,
    &stage_quick_sync_clear
    };
