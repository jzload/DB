...
class partition_info {
  ...
  */
  ...
  union {
    ...
    part_range_str_list_val *range_str_array;
#endif
...
#ifdef HAVE_ZSQL_RANGE_STR
  size_t max_char_len;
  bool check_partition_value_len(partition_element *part_def);
  bool cmp_str_for_range_str(const partition_element *first_arg,
                            const partition_element *second_arg);
  bool check_support_range_str(void);
#endif

  bool fix_partition_values(THD *thd, part_elem_value *val,
  ...
  */
  ...
  bool is_same_partitioning(partition_info *new_part_info);
  bool compare_part_info(partition_info *new_part_info);
  bool has_same_part_fields(partition_info *new_part_info);
  bool compare_part_elements(partition_info *new_part_info);
