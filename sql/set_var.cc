...
do {                                                \
  T val= *(reinterpret_cast<T*>(const_cast<uchar*>(value)));        \
  CMD;                                              \
} while (0)

#define case_for_integers(CMD)                             \
    case SHOW_SIGNED_INT:      do_num_val (int,CMD);       \
    case SHOW_SIGNED_LONG:     do_num_val (long,CMD);      \
    case SHOW_SIGNED_LONGLONG: do_num_val (longlong,CMD);  \
    case SHOW_INT:             do_num_val (uint,CMD);      \
    case SHOW_LONG:            do_num_val (ulong,CMD);     \
    case SHOW_LONGLONG:        do_num_val (ulonglong,CMD); \
...
#define case_get_string_as_lex_string               \
    case SHOW_CHAR:                                 \
      sval.str= reinterpret_cast<char*>(const_cast<uchar*>(value));       \
    ...
    case SHOW_LEX_STRING:                           \
      sval= *(reinterpret_cast<LEX_STRING*>(const_cast<uchar*>(value)));    \
      break
                          ...
                          THD *thd, enum_var_type type, LEX_STRING *base)
{
  ...
  *is_null= false;
  ...
  switch (show_type())
  {
    ...
    case SHOW_MY_BOOL:  return *(const_cast<uchar*>(value));
    default:            
...
String *sys_var::val_str_nolock(String *str, THD *thd, const uchar *value)
{
  ...
  switch (show_type())
  {
    ...
    case SHOW_MY_BOOL:
      sval= to_lex_string(bools[(int)*(const_cast<uchar*>(value))]);
      break;
    default:
      my_error(ER_VAR_CANT_BE_READ, MYF(0), name.str);
      return 0;
...
Item *sys_var::copy_value(THD *thd) {
  ...
  switch (get_var_type()) {
    ...
    case GET_STR_PTR: {
      const char *tmp_str_val = *(const char**)((void*)session_var_ptr(thd));
      DBUG_ASSERT(tmp_str_val);
      return new Item_string(tmp_str_val, strlen(tmp_str_val),
                             system_charset_info);
    }
