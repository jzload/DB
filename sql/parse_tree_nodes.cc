...
PT_key_part_specification::PT_key_part_specification(Item *expression,
                                                     enum_order order)
    : m_expression(expression), m_order(order), m_prefix_length(0) {
    m_column_name.str    = NULL;
...
Sql_cmd *PT_update::make_cmd(THD *thd) {
  ...
  if (pc.has_row_item && is_multitable) {
     my_error(ER_OPERAND_COLUMNS, MYF(0), column_list->elements());
     return NULL;
  }
 #endif
  /*
...
Sql_cmd *PT_insert::make_cmd(THD *thd) {
  ...
  if (is_insert_all_into) return make_cmd_for_insert_all(thd);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
/*
 * Added for 'declare begin end' block, 
 * which is equalivant to CREATE_PROCEDURE + CALL_PROCEDURE 
 */
Sql_cmd *PT_call_decl_block::make_cmd(THD *thd) {
  LEX *const lex = thd->lex;
  lex->sql_command = SQLCOM_DECL_BLOCK;

  return new (thd->mem_root) Sql_cmd_call_decl_block(sp);
}
...
bool PT_query_specification::contextualize(Parse_context *pc) {
  ...
  //a flag set during transform_swcb(), indicating that names of schema and table should be replaced
  if(get_need_replace_schema_table()) {
    pc->select->if_need_replace_schema_table = true;
  }
  ...
  bool contains_fj_hint = (opt_hints) ? opt_hints->has_full_join_anti_block_hint()
                                        : false;
#endif
  if (!from_clause.empty()) {
#ifdef HAVE_ZSQL_FULL_JOIN
    if (contains_fj_hint) {
      if (from_clause.size() == 1 && from_clause[0]->is_joined_table())
        from_clause[0]->set_full_join_anti();
    }
#endif
...
#ifdef HAVE_ZSQL_FULL_JOIN
      bool contains_other_hints = false;
      if (contains_fj_hint) {
        if (opt_hints->keep_only_full_join_anti_block_hint(contains_other_hints) ||
            opt_hints->contextualize(pc))
          return true;
      }
      if (!contains_fj_hint || contains_other_hints)
#endif
      push_warning_printf(
...
bool PT_connect_engine_option::contextualize(
    Table_ddl_parse_context *pc) {
  if (super::contextualize(pc)) return true;
  ...
  LEX *const lex = thd->lex;
  new (thd->mem_root)
          engine_option_value(option_name, option_value, flag,
                              &pc->create_info->option_list,
                              &lex->option_list_last);
  if(0 == native_strncasecmp(option_name.str, tabname_str.str, 
                             tabname_str.length))
    pc->create_info->tabname_value =  option_value;
  else if(0 == native_strncasecmp(option_name.str, option_list_str.str, 
                                  option_list_str.length))
    pc->create_info->option_list_value =  option_value;
  else if(0 == native_strncasecmp(option_name.str, table_type_str.str, 
                                  table_type_str.length))
    pc->create_info->table_type_value =  option_value;

  pc->create_info->used_fields |= HA_CREATE_WITH_ODBC_OPTIONS;
  return false;
}
...
#ifdef HAVE_ZSQL_CONNECT_ENGINE
      ,field_def->connect_flag, field_def->is_connect
#endif // HAVE_ZSQL_CONNECT_ENGINE
      );
}
...
bool PT_table_locking_clause::set_lock_for_tables(Parse_context *pc) {
  ...
  {
      if (get_lock_descriptor().type == TL_READ_WITH_SHARED_LOCKS)
      {
          my_error(ER_NOT_SUPPORTED_YET, MYF(0), "SELECT FOR SHARE OF ITEMS");
          return true;
      }
      ...
      {
          item = new (pc->mem_root) Item_field(POS(), NullS, table_ident->db.str, table_ident->table.str);

          if (!item || item->itemize(pc, (Item**)(&item))) return true;

          select->m_lock_item_list.push_back(item);
      }

      select->m_table_locking = this;

      return false;
  }

  for (Table_ident *table_ident : m_tables) {
    table_list = select->find_table_by_name(table_ident);
    if (table_list == NULL)
