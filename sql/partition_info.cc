...
#include "sql/item_func.h"  // HAVE_ZSQL_RANGE_STR
...
static bool compare_elem_basic_properties(partition_element *part_elem,
                                          partition_element *new_part_elem);

static bool compare_part_elems_no_subpart(partition_type part_type,
                                          partition_element *part_elem,
                                          partition_element *new_part_elem);

static bool compare_range_part_elemes(partition_element *part_elem,
                                      partition_element *new_part_elem);

#if 0
static bool compare_hash_part_elemes(partition_element *part_elem,
                                     partition_element *new_part_elem);
#endif

static bool compare_list_part_elemes(partition_element *part_elem,
                                     partition_element *new_part_elem);
#endif // HAVE_ZSQL_COPY_TABLE

#ifdef HAVE_ZSQL_RANGE_STR
static uint get_max_char_length(Item_func *part_expr,Field *field);
#endif 
...
bool partition_info::add_named_partition(const char *part_name, size_t length) {
  ...
  if (part_def->is_subpart) {
    ...
    // `truncate partition` handles only normal partitions under Oracle parse mode
    if (current_thd->lex->trunc_part_type == trunc_partition_type::PARTITION &&
        ORACLE_PARSE_MODE == thd->variables.m_opt_parse_mode) {
      my_error(ER_UNKNOWN_PARTITION, MYF(0), part_name, table->alias);
      return true;
    }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    // `truncate subpartition` handles only subpartitions
    if (current_thd->lex->trunc_part_type == trunc_partition_type::SUBPARTITION) {
      my_error(ER_UNKNOWN_PARTITION, MYF(0), part_name, table->alias);
      return true;
    }
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_RANGE_STR
    if (is_range_str) {
      part_range_str_list_val *loc_range_str_array = NULL;
      size_t t_size = sizeof(part_range_str_list_val);

      range_str_array = (part_range_str_list_val *)sql_calloc(num_parts * t_size);
      if (unlikely(range_str_array == NULL)) {
        mem_alloc_error(num_parts * t_size);
        goto end;
      }

      loc_range_str_array = range_str_array;
      partition_element *part_first_def = NULL;
      Field *field = part_field_array[0];
      // max key length is 3072 bytes so substr can not length then 800 char
      max_char_len = get_max_char_length(down_cast<Item_func *>(part_expr), field);
      i = 0;
      do {
        part_def = it++;
        if (check_partition_value_len(part_def)) goto end;

        if (!first) {
          if (!cmp_str_for_range_str(part_first_def, part_def))
            goto range_not_increasing_error;
        }

        if (part_def->max_value)
        {
          loc_range_str_array[i].max_value = true;
          loc_range_str_array[i].value_str = NULL;
        } else {
          loc_range_str_array[i].max_value = false;
          loc_range_str_array[i].value_str = sql_strdup(part_def->range_str);
        }

        part_first_def = part_def;
        first = false;
      } while (++i < num_parts);
    } else {
#endif
...
/*
  fill space if String len less than split_length
  SYNOPSIS
    fill_char_string()
    s                              String
    split_length                   char length
*/
static void fill_char_string(String &s, size_t split_length) {
  if (split_length > s.length()) {
    s.append_with_prefill_to_char_ff(nullptr, 0, (split_length - s.length()),
                                     ' ');
  }
}

/*
  Compare two partition_element values in RANGE substring partitioning
  SYNOPSIS
    cmp_str_for_range_str()
    first_arg                   First partition_element argument
    first_arg                   Second partition_element argument
  RETURN VALUES
    true                        first < second
    false                       first >= second
*/
bool partition_info::cmp_str_for_range_str(
    const partition_element *first_arg, const partition_element *second_arg) {
  if (NULL == first_arg || NULL == second_arg) return false;

  if (first_arg->max_value) return false;

  if (second_arg->max_value) return true;

  Field **field = part_field_array;
  size_t split_length = max_char_len * ((*field)->charset()->mbmaxlen);
  String first_arg_str(first_arg->range_str, (*field)->charset());
  String second_arg_str(second_arg->range_str, (*field)->charset());
  size_t first_arg_len = 0;
  size_t second_arg_len = 0;

  switch ((*field)->type()) {
    case MYSQL_TYPE_STRING:
      // only char need fill space char need get length  goto next  case
      fill_char_string(first_arg_str, split_length);
      fill_char_string(second_arg_str, split_length);

    case MYSQL_TYPE_VAR_STRING:
    case MYSQL_TYPE_VARCHAR:
      first_arg_len = split_length > first_arg_str.length()
                          ? first_arg_str.length()
                          : split_length;
      second_arg_len = split_length > second_arg_str.length()
                           ? second_arg_str.length()
                           : split_length;
      break;

    // can not happend it only string or var_string
    default:
      my_error(ER_PARTITION_FUNCTION_IS_NOT_ALLOWED, MYF(0));
      return false;
  }
  set_if_smaller(first_arg_len, (uint)~0L);
  set_if_smaller(second_arg_len, (uint)~0L);

  // A negative number if first_arg < second_arg
  // A positive number if first_arg > second_arg
  // 0 if strings are equal

  int res = (*field)->charset()->coll->strnncollsp(
      (*field)->charset(), (const uchar *)first_arg_str.ptr(), first_arg_len,
      (const uchar *)second_arg_str.ptr(), second_arg_len);
  return res < 0;
}


/**
  check if support range str or not

  @return Operation status
    @retval false   not support range str
    @retval true    support range str
*/

bool partition_info::check_support_range_str(void)
{
  if (part_expr == NULL)
    return false;

  if (part_type == partition_type::RANGE &&
      part_expr->type() == Item::FUNC_ITEM &&
      part_expr->result_type() == Item_result::STRING_RESULT) {
        const Item_func *func_item = down_cast<const Item_func *>(part_expr);
        if (strcmp(func_item->func_name(), "substr") == 0) {
          return true;
      }
  }
  ...
  if (check_single_table_access(thd, SELECT_ACL, &table_list, true)) {
    ...
    if (column_list)
      ...
      if (part_expr->null_value)
        buf_ptr = "NULL";
      else if (is_range_str) /* #ifdef HAVE_ZSQL_RANGE_STR */
      {
        String str;
        String *res = part_expr->val_str(&str);
        tmp.append("'").append(res->c_ptr_safe()).append("'");
        buf_ptr = tmp.c_str();
      } else
...
void partition_info::report_part_expr_error(bool use_subpart_expr) {
  ...
  if (expr->type() == Item::FIELD_ITEM || is_range_str) {
...
bool partition_info::fix_partition_values(THD *thd,
  ...
  if (col_val->max_value) {
    ...
    if ((val->null_value = item_expr->null_value)) {
      ...
      if (is_range_str) { /* #ifdef HAVE_ZSQL_RANGE_STR */
       if (item_expr->result_type() != STRING_RESULT) {
        my_error(ER_VALUES_IS_NOT_STRING_TYPE_ERROR, MYF(0), part_elem->partition_name);
        return true;
       }
      } else {
        if (item_expr->result_type() != INT_RESULT) {
          my_error(ER_VALUES_IS_NOT_INT_TYPE_ERROR, MYF(0), part_elem->partition_name);
          return true;
        }
      }
    }
    if (part_type == partition_type::RANGE) {
      ...
      if (is_range_str) { /* HAVE_ZSQL_RANGE_STR */
        char buffer[MAX_KEY_LENGTH];
        String str(buffer, sizeof(buffer), &my_charset_bin);
        String *res = item_expr->val_str(&str);
        part_elem->range_str = strmake_root(thd->mem_root, res->ptr(), res->length());
      }
      else
...
/**
  Get substr string  output  max  character length .

  @param Item_func   Item object to substr
  @param field  Field for which the item will substr

  @return Column uint
    @retval the max character length 
  
*/
static uint get_max_char_length(Item_func *part_expr,Field *field){
  int char_len = field->charset()->mbmaxlen;
  size_t substr_out_len = part_expr->max_length;
  return substr_out_len/char_len;
}


/**
  check if partition value length than max char_length .

  @param partition_element   partition def

  @return Column bool
    @retval true failed substr string shorter than cloumn values
            false success substr longer than  cloumn values

*/
bool partition_info::check_partition_value_len(partition_element *part_def) {
  DBUG_TRACE;
  DBUG_ASSERT(part_def != NULL);
  bool result = false;
  size_t str_char_len = 0;

  if (part_def->range_str == NULL) return 0;
  Field *field = part_field_array[0];
  String char_str(part_def->range_str, field->charset());
  str_char_len = char_str.numchars();
  // size_t str_len = char_str.length();
...
#ifdef HAVE_ZSQL_RANGE_STR
  if (check_support_range_str()) {
    is_range_str = true;
    DBUG_ASSERT(!column_list);
  }

  if (is_range_str && part_expr != NULL) {
    const Item_func *func_item = down_cast<const Item_func *>(part_expr);
    if (func_item->arguments()[0]->type() != Item::FIELD_ITEM) {
      my_error(ER_PART_FUNC_TYPE_INCORRECT, MYF(0));
      return true;
    }
  }
#endif
...
#ifdef HAVE_ZSQL_COPY_TABLE
bool 
partition_info::is_same_partitioning(partition_info *new_part_info)
{
  if (compare_part_info(new_part_info))
    return false;

  if (compare_part_elements(new_part_info))
    return false;
...
bool partition_info::compare_part_info(partition_info *new_part_info)
{
  if (part_type == partition_type::NONE)  // don't support subpart
    return true;
  ...
  if (part_type != partition_type::HASH)
  {
    /*
      ...
      Also COLUMNS partitioning was added in 5.5, so treat that as different.
    */
    if (new_part_info->num_subparts != num_subparts ||
        new_part_info->subpart_field_list.elements !=
          subpart_field_list.elements ||
  ...
  {
    /* Check if KEY partitioned. */
    if (new_part_info->part_field_list.elements != part_field_list.elements)
      return true;
  }

  if (!has_same_part_fields(new_part_info))
    return true;

  if (key_algorithm != new_part_info->key_algorithm)
    return true;
...
bool partition_info::has_same_part_fields(partition_info *new_part_info)
{
  /* Check that it will use the same fields in KEY (fields) list. */
  List_iterator<char> old_field_name_it(part_field_list);
  List_iterator<char> new_field_name_it(new_part_info->part_field_list);
...
bool partition_info::compare_part_elements(partition_info *new_part_info)
{
  if (use_default_partitions)
    return false;
  ...
  do {
    ...
    if (compare_elem_basic_properties(part_elem, new_part_elem))
      return true;

    if (is_sub_partitioned())
    {
      // todo: process sub partition
      return true;
    }

    if (compare_part_elems_no_subpart(part_type, part_elem, new_part_elem))
      return true;
...
// caller ensure basic properties are same
static bool compare_part_elems_no_subpart(partition_type part_type,
                                          partition_element *part_elem,
                                          partition_element *new_part_elem)
{
  ...
  case partition_type::RANGE:
    ret = compare_range_part_elemes(part_elem,new_part_elem);
    break;
  case partition_type::HASH:
    ret = false; //compare_hash_part_elemes(part_elem,new_part_elem);
    break;
  case partition_type::LIST:
    ret = compare_list_part_elemes(part_elem,new_part_elem);
    break;
  default:
    break;
  }
...
static bool compare_range_part_elemes(partition_element *part_elem,
                                      partition_element *new_part_elem)
{
  if (part_elem->range_value == new_part_elem->range_value)
    return false;
