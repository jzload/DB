...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  virtual bool is_sp_instr_jump() const { return false; }

  virtual bool is_sp_instr_lex_branch() const { return false; }
#endif
...
class sp_lex_instr : public sp_instr {
 public:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  LEX* get_lex() const { return m_lex; }
  /* #endif */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  void set_first_execution(bool flag) {
    m_first_execution = flag;
  }
#endif
...
class sp_instr_stmt : public sp_lex_instr {
 public:
  sp_instr_stmt(uint ip, LEX *lex, LEX_CSTRING query)
      : sp_lex_instr(ip, lex->get_sp_current_parsing_ctx(), lex, true),
        m_query(query),
        is_dynamic_stmt(false), /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
        m_dynamic_stmt(NULL),
        m_valid(true) {}
 ...
 public:
  ...
  LEX_CSTRING m_query;
  bool is_dynamic_stmt;  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  Item *m_dynamic_stmt;
...
class sp_instr_freturn : public sp_lex_instr {
 public:
  ...
  sp_instr_freturn(uint ip, enum_sp_type type_sp, LEX *lex,
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  virtual bool is_sp_instr_jump() const override { return true; }
#endif
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  virtual bool is_sp_instr_lex_branch() const override { return true; }
#endif
 ...
 public:
  sp_instr_cpush(uint ip, sp_pcontext *ctx, LEX *cursor_lex,
      ...
      : sp_lex_instr(ip, ctx, cursor_lex, true),
        m_cursor_type(STATIC_CURSOR),  /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
        m_cursor_query_parsed(false),
        m_splocal(NULL),
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  void set_splocal(Item_splocal *splocal) { m_splocal = splocal; }

  void set_query(LEX_CSTRING *sql_query) {
    //m_cursor_query.str = sql_query->str;
    //m_cursor_query.length = sql_query->length;
    m_cursor_query  = *sql_query;
  }

  enum_ora_cursor_type m_cursor_type;
  bool m_cursor_query_parsed; // only for ref cursor, if true, lex already parsed
  Item_splocal *m_splocal;
  /* endif */
