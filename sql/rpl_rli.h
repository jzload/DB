...
/* Instrumentation key for performance schema for mts_temp_table_LOCK */
  ...
  */
  ...
  ulonglong group_master_log_timestamp;
 ...
 */
  ...
  */
  ...
  inline ulonglong get_master_log_timestamp() {return group_master_log_timestamp;}
  inline void set_master_log_timestamp(ulonglong ts)
  {
    group_master_log_timestamp = ts;
  }
 ...
 */
  ...
  */
  ...
  /**
    rli_stop_slave_fast store the flag when exec the cmd stop slave fast;
    If rli_slave_stop_fast is set, we can set rli->sql_thread_kill_accepted whitout
    wait up to 60s.
  */
  bool rli_slave_stop_fast;
  void set_slave_stop_fast(bool value)
  {
    if(value != rli_slave_stop_fast)
      rli_slave_stop_fast = value;
    return;
  }
};
...
inline bool is_slave_reapply_thread(const THD *thd)
{
  return (thd->system_thread == SYSTEM_THREAD_SLAVE_WORKER ||
...
inline bool is_slave_repl_thread(const THD *thd)
{
  return (thd->system_thread == SYSTEM_THREAD_SLAVE_IO ||
          thd->system_thread == SYSTEM_THREAD_SLAVE_WORKER ||
          thd->system_thread == SYSTEM_THREAD_SLAVE_SQL);
}
