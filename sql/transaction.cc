...
extern bool support_quick_sync(THD *thd);
extern bool send_bellow_lwm_error_during_execute(THD *thd);
...
bool cleanup_after_trans_commit(THD *thd, bool error)
{
    DBUG_TRACE;
    if (!error)
    {
    ...
    */
    ...
    // above comes from trans_commit , fllow comes from mysql_execute_commnad
    thd->mdl_context.release_transactional_locks();
    /* Begin transaction with the same isolation level. */
    if (thd->m_tx_chain) {
      if (trans_begin(thd)) return true;
    } else {
    ...
    if (thd->m_tx_release) thd->killed = THD::KILL_CONNECTION;
...
bool trans_commit(THD *thd, bool ignore_global_read_lock) {
  ...
  thd->m_trx_type = Transaction_ctx::SESSION;
  if (!thd->m_tx_chain  &&  
      !thd->m_tx_release && 
      support_quick_sync(thd))
    thd->set_maybe_quick_sync();
...
bool cleanup_after_trans_commit_stmt(THD *thd, bool error)
{
  DBUG_TRACE;
  ...
  if (error == false && !thd->in_active_multi_stmt_transaction())
    if (thd->rpl_thd_ctx.session_gtids_ctx().notify_after_transaction_commit(thd))
      sql_print_warning("Failed to collect GTID to send in the response packet!");
