  ...
  */
  if (!thd->lock) {
    ...
    */
    int out_value = 0;
    int hook_error =
        RUN_HOOK_INLINE(binlog_relay_io, applier_log_event, (thd, out_value));
...
bool is_need_stat(Log_event_type type)
{  
  if (type == binary_log::ROTATE_EVENT || 
      type == binary_log::STOP_EVENT ||
...
bool is_need_stat(Log_event *ev)
{
  Log_event_type type = ev->get_type_code();
  
  return is_need_stat(type);
}
