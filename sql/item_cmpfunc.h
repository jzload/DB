...
class Item_func_case : public Item_func {
 ...
 public:
  ...
  /**
    This constructor is used for the decode function
    first_expr_num: 'expr' parameter index
    else_expr_num: 'default' parameter index
  */
  Item_func_case(const POS &pos, List<Item> &args_list)
      : super(pos),
        first_expr_num(-1),
        else_expr_num(-1),
        cached_result_type(INT_RESULT),
        ...
        case_item(0)
    {
      ...
      Item *first_expr_arg = args_list.pop();

      if (args_list.elements & 1) // is odd number
      {
        ncases = args_list.elements - 1;
        else_expr_num = args_list.elements - 1;
      }
      else
        ncases = args_list.elements;

      args_list.push_back(first_expr_arg);
      first_expr_num = args_list.elements - 1;

      set_arguments(args_list, true);
      memset(&cmp_items, 0, sizeof(cmp_items));
  }
