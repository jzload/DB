...
#include "sql/oracle_compatibility/oracle_routine.h" // ORACLE PROCEDURE
#include "sql/oracle_compatibility/merge_into.h" // MERGE INTO
...
{
  if(firstTable->field_translation) {
    // merged
    for (Field_translator *transl= firstTable->field_translation;
         transl < firstTable->field_translation_end;
         transl++) {
      DBUG_ASSERT(transl->item->fixed);
      if( addOneVariable(thd, ctx, transl->name))
        return true;
    }
    ...
    // unexpected case, table not found
    my_error(ER_PROCEDURE_EXECUTE_FAIL, MYF(0), "table not found");
    return true;
  }
...
static bool declVarsByTable(THD *thd, sp_pcontext *ctx, TABLE_LIST *firstTable,
                            const char *dbName, const char *tabName, const char *fieldName)
{
  while (firstTable)
  {
  ...
  if (!firstTable || !firstTable->table) {
    const char *full_name = construct_full_name(thd, dbName, tabName, fieldName);
    if(!full_name) full_name = "[unknown]";

    size_t err_len = 512;
    char *err_msg = new (thd->mem_root) char[err_len];
    snprintf(err_msg, err_len, "unknown variable: %s", full_name);

    my_error(ER_PROCEDURE_EXECUTE_FAIL, MYF(0), err_msg);
    return true;
  }
                                   ...
                                   Item_field *itemField)
{
  ...
  if (declVarsByTable(thd, ctx, table_first,
                      itemField->db_name, itemField->table_name, itemField->field_name))
    return true;
                                   ...
                                   bool *openTable)
{
  ...
  /* bug-fix: sql-cursor-for-loop leads to mysqld-crash if derived table accessed */
  {
    if ((select_lex->derived_table_count || select_lex->table_func_count) &&
        select_lex->resolve_placeholder_tables(thd, true))
      goto func_error;

    // Wait with privilege checking until all derived tables are resolved.
    if (select_lex->derived_table_count &&
        select_lex->check_view_privileges(thd, SELECT_ACL, SELECT_ACL))
      goto func_error;
  }
                                        ...
                                        uint cursorIdx)
{
  ...
  sp_instr_cpush *i= new(thd->mem_root) sp_instr_cpush(sp->instructions(), ctx,
...
static bool createInstrCopen(THD *thd, LEX_STRING *cursorName, bool scope)
{
  ...
  sp_instr_copen *copen= new(thd->mem_root) sp_instr_copen(sp->instructions(), pctx, idx);
...
/* scope:find cursor in current context or global context */
static bool createInstrCclose(THD *thd, LEX_STRING *cursorName, bool scope)
{
  uint idx;
  LEX *lex= thd->lex;
  sp_head *sp= lex->sphead;
  ...
  sp_instr_cclose *cclose= new(thd->mem_root) sp_instr_cclose(sp->instructions(), pctx, idx);
  if (cclose == NULL || sp->add_instr(thd, cclose))
    return true;
                              ...
                              const Mem_root_array<sp_variable *> *spc_vars)
{
  ...
  sp_head *sp= lex->sphead;
  sp_pcontext *pctx= lex->get_sp_current_parsing_ctx();
  sp_instr_cfetch *cfetch= new(thd->mem_root) sp_instr_cfetch(sp->instructions(),
                                               pctx, cursorIdx);
...
static bool createInstrJumpIfNot(THD *thd, Item* jump_expr)
{
  ...
  sp_instr_jump_if_not *jin= new(thd->mem_root) sp_instr_jump_if_not(ip, lex, jump_expr,
                                                      expr_query);
  if (jin == NULL ||
      // Jumping forward
                                 ...
                                 bool reverse)
{
  ...
  sp_instr_set *is= new(thd->mem_root) sp_instr_set(lex->sphead->instructions(),
                                     lex,
                                     idx,
                                     low_bound,
                                     value_query,
  ...
  sp_instr_jump_if_not *i= new(thd->mem_root) sp_instr_jump_if_not(ip,lex,expr,expr_query);
  if (i == NULL ||
...
static bool sp_create_instr_jump(THD *thd, uint)
{
  ...
  sp_instr_jump *i= new(thd->mem_root) sp_instr_jump(ip, spc, lab->ip);
...
/* HAVE_ZSQL_MERGE_INTO begin */
// Specially used in merge into, convert 't1 dt' to '(select * from t1) dt'
static PT_table_reference* convert_single_table_to_derived_table(
                                    const POS &pos,
                                    MEM_ROOT *mem_root,
                                    PT_table_factor_table_ident *table) {
  if (nullptr == table) return nullptr;
  // If single table have no alias, use this table without convert.
  if (nullptr == table->table_alias()) return table;
  ...
  PTI_simple_ident_ident *ident_star =
                          new (mem_root) PTI_simple_ident_ident(pos, star);
  if (ident_star == nullptr) return nullptr;

  PT_select_item_list *item_list1 = new (mem_root) PT_select_item_list();
  ...
  table_reference_list1.init(mem_root);
  if (table_reference_list1.push_back(table)) return nullptr;

  PT_query_specification *query_specification2 = new (mem_root)
                          PT_query_specification(options, item_list1,
  ...
  PT_query_expression *query_expression2 =
                       new (mem_root) PT_query_expression(
                       query_expression_body_primary2,
                       nullptr, nullptr, nullptr);
  if (query_expression2 == nullptr) return nullptr;

  PT_subquery *subquery = new (mem_root) PT_subquery(pos, query_expression2);
  if (subquery == nullptr) return nullptr;

  Create_col_name_list column_names;
  column_names.init(mem_root);
  LEX_CSTRING alias = {table->table_alias(), strlen(table->table_alias())};
  PT_derived_table* derived_table = new (mem_root) PT_derived_table(false,
                                                subquery, alias, &column_names);
  return derived_table;
}
/* HAVE_ZSQL_MERGE_INTO end */
...
%token<lexer.keyword> TABNAME_SYM
%token<lexer.keyword> QUICKSYNC_SYM
%token<lexer.keyword> THREADPOOL_SYM
%token<lexer.keyword> COPY_SYM
%token<lexer.keyword> SYNCREPLAY_SYM
%token<lexer.keyword> MY_LEX_FOR_LOOP_SEP
%token<lexer.keyword> SYS_REFCURSOR_SYM
%token<lexer.keyword> EXCEPTION_SYM
%token<lexer.keyword> CLOB_SYM
%token<lexer.keyword> IMMEDIATE_SYM
%token<lexer.keyword> NOT_FOUND_SYM
%token<lexer.keyword> SQLCODE_SYM
%token<lexer.keyword> SQLERRM_SYM
%token<lexer.keyword> ROWNUM_SYM
%token<lexer.keyword> WITHIN_SYM /*resolve warning:sql_yacc.yy:17009.11-20: warning: type clash on default action: <lexer.keyword> != <> [-Wother]*/
%token LISTAGG_SYM
%token<lexer.keyword> CONNECT_SYM
%token<lexer.keyword> PRIOR_SYM
%token<lexer.keyword> VARCHAR2_SYM
%token SYSTIMESTAMP_SYM
%token MINUS_SYM
/* HAVE_ZSQL_MERGE_INTO begin */
%token<lexer.keyword>  MATCHED
/* HAVE_ZSQL_MERGE_INTO end */
/*
...
%type <lexer.lex_str>
        ...
        LEX_HOSTNAME ULONGLONG_NUM select_alias ident opt_ident ident_or_text table_alias_ident
...
%type <item>
        ...
        field_or_var limit_option row_count
        ...
        opt_having_clause
        opt_simple_limit
        opt_merge_into_subclause_where
...
%type <item_list2>
        ...
        fields_expression_for_insert_all
...
%type <lexer.keyword> ident_keyword label_keyword role_keyword ident_keywords_table_alias
        lvalue_keyword
...
%type <limit_options> limit_options fetch_clause

%type <limit_clause> limit_clause opt_limit_clause
        opt_limit_or_fetch_clause limit_or_fetch_clause
...
/* HAVE_ZSQL_MERGE_INTO begin */
%type <join_table_on> merge_joined_table
/* HAVE_ZSQL_MERGE_INTO end */
...
%type <hierarchical> opt_hierarchical_query_clause
...
/* HAVE_ZSQL_MERGE_INTO begin */
        merge_target_table merge_source_table
/* HAVE_ZSQL_MERGE_INTO end */

%type <query_expression_body> query_expression_body query_expression_body_for_insert_all
...
%type <query_expression> query_expression query_expression_parens
        query_expression_or_parens as_create_query_expression query_expression_for_insert_all
...
%type <table_ident> table_ident_opt_wild table_or_column_ident_opt_wild

%type <table_ident_list> table_alias_ref_list table_locking_list table_or_column_alias_ref_list
...
/*HAVE_ZSQL_INSERT_ALL begin*/
%type <table_fields_values_list> table_fields_values_expression_for_insert_all table_fields_expression_for_insert_all

%type <table_fields_values> table_fields_values_item table_fields_item
/*HAVE_ZSQL_INSERT_ALL end*/

/* HAVE_ZSQL_MERGE_INTO begin */
%type <merge_fields_values> merge_update_clause merge_insert_clause merge_insert_or_update
%type <merge_clauses> merge_update_insert_clause
/* HAVE_ZSQL_MERGE_INTO end */
...
/*
   Types from ZSQL Independent research and development
*/
%type <lexer.lex_str>
        sp_end_name

%type <lex_cstr>
        leave_label_ident ora_label_ident

%type <num>
        sp_opt_inout2
        sp_opt_ora_variable_declares sp_ora_variable_declares sp_ora_variable_declare
        exit_option_expr
        opt_reverse
...
%type <sp_default>
        sp_ora_default
        sp_ora_default_no_pushexpr

%type <lex>
        sp_cursor_stmt

%type <sp_for_loop_ident>
        low_bound_or_cursor opt_up_bound

%type <sp_cursor_open_stmt>
        open_cursor_statement

%type <length_qualifier> qualifier

%type <varchar2_data_type> varchar2_field_length
...
sql_statement:
        ...
        | sp_decl_block END_OF_INPUT
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
            sp_finish_parsing(thd);
            lex->sql_command= SQLCOM_DECL_BLOCK;
            *parse_tree = NEW_PTN PT_call_decl_block(lex->sphead);
          }
        ;
...
execute_immediate:
          EXECUTE_SYM IMMEDIATE_SYM simple_expr
          {
            THD *thd= YYTHD;
            LEX *lex= thd->lex;
            ITEMIZE($3, &$3);
...
create:
        ...
        | CREATE or_replace view_or_trigger_or_sp_or_event
          {
...
sp_pdparam:
          sp_opt_inout ident sp_opt_inout2 type opt_collate sp_ora_default_no_pushexpr
          {
            THD *thd= YYTHD;
            Item *dflt_value_item= $6.expr;
            if (dflt_value_item)
            ...
            {
              var_mode= (sp_variable::enum_mode)$1;
            }
            else
            {
              var_mode= (sp_variable::enum_mode)$3;
            }

            CONTEXTUALIZE($4);
            enum_field_types field_type= $4->type;
            const CHARSET_INFO *cs= $4->get_charset();
            if (merge_sp_var_charset_and_collation(cs, $5, &cs))
              MYSQL_YYABORT;
            ...
            {
              spvar->mode= (sp_variable::enum_mode)$3;
            ...
            if (spvar->field_def.init(thd, "", field_type,
                                      $4->get_length(), $4->get_dec(),
                                      $4->get_type_flags(),
                                      NULL, NULL, &NULL_CSTR, 0,
                                      $4->get_interval_list(),
                                      cs ? cs : thd->variables.collation_database,
                                      $5 != nullptr, $4->get_uint_geom_type(),
                                      nullptr, nullptr, {},
                                      dd::Column::enum_hidden_type::HT_VISIBLE))
            {
              MYSQL_YYABORT;
...
sp_opt_inout2:
        ...
        | IN_SYM
          {
            if (YYTHD->variables.m_opt_parse_mode != ORACLE_PARSE_MODE)
            {
              my_error(ER_ORACLE_MARIADB_MIXED, MYF(0));
              MYSQL_YYABORT;
            }
        ...
        | IN_SYM OUT_SYM
          {
            ...
            $$= sp_variable::MODE_INOUT;
          }
        ;

sp_ora_proc_stmts:
          /* Empty */ {}
        | sp_ora_proc_stmts  sp_ora_proc_stmt ';'
        ;

sp_ora_proc_stmts1:
          sp_ora_proc_stmt ';' {}
        | sp_ora_proc_stmts1 sp_ora_proc_stmt ';'
        ;
...
sp_ora_proc_stmt:
          sp_proc_stmt_statement
        | sp_ora_proc_stmt_return
        | sp_ora_proc_stmt_if
        | sp_ora_labeled_block
        | sp_ora_unlabeled_block
        | sp_ora_labeled_control
        | sp_ora_proc_stmt_unlabeled
//      | case_stmt_specification
        | sp_decl_block
        | sp_proc_stmt_exec_proc
        | sp_proc_stmt_exit
        | sp_proc_stmt_leave
        | sp_proc_stmt_iterate
        | sp_proc_stmt_open
...
sp_proc_stmt_exec_proc:
          IDENT_sys opt_paren_expr_list
          {
            THD *thd= YYTHD;
            LEX *lex= thd->lex;
            lex->sphead->reset_lex(thd);
            Lex->sphead->m_tmp_query= @1.raw.end;

            sp_name *call_spname= makeSpname(thd,NULL,&$1);
            if (!call_spname)
              MYSQL_YYABORT;

            // initCallCmdAndAddRoutineT(thd, call_spname, SP_TYPE_PROCEDURE);
            Parse_tree_root * call_tree = NEW_PTN PT_call(call_spname, $2);
            MAKE_CMD(call_tree);

            sp_head *sp= Lex->sphead;
            const char *expr_start_ptr= @1.raw.end;
            LEX_CSTRING query= make_string(thd, expr_start_ptr, @2.raw.end);
        ...
        | IDENT_sys '.' IDENT_sys opt_paren_expr_list
          {
            THD *thd= YYTHD;
            Lex->sphead->reset_lex(thd);
            Lex->sphead->m_tmp_query= @3.raw.end;
            sp_name *call_spname = makeSpname(thd, &$1, &$3);
            if (!call_spname)
              MYSQL_YYABORT;

            // initCallCmdAndAddRoutineT(thd, call_spname, SP_TYPE_PROCEDURE);
            Parse_tree_root * call_tree = NEW_PTN PT_call(call_spname, $4);
            MAKE_CMD(call_tree);

            sp_head *sp= Lex->sphead;
            const char *expr_start_ptr= @3.raw.end;
            LEX_CSTRING query= make_string(thd, expr_start_ptr, @4.raw.end);
...
sp_proc_stmt_exit:
          EXIT_SYM leave_label_ident exit_option_expr
          {
            if (YYTHD->variables.m_opt_parse_mode != ORACLE_PARSE_MODE)
            {
              my_error(ER_ORACLE_MARIADB_MIXED, MYF(0), "");
            ...
            else
            {
              ...
              */
              ...
              if (n)
              {
                sp_instr_hpop *hpop= new(thd->mem_root) sp_instr_hpop(ip++, ctx);
                if (hpop == NULL)
                  MYSQL_YYABORT;
              ...
              if (n)
              {
                sp_instr_cpop *cpop= new(thd->mem_root) sp_instr_cpop(ip++, ctx, n);
                if (cpop == NULL)
                  MYSQL_YYABORT;
              ...
              if ($3)                      // exit when expr, jump if not
              {
...
exit_option_expr:
        ...
        | WHEN_SYM
          ...
          expr
          {
            ...
            sp_instr_jump_if_not *i = new(thd->mem_root) sp_instr_jump_if_not(ip,lex,$3,
                                                      expr_query);
            if (i == NULL ||
...
sp_decl_block:
          DECLARE_SYM
          {
            ...
            /* add for standalone declare+begin+end block in oracle */
            if (!lex->sphead)
            {
              if(MYSQL_PARSE_MODE == thd->variables.m_opt_parse_mode)
              {
              ...
              {
                // reference to the code of 'create procedure'
                YYTHD->lex->definer= 0;
                Lex->create_info= YYTHD->alloc_typed<HA_CREATE_INFO>();
                if (Lex->create_info == NULL) {
                  my_error(ER_OOM, MYF(0));
                ...
                lex->stmt_definition_begin= @1.cpp.start;
                sp_name *out_sp_name = NULL;
                sp_head *sp= sp_start_parsing_noname(thd, enum_sp_type::PROCEDURE, &out_sp_name);

                if (unlikely(!sp))
                {
                  my_error(ER_GENERATE_SP_NAME, MYF(0));
                  MYSQL_YYABORT;
                }

                lex->sphead= sp;
                lex->spname= out_sp_name;
          ...
          sp_ora_variable_declares sp_ora_unlabeled_block
          { }
        ;
...
sp_ora_proc_stmt_return:
          ...
          sp_opt_return_expr
          {
            ...
            if (!$3 && !isReturnCmdLegal(thd, sp, NULL))
                return true;
...
sp_ora_if:
          ...
          expr THEN_SYM
          {
            ...
            sp_instr_jump_if_not *i= new (lex->thd->mem_root)
              sp_instr_jump_if_not(ip, lex, $2, expr_query);
            if (i == NULL ||
...
sp_ora_elseifs:
          /* Empty */
        | ELSEIF_SYM sp_ora_if
        | ELSE sp_ora_proc_stmts1
        ;
...
sp_proc_stmt_open:
        ...
        | OPEN_SYM ident FOR_SYM open_cursor_statement
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
            uint offset;
            ...
            copen= new(thd->mem_root) sp_instr_copen(sp->instructions(), pctx, offset);
            if (copen == NULL || sp->add_instr(thd, copen))
              MYSQL_YYABORT;

            if (!(cpush= pctx->get_cpush_by_cursor_name($2, false)))
            {
              my_error(ER_UNKNOWN_ERROR, MYF(0), "");
              MYSQL_YYABORT;
            }
            ...
            //if (lex->is_metadata_used())
            {
              cursor_query= make_string(thd, @3.raw.end, @4.raw.end);
...
open_cursor_statement:
          ident
          {
            ...
            if (!(spv= pctx->find_variable($1.str, $1.length, false)))
            {
              my_error(ER_SP_UNDECLARED_VAR, MYF(0), $1.str);
              MYSQL_YYABORT;
            }
...
sp_ora_labeled_block:
          label_ident ':' BEGIN_SYM
          {
            ...
            lex->name= to_lex_string($1);
          }
          sp_ora_block_content sp_opt_label
          {
            if ($6.str)
            {
              LEX *lex= Lex;
              sp_pcontext *pctx= lex->get_sp_current_parsing_ctx();
              sp_label *lab= pctx->find_label($1);
              if (my_strcasecmp(system_charset_info, $6.str, lab->name.str) != 0)
              {
        ...
        | IDENT_sys SET_VAR
          {
            ...
            sp_create_assignment_lex(thd, @2.raw.end);
          }
          expr
          {
            ITEMIZE($4, &$4);
            ...
            sp_pcontext *spc= lex->get_sp_current_parsing_ctx();
            sp_variable *spv= NULL;
            if (!spc || !(spv= spc->find_variable($1.str, $1.length, false)))
              MYSQL_YYABORT;
            ...
            if (sp_create_assignment_instr(thd, @4.raw.end))
              MYSQL_YYABORT;
          }
        ;
...
sp_ora_unlabeled_block:
          ...
          sp_ora_block_content sp_end_name
          {
            if ($4.length)
            {
              sp_head *sp = Lex->sphead;
              if (my_strnncoll(system_charset_info, (const uchar*)$4.str, $4.length,
                               (const uchar*)sp->m_name.str, sp->m_name.length))
              {
                my_error(ER_SP_NAME_MISMATCH, MYF(0), $4.str, sp->m_name.str);
                MYSQL_YYABORT;
              }
            }
          }
...
sp_ora_unlabeled_control:
          LOOP_SYM sp_ora_proc_stmts1 END LOOP_SYM
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
            sp_head *sp= lex->sphead;
        ...
        | FOR_SYM
          {
            if (ORACLE_PARSE_MODE != YYTHD->variables.m_opt_parse_mode)
            {
              my_error(ER_ORACLE_MARIADB_MIXED, MYF(0),"");
              MYSQL_YYABORT;
            }
...
pop_unlabled_control_label:
        ...
        | label_ident
          {
            if (MYSQL_PARSE_MODE == YYTHD->variables.m_opt_parse_mode)
            {
              my_error(ER_SYNTAX_ERROR, MYF(0), "");
              MYSQL_YYABORT;
            }
...
pop_sp_empty_label:
          {
            ...
            sp_label *lab = pctx->pop_label();
            //if there is a close_cursor instr at the end, do it after jump out!
            sp->m_parser_data.do_backpatch(lab, sp->instructions()-(lex->need_close_cur?1:0));
            lex->need_close_cur = false;
            lex->cursor_name_to_close = {NULL,0};
...
ora_while_body:
          ...
          {                             /*$2*/
            ...
            if (lex->is_metadata_used())
            {
              expr_query= make_string(thd, expr_start_ptr, @1.raw.end);
              if (!expr_query.str)
                MYSQL_YYABORT;
            }
            ...
            sp_instr_jump_if_not *i= NEW_PTN
                sp_instr_jump_if_not(sp->instructions(), lex, $1, expr_query);
...
/* low_bound_or_cursor just for parse for loop,maybe variable, cursor and literal  */
low_bound_or_cursor:
          ident
          {
            THD *thd= YYTHD;
            ...
            if (spc && (spv= spc->find_variable($1.str, $1.length, false)))
            {
              ...
              $$.item= splocal;
              $$.name= $1;
              $$.type= 2 ; // 0, err; 1, literal; 2, sp_local; 3, implicit cusor
              lex->safe_to_cache_query=0;
            }
            else if (spc && spc->find_cursor($1, &pos, false))
            {
              $$.item= NULL;
              $$.name= $1;
              $$.type= 3;
...
opt_up_bound:
        ...
        | MY_LEX_FOR_LOOP_SEP low_bound_or_cursor
          {
            $$.item= $2.item;
            $$.type= $2.type;
            $$.name= $2.name;
...
for_body:
          ident IN_SYM opt_reverse low_bound_or_cursor opt_up_bound LOOP_SYM
          {
            THD *thd= YYTHD;
            Lex->parse_place= IN_PARSE_INVALID;
            Lex->need_close_cur = false;
            ...
            if ($4.type == 3)          /* for id in cursor_name */
            {
              spct->m_cursor_value_name= ($1).str;
              if (!$5.empty)
              {
                my_error(ER_FOR_LOOP_SYNTAX_ERROR, MYF(0), "");
                MYSQL_YYABORT;
              }
              if (!init_for_loop_cursor(thd, $4.name))
                MYSQL_YYABORT;
            }
            else                /* for id in 1..var  note:literal or variable */
            {
              ...
              ITEMIZE($4.item, &($4.item));
              ITEMIZE($5.item, &($5.item));

              if (!init_for_loop_normal(thd, $1, $4.item, $5.item, $3))
                  MYSQL_YYABORT;
            }
          }
          sp_ora_proc_stmts1 END LOOP_SYM
          {
            LEX *lex= Lex;
            THD *thd= YYTHD;
            sp_pcontext *ctx= Lex->get_sp_current_parsing_ctx();

            if ($4.type == 3)        //it is cursor
            {
              uint ip= lex->sphead->instructions();
              sp_label *lab= ctx->last_label();
              lab->ip += 1;
              /* Jumping back */
              sp_instr_jump *i= new(thd->mem_root) sp_instr_jump(ip, ctx, lab->ip);
              if (i == NULL || lex->sphead->add_instr(thd, i))
                MYSQL_YYABORT;

              lex->need_close_cur = true;
              lex->cursor_name_to_close = $4.name;
              if(createInstrCclose(thd, &($4.name), false))
                MYSQL_YYABORT;
            ...
            else                    //it is variable or literal
            {
              ...
              sp_create_assignment_lex(thd, @10.raw.end);
              //Lex->sphead->m_tmp_query= m_tmp_query;

              if (create_control_variable_assignment(thd, $1, $3))
                MYSQL_YYABORT;
        ...
        | ident IN_SYM '(' sp_cursor_stmt ')' LOOP_SYM
          {
            //Lex->spcont->m_cursor_value_name= ($1).str;
            THD *thd= YYTHD;
            Lex->need_close_cur = false;
            sp_pcontext *pctx= Lex->get_sp_current_parsing_ctx();
            pctx->m_cursor_value_name= ($1).str;
            ...
            // declare variables
            if (declareImplicitVars(thd, $4, pctx))
              MYSQL_YYABORT;

            if (!(lexCursorName=createImplicitCursorName(cursorIdx)))
              MYSQL_YYABORT;
            Lex->cursor_name_to_close = *lexCursorName;

            // declare a cursor
            if (!(cpush= CreateInstrCpush(thd, $4, pctx, &cursor_query,
          ...
          sp_ora_proc_stmts1 END LOOP_SYM
          {
            ...
            //need to close the cursor after 'for loop'
            lex->need_close_cur = true;
            if(createInstrCclose(thd, &(lex->cursor_name_to_close), false))
              MYSQL_YYABORT;
...
sp_exception_handle:
          /* empty*/ {}
        | EXCEPTION_SYM WHEN_SYM OTHERS_SYM THEN_SYM
          {
            if (MYSQL_PARSE_MODE == YYTHD->variables.m_opt_parse_mode)
            {
              my_error(ER_ORACLE_MARIADB_MIXED, MYF(0), "");
            ...
            sp_instr_hpush_jump *i=
                new(thd->mem_root) sp_instr_hpush_jump(sp->instructions(), pctx, h);
            if (i == NULL || sp->add_instr(thd, i))
              MYSQL_YYABORT;
          ...
          sp_ora_proc_stmts1
          {
            ...
            i= new(thd->mem_root) sp_instr_hreturn(sp->instructions(), ctx);
            if (i == NULL || sp->add_instr(thd, i))
              MYSQL_YYABORT;
...
ts_option_engine:
          opt_storage ENGINE_SYM opt_equal engine_ident_or_text
          {
...
create_table_option:
          ENGINE_SYM opt_equal engine_ident_or_text
          {
        ...
        | SECONDARY_ENGINE_SYM opt_equal engine_ident_or_text
          {
        ...
        | ENCRYPTION_SYM opt_equal TEXT_STRING_sys
	  {
        ...
        | TABNAME_SYM equal TEXT_STRING_sys
          {
            LEX_STRING name;
            ...
            $$= NEW_PTN PT_connect_engine_option(name, $3, true);
          }
        | OPTION_LIST_SYM equal TEXT_STRING_sys
          {
            LEX_STRING name;
            if ($3.length > ENGINE_OPTION_MAX_LENGTH){
        ...
        | TABLE_TYPE_SYM equal ident
          {
            ...
            $$= NEW_PTN PT_connect_engine_option(name, $3, false);
          }
        ;
...
type:
        ...
        | numeric_type float_options field_options
          {
            ...
            /* For oracle NUMERIC/DECIMAL without arguments:
             *   When the precision is not given, use maximum number of
             *   digits(65) as the default.
             *
             *   The maximum number of digits for DECIMAL in Mysql is 65.
             *   The number of digits of DECIMAL in Mysql is 10.
             */
            if (YYTHD->variables.m_opt_parse_mode == ORACLE_PARSE_MODE) { // oracle
              if ($1 == Numeric_type::NUMERIC ||
                  $1 == Numeric_type::DECIMAL) {
                if ($2.length == nullptr) {
                  $2.length = YYTHD->strmake("65", sizeof("65") - 1);
                }
              }
            }
#endif

            /* convert the temporary numeric type to the valid one */
            if ($1 == Numeric_type::NUMBER ||
                $1 == Numeric_type::NUMERIC ||
                $1 == Numeric_type::FIXED) {
              $1 = Numeric_type::DECIMAL;
            }
        ...
        | VARCHAR2_SYM varchar2_field_length
          {
            CHARSET_INFO *charset = NULL;
            Semantics_type length_qualifier = $2.qualifier;
            if (Semantics_type::DEFAULT == length_qualifier)
            {
              if (SEMANTICS_CHAR == YYTHD->variables.nls_length_semantics)
                length_qualifier = Semantics_type::CHAR;
              else
                length_qualifier = Semantics_type::BYTE;
            }

            if (Semantics_type::BYTE == length_qualifier)
              charset = &my_charset_bin;

            $$= NEW_PTN PT_char_type(Char_type::VARCHAR, $2.c_str, charset,
                                     false);
          }
        | VARCHAR2_SYM
          {
            ...
            {
              $$= NEW_PTN PT_char_type(Char_type::VARCHAR, "4000", NULL, false);
            }
            else
            {
              my_error(ER_SYNTAX_ERROR, MYF(0));
...
numeric_type:
        ...
        | NUMBER_SYM  { $$= Numeric_type::NUMBER; }
        | NUMERIC_SYM { $$= Numeric_type::NUMERIC; }
        | FIXED_SYM   { $$= Numeric_type::FIXED; }
        ;
...
varchar2_field_length:
          '(' LONG_NUM qualifier ')' {
            $$.c_str = $2.str;
            $$.qualifier = $3;
          }
        | '(' ULONGLONG_NUM qualifier ')' {
            $$.c_str = $2.str;
            $$.qualifier = $3;
          }
        | '(' DECIMAL_NUM qualifier ')' {
            $$.c_str = $2.str;
            $$.qualifier = $3;
          }
        | '(' NUM qualifier ')' {
...
qualifier:
          /* empty */  { $$= Semantics_type::DEFAULT; }
        | BYTE_SYM     { $$= Semantics_type::BYTE; }
        | CHAR_SYM     { $$= Semantics_type::CHAR; }
        ;
...
column_attribute:
        ...
        | FLAG_SYM equal real_ulonglong_num
          {
            $$= NEW_PTN PT_flag_column_attr($3, true);
          }
        ;
...
/* HAVE_ZSQL_SYSDATE begin */
        | SYSDATE_SYM func_datetime_precision
          {
            $$= NEW_PTN Item_func_now_local(@$, $2);
          }
/* HAVE_ZSQL_SYSDATE end */
        | signed_literal
        ;
...
*/
        ...
        | TRUNCATE_SYM partition_or_subpartition all_or_alt_part_name_list opt_update_global_indexes
          {
...
partition_or_subpartition:
          PARTITION_SYM
          {
            Lex->trunc_part_type = trunc_partition_type::PARTITION;
          }
        | SUBPARTITION_SYM
          {
            Lex->trunc_part_type = trunc_partition_type::SUBPARTITION;
          }

opt_update_global_indexes:
          /* Empty */ {}
        | UPDATE_SYM GLOBAL_SYM INDEXES {}
        ;
/* HAVE_ZSQL_ORACLE_COMPATIBILITY end */
...
slave:
      ...
      | STOP_SYM SLAVE opt_slave_thread_option_list opt_fast opt_channel
        {
          LEX *lex=Lex;
...
opt_fast:
          /* empty */ {Lex->slave_stop_fast=false;}
        | FAST_SYM {Lex->slave_stop_fast=true;}
        ;
...
copy:
        COPY_SYM TABLE_SYM FROM table_ident TO_SYM table_ident
        {
          Lex->sql_command= SQLCOM_COPY_TABLE;
          LEX *lex=Lex;
          SELECT_LEX *sl= Select;
          /* $4 is like the flush table, $6 is like alter table. */
          if (!sl->add_table_to_list(lex->thd, $4, NULL, TL_OPTION_UPDATING,
                                     TL_READ_NO_INSERT, MDL_SHARED_HIGH_PRIO) ||
              !sl->add_table_to_list(lex->thd, $6, NULL, TL_OPTION_UPDATING,
...
query_expression_body:
        ...
        /* HAVE_ZSQL_MINUS begin { */
        | query_expression_body MINUS_SYM query_primary
          {
            $$= NEW_PTN PT_union(NEW_PTN PT_query_expression($1), @1,
                                 true, $3, EXCEPT_TYPE);
          }
        | query_expression_parens MINUS_SYM query_primary
          {
            if ($1 == NULL)
              MYSQL_YYABORT; // OOM

            $$= NEW_PTN PT_union($1, @1, true, $3, EXCEPT_TYPE);
          }
        | query_expression_body MINUS_SYM query_expression_parens
          {
            if ($3 == NULL)
              MYSQL_YYABORT; // OOM
            ...
            auto lhs_qe= NEW_PTN PT_query_expression($1);
            PT_nested_query_expression *nested_qe=
              NEW_PTN PT_nested_query_expression($3);

            $$= NEW_PTN PT_union(lhs_qe, @1, true, nested_qe, EXCEPT_TYPE);
          }
        | query_expression_parens MINUS_SYM query_expression_parens
          {
            if ($1 == NULL || $3 == NULL)
              MYSQL_YYABORT; // OOM
            ...
            PT_nested_query_expression *nested_qe=
              NEW_PTN PT_nested_query_expression($3);
            $$= NEW_PTN PT_union($1, @1, true, nested_qe, EXCEPT_TYPE);
          }
        /* HAVE_ZSQL_MINUS end } */
        ;
...
table_locking_list:
          OF_SYM table_or_column_alias_ref_list { $$= $2; }
        ;
...
bit_expr:
            ...
            else
            {
                 ...
                 ((Parse_tree_item*)percent_right)->b_fielditem_simple())
              {
                ...
                if (0 == strcasecmp("found",psimp_idt_idt->ident.str))
                {
                    ITEMIZE(percent_left, &percent_left);
                    ITEMIZE(percent_right, &percent_right);
                    Item_field *leftField =  down_cast<Item_field *>(percent_left);
                    $$ = make_cursor_attr_func(thd, const_cast<char*>(leftField->field_name),
                                               Item_func_cursor_attribute::FOUND);
                }
                else if (0 == strcasecmp("notfound", psimp_idt_idt->ident.str))
                {
                ...
                else if (0 == strcasecmp("isopen", psimp_idt_idt->ident.str))
                {
                    ITEMIZE(percent_left, &percent_left);
                    ITEMIZE(percent_right, &percent_right);
                    Item_field *leftField  =  down_cast<Item_field *>(percent_left);
                    $$= make_cursor_attr_func(thd, const_cast<char*>(leftField->field_name),
                                              Item_func_cursor_attribute::ISOPEN);
                }
                else if (0 == strcasecmp("rowcount", psimp_idt_idt->ident.str))
                {
                    ITEMIZE(percent_left, &percent_left);
                    ITEMIZE(percent_right, &percent_right);
                    Item_field *leftField  =  down_cast<Item_field *>(percent_left);
                    $$= make_cursor_attr_func(thd, const_cast<char*>(leftField->field_name),
                                              Item_func_cursor_attribute::ROWCOUNT);
                }
                else
                {
                  $$= NEW_PTN Item_func_mod(@$, $1, $3);
        ...
        | simple_expr
        | PRIOR_SYM simple_expr
          {
            $$= NEW_PTN PTI_prior_item(@$, $2);
          }
        ;
...
function_call_nonkeyword:
        ...
        /* HAVE_ZSQL_ORACLE_COMPATIBILITY begin */
        | SYSTIMESTAMP_SYM func_systimestamp_precision
          {
            $$= NEW_PTN Item_func_now_systimestamp(@$, static_cast<uint8>($2));
...
sum_expr:
        ...
        */
        ...
        | LISTAGG_SYM '(' expr opt_listagg_separator ')'
          WITHIN_SYM GROUP_SYM '(' ORDER_SYM BY order_list ')' opt_windowing_clause
          {
            PT_item_list *args= NEW_PTN PT_item_list;
            if (args == NULL || args->push_back($3))
              MYSQL_YYABORT; // OOM

            $$= NEW_PTN Item_func_listagg(@$, args, $4, $11, $13);
          }
        ;
...
sp_expr:
        expr sp_opt_expr
        {
          ...
          else
          {
            ...
            ITEMIZE($2, &$2);
            const POS pos= @$;
            Item *mark_param = createSpNotationItem(pos, thd, $1, $2);
            if (!mark_param)
            {
              MYSQL_YYABORT;
            }
        ...
        | table_reference outer_join_type table_reference ON_SYM expr
          {
            ...
            /* FULL JOIN */
            // Create a derived table for full join
            if ($2 == JTT_FULL)
            {
              YYTHD->syntax_error("can't support fulljoin"); 
              MYSQL_YYABORT;  
            }
            else
#endif
        ...
        | table_reference outer_join_type table_reference USING '(' using_list ')'
          {
            ...
            if ($2 == JTT_FULL)
            {
              $$ = NEW_PTN PT_full_joined_table($1, @2, $2, $3, $6);
              if ($$ == NULL)
                MYSQL_YYABORT;  //OOM
            }
            else
...
#ifdef HAVE_ZSQL_FULL_JOIN
            /* FULL JOIN */
            if ($2 == JTT_NATURAL_FULL)
            {
              $$ = NEW_PTN PT_full_joined_table($1, @2, $2, $3);
              if ($$ == NULL)
                MYSQL_YYABORT;  //OOM
            }
            else
...
natural_join_type:
        ...
        | NATURAL FULL opt_outer JOIN_SYM
        {
#ifdef HAVE_ZSQL_FULL_JOIN
          $$= JTT_NATURAL_FULL;
#endif
...
or_replace:
          /* empty */  { Lex->oracle_options &= (~ORA_CREATE_OR_REPLACE); }
        | OR_SYM REPLACE_SYM
...
/*
  hierarchical query clause in select
*/

opt_hierarchical_query_clause:
          /* empty */ { $$= NULL; }
        | START_SYM WITH expr CONNECT_SYM BY expr
         {
           YYTHD->syntax_error("can't support start with connect by"); 
           MYSQL_YYABORT;          
         }
        ;
...
opt_limit_or_fetch_clause:
          /* empty */           { $$= NULL; }
        | limit_or_fetch_clause { $$= $1; }
        ;

limit_or_fetch_clause:
          limit_clause { $$= $1; }
        | fetch_clause { $$= NEW_PTN PT_limit_clause($1); }
        ;

fetch_clause:
        FETCH_SYM first_or_next row_count row_or_rows ONLY_SYM
        {
          $$.opt_offset= NULL;
          $$.limit= $3;
...
table_or_column_alias_ref_list:
          table_or_column_ident_opt_wild
          {
            $$.init(YYMEM_ROOT);
            if ($$.push_back($1))
              MYSQL_YYABORT; // OOM
          }
        | table_or_column_alias_ref_list ',' table_or_column_ident_opt_wild
          {
            $$= $1;
            if ($$.push_back($3))
              MYSQL_YYABORT; // OOM
...
merge_stmt:
          MERGE_SYM                   /* #1 */
          INTO                        /* #2 */
          merge_joined_table          /* #3 */
          merge_update_insert_clause  /* #4 */
          {
            $$= NEW_PTN PT_merge($3,
                                 $4.insert_fields_values,
                                 $4.update_fields_values,
                                 $4.merge_option);
            if (nullptr == $$) {
              MYSQL_YYABORT;
            }
            Lex->oracle_options &= (~(ORA_MERGE_INTO_WITH_UPDATE |
                                      ORA_MERGE_INTO_WITH_INSERT));
            Lex->oracle_options |= $4.merge_option;
          }

merge_update_insert_clause:
          WHEN_SYM merge_insert_or_update
          {
            if ($2->merge_option == ORA_MERGE_INTO_WITH_UPDATE) {
              $$.update_fields_values = $2;
              $$.insert_fields_values = nullptr;
              $$.merge_option = ORA_MERGE_INTO_WITH_UPDATE;
            }
            else if ($2->merge_option == ORA_MERGE_INTO_WITH_INSERT) {
              $$.insert_fields_values = $2;
              $$.update_fields_values = nullptr;
        ...
        | merge_update_insert_clause WHEN_SYM merge_insert_or_update
          {
            if ($1.merge_option & $3->merge_option) {
                YYTHD->syntax_error_at(@2);
                MYSQL_YYABORT;
            }
            ...
            if ($3->merge_option == ORA_MERGE_INTO_WITH_UPDATE) {
              $$.update_fields_values = $3;
              $$.merge_option |= ORA_MERGE_INTO_WITH_UPDATE;
            }
            else if ($3->merge_option == ORA_MERGE_INTO_WITH_INSERT) {
              $$.insert_fields_values = $3;
              $$.merge_option |= ORA_MERGE_INTO_WITH_INSERT;
            }
            else {
              YYTHD->syntax_error();
...
merge_joined_table:
          merge_target_table          /* #1 */
          USING                       /* #2 */
          merge_source_table          /* #3 */
          ON_SYM                      /* #4 */
          '('                         /* #5 */
          expr                        /* #6 */
          ')'                         /* #7 */
          {
            $$ = NEW_PTN PT_joined_table_on($1, @$, JTT_RIGHT, $3, $6);//type=JTT_RIGHT
            if (nullptr == $$) {
              MYSQL_YYABORT;
            }
            $$->join_in_merge_into = true;
          };
...
merge_source_table:
          ...
          {
            /**
               Single table without alias will not to conver to a derived table
               and cannot be materialized if needed.
               Single table with alias like: "t1 dt", will be convert to derived table:
               "(select * from t1) dt", so that the derived table can be materialized if needed.
               Occasion to materialize: "merge t1 using t1 dt ...".
            */
            $$ = convert_single_table_to_derived_table(
                 @$,YYMEM_ROOT,down_cast<PT_table_factor_table_ident *>($1));
            if (nullptr == $$) {
              MYSQL_YYABORT;
            }
          }
...
merge_update_clause:
          ...
          SET_SYM               /* #4 */
          update_list           /* #5 */
          opt_merge_into_subclause_where      /* #6 */
          //opt_merge_delete_clause     /* #8 */
          {
            $$ = NEW_PTN Merge_fields_values($5.column_list,
                                             $5.value_list,
                                             $6,
                                             ORA_MERGE_INTO_WITH_UPDATE);
            if (nullptr == $$) {
              MYSQL_YYABORT;
            }
          };
...
merge_insert_list:
          VALUES row_value /* value_or_values */
          {
            $$.column_list = NEW_PTN PT_item_list;
            if (nullptr == $$.column_list) {
              MYSQL_YYABORT;
            }
            $$.value_list= $2;
          }
...
opt_merge_into_subclause_where:
          /* empty */   { $$ = nullptr; }
        | WHERE expr    { $$ = NEW_PTN PTI_nonspecific_context(@2, $2); }
        ;
/*HAVE_ZSQL_MERGE_INTO end*/
/*
...
/* HAVE_ZSQL_INSERT_ALL begin */
        | INSERT_SYM                                       /* #1 */
          ALL                                              /* #2 */
          INTO                                             /* #3 */
          table_fields_values_expression_for_insert_all    /* #4 */
          SELECT_SYM                                       /* #5 */
          select_item_list                                 /* #6 */
          FROM                                             /* #7 */
          DUAL_SYM                                         /* #8 */
          {
            YYTHD->syntax_error("can't support insert all into"); 
            MYSQL_YYABORT;   
          }
        | INSERT_SYM                              /* #1 */
          ALL                                     /* #2 */
          INTO                                    /* #3 */
          table_fields_expression_for_insert_all  /* #4 */
          query_expression_for_insert_all         /* #5 */
          {
            YYTHD->syntax_error("can't support insert all into"); 
            MYSQL_YYABORT;  
          }
...
query_expression_body_for_insert_all:
/* HAVE_ZSQL_INSERT_ALL begin */
          query_primary
          {
            $$= NEW_PTN PT_query_expression_body_primary($1);
          }
        | query_expression_body_for_insert_all UNION_SYM union_option query_primary
          {
            $$= NEW_PTN PT_union(NEW_PTN PT_query_expression($1), @1, $3, $4);
          }
        | query_expression_body_for_insert_all UNION_SYM union_option query_expression_parens
          {
            if ($4 == NULL)
              MYSQL_YYABORT; // OOM
            ...
            auto lhs_qe= NEW_PTN PT_query_expression($1);
            if (NULL == lhs_qe) MYSQL_YYABORT;

            PT_nested_query_expression *nested_qe=
              NEW_PTN PT_nested_query_expression($4);
            if (NULL == nested_qe) MYSQL_YYABORT;

            $$= NEW_PTN PT_union(lhs_qe, @1, $3, nested_qe);
          }
        | query_expression_body_for_insert_all MINUS_SYM query_primary
          {
            $$= NEW_PTN PT_union(NEW_PTN PT_query_expression($1), @1,
                                 true, $3, EXCEPT_TYPE);
          }
        | query_expression_body_for_insert_all MINUS_SYM query_expression_parens
          {
            if ($3 == NULL)
              MYSQL_YYABORT; // OOM
...
query_expression_for_insert_all:
          query_expression_body_for_insert_all
          opt_order_clause
          opt_limit_or_fetch_clause
          opt_locking_clause_list
          {
            if (NULL == $1) MYSQL_YYABORT;
            $$= NEW_PTN PT_query_expression($1, $2, $3, $4);
          }
        ;
...
table_fields_item:
           table_ident fields_expression_for_insert_all
           {
             $$.fields = NEW_PTN table_fields_pair;
             if (NULL == $$.fields) {
               MYSQL_YYABORT;
             }
             $$.fields->table = $1;
             $$.fields->column_list = $2;
           }
         ;

table_fields_expression_for_insert_all:
          table_fields_expression_for_insert_all INTO table_fields_item
          {
            $$.table_columns_list->push_back($3.fields);
          }
        | table_fields_item
          {
            $$.table_columns_list = NEW_PTN List<struct table_fields_pair>;
            if (NULL == $$.table_columns_list) {
              MYSQL_YYABORT;
            }
            $$.table_columns_list->push_back($1.fields);
          }
/* HAVE_ZSQL_INSERT_ALL end */
        ;

table_fields_values_expression_for_insert_all:
/* HAVE_ZSQL_INSERT_ALL begin */
          table_fields_values_expression_for_insert_all INTO table_fields_values_item
          {
            $$.table_columns_list->push_back($3.fields);
            $$.table_values_list->push_back($3.values);
          }
        | table_fields_values_item
          {
            $$.table_columns_list = NEW_PTN List<table_fields_pair>;
            $$.table_values_list  = NEW_PTN List<table_values_pair>;
            if (NULL == $$.table_columns_list || NULL == $$.table_values_list) {
              MYSQL_YYABORT;
            }

            $$.table_columns_list->push_back($1.fields);
            $$.table_values_list->push_back($1.values);
          }
        ;

table_fields_values_item:
          table_ident insert_from_constructor
          {
            $$.fields = NEW_PTN table_fields_pair;
            if (NULL == $$.fields) {
              MYSQL_YYABORT;
            ...
            $$.fields->table = $1;
            $$.fields->column_list = $2.column_list;

            $$.values = NEW_PTN table_values_pair;
            if (NULL == $$.values) {
              MYSQL_YYABORT;
            }
            $$.values->table = $1;
            $$.values->row_values_list = $2.row_value_list;
          }
/* HAVE_ZSQL_INSERT_ALL end */
        ;
...
update_elem:
          ...
          | '(' simple_ident_nospvar ',' expr_list ')' equal  row_subquery
          {
            auto *item_row = NEW_PTN Item_row(@$, $2, $4->value);
            auto *single_row = NEW_PTN PTI_singlerow_subselect(@$, $7);
            if (item_row == nullptr || single_row == nullptr) {
...
truncate_stmt:
          TRUNCATE_SYM opt_table table_ident opt_no_purge
          {
            $$= NEW_PTN PT_truncate_table_stmt($3);
          }
        ;

/* Only used for ZSQL Syntax compatibility.
 * Not report error when call "truncate table xxx no purge" */
opt_no_purge:
         /* empty */
        | NO_SYM PURGE
        ;
...
show_param:
        ...
        | ENGINE_SYM engine_ident_or_text show_engine_param
          {
        ...
        | SLAVE SYNCREPLAY_SYM STATUS_SYM
          {
...
opt_export_format:
        ...
        | GDB_FORMAT_SYM text_string
          {
            $$.pump_type= PUMP_UNKNOWN;
            if (!$2->ptr()) {
              YYTHD->syntax_error_at(@2);
              MYSQL_YYABORT;
            }

            if(0 == my_strcasecmp(system_charset_info, $2->ptr(), "ECSV")) {
              $$.pump_type= PUMP_EXPORT_ECSV;
              Lex->datapump_csv= true;
            } else if(0 == my_strcasecmp(system_charset_info, $2->ptr(), "nospace_filling")) {
              $$.pump_type= PUMP_EXPORT_NOSPACE_CSV;
              Lex->datapump_csv= true;
            } else if(0 == my_strcasecmp(system_charset_info, $2->ptr(), "JCSV")) {
              $$.pump_type= PUMP_EXPORT_JCSV;
              Lex->datapump_csv= true;
              Lex->zsql_options |= ZSQL_OPTION_PUMP_EXPORT_JCSV;  // HAVE_ZSQL_GDB_FORMAT
            } else {
              YYTHD->syntax_error_at(@2);
              MYSQL_YYABORT;
            }
        ...
        | GDB_FORMAT_SYM text_string
          {
            ...
            /* use jcsv instead of NO_ESCAPES, as requirement */
            if(0 == my_strcasecmp(system_charset_info, $2->ptr(), "JCSV")) {
              $$.pump_type= PUMP_IMPORT_NO_ESCAPES;
              Lex->datapump_csv= true;
            } else {
              YYTHD->syntax_error_at(@2);
              MYSQL_YYABORT;
...
/* HAVE_ZSQL_MERGE_INTO begin */
/**
   MERGE_SYM cannot be ident but can be used to assign engine like "engine=merge".
*/
engine_ident_or_text:
          ident_or_text { $$=$1;}
        | MERGE_SYM
...
/* FULL JOIN: FULL cannot be used as table alias */
ident_keywords_table_alias:
          ident_keywords_unambiguous
        | ident_keywords_ambiguous_1_roles_and_labels
        | ident_keywords_ambiguous_2_labels
...
set_expr_or_default:
        ...
        | MERGE_SYM
          {
            /**
              set default_storage_engine=merge;
              MERGE_SYM cannot be a ident but can be set to a variable.
            */
            $$= NEW_PTN Item_string(@$, "MERGE", 5, system_charset_info);
          }
        ;
...
sp_tail_body:
        ...
        | sp_ora_chistic sp_ora_proc_stmt
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
...
sp_opt_ora_variable_declares:
          /* empty */ { $$= 0; }
        | sp_ora_variable_declares { $$= $1; }
        ;

sp_ora_variable_declares:
          sp_ora_variable_declare
          { $$= 1; }
        | sp_ora_variable_declares sp_ora_variable_declare
          { $$= $1 + 1; }
        ;
...
sp_ora_variable_declare:
          IDENT_sys
          {
            ...
            if (spc->find_variable($1.str, $1.length, true))
            {
              my_error(ER_SP_DUP_VAR, MYF(0), $1.str);
              MYSQL_YYABORT;
            }
          ...
          type opt_collate sp_ora_default ';'
          {
            THD *thd= YYTHD;
            LEX *lex= thd->lex;
            sp_head *sp= lex->sphead;
            ...
            if (dflt_value_item)
            {
              ...
              if (lex->is_metadata_used())
              {
                dflt_value_query= make_string(thd, expr_start_ptr, @5.raw.end);
                if (!dflt_value_query.str)
                  MYSQL_YYABORT;
              }
            }
        ...
        | CURSOR_SYM IDENT_sys IS sp_cursor_stmt ';'
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
            sp_head *sp= lex->sphead;
            ...
            i= new(thd->mem_root) sp_instr_cpush(sp->instructions(), pctx, $4,cursor_query,
                                  pctx->current_cursor_count());
            if (i == NULL || sp->add_instr(thd,i) || pctx->add_cursor($2))
              MYSQL_YYABORT;
            LEX_STRING *csrName= (LEX_STRING*)sql_memdup(&$2, sizeof(LEX_STRING));
            pctx->push_cursor_instr(csrName,i,pctx->current_cursor_count());
          }
        | IDENT_sys SYS_REFCURSOR_SYM ';'
          {
            THD *thd= YYTHD;
            LEX *lex= Lex;
            sp_head *sp= lex->sphead;
            ...
            if (pctx->find_cursor($1, &offp, TRUE))
            {
            ...
            i= new(thd->mem_root) sp_instr_cpush(sp->instructions(), pctx, sublex,cursor_query,
                                  pctx->current_cursor_count());
            if (i == NULL ||  sp->add_instr(thd,i) || pctx->add_cursor($1))
              MYSQL_YYABORT;
            i->m_cursor_type = REF_CURSOR;
            LEX_STRING *csr_name= (LEX_STRING*)sql_memdup(&$1, sizeof(LEX_STRING));
            if(!csr_name) {
              MYSQL_YYABORT;
            }
            pctx->push_cursor_instr(csr_name, i, pctx->current_cursor_count());
          }
        ;
