...
bool SELECT_LEX::prepare(THD *thd) {
  ...
  */
  ...
  if (m_table_locking)
  {
      // Set up the OF cluase
      if (setup_of_clause(thd)) return true;

      // acquire tables' sw lock.
      if (acquire_tables_sw_lock(thd)) return true;
  }
...
bool simplify_const_condition(THD *thd, Item **cond, bool remove_cond,
                                     bool *ret_cond_value) {
...
loop:
  ...
  for (TABLE_LIST *tl = get_table_list(); tl; tl = tl->next_local) {
    ...
    // If tl is source table in merge into SQL command and contain target table.
    // materialize tl.
    if (materialize_for_merge_into(thd, tl)) return true;
  ...
  */
  ...
  /*
    ...
    semi-join (which is done in flatten_subqueries()). The requirements are:
      ...
      14. SELECT_LEX in subquery predicate do not have rownum
  */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
      && !predicate->unit->slave->with_rownum                     // 14
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
      )
...
#ifdef HAVE_ZSQL_FULL_JOIN
  /* FULL JOIN */
  bool is_fj_first_select = false;
  if (check_and_create_full_join_fields_list(thd,
                                             is_fj_first_select))
    return true;
#endif
    ...
    if (item->type() == Item::FIELD_ITEM && (item_field = (Item_field *)item) &&
      ...
      */
      if (subsel && subsel->substype() == Item_subselect::EXISTS_SUBS &&
          ...
          && allow_replace_wild_in_exists_subselect(subsel->unit)
...
#ifdef HAVE_ZSQL_FULL_JOIN
                          /* FULL JOIN */
                          , is_fj_first_select, full_join_fields_list
#endif
                          ))
          return true;
      }
  ...
  if (derived_select->is_ordered()) {
    ...
    */
    ...
    if ((lex->sql_command == SQLCOM_SELECT ||
      ...
      for (ORDER *ord = derived_select->order_list.first;
           ord; ord = ord->next) {
        if (ord->used_alias)
          ord->item_ptr = *ord->item;
      }
