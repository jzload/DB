 ...
 public:
  JOIN(THD *thd_arg, SELECT_LEX *select)
      : select_lex(select),
        ...
        m_rownum_is_pushed_to_sort(false),
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  /**
    When there is only one non-const input table and the sort is moved to table,
    rownum should be evaluated in front of a sort,
    so that it does not need to be evaluated again.
  */
  bool m_rownum_is_pushed_to_sort;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  bool replace_rownum_item();
  bool make_rownum_info();
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  /**
