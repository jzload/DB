  ...
  */
  ...
  if (net->error && net->vio != 0) {
    if (!thd->killed) {
      ...
      if (0 != thd->req_wait_time)
      {
        if(0 != thd->pre_dispatch_time)
          queue_time = (thd->pre_dispatch_time - thd->req_wait_time) / 1000;
        else
          queue_time = (my_micro_time() - thd->req_wait_time) / 1000;
      }
      ...
      LogErr(INFORMATION_LEVEL, ER_ABORTED_CONNECTION_WAIT_TIME,
             thd->thread_id(), queue_time);
    }
  }
}
