...
bool process_single_event(THD *thd, Master_info *mi, const char *event_buf, uint event_len);
    ...
    if (is_parallel_warn || (!rli->is_parallel_exec() &&
      ...
      if (rli->abort_slave) {
        ...
        */
        ...
        /* for stop slave fast without wait up to 60s. */
        if(rli->rli_slave_stop_fast)
        {
          rli->sql_thread_kill_accepted = true;
          rli->set_slave_stop_fast(false);
        }
...
void show_slave_sync_replay_status_metadata(List<Item> &field_list)
{
  field_list.push_back(new Item_return_int("Sync_Log_Timestamp", 21,MYSQL_TYPE_LONGLONG));
  field_list.push_back(new Item_return_int("Replay_Log_Timestamp", 21,MYSQL_TYPE_LONGLONG));
  field_list.push_back(new Item_return_int("Relaylog_Completed", 3,MYSQL_TYPE_TINY));
}

bool show_slave_sync_replay_status_send_data(THD *thd, Master_info *mi)
{
  DBUG_ENTER("show_slave_sync_replay_status_send_data");
  ...
  protocol->store((ulonglong) mi->get_master_log_timestamp());
  protocol->store((ulonglong) mi->rli->get_master_log_timestamp());
  protocol->store_tiny((longlong)mi->transaction_parser.is_not_inside_transaction());
...
bool show_slave_sync_replay_status(THD *thd)
{
  List<Item> field_list;
  Protocol *protocol= thd->get_protocol();
  Master_info *mi= NULL;
  ...
  DBUG_ENTER("show_slave_sync_replay_status(THD)");
  channel_map.assert_some_lock();

  show_slave_sync_replay_status_metadata(field_list);
  if (thd->send_result_metadata(&field_list,
                                Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF))
  {
    goto err;
  ...
  for (mi_map::iterator it= channel_map.begin(); it!=channel_map.end(); it++)
  {
    ...
    {
      if (show_slave_sync_replay_status_send_data(thd, mi))
        goto err;
...
bool show_slave_sync_replay_status_cmd(THD *thd)
{
  LEX *lex= thd->lex;
  bool res;

  DBUG_ENTER("show_slave_sync_replay_status_cmd");

  channel_map.rdlock();

  if (!lex->mi.for_channel)
  {
    res= show_slave_sync_replay_status(thd);
  }
  else
  {
     // dosent support multisource
     res = false;     
  }
...
static int request_dump(THD *thd, MYSQL *mysql, MYSQL_RPL *rpl, Master_info *mi,
  ...
  *suppress_warnings = false;
  if (RUN_HOOK_INLINE(binlog_relay_io, before_request_transmit,
               (thd, mi, binlog_flags)))
    return 1;
...
apply_event_and_update_pos(Log_event **ptr_ev, THD *thd, Relay_log_info *rli) {
  ...
  DBUG_PRINT("exec_event",
             ("%s(type_code: %d; server_id: %u)", ev->get_type_str(),
  ...
  */
  ...
  if (ev) {
    ...
    */
    ...
    if ((exec_res == 0) && (*ptr_ev) && !(ev->is_artificial_event() || ev->is_relay_log_event()))
    {
      // update rli->master_total_log_pos && master_log_timestamp if its not paralell replay
      if (is_need_stat(ev))
      {
        rli->set_master_log_timestamp(rli->last_master_timestamp);
      }
    }
...
extern "C" void *handle_slave_io(void *arg) {
  ...
  my_thread_init();
  {
    ...
    if (RUN_HOOK_INLINE(binlog_relay_io, thread_start, (thd, mi))) {
      mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR,
                 ER_THD(thd, ER_SLAVE_FATAL_ERROR),
...
requesting master dump") ||
      ...
      while (!io_slave_killed(thd, mi)) {
        ...
        */
        ...
        packet_len = read_event(mysql, &rpl, mi, &suppress_warnings);
        if (check_io_slave_killed(thd, mi,
                                  "Slave I/O thread killed while \
reading event"))
          goto err;
        ...
        packet = (const char *)mysql->net.read_pos + 1;
        DBUG_PRINT("info", ("IO thread received event of type %s",
                            Log_event::get_type_str(
                                (Log_event_type)event_buf[EVENT_TYPE_OFFSET])));
        if (RUN_HOOK_INLINE(binlog_relay_io, after_read_event,
                     (thd, mi, (const char *)mysql->net.read_pos + 1, packet_len,
                      &packet, &packet_len))) {
          mi->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR,
                     ER_THD(thd, ER_SLAVE_FATAL_ERROR),
        ...
        while (packet_len>0)
        {
          event_len = uint4korr(event_buf+EVENT_LEN_OFFSET);

          // how to process error
          if (process_single_event(thd, mi, event_buf, event_len))
            goto err;

          event_buf += event_len;
          packet_len -= event_len;
        }
        ...
        event_buf = event_buf - event_len;
      

        if (RUN_HOOK_INLINE(binlog_relay_io, after_queue_event,
  ...
  err:
    ...
    (void)RUN_HOOK_INLINE(binlog_relay_io, thread_stop, (thd, mi));
    /*
...
bool mts_checkpoint_routine(Relay_log_info *rli, bool force) {
  ...
  */
  ...
  rli->set_master_log_timestamp(rli->gaq->lwm.ts);
...
extern "C" void *handle_slave_sql(void *arg) {
  ...
  my_thread_init();
  {
    ...
    if (RUN_HOOK_INLINE(binlog_relay_io, applier_start, (thd, rli->mi))) {
      mysql_cond_broadcast(&rli->start_cond);
      mysql_mutex_unlock(&rli->run_lock);
      rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR,
                  ER_THD(thd, ER_SLAVE_FATAL_ERROR),
...
QUEUE_EVENT_RESULT queue_event(Master_info *mi, const char *buf,
  ...
  ulong s_id;
  int lock_count = 0;
  int is_artificial_event = 0;
  int is_relaylog_event = 0;
  ...
  */
  ...
  is_artificial_event = uint2korr(buf + FLAGS_OFFSET) & LOG_EVENT_ARTIFICIAL_F;
  is_relaylog_event = uint2korr(buf + FLAGS_OFFSET) & LOG_EVENT_RELAY_LOG_F;
  
  if (!is_relaylog_event && !is_artificial_event)
  {
    if (is_need_stat(event_type))
    {
      mi->set_master_log_timestamp(uint4korr(buf));
    }
  }
...
int stop_slave(THD *thd, Master_info *mi, bool net_report, bool for_one_channel,
  ...
  */
  ...
  if (thread_mask) {
    if (thread_mask & (SLAVE_SQL|SLAVE_FORCE_ALL))
      mi->rli->set_slave_stop_fast(thd->lex->slave_stop_fast);
    slave_errno =
...
int reset_slave(THD *thd, Master_info *mi, bool reset_all) {
  ...
  */
  ...
  (void)RUN_HOOK_INLINE(binlog_relay_io, after_reset_slave, (thd, mi));
  ...
  */
  if (reset_all) {
    /*
       'RESET SLAVE ALL' means this DB is no longer a slave host. Master host should 
       record gtid_idx file, but here can't do this until binlog first rotate after 
       'RESET SLAVE ALL', to avoid record hole.
    */
    mysql_bin_log.set_gtmgtid_idx_write_state(GTMGTID_IDX_WRITE_NONE);
...
/**
  process one single event for dump-io thread optimize.
  ...
  @return false is returned if there has no error, otherwise true is returned.
 */
bool process_single_event(THD *thd, Master_info *mi, const char *event_buf, uint event_len)
{
  Relay_log_info *rli= mi->rli;
