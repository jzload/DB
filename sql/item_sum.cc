...
bool Item_sum_num::fix_fields(THD *thd, Item **ref) {
  ...
  for (uint i = 0; i < arg_count; i++) {
    ...
    set_item_option(args[i]->item_options);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    if (value != NULL && check_item_option(HAVE_TO_NUMBER_ITEM))
      value->set_item_option(HAVE_TO_NUMBER_ITEM);
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  */
  if (result->length() > item->group_concat_max_len) {
    ...
    */
    ...
    uint error_code = ER_CUT_VALUE_GROUP_CONCAT;
    if (item->sum_func() == Item_sum::Sumfunctype::LISTAGG_FUNC) error_code = ER_CUT_VALUE_LISTAGG;

    push_warning_printf(
        current_thd, Sql_condition::SL_WARNING, error_code,
        ER_THD(current_thd, error_code), item->row_count);
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref) {
  ...
  */
  ...
  if (sum_func() == LISTAGG_FUNC)
      group_concat_max_len = static_cast<uint>(thd->variables.m_listagg_max_len);
  else
      group_concat_max_len = static_cast<uint>(thd->variables.group_concat_max_len);
...
bool Item_func_group_concat::setup(THD *thd) {
  ...
  DBUG_ASSERT((m_is_window_function && !aggr_select) || select == aggr_select);
  ...
  */
  ...
  count_field_types(select, tmp_table_param, all_fields, false, true);
...
String *Item_func_group_concat::val_str(String *) {
  ...
  if (table && table->blob_storage &&
    ...
    uint error_code = ER_CUT_VALUE_GROUP_CONCAT;
    if (sum_func() == LISTAGG_FUNC) error_code = ER_CUT_VALUE_LISTAGG;

    push_warning_printf(
        current_thd, Sql_condition::SL_WARNING, error_code,
        ER_THD(current_thd, error_code), row_count);
  }
