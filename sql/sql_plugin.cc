...
*/
sys_var *find_plugin_sysvar(st_plugin_int *plugin, SYS_VAR *plugin_var)
{
...
static bool is_semisync_plugin(const LEX_STRING *dl)
{
    if(dl->length == strlen("semisync_master.so"))
    {
        if(!strcmp(dl->str, "semisync_master.so"))
        {
            sql_print_warning("The semisync_master plugin is inline, can not install.");
            return true;
        }
    }

    if(dl->length == strlen("semisync_slave.so"))
    {
        if(!strcmp(dl->str, "semisync_slave.so"))
        {
            sql_print_warning("The semisync_slave plugin is inline, can not install.");
            return true;
        }
    }
...
static bool plugin_add(MEM_ROOT *tmp_root, LEX_CSTRING name,
  ...
  if(is_semisync_plugin(dl))
  {
    mysql_rwlock_unlock(&LOCK_system_variables_hash);
    mysql_mutex_unlock(&LOCK_plugin);
    if (report & REPORT_TO_USER)
      report_error(report, ER_INSTALL_PLUGIN_FAILED, dl->str);
    return true;
  }
