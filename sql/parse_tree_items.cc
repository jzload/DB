...
bool PTI_table_wild::itemize(Parse_context *pc, Item **item) {
  ...
  //replace db & table name
  fix_db_table_name(pc);

  Item_replace_info *ri = &name_replace_info;
  *item = new (pc->mem_root) Item_field(POS(), ri->replace_new_schema_name, ri->replace_new_table_name, "*");
  if (*item == NULL || (*item)->itemize(pc, item)) return true;
  pc->select->with_wild++;

  return pass_item_replace_info(*item, this);
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
    //used in recursive syntax, replace shema/table names
    fix_db_table_name(pc);

    Item_replace_info *ri = &name_replace_info;
    if ((pc->select->parsing_place != CTX_HAVING) ||
        (pc->select->get_in_sum_expr() > 0)) {
      *res = new (pc->mem_root) Item_field(POS(), ri->replace_new_schema_name, ri->replace_new_table_name, ident.str);
    } else {
      *res = new (pc->mem_root) Item_ref(POS(), ri->replace_new_schema_name, ri->replace_new_table_name, ident.str);
    }
    if (*res == NULL || (*res)->itemize(pc, res)) return true;

    return pass_item_replace_info(*res, this);
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
  }
  return *res == NULL;
}
...
static bool isCursorNameInOracleMode(THD *thd, const char *name)
{
  LEX *lex = thd->lex;
  ...
  if (ORACLE_PARSE_MODE == thd->variables.m_opt_parse_mode &&
      lex->sphead &&
      lex->sphead->m_type == enum_sp_type::PROCEDURE &&
      ctx &&
      ctx->m_cursor_value_name &&
      0 == my_strcasecmp(system_charset_info, name, ctx->m_cursor_value_name)
     )
    return true;
...
bool PTI_simple_ident_q_3d::itemize(Parse_context *pc, Item **res) {
  ...
  if ((pc->select->parsing_place != CTX_HAVING) ||
      (pc->select->get_in_sum_expr() > 0)) {
    *res = new (pc->mem_root) Item_field(POS(), ri->replace_new_schema_name, ri->replace_new_table_name, field);
  } else {
    *res = new (pc->mem_root) Item_ref(POS(), ri->replace_new_schema_name, ri->replace_new_table_name, field);
  }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    sp_pcontext *ctx= lex->get_sp_current_parsing_ctx();
    if (isCursorNameInOracleMode(thd, table)) {
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
      if(pc->select->if_need_replace_schema_table) {
        //sp not considered
        my_error(ER_START_WITH_CONNECT_BY, MYF(0), "not supported in sp currently");
        return true;
      }
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
      
      sp_variable *spv = nullptr;
      LEX_STRING var_name = {const_cast<char*>(field), strlen(field)};

      if ((spv = ctx->find_variable(field, strlen(field), true))) {
        /* We're compiling a stored procedure and found a variable */
        if (! lex->parsing_options.allows_variable) {
          my_error(ER_VIEW_SELECT_VARIABLE, MYF(0));
          return true;
        ...
        *res = splocal;
        ...
        if((*res)->itemize(pc, res))
          return true;
      } else {
        const char *full_name = construct_full_name(thd, nullptr, table, field);
        if(!full_name) full_name = "[unknown]";

        my_error(ER_SP_UNDECLARED_VAR, MYF(0), full_name);
        return true;
      }
    } else {
