...
bool JOIN::optimize() {
  ...
  if (select_lex->olap == ROLLUP_TYPE && select_lex->with_rownum)
  {
    my_error(ER_MISUSE_ROWNUM, MYF(0), "with ROLLUP at the same time");
    return true;
  }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  // extract conds comparing rownum item and const items from and_cond
  // no need to extract when plan_is_const is true.
  if (where_cond && (select_lex->m_rownum_option & ROWNUM_IN_WHERE_COND) &&
      extract_rownum_from_where_cond(thd, &where_cond, &where_cond_rownum))
    return true;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  if (having_cond) {
  ...
  */
  ...
  // Check if all where condition in merge into clause is optimized to impossible where
  if (select_lex->query_result()->no_merge_result_attached()) {
    zero_result_cause = "Impossible WHERE";
    best_rowcount = 0;
    create_iterators_for_zero_rows();
    goto setup_subq_exit;
  ...
  */
  ...
  /*
    ...
    (9) If there is rownum item in select list, and ORDER BY exists or
    the first windowing step needs sorting, we need a tmp table and sort on the
    tmp table instead of the const tables.
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
        || ((select_lex->m_rownum_option & ROWNUM_IN_SELECT_LIST_OR_HAVING) &&  // (9)
         (order || (m_windows.elements != 0 && m_windows[0]->needs_sorting())))
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
       )
      need_tmp_before_win = true;
  }
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  if (make_rownum_info()) return true;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
