...
bool Sql_cmd_create_table::execute(THD *thd) {
  ...
  //if CONNECT engine table is created with primary key, return error.
  if((lex->create_info->used_fields & HA_CREATE_WITH_ODBC_OPTIONS)
      && lex->alter_info->key_list.size()){
    lex->alter_info->key_list.clear();
    //my_error(ER_CANT_CREATE_TABLE, MYF(0), "on CONNECT engine with primary key");
