...
bool Sql_cmd_load_table::execute_inner(THD *thd,
  ...
  */
  ...
  const int escape_char =
      (escaped->length() &&
       m_exchange.datapump.pump_type != PUMP_IMPORT_NO_ESCAPES &&
...
bool Sql_cmd_load_table::is_null_field(THD *thd, READ_INFO &read_info,
                                       ...
                                       uchar *field_value,
                                       uint field_length) {
  if (thd->variables.import_space_as_null) {
    if (!read_info.enclosed && 0 == field_length)
      return true;
  } else if ((!read_info.enclosed &&
              (enclosed_length && field_length == 4 &&
               !memcmp(field_value, STRING_WITH_LEN("NULL")))) ||
             (field_length == 1 && read_info.found_null))
    return true;
