...
bool Prepared_statement::prepare(const char *query_str, size_t query_length) {
  ...
  */
  DBUG_ASSERT(lex->sphead == NULL || lex->sql_command == SQLCOM_DECL_BLOCK || error != 0);
...
bool Prepared_statement::execute(String *expanded_query, bool open_cursor) {
  ...
  */
  ...
  if (!error) {
    ...
    if (open_cursor) {
      lex->safe_to_cache_query = 0;
      // a flag to avoid temptable engine usage
      lex->zsql_options |= ZSQL_OPTION_USE_CURSOR_FETCH;
      ...
      */
      ...
      thd->executing_prepare_stmt = true;
      error = mysql_execute_command(thd, true);
      thd->executing_prepare_stmt = false;
  ...
  */
  ...
  if (error == 0 &&
    ( (this->lex->sql_command == SQLCOM_CALL) ||
    (this->lex->sql_command == SQLCOM_DECL_BLOCK) )) //HAVE_ZSQL_ORACLE_COMPATIBILITY
