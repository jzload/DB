...
/*
extern bool adjust_union_parent(Parse_context *pc, PT_query_specification *connect_by_query_node, PT_union *union_parent);
extern bool adjust_query_expr_parent(Parse_context *pc, PT_query_specification *connect_by_node, \
    PT_query_expression *old_query);
*/
...
class PT_order_expr : public Parse_tree_node, public ORDER {
 ...
 public:
  ...
  void *clone(Parse_tree_transformer *ptt) const override; //HAVE_ZSQL_ORACLE_COMPATIBILITY
};
...
class PT_table_reference : public Parse_tree_node {
  ...
  */
  ...
  virtual bool is_joined_table() const { return false; }
  virtual void set_full_join_anti() { DBUG_ASSERT(false); };
#endif
};
...
class PT_table_factor_table_ident : public PT_table_reference {
 ...
 public:
  ...
  const char *table_alias() { return opt_table_alias; } //HAVE_ZSQL_START_WITH_CONNECT_BY

  Table_ident *get_table_ident() { return table_ident; } //HAVE_ZSQL_START_WITH_CONNECT_BY
...
class PT_table_factor_joined_table : public PT_table_reference {
 ...
 public:
  ...
  void *clone(Parse_tree_transformer *ptt) const override; //HAVE_ZSQL_ORACLE_COMPATIBILITY
#ifdef HAVE_ZSQL_FULL_JOIN
  bool is_joined_table() const override { return true; }
  void set_full_join_anti() override;
#endif
...
class PT_joined_table : public PT_table_reference {
 ...
 public:
...
#ifdef HAVE_ZSQL_FULL_JOIN
                /* FULL JOIN */
                type == JTT_NATURAL_FULL || type == JTT_FULL ||
#endif
                type == JTT_RIGHT);
  }
  ...
  bool is_joined_table() const override { return true; }
  void set_full_join_anti() override {
    is_full_join_anti = true;
  }
  bool is_full_join_anti{false};
  bool is_full_join()
  {
    return m_type == JTT_FULL || m_type == JTT_NATURAL_FULL;
  }
#endif

#ifdef HAVE_ZSQL_MERGE_INTO
  bool join_in_merge_into;

  void convert_to_straight_join() {
    // Designed for merge into
    // covert from right join to straight join when there is only update clause
    DBUG_ASSERT(m_type & JTT_RIGHT);
    // works only when the right join is converted to inner join
    tr2->straight = true;
  }
#endif /* HAVE_ZSQL_MERGE_INTO */
...
#ifdef HAVE_ZSQL_MERGE_INTO
    if (!join_in_merge_into) {
      if (tab1_node->contextualize(pc))
        return true;
    }
    else {
      Yacc_state &yyps = pc->thd->m_parser_state->m_yacc;
      thr_lock_type tmp_lock_type = yyps.m_lock_type;
      enum_mdl_type tmp_mdl_type  = yyps.m_mdl_type;
      ...
      yyps.m_lock_type = tmp_lock_type;
      yyps.m_mdl_type  = tmp_mdl_type;
    }

    if (tab2_node->contextualize(pc))
      return true;
#else
...
#ifdef HAVE_ZSQL_FULL_JOIN
    /* FULL JOIN */
    if (is_full_join_anti && !tr2->nested_join)
      tr2 = pc->select->nest_last_join(pc->thd, 1);
#endif
...
class PT_hierarchical : public Parse_tree_node {
  typedef Parse_tree_node super;
 ...
 public:
  PT_hierarchical(Item *recurs_begin_arg, Item *recurs_end_arg)
      : m_recurs_begin(recurs_begin_arg), m_recurs_end(recurs_end_arg) {}

  Item *recurs_begin() { return m_recurs_begin; }
  Item *recurs_end() { return m_recurs_end; }

  void *clone(Parse_tree_transformer *ptt) const override;
};
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
...
class PT_query_expression_body : public Parse_tree_node {
  ...
  */
  ...
  virtual bool is_query_body_contain_insert_all_select_from_dual() {return false;}
#endif /* HAVE_ZSQL_INSERT_ALL */
...
#ifdef HAVE_ZSQL_INSERT_ALL
  virtual bool is_query_primary_contain_insert_all_select_from_dual() {return false;}
#endif /* HAVE_ZSQL_INSERT_ALL */
...
class PT_query_specification : public PT_query_primary {
 ...
 public:
  PT_query_specification(
      ...
      PT_hierarchical *opt_hierarchical_clause_arg, //HAVE_ZSQL_START_WITH_CONNECT_BY
      ...
      : opt_hints(opt_hints_arg),
        ...
        opt_hierarchical_clause(opt_hierarchical_clause_arg), //HAVE_ZSQL_START_WITH_CONNECT_BY
  ...
  PT_query_specification(
      ...
      : opt_hints(NULL),
        ...
        from_clause(from_clause_arg),
        opt_where_clause(opt_where_clause_arg),
        opt_hierarchical_clause(NULL), //HAVE_ZSQL_START_WITH_CONNECT_BY
...
#ifdef HAVE_ZSQL_START_WITH_CONNECT_BY
  PT_hierarchical *hierarchical_clause() {return opt_hierarchical_clause;}

  void set_hierarchical_clause(PT_hierarchical *new_val) {opt_hierarchical_clause = new_val;}

  PT_hint_list *get_opt_hints() { return opt_hints; }

  void set_opt_hints(PT_hint_list *opt_hints_arg) { opt_hints = opt_hints_arg; }

  Query_options get_options() { return options; }

  void set_options(Query_options options_arg) { options = options_arg; }

  PT_item_list * get_item_list() { return item_list; }

  void set_item_list(PT_item_list *arg) { item_list = arg; }

  const  Mem_root_array_YY<PT_table_reference *> &get_from_clause() { return from_clause; }

  void set_from_clause(Mem_root_array_YY<PT_table_reference *> from_clause_arg) { from_clause = from_clause_arg; }

  Item * where_clause() { return opt_where_clause; }

  void set_where_clause(Item *new_where_clause) { opt_where_clause = new_where_clause; }

  PT_group *group_clause() { return opt_group_clause; }

  void set_group_clause(PT_group *group_clause_param) { opt_group_clause = group_clause_param; }

  Item *having_clause() { return opt_having_clause; }

  void set_having_clause(Item *having_clause_param) { opt_having_clause = having_clause_param; }

  PT_window_list *window_clause() { return opt_window_clause; }

  bool get_need_replace_schema_table() { return need_replace_schema_table; }

  void set_need_replace_schema_table(bool val) { need_replace_schema_table = val; }

#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */

#ifdef HAVE_ZSQL_INSERT_ALL
  bool is_query_primary_contain_insert_all_select_from_dual() {
...
class PT_query_expression final : public Parse_tree_node {
 public:
  ...
  bool contextualize(Parse_context *pc) override {
    ...
    if(pc->thd->lex->is_swcb && transform_swcb(pc, transformed)) {
      return true;
    }
    if(transformed) { pc->select->can_fix_outer_field = false; }
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */
  ...
  /// Called by HAVE_ZSQL_START_WITH_CONNECT_BY
  void set_body(PT_query_expression_body *new_body) { m_body = new_body; }
  ...
  PT_with_clause *with_clause() {
    return m_with_clause;
  }

  void set_with_clause(PT_with_clause *new_with_clause) { m_with_clause = new_with_clause; }

  PT_order *order() { return m_order; }
  PT_limit_clause *limit() { return m_limit; }
  PT_locking_clause_list *locking_clauses() { return m_locking_clauses; }

  bool transform_swcb(Parse_context *pc, bool &transformed) ;
#endif /* HAVE_ZSQL_START_WITH_CONNECT_BY */

#ifdef HAVE_ZSQL_INSERT_ALL
  bool is_insert_all_select_from_dual() {
    return m_body->is_query_body_contain_insert_all_select_from_dual();
  }
#endif /* HAVE_ZSQL_INSERT_ALL */
...
class PT_query_expression_body_primary : public PT_query_expression_body {
 public:
  ...
  PT_query_primary *query_primary() { return m_query_primary; } //HAVE_ZSQL_START_WITH_CONNECT_BY

#ifdef HAVE_ZSQL_INSERT_ALL
  bool is_query_body_contain_insert_all_select_from_dual() {
    return m_query_primary->is_query_primary_contain_insert_all_select_from_dual();
  }
#endif /* HAVE_ZSQL_INSERT_ALL */
...
/* PT_union: used in yacc for UNION, MINUS */
class PT_union : public PT_query_expression_body {
 public:
...
#ifdef HAVE_ZSQL_MINUS
           , sub_select_type linkage_type = UNION_TYPE
#endif
           )
      : m_lhs(lhs),
        m_lhs_pos(lhs_pos),
...
#ifdef HAVE_ZSQL_MINUS
        m_linkage_type(linkage_type),
#endif
        m_rhs(rhs) {}

  bool contextualize(Parse_context *pc) override;

  /* For UNION, MINUS, is_union() is true */
  ...
  PT_query_primary *rhs() { return m_rhs; } //HAVE_ZSQL_START_WITH_CONNECT_BY

  void set_rhs(PT_query_primary *rhs_val) { m_rhs = rhs_val; } //HAVE_ZSQL_START_WITH_CONNECT_BY
 ...
 private:
  ...
  bool m_is_distinct;
#ifdef HAVE_ZSQL_MINUS
  sub_select_type m_linkage_type;  /* UNION, or MINUS(except) ... */
#endif // HAVE_ZSQL_MINUS
...
class PT_insert final : public Parse_tree_root {
...
#ifdef HAVE_ZSQL_INSERT_ALL
  bool is_insert_all_into;
  List<table_values_pair> *table_values_list;
  List<table_fields_pair> *table_columns_list;
#endif /* HAVE_ZSQL_INSERT_ALL */
 ...
 public:
  PT_insert(bool is_replace_arg, PT_hint_list *opt_hints_arg,
    ...
    is_insert_all_into = false;
  }

#ifdef HAVE_ZSQL_INSERT_ALL
  //constructed function for sql 'insert all into values select'
  PT_insert(bool is_replace_arg, PT_hint_list *opt_hints_arg,
            thr_lock_type lock_option_arg, bool ignore_arg,
            Table_ident *table_ident_arg, List<String> *opt_use_partition_arg,
            PT_item_list *column_list_arg,
            ...
            PT_item_list *opt_on_duplicate_column_list_arg,
            PT_item_list *opt_on_duplicate_value_list_arg,
            bool is_insert_all_into_flag,
            List<table_values_pair> *table_values_list_arg,
            List<table_fields_pair> *table_columns_list_arg)
      : is_replace(is_replace_arg),
        opt_hints(opt_hints_arg),
        lock_option(lock_option_arg),
        ignore(ignore_arg),
        ...
        opt_on_duplicate_column_list(opt_on_duplicate_column_list_arg),
        opt_on_duplicate_value_list(opt_on_duplicate_value_list_arg),
        is_insert_all_into(is_insert_all_into_flag),
        table_values_list(table_values_list_arg),
        table_columns_list(table_columns_list_arg) {
    // REPLACE statement can't have IGNORE flag:
    DBUG_ASSERT(!is_replace || !ignore);
    // REPLACE statement can't have ON DUPLICATE KEY UPDATE clause:
    DBUG_ASSERT(!is_replace || opt_on_duplicate_column_list == NULL);
  ...
  Sql_cmd *make_cmd_for_insert_all(THD *thd);

  bool Contextualize_for_insert_all(THD *thd, Parse_context &pc);
#endif /* HAVE_ZSQL_INSERT_ALL */
...
class PT_call_decl_block final : public Parse_tree_root {
  sp_head *sp;

 public:
  PT_call_decl_block(sp_head *sp_arg)
      : sp(sp_arg) {}
...
/**
   used for connect engine
  @ingroup PT_connect_engine_option
*/
class PT_connect_engine_option : public PT_create_table_option {
  using super = PT_create_table_option;

  LEX_STRING option_name{nullptr, 0};
  LEX_STRING option_value{nullptr, 0};
  bool flag{false};

 public:
  explicit PT_connect_engine_option(LEX_STRING &name,
                                    LEX_STRING &value, bool flag)
      : option_name(name), option_value(value), flag(flag) {}
...
class PT_load_table final : public Parse_tree_root {
 public:
  PT_load_table(enum_filetype filetype, thr_lock_type lock_type,
                ...
                Datapump_format opt_import_format)
