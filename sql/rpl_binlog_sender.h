...
class Binlog_sender : Gtid_mode_copy {
  ...
  */
  ...
  /* reset transmit_packet for first event before send,
   * used for send-recv events optimize.
   */
  bool m_need_reset_transmit;

  /*
   * header info for a dump packet,
   * PACKET_FLAG_SYNC_INDEX: sync flag pos in header
   */
  const static unsigned char PACKET_FLAG_SYNC;
  const static int PACKET_FLAG_SYNC_INDEX;

  /* Combine many events to one packet, if the packet length > PACKET_MAX_COMBINE_SIZE,
   * should send the packet to avoid a too long packet(like 1G will cause error).
   */
  const static uint32 PACKET_MAX_COMBINE_SIZE;
  ...
  */
  ...
  inline bool is_need_send_packet();
  inline int merge_send_packets(bool &need_send);
  inline bool reduce_packet_for_exculde_gtid(uint32 event_len);
};
