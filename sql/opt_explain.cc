...
class Explain_join : public Explain_table_base {
 ...
 private:
  ...
  bool Explain_print_table_for_insert_all_select(const THD *query_thd,
...
bool Explain_union_result::explain_table_name() {
  ...
  Query_Blocks_Linkage_Type linkage_type = get_query_blocks_linkage_type(sl);
  ...
  if (Query_Blocks_Linkage_Type::ALL_ARE_UNION == linkage_type) {
    ...
    memcpy(table_name_buffer, STRING_WITH_LEN("<minus"));
    len = sizeof("<minus") - 1;
  } else {
    memcpy(table_name_buffer, STRING_WITH_LEN("<union/minus"));
    len = sizeof("<union/minus") - 1;
  }
#else
  memcpy(table_name_buffer, STRING_WITH_LEN("<union"));
...
#ifdef HAVE_ZSQL_MERGE_INTO
    case SQLCOM_MERGE_INTO:
      if (table == query_plan->get_lex()->select_lex->leaf_tables->table)
        fmt->entry()->mod_type = MT_MERGE;
      break;
#endif /* HAVE_ZSQL_MERGE_INTO */
    default:;
...
#ifdef HAVE_ZSQL_INSERT_ALL
bool Explain_join::Explain_print_table_for_insert_all_select(const THD *query_thd,
                                        Sql_cmd_insert_select *tmp_sql_cmd) {
  qep_row *join_entry = fmt->entry();
  tmp_sql_cmd->is_explain_insert_all_executed = true;

  TABLE_LIST *const table_list = query_thd->lex->query_tables;
  ...
  for (cur_table = table_list; cur_table && ((tb_field = it_field++));
    ...
    join_entry->mod_type = MT_INSERT;
    table = cur_table->table;
    tab = NULL;
    if (prepare_columns() || fmt->flush_entry()) return true; /* purecov: inspected */
  }
...
#ifdef HAVE_ZSQL_INSERT_ALL
  /*
     explain the destinate table in insert all select,
     here we don't use condition:if(lex->sql_command == SQLCOM_INSERT_SELECT),
     because when we use the condition, we still need check tmp_sql_cmd->is_insert_all_global to
     make clear it is insert all or normal insert, so we use dynamic_cast directly to let
     the if branch only do insert all select work.
  */
  Sql_cmd_insert_select *tmp_sql_cmd =
      dynamic_cast<Sql_cmd_insert_select*>(query_thd->lex->m_sql_cmd);
  if (NULL != tmp_sql_cmd && tmp_sql_cmd->is_insert_all_global
        && !tmp_sql_cmd->is_explain_insert_all_executed) {
    if (Explain_print_table_for_insert_all_select(query_thd, tmp_sql_cmd)) return true;
  } else {
#endif /* HAVE_ZSQL_INSERT_ALL */
...
#ifdef HAVE_ZSQL_INSERT_ALL
bool Explain_table_for_insert_all_values(THD *const explain_thd, const THD *query_thd,
                                  SELECT_LEX *select, const Modification_plan *plan,
  ...
  for (cur_table = query_thd->lex->query_tables; cur_table && ((tb_field = it_field++));
       cur_table = cur_table->next_global) {
    ret = Explain_table(explain_thd, query_thd, select, cur_table->table,
                    plan->tab, plan->key, plan->limit,
                    plan->need_tmp_table, plan->need_sort, plan->mod_type,
                    plan->used_key_is_modified, plan->message)
          .send() || explain_thd->is_error();
    if (ret) return true;
  }
  ...
  */
  ...
  if (!plan || plan->zero_result) {
    ...
    if (other &&
      ...
      /* process insert all values,here we don't use condition:if(lex->sql_command == SQLCOM_INSERT),
         because when we use the condition, we still need check tmp_sql_cmd->is_insert_all_global to
         make clear it is insert all or normal insert, thus increse lizard, so we use dynamic directly
         to let the if branch only do insert all values work.
      */
      Sql_cmd_insert_values *tmp_sql_cmd =
          dynamic_cast<Sql_cmd_insert_values*>(query_thd->lex->m_sql_cmd);
      if (NULL != tmp_sql_cmd && tmp_sql_cmd->is_insert_all_global) {
        ret = Explain_table_for_insert_all_values(explain_thd, query_thd, select, plan, tmp_sql_cmd);
      } else {
...
#ifdef HAVE_ZSQL_MERGE_INTO
        case SQLCOM_MERGE_INTO:
          // "Merge into v1" or "Merge into tm1"?
          explain = string("-> Merge into ") +
                    query_plan->get_lex()->query_tables->table_name +
                    "\n";
          base_level = 1;
          break;
#endif /* HAVE_ZSQL_MERGE_INTO */
...
bool explain_query(THD *explain_thd, const THD *query_thd,
  ...
  /*
    ...
    3) Allow only SELECT, INSERT/REPLACE ... SELECT, MERGE INTO, Multi-DELETE and
...
#ifdef HAVE_ZSQL_MERGE_INTO
       query_thd->query_plan.get_command() == SQLCOM_MERGE_INTO ||
#endif /* HAVE_ZSQL_MERGE_INTO */
