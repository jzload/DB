...
sp_head::sp_head(MEM_ROOT &&mem_root, enum_sp_type type)
    : m_type(type),
      ...
      /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
      m_not_mark_params(&main_mem_root),
      m_mark_params(&main_mem_root),
      m_block_instr(&main_mem_root),
      m_exception_instr(&main_mem_root),
      m_with_notaion(false),
      /* #endif */
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    err_status = set_sp_params_value(thd, proc_runtime_ctx, args);
#else
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    err_status = set_out_params_values(thd, parent_sp_runtime_ctx,
                                       proc_runtime_ctx, args);
#else
    List_iterator<Item> it_args(*args);
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
static bool is_variable_in_array(sp_variable *spvar,
                                 Mem_root_array<sp_variable*> *vars) {
  uint element = vars->size();
...
static bool get_index_in_array(uint *idx, sp_variable *spvar,
                               const Mem_root_array<sp_variable*> *vars) {
  uint element = vars->size();
  for (uint i = 0; i < element; i++) {
    if (spvar == vars->at(i)) {
      *idx = i;
...
static sp_variable *get_spvar_from_array(uint idx,
                        const Mem_root_array<sp_variable*> *vars) {
  if (idx >= vars->size())
    return NULL;
...
static sp_instr *get_one_instr(Mem_root_array<sp_instr *> *instructions,
                               uint i) {
  return (instructions && i < (uint) instructions->size())
             ? instructions->at(i) : NULL;
}

bool sp_head::check_params(THD* thd, List<Item> *args) {
  if (MYSQL_PARSE_MODE == thd->variables.m_opt_parse_mode) {
    return check_maria_mode_params(args);
  }

  return check_oracle_mode_params(args);
}

bool sp_head::check_maria_mode_params(List<Item> *args) {
  uint args_count = args != NULL ? args->elements : 0;
  uint def_params_count = m_root_parsing_ctx->context_var_count();
  return check_params_count(args_count, def_params_count);
}

bool sp_head::check_oracle_mode_params(List<Item> *args) {
  if (check_and_set_notaion_flag(args)) {
    return check_oralce_notation_params(args);
  }

  return check_oralce_normal_params(args);
}

bool sp_head::check_oralce_notation_params(List<Item> *args) {
  /*
    produce
    m_mark_values && m_mark_params && m_not_mark_values && m_not_mark_params
  */
  if (init_notation_resource(args)) {
    return true;
  }

  if (is_param_instantiated_multiply()) {
    return true;
  }

  // every in_mode variable need input value
  if (is_in_params_without_value()) {
    return true;
  }

  /*
  if (is_notation_params_contain_out_varaible())
  {
    my_error(ER_SP_WRONG_MODE_OF_ARGS, MYF(0), "");
    return true;
  }
  */
...
bool sp_head::init_notation_resource(List<Item> *args) {
  bool marked_item_found = false;
  Item *item;
  List_iterator<Item> iter(*args);
  while ((item = iter++) != nullptr) {
    if (item->type() == Item::NOTATION_PARAM_ITEM) {
      LEX_STRING &name = ((Item_sp_notation_param*)item)->m_name;
      sp_variable *spvar = m_root_parsing_ctx->find_variable(name.str,
                                                             name.length,
                                                             true);
      if (!spvar) {
        my_error(ER_SP_NOTATION_NAME_MISMATCH, MYF(0), name);
        return true;
      }
      m_mark_params.push_back(spvar);
      m_mark_values.push_back(item);
      marked_item_found = true;
    } else {
      if (marked_item_found) {
        my_error(ER_SP_POSITION_NOTATION_PARAMS_MIXED, MYF(0), "");
        return true;
      }
      m_not_mark_values.push_back(item);
    }
  }

  uint total_param_count = m_root_parsing_ctx->context_var_count();
  for (uint i = 0; i<total_param_count; i++) {
    sp_variable *spvar = m_root_parsing_ctx->find_variable(i);

    if (!is_variable_in_array(spvar, &m_mark_params))
      m_not_mark_params.push_back(spvar);
  }

  return false;
}

bool sp_head::is_param_instantiated_multiply() {
  uint max_unmarked_param_pos = m_not_mark_values.elements;

  for (uint i = 0; i < max_unmarked_param_pos; i++) {
    sp_variable *spvar = m_root_parsing_ctx->find_variable(i);
    if (!spvar)
      continue;

    if (is_variable_in_array(spvar, &m_mark_params)) {
      my_error(ER_SP_MULTIPLE_INSTANCE_ARGUMENT, MYF(0) ,"");
      return true;
    }
  }
...
bool sp_head::is_in_params_without_value() {
  if (m_not_mark_values.elements > m_not_mark_params.size()) {
    my_error(ER_SP_WRONG_NO_OF_ARGS2, MYF(0), m_name.str);
    return true;
  }

  if (m_mark_values.elements != m_mark_params.size()) {
    my_error(ER_SP_WRONG_NO_OF_ARGS2, MYF(0), m_name.str);
    return true;
  }

  uint offset = m_not_mark_params.size() - m_not_mark_values.elements;
  uint start_idx = m_not_mark_values.elements;
  for (uint i = 0; i < offset; i++) {
    sp_variable *spvar = get_spvar_from_array(start_idx+i, &m_not_mark_params);
    if (!spvar->default_value && spvar->mode == sp_variable::MODE_IN) {
      my_error(ER_SP_WRONG_NO_OF_ARGS2, MYF(0), m_name.str);
      return true;
    }
  }
...
bool sp_head::check_oralce_normal_params(List<Item> *args) {
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  //bugfix: in case args is nullptr
  uint elements_count = args ? args->elements : 0;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */

  uint total_param_count = m_root_parsing_ctx->context_var_count();
  if (0 == total_param_count) {
    return false;
  }

  sp_variable *last_var =
      m_root_parsing_ctx->find_variable(total_param_count - 1);
  if (!last_var->default_value) {
    return check_params_count(elements_count, total_param_count);
  }

  return check_default_params_count(elements_count);
}

bool sp_head:: check_params_count(uint input_count, uint def_count) {
  if (input_count != def_count) {
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "PROCEDURE",
             m_qname.str, def_count, input_count);
    return true;
  }
...
bool sp_head::check_default_params_count(uint input_count) {
  uint total_param_count = m_root_parsing_ctx->context_var_count();
  uint valid_default_param_count  = get_valid_defalut_param_count();
  uint at_least_input_parma_count =
      total_param_count - valid_default_param_count;

  if (input_count < at_least_input_parma_count ||
      input_count > total_param_count) {
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "PROCEDURE",
             m_qname.str, at_least_input_parma_count, input_count);
    return true;
  }
...
uint sp_head::get_valid_defalut_param_count() {
  sp_variable *spvar;
  uint params_count = m_root_parsing_ctx->context_var_count();
  uint valid_default_param_count = 0;

  for (uint i = params_count; i > 0; i--) {
    spvar = m_root_parsing_ctx->find_variable(i - 1);
    if (!spvar->default_value)
      break;
    valid_default_param_count++;
  }

  return valid_default_param_count;
}

bool sp_head::check_and_set_notaion_flag(List<Item> *args) {
  if (args == NULL)
    return false;
...
bool sp_head::set_params_with_default_value(THD *thd,
                                            sp_rcontext *nctx,
                                            uint start_idx) {
  const Mem_root_array<sp_variable*> *params_array =
      m_root_parsing_ctx->get_vars();
  uint params_count = params_array->size();

  sp_variable *spvar;
  for (uint i = start_idx; i < params_count; i++) {
    spvar = params_array->at(i);
    if (!spvar)
      continue;

    if (!spvar->default_value) {
      my_error(ER_SYNTAX_ERROR, MYF(0),"Procedure's input parameter error");
      return true;
    }

    if (nctx->set_variable(thd, i, &(spvar->default_value)))
      return true;
  }
...
bool sp_head::is_notation_params_contain_out_varaible() {
  uint elements = m_mark_params.size();
  for (uint i = 0 ; i < elements; i++) {
    sp_variable *spvar = m_mark_params.at(i);
    if (spvar && sp_variable::MODE_OUT == spvar->mode) {
      return true;
    }
  }
...
bool sp_head::set_sp_params_value(THD *thd,
                                  sp_rcontext *nctx,
                                  List<Item>  *args) {
  if (m_with_notaion) {
    return set_sp_params_value_with_notation(thd, nctx, args);
  }

  return set_sp_params_value_without_notation(thd, nctx, args);
}

bool sp_head::set_sp_params_value_without_notation(THD *thd,
                                                   sp_rcontext *nctx,
                                                   List<Item>  *args) {
  uint params = m_root_parsing_ctx->context_var_count();
  List_iterator<Item> it_args(*args);
  ...
  for (uint i = 0 ; i < params ; i++) {
    ...
    if (fill_variable_info(thd, nctx, spvar, arg_item, i))
      return true;
  }

  if (thd->variables.m_opt_parse_mode == ORACLE_PARSE_MODE  &&
      set_params_with_default_value(thd, nctx, args->elements)) {
    return true;
  }
...
bool sp_head::fill_variable_info(THD *thd,
                                 sp_rcontext *nctx,
                                 sp_variable *spvar,
                                 Item *var_value,
                                 uint  var_idx) {
  if (spvar->mode != sp_variable::MODE_IN) {
    Settable_routine_parameter *srp =
        var_value->get_settable_routine_parameter();
    if (!srp) {
      my_error(ER_SP_NOT_VAR_ARG, MYF(0), var_idx+1, m_qname.str);
      return true;
    }
  ...
  if (spvar->mode == sp_variable::MODE_OUT) {
    Item_null *null_item = new (thd->mem_root) Item_null();
    Item *tmp_item = null_item;

    if (!null_item || nctx->set_variable(thd, var_idx, &tmp_item)) {
      DBUG_PRINT("error", ("set variable failed"));
      return true;
    }
  } else {
    if (nctx->set_variable(thd, var_idx, &var_value)) {
...
bool sp_head::set_sp_params_value_with_notation(THD *thd,
                                                sp_rcontext *nctx,
                                                List<Item>  *) {
  uint mark_params = m_mark_params.size();
  List_iterator<Item> iter_mark(m_mark_values);

  for (uint i = 0; i < mark_params; i++) {
    uint idx = 0;
    Item_sp_notation_param *mark_value = (Item_sp_notation_param*)(iter_mark++);
    sp_variable *mark_var = m_mark_params.at(i);

    if (get_index_in_array(&idx, mark_var, m_root_parsing_ctx->get_vars())) {
      DBUG_PRINT("error", ("set variable 2 failed"));
      return true;
    }

    if (fill_variable_info(thd, nctx, mark_var, mark_value->m_value, idx))
      return true;
  }

  uint unmark_params_count = m_not_mark_params.size();
  uint unmark_values_count = m_not_mark_values.elements;
  List_iterator<Item> iter_unmark(m_not_mark_values);
  for (uint i = 0; i < unmark_params_count; i++) {
    uint idx = 0;
    Item *var_value = nullptr;

    if (i < unmark_values_count)
      var_value = iter_unmark++;

    sp_variable *unmark_var = m_not_mark_params.at(i);
    if (get_index_in_array(&idx, unmark_var, m_root_parsing_ctx->get_vars()))
      return true;

    if (i >= unmark_values_count)
      var_value = unmark_var->default_value;
    ...
    if (fill_variable_info(thd, nctx, unmark_var, var_value, idx))
      return true;
  }
...
bool sp_head::set_out_params_values(THD *thd,
                                    sp_rcontext *octx,
                                    sp_rcontext *nctx,
                                    List<Item>  *args) {
  if (m_with_notaion) {
    return set_out_params_value_when_exists_notaion(thd, octx, nctx, args);
  }

  return set_out_params_value_when_no_notaion(thd, octx, nctx, args);
}

bool sp_head::set_out_params_value_when_exists_notaion(THD *thd,
                                                       sp_rcontext *octx,
                                                       sp_rcontext *nctx,
                                                       List<Item>  *) {
  // set unmarked out mode variable ,umarked param must be in the front
  uint unmarked_value_count = m_not_mark_values.elements;

  if (set_position_out_params_value(thd, octx, nctx,
                                    &m_not_mark_values,
                                    unmarked_value_count)) {
    return true;
  }

  return set_notation_out_params_value(thd, octx, nctx);
}

bool sp_head::set_out_params_value_when_no_notaion(THD *thd,
                                                   sp_rcontext *octx,
                                                   sp_rcontext *nctx,
                                                   List<Item>  *args) {
  uint params = m_root_parsing_ctx->context_var_count();
  return set_position_out_params_value(thd, octx, nctx, args, params);
}

// args, input args, such as 'a1,a2' of sp_name(a1,a2)
bool sp_head::set_position_out_params_value(THD *thd,
                                            sp_rcontext *octx,
                                            sp_rcontext *nctx,
                                            List<Item>  *args,
                                            uint params_count) {
  List_iterator<Item> it_arg(*args);

  for (uint i = 0; i < params_count; i++) {
    Item *arg_item = it_arg++;
    ...
    if (set_out_variable_value(thd, octx, nctx, spvar, arg_item, i))
      return true;
  }
...
bool sp_head::set_notation_out_params_value(THD *thd,
                                            sp_rcontext *octx,
                                            sp_rcontext *nctx) {
  uint marked_value_count = m_mark_values.elements;
  List_iterator<Item> it_mark(m_mark_values);
  for (uint i = 0; i < marked_value_count; i++) {
    uint idx = 0;
    Item_sp_notation_param *mark_item = (Item_sp_notation_param*)it_mark++;

    if (!mark_item)
      break;

    sp_variable *spvar = get_spvar_from_array(i, &m_mark_params);

    if (spvar->mode == sp_variable::MODE_IN)
      continue;

    if (get_index_in_array(&idx, spvar, m_root_parsing_ctx->get_vars())) {
      my_error(ER_UNKNOWN_ERROR, MYF(0), "");
      return true;
    }

    if (set_out_variable_value(thd, octx, nctx, spvar, mark_item->m_value, idx))
      return true;
  }
...
bool sp_head::set_out_variable_value(THD *thd,
                                     sp_rcontext *octx,
                                     sp_rcontext *nctx,
                                     sp_variable *spvar,
                                     Item *var_value,
                                     uint idx) {
  Settable_routine_parameter *srp = var_value->get_settable_routine_parameter();

  DBUG_ASSERT(srp);

  if (srp->set_value(thd, octx, nctx->get_item_addr(idx))) {
    DBUG_PRINT("error", ("set value failed"));
    return true;
  }

  Send_field *out_param_info = new (thd->mem_root) Send_field();
  nctx->get_item(idx)->make_field(out_param_info);
  out_param_info->db_name = m_db.str;
  out_param_info->table_name = m_name.str;
  out_param_info->org_table_name = m_name.str;
  out_param_info->col_name = spvar->name.str;
...
void sp_head::reset_notation_resource(THD* thd) {
  if (thd->variables.m_opt_parse_mode != ORACLE_PARSE_MODE)
    return;
  ...
  m_mark_values.empty();
  m_not_mark_values.empty();

  m_mark_params.clear();
  m_not_mark_params.clear();

  m_block_instr.clear();
  m_exception_instr.clear();
}

int sp_head::backpatch_sphead() {
  m_block_instr.clear();
  m_exception_instr.clear();
  m_block_instr.reserve(0);
  m_exception_instr.reserve(0);

  // cpoy_instrs(0,m_tmp_ip1,&m_decl_instr);
  if (cpoy_instrs(m_tmp_ip1, m_tmp_ip2, &m_block_instr))
    return 1;
  if (cpoy_instrs(m_tmp_ip2, m_instructions.size(), &m_exception_instr))
    return 1;

  /// set offset for all instr
  set_instr_offset(&m_block_instr,m_exception_instr.size(),false);
  set_instr_offset(&m_exception_instr,m_tmp_ip2-m_tmp_ip1,true);

  /// reset m_instr
  m_instructions.resize(m_tmp_ip1) ;
  if (cpoy_instrs2(&m_exception_instr))
    return 1;
  if (cpoy_instrs2(&m_block_instr))
    return 1;
...
/*
  cpoy elements from m_instr to other

  start_idx: start_pos in from_array, include itself
  end_idx:   end_pos ,not include itself
*/

int sp_head::cpoy_instrs(uint start_idx, uint end_idx,
                         Mem_root_array<sp_instr *> *to_array) {
  for (uint i = start_idx; i < end_idx; i++) {
    sp_instr *ip = get_instr(i);
    if (ip) {
      if (to_array->push_back(ip))
        return 1;
    }
  }
...
// copy instr in from_array to m_instr
int sp_head::cpoy_instrs2(Mem_root_array<sp_instr *> *from_array) {
  uint elements = from_array->size();
  for (uint i = 0; i < elements; i++) {
    sp_instr *ip;
    ip = get_one_instr(from_array, i);
    m_instructions.push_back(ip);
  }

  return 0;
}

void sp_head::set_instr_offset(Mem_root_array<sp_instr *> * array,
                               uint offset, bool minnus) {
  uint elements = array->size();
  for (uint i = 0; i < elements; i++) {
    sp_instr *ip ;
    ip = get_one_instr(array,i);
    if(!ip)
        return;
    ...
    if (ip->is_sp_instr_jump()) {
      sp_instr_jump *meta_ip = (sp_instr_jump*)ip;
      if (minnus) {
        if (meta_ip->m_dest)
          meta_ip->m_dest -= offset;
      } else {
        if (meta_ip->m_dest)
          meta_ip->m_dest += offset;
      }
    } else if (ip->is_sp_instr_lex_branch()) {
        sp_lex_branch_instr *meta_ip = (sp_lex_branch_instr*)ip;
        if (minnus) {
          if (meta_ip->m_dest)
            meta_ip->m_dest -= offset;
          if (meta_ip->m_cont_dest)
            meta_ip->m_cont_dest -= offset;
        } else {
          if (meta_ip->m_dest)
            meta_ip->m_dest += offset;
          if (meta_ip->m_cont_dest)
