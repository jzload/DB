...
extern void thd_set_net_read_write(THD *thd, uint val);

static void commit_all_follower_threads(THD *leader)
{
  if(!leader)
    return;
  ...
  // skip leader
  THD *tmp = NULL;
  THD *follower = leader->next_to_commit;
  leader->tx_commit_pending= false;
  leader->next_to_commit = NULL;

  while (follower)
  {
    tmp = follower->next_to_commit;
    follower->tx_commit_pending= false;
    follower->next_to_commit = NULL;

    if(false == follower->is_quick_sync())
    {/* no quick_sync trx. */
      follower = tmp;
      continue;
    }
    /* quick sync trx */
    threadpool_append_commit_to_high_prio_queue(follower);
    follower = tmp;
  }
  return;
}
...
class binlog_cache_data {
  ...
  */
  ...
  bool has_big_transaction(uint64_t trans_max_size) {
    if (get_byte_position() > trans_max_size)
      return true;
...
class binlog_cache_mngr {
  ...
  */
  ...
  bool has_big_transaction(uint64_t trans_max_size) {
    if (trx_cache.has_big_transaction(trans_max_size) ||
        stmt_cache.has_big_transaction(trans_max_size))
      return true;
...
void Stage_manager::signal_done(THD *queue) {
  ...
  /*
     Fix a bug by the mixed trx with qucik_sync and semi_sync.
     First set the semi_sync trx flag of pending;
     Second add quick_sync trx to threadpool;
     Third broadcast the cond of m_cond_done;
     the 1th and 2th need be lock by m_lock_done.
  */
  commit_all_follower_threads(queue);
  mysql_mutex_unlock(&m_lock_done);
  ...
  if (!error && thd->lex->sql_command == SQLCOM_XA_ROLLBACK) {
    ...
    if (!error && !thd->is_error()) {
      ...
      */
        ...
        */
        (void)RUN_HOOK_INLINE(transaction, after_commit, (thd, all));
      }
    }
  }
  /*
...
MYSQL_BIN_LOG::MYSQL_BIN_LOG(uint *sync_period)
    : name(nullptr),
      ...
      gtmgtid_idx_switch(opt_gtmgtid_idx),
  ...
  */
  index_file_name[0] = 0;
  memset(&gtmgtid_idx_file, 0, sizeof(gtmgtid_idx_file));
}
...
bool MYSQL_BIN_LOG::open_binlog(
  ...
  gtmgtid_idx_switch = opt_gtmgtid_idx;
  if (open_gtmgtid_idx_file_and_write_title())
      goto err;
...
bool MYSQL_BIN_LOG::reset_logs(THD *thd, bool delete_only) {
  ...
  */
  ...
  for (;;) {
    if (del_gtmgtid_idx_file_allow_opened(linfo.log_file_name))
      goto err;
...
int MYSQL_BIN_LOG::register_purge_index_entry(const char *entry) {
  ...
  size_t entry_len = my_strnlen(entry, FN_REFLEN - 1);

  if ((error = register_purge_gtmgtid_idx_entry(entry, entry_len)))
    return error;
  ...
  */
  if (stmt_stuff_logged || trx_stuff_logged) {
    ...
    */
    ...
    if (is_quick_sync_trans(thd))
    {
        thd->set_curtrans_quick_sync();
        thd->set_quick_sync();
    }
...
std::pair<int, my_off_t> MYSQL_BIN_LOG::flush_thread_caches(THD *thd) {
  ...
  thd->set_prev_trans_end_pos(m_binlog_file->position());
  ...
  for (THD *head = first; head; head = head->next_to_commit) {
    if (head->get_transaction()->m_flags.run_hooks &&
      ...
      */
      Thd_backup_and_restore switch_thd(thd, head);
      bool all = head->get_transaction()->m_flags.real_commit;
      (void)RUN_HOOK_INLINE(transaction, after_commit, (head, all));
      /*
...
int MYSQL_BIN_LOG::flush_cache_to_file(my_off_t *end_pos_var) {
  if (m_binlog_file->flush() || flush_cache_to_gtmgtid_idx_file()) {
    THD *thd = current_thd;
    thd->commit_error = THD::CE_FLUSH_ERROR;
...
int MYSQL_BIN_LOG::finish_commit(THD *thd) {
  ...
  */
  set_commit_end_time(thd);
  set_ordered_commit_time(thd);
...
static inline int call_after_sync_hook(THD *queue_head) {
  ...
  if (DBUG_EVALUATE_IF("simulate_after_sync_hook_error", 1, 0) ||
      RUN_HOOK_INLINE(binlog_storage, after_sync, (queue_head, log_file, pos))) {
...
int MYSQL_BIN_LOG::ordered_commit(THD *thd, bool all, bool skip_commit) {
  ...
  */
  ...
  thd->m_session_stage = ENUM_BEFORE_FLUSH;
  set_ordercommit_and_flush_start_time(thd);
  ...
  */
  ...
  thd->m_session_stage = ENUM_FLUSH_BINLOG;
  flush_error =
  ...
  */
  if (flush_error == 0) {
    ...
    if (RUN_HOOK_INLINE(binlog_storage, after_flush,
  ...
  thd->m_session_stage = ENUM_BEFORE_SYNC;
  set_flush_end_and_sync_start_time(thd);
  ...
  */
  ...
  if (flush_error == 0 && total_bytes > 0) {
    ...
    thd->m_session_stage = ENUM_SYNC_BINLOG;
  ...
  update_binlog_current_timestamp((int64)my_time(0));

  DEBUG_SYNC(thd, "bgc_after_sync_stage_before_commit_stage");

  leave_mutex_before_commit_stage = &LOCK_sync;
  thd->m_session_stage = ENUM_BEFORE_COMMIT;
  set_sync_end_and_commit_start_time(final_queue);
  /*
...
commit_stage:
  ...
  if ((opt_binlog_order_commits || Clone_handler::need_commit_order()) &&
    ...
    // commit_queue always = thd, if not ,something wrong
    THD *commit_queue =
        stage_manager.fetch_queue_for(Stage_manager::COMMIT_STAGE);
    DBUG_EXECUTE_IF("semi_sync_3-way_deadlock",
                    DEBUG_SYNC(thd, "before_process_commit_stage_queue"););
    DBUG_ASSERT(commit_queue == thd);
    commit_queue->m_leader_thread = true;
    commit_queue->m_ordered_commit = true;

    if (flush_error == 0 && sync_error == 0)
    {
        // quick_sync_mode return
        commit_queue->m_session_stage = ENUM_BEFORE_SYNC_HOOK1;
        if (call_after_sync_hook_ext(thd,commit_queue,do_rotate))
        {
            return 0;
        }
    }
    ...
    */
    ...
    final_queue->m_leader_thread = true;
    final_queue->m_ordered_commit = false;
    if (flush_error == 0 && sync_error == 0)
    {
        // quick_sync_mode return
        DBUG_ASSERT(final_queue == thd);

        final_queue->m_session_stage = ENUM_BEFORE_SYNC_HOOK2;
        if (call_after_sync_hook_ext(thd,final_queue,do_rotate))
        {
            return 0;
        }
    }
  ...
  */
  ...
  /* Commit done so signal all waiting threads, include quick_sync trx and semi_sync trx. */
  ...
  */
  thd->reset_quick_sync();
  return thd->commit_error == THD::CE_COMMIT_ERROR;
}


bool  MYSQL_BIN_LOG::quick_sync_commit(THD *thd)
{
    if (thd->m_leader_thread)
    {
      quick_sync_leader_exectue(thd);       // how to process error
    }
    else
    {
      quick_sync_follower_execute(thd);    // how to process error
    }

    return 0;
}

void MYSQL_BIN_LOG::add_follower_thread_to_work_queue(THD *leader)
{
    /* Must set next_to_commit to NULL after use.*/
    THD *follower = leader->next_to_commit;
    THD *next_follower = NULL;
    leader->next_to_commit = NULL;
    while (follower)
    {
        next_follower = follower->next_to_commit;
        follower->next_to_commit = NULL;
        threadpool_append_commit_to_high_prio_queue(follower);
...
int MYSQL_BIN_LOG::quick_sync_leader_exectue(THD *thd)
{
    if (thd->m_ordered_commit)
    {
        quick_sync_leader_ordered_commit(thd);
        add_follower_thread_to_work_queue(thd);
    }
    else
    {
        // if ordered_commit = 0, follower go first, leader rorate binlog wating follower finished
        add_follower_thread_to_work_queue(thd);
        quick_sync_leader_paralell_commit(thd);
    }

    quick_sync_clearup_and_reply(thd);
    return 0;
}

int MYSQL_BIN_LOG::quick_sync_leader_ordered_commit(THD *thd)
{
  DBUG_ENTER("MYSQL_BIN_LOG::quick_sync_leader_ordered_commit");
  thd->m_session_stage = ENUM_COMMIT_TRANS;
  THD *final_queue = thd->m_quick_sync_commit_queue;

  // if produce error during flush or sync, cant come here
  // if (opt_binlog_order_commits || binlog_error_action != ABORT_SERVER)
  // if (final_queue->m_ordered_commit)
  {
    process_commit_stage_queue(thd, final_queue);
    mysql_mutex_unlock(&LOCK_commit);
    process_after_commit_stage_queue(thd, final_queue);
  }
  ...
  */
   ...
   */
  if (DBUG_EVALUATE_IF("force_rotate", 1, 0) ||
      (thd->m_do_rotate && thd->commit_error == THD::CE_NONE &&
       !is_rotating_caused_by_incident))
  {
    mysql_mutex_lock(&LOCK_rotate_binlog);
    mysql_cond_signal(&COND_rotate_binlog);
    mysql_mutex_unlock(&LOCK_rotate_binlog);
  }
...
int   MYSQL_BIN_LOG::quick_sync_leader_paralell_commit(THD *thd)
{
  DBUG_ENTER("MYSQL_BIN_LOG::quick_sync_leader_paralell_commit");
  thd->m_session_stage = ENUM_COMMIT_TRANS;
...
int MYSQL_BIN_LOG::quick_sync_follower_execute(THD *follower)
{
    finish_commit(follower);
    quick_sync_clearup_and_reply(follower);
    return 0;
}

/********************************************************************
all:    comes from ordered_commit; commit or implicit commit is true; end of stmt is false
error:whether make error during commit by ack work thread
*********************************************************************/
int MYSQL_BIN_LOG::quick_sync_clearup_and_reply(THD *thd)
{
  // dosent consider about error made by engine commit
  bool error = false;
  thd->m_session_stage = ENUM_REPLY_TO_CLINET;

  mark_xid_state_binlogged(thd);                        // MYSQL_BIN_LOG::commit

  cleanup_after_ha_commit_trans(thd,error);          // ha_commit_trans

  if (thd->m_trx_type == Transaction_ctx::STMT)
  {
      cleanup_after_trans_commit_stmt(thd,error);   // trans_commit_stmt
  }
  else
  {
      if (cleanup_after_trans_commit(thd,error))     // trans_commit + part of mysql_execute-command
          error = true;
  }

  cleanup_after_execute_command(thd,error);      // mysql_execute-command

  cleanup_after_mysql_parse(thd); // mysql_parse

  if ((thd->m_multi_statements)    &&
     (0 == ((thd->m_multi_statements) & SERVER_MULTI_STATEMENTS_END)))
  {
      // if error when execute_multi_statements_query, dosent execute follow statments
      // reply error to client
      // wakeup a work thread to execute follow statements, attention to error process

      // threadpool_append_commit_to_high_prio_queue(thd);
      // execute_multi_statements_query(thd);
      thd->m_need_execute = true;
      return 0;
  }

  int retval = cleanup_after_dispatch_command(thd); // dispatch_command
...
void MYSQL_BIN_LOG::mark_xid_state_binlogged(THD *thd)
{
  bool all = thd->m_trx_type;
...
// ret:1, work thread commit
// ret:0, app thread commit
// this func dosent cause sync error
bool MYSQL_BIN_LOG::call_after_sync_hook_ext(THD *thd MY_ATTRIBUTE((unused)),
                                             THD *commit_queue,
                                             bool  do_rotate)
{
    DBUG_ASSERT(commit_queue == thd);
    bool quicksync_mode = false;
    if (is_group_quick_sync(commit_queue))
    {
        commit_queue->m_group_quick_sync = true;
        commit_queue->m_do_rotate = do_rotate ;
        commit_queue->m_quick_sync_commit_queue = commit_queue; // dont need? thd  == commit_queue?
        quicksync_mode = true;
    }
    call_after_sync_hook(commit_queue);

    return quicksync_mode;
}

void MYSQL_BIN_LOG::rotate_wraper(THD *thd)
{
    /*
...
bool MYSQL_BIN_LOG::is_quick_sync_trans(THD *thd)
{
  if (thd->is_maybe_quick_sync() &&
      (Connection_handler_manager::thread_handling ==
      Connection_handler_manager::SCHEDULER_THREAD_POOL) &&
      opt_bin_log &&
      replication_plugin_satisfied_quick_sync() &&
      !is_slave_repl_thread(thd) &&
      !stmt_causes_implicit_commit(thd, CF_AUTO_COMMIT_TRANS))
    return true;

  return false;
}

void MYSQL_BIN_LOG::set_ordercommit_and_flush_start_time(THD *thd)
{
  uint64_t curr_time =  ulonglong2uint64_t(thd->current_utime());

  thd->order_commit_time = curr_time;

  thd->flush_time = curr_time;
}

void MYSQL_BIN_LOG::set_flush_end_and_sync_start_time(THD *thd)
{
  uint64_t curr_time = ulonglong2uint64_t(thd->current_utime());
  ...
  while (follower)
  {
    follower->flush_time = curr_time - follower->flush_time;
    follower->sync_time  = curr_time;
    follower = follower->next_to_commit;
  }
}

void MYSQL_BIN_LOG::set_sync_end_and_commit_start_time(THD *thd)
{
  uint64_t curr_time = ulonglong2uint64_t(thd->current_utime());
  ...
  while (follower)
  {
    follower->sync_time = curr_time - follower->sync_time;
    follower->commit_time = curr_time;
    follower = follower->next_to_commit;
  }
}

void MYSQL_BIN_LOG::set_commit_end_time(THD *thd)
{
  if (0 != thd->commit_time) // make error before enter commit phase
  {
    thd->commit_time = ulonglong2uint64_t(thd->current_utime()) - thd->commit_time;
  }
}

void MYSQL_BIN_LOG::set_ordered_commit_time(THD *thd)
{
  thd->order_commit_time = ulonglong2uint64_t(thd->current_utime()) - thd->order_commit_time;
}
...
*/
    ...
    */
    ...
    if (lex->sql_command == SQLCOM_MERGE_INTO &&
        (lex->oracle_options & ORA_MERGE_INTO_WITH_UPDATE))
      lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_MERGE_UPDATE);
...
bool THD::is_ddl_gtid_compatible() {
  ...
  if (lex->sql_command == SQLCOM_CREATE_TABLE &&
    ...
    */
    if (variables.binlog_format != BINLOG_FORMAT_ROW ||
	variables.gtid_next.type != AUTOMATIC_GTID ||
...
void update_binlog_current_timestamp(longlong ts)
{
  binlog_current_timestamp.store(ts);
}

bool THD::has_big_transaction() {
  binlog_cache_mngr *cache_mngr = thd_get_cache_mngr(this);

  if (cache_mngr &&
      cache_mngr->has_big_transaction(variables.transaction_max_binlog_size))
    return true;
