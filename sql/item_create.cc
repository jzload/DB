...
class To_char_instantiator {
 public:
  ...
  Item *instantiate(THD *thd, PT_item_list *args) {
    switch (args->elements()) {
      case 1:
        return new (thd->mem_root) Item_func_to_char(POS(), (*args)[0]);
      case 2: {
        Item *arg1 = (*args)[0];
        Item *arg2 = (*args)[1];

        enum_tochar_mode e_tochar_mode = ENUM_TOCHAR_DATE_MODE;
        if (!ChooseModeByStr(arg2, e_tochar_mode)){
            my_error(ER_WRONG_PARAMETERS_TO_NATIVE_FCT, MYF(0), "to_char");
            return nullptr;
          }

        if (e_tochar_mode == ENUM_TOCHAR_NUM_MODE) {
          return new (thd->mem_root) Item_func_to_char(POS(), arg1, arg2);
        } else if (e_tochar_mode == ENUM_TOCHAR_HEX_MODE) {
          return nullptr;
        } else {
          Item *format_item = ((PTI_udf_expr *)arg2)->get_item();
          LEX_STRING format_literal =
              ((PTI_text_literal *)format_item)->get_literal();

          const CHARSET_INFO *str_charset =
              format_item->get_str_value().charset();
          char format_buff[64] = {0};
          String *new_format_str =
              new (thd->mem_root)String(format_buff, str_charset);

          uint year_flag = 0;
          uint month_type = 0;
          uint ms_len = 0;
          if (!OracleStrToMysqlStr(format_literal, new_format_str,
                                  &year_flag, &month_type, &ms_len)) {
            my_error(ER_WRONG_PARAMETERS_TO_NATIVE_FCT, MYF(0), "to_char");
            return nullptr;
          }

          LEX_STRING new_format;
          new_format.str = (char*)new_format_str->ptr();
          new_format.length = new_format_str->length();
          ((PTI_text_literal *)format_item)->set_literal(new_format);

          return new (thd->mem_root)
              Item_func_oracle_date_format(POS(), arg1, arg2, 0, ms_len, new_format_str);
        }
      }
      default:
        DBUG_ASSERT(false);
...
class Decode_instantiator {
 public:
   ...
   Item *instantiate(THD *thd, PT_item_list *args)
   {
      return new (thd->mem_root) Item_func_decode(POS(), args->value);
   }
};
...
static const std::pair<const char *, Create_func *> func_array[] = {
...
#ifdef HAVE_ZSQL_ASYMMETRIC_ENCRYPT
    {"ASYMMETRIC_DECRYPT", SQL_FN(Item_func_asymmetric_decrypt, 3)},
    {"ASYMMETRIC_ENCRYPT", SQL_FN(Item_func_asymmetric_encrypt, 3)},
    {"CREATE_ASYMMETRIC_PRIV_KEY",
     SQL_FN(Item_func_create_asymmetric_priv_key, 2)},
    {"CREATE_ASYMMETRIC_PUB_KEY",
     SQL_FN(Item_func_create_asymmetric_pub_key, 2)},
#endif /* HAVE_ZSQL_ASYMMETRIC_ENCRYPT */
    ...
    {"NVL", SQL_FN(Item_func_nvl, 2)},
    {"NVL2", SQL_FN(Item_func_nvl2, 3)},
    {"TO_TIMESTAMP", SQL_FN_V(Item_func_oracle_to_timestamp, 1, 2)},
    {"DECODE", SQL_FACTORY(Decode_instantiator)},
    {"MONTHS_BETWEEN", SQL_FN(Item_func_months_between,2)},
