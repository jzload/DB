...
#include "sql_copy.h"         // mysql_copy_table
...
const LEX_CSTRING session_stage[]={
  { STRING_WITH_LEN("INIT") },
  { STRING_WITH_LEN("Do_command") },
  { STRING_WITH_LEN("Before_flush") },
  { STRING_WITH_LEN("Flush_binlog") },
  { STRING_WITH_LEN("Before_sync") },
  { STRING_WITH_LEN("Sync_binlog") },
  { STRING_WITH_LEN("Before_commit") },
  { STRING_WITH_LEN("Before_sync_hook1") },
  { STRING_WITH_LEN("Before_sync_hook2") },
  { STRING_WITH_LEN("Commit_trans") },
  { STRING_WITH_LEN("Reply_to_client") },
  { STRING_WITH_LEN("Epoll_add") },
  { STRING_WITH_LEN("Epoll_mod") }
};
...
void init_sql_command_flags(void) {
  ...
  */
  ...
  sql_command_flags[SQLCOM_RENAME_TABLE] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
#ifdef HAVE_ZSQL_COPY_TABLE
  sql_command_flags[SQLCOM_COPY_TABLE]= CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
#endif // HAVE_ZSQL_COPY_TABLE
...
#ifdef HAVE_ZSQL_MERGE_INTO
  sql_command_flags[SQLCOM_MERGE_INTO] = CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE |
                                         CF_CAN_GENERATE_ROW_EVENTS |
                                         CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED;
#endif /* HAVE_ZSQL_MERGE_INTO */
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_THREADPOOL_STATUS] = CF_STATUS_COMMAND;
  sql_command_flags[SQLCOM_SHOW_QUICKSYNC_STATUS] = CF_STATUS_COMMAND;
  sql_command_flags[SQLCOM_SHOW_SLAVE_SYNCREPLAY_STATUS]= CF_STATUS_COMMAND;
  ...
  */
  ...
  sql_command_flags[SQLCOM_DECL_BLOCK] = CF_REEXECUTION_FRAGILE |
                                   CF_CAN_GENERATE_ROW_EVENTS |
                                   CF_OPTIMIZER_TRACE;
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  sql_command_flags[SQLCOM_DECL_BLOCK] |= CF_PREOPEN_TMP_TABLES;
#ifdef HAVE_ZSQL_MERGE_INTO
  sql_command_flags[SQLCOM_MERGE_INTO] |= CF_PREOPEN_TMP_TABLES;
#endif /* HAVE_ZSQL_MERGE_INTO */
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
...
#ifdef HAVE_ZSQL_COPY_TABLE
  sql_command_flags[SQLCOM_COPY_TABLE] |= CF_DISALLOW_IN_RO_TRANS;
#endif // HAVE_ZSQL_COPY_TABLE
  ...
  */
...
#ifdef HAVE_ZSQL_MERGE_INTO
  sql_command_flags[SQLCOM_MERGE_INTO] |= CF_ALLOW_PROTOCOL_PLUGIN;
#endif /* HAVE_ZSQL_MERGE_INTO */
  ...
  sql_command_flags[SQLCOM_SHOW_THREADPOOL_STATUS] |= CF_ALLOW_PROTOCOL_PLUGIN;
  sql_command_flags[SQLCOM_SHOW_QUICKSYNC_STATUS] |= CF_ALLOW_PROTOCOL_PLUGIN;
  sql_command_flags[SQLCOM_SHOW_SLAVE_SYNCREPLAY_STATUS] |= CF_ALLOW_PROTOCOL_PLUGIN;
...
#ifdef HAVE_ZSQL_COPY_TABLE
  sql_command_flags[SQLCOM_COPY_TABLE] |= CF_ALLOW_PROTOCOL_PLUGIN;
#endif // HAVE_ZSQL_COPY_TABLE
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  sql_command_flags[SQLCOM_DECL_BLOCK] |= CF_ALLOW_PROTOCOL_PLUGIN;
#endif /* HAVE_ZSQL_ORACLE_COMPATIBILITY */
  ...
  */
  ...
  sql_command_flags[SQLCOM_DROP_SRS] |=
      CF_NEEDS_AUTOCOMMIT_OFF | CF_POTENTIAL_ATOMIC_DDL;
#ifdef HAVE_ZSQL_COPY_TABLE
  sql_command_flags[SQLCOM_COPY_TABLE] |=
      CF_NEEDS_AUTOCOMMIT_OFF | CF_POTENTIAL_ATOMIC_DDL;
#endif // HAVE_ZSQL_COPY_TABLE
...
void THD::reset_all_quick_sync_flags()
{
  reset_quick_sync_refcount();
  m_multi_statements = 0;
  m_multi_statements_end_pos = NULL;
  m_quick_saved_secondary_engine = Secondary_engine_optimization::PRIMARY_ONLY;
  m_quick_parser_state->quick_sync_reset();
  reset_quick_sync_for_next_stmt();
}

bool cleanup_after_do_command(THD *thd)
{
  DBUG_TRACE;
  ...
  thd->m_quick_parser_state->m_lip.found_semicolon = NULL;  // when error, not null
  return false;
}
...
bool do_command(THD *thd) {
  ...
  thd->m_session_stage = ENUM_DO_COMMAND;
  ...
  */
  ...
  thd->reset_all_quick_sync_flags();
...
/*
follows dosent support
proceudre
function
trigger
prepare
*/
bool support_quick_sync(THD *thd)
{
   if (thd->get_command() != COM_QUERY)
    return false;

   // procedure
   if (thd->sp_runtime_ctx || thd->lex->get_sp_current_parsing_ctx())
     return false;
...
void thd_set_quick_sync_error(THD *thd)
{
    if (thd->m_quick_sync_err != 0)
    {
      // thd->get_stmt_da()->set_overwrite_status(true);

      if(GROUP_BELOW_LWM_ALARM_ERR == thd->m_quick_sync_err)
      {
        // my_error(ER_QUICK_SYNC_BELOW_LWM_ALARM, MYF(0), "");
        thd->server_status |= SERVER_STATUS_GROUP_BELOW_LWM;
      }
      else if(GROUP_BELOW_HWM_ALARM_ERR == thd->m_quick_sync_err)
      {
        // my_error(ER_QUICK_SYNC_BELOW_HWM_ALARM, MYF(0), "");
        thd->server_status |= SERVER_STATUS_GROUP_BELOW_HWM;
      }
...
// if relication mode is bellow lwm, send error
void err_replication_bellow_lwm(THD *thd)
{
  thd->get_stmt_da()->set_overwrite_status(true);
  my_error(ER_QUICK_SYNC_BELOW_LWM_ALARM, MYF(0), "");
  thd->get_stmt_da()->set_overwrite_status(false);
...
// readonly is priorited to bellow_lwm
void err_readonly_or_bellow_lwm(THD *thd)
{
  if (opt_readonly)
    err_readonly(thd);
  else
    err_replication_bellow_lwm(thd);
}

// just like check_readonly,if mysql upgrade, pay attention to it
bool is_replication_slave_operations(THD *thd)
{
...
(4) create temp table
(5) drop temp table
(6) update temp table or pfs tables
*/
bool is_support_run_bellow_repl_lwm(THD *thd)
{
  if (thd->security_context()->check_access(SUPER_ACL))
    return true;

  if (is_replication_slave_operations(thd))
    return true;
  ...
  const bool create_or_drop_databases=
    (lex->sql_command == SQLCOM_CREATE_DB) ||
    (lex->sql_command == SQLCOM_DROP_DB);
  if (create_or_drop_databases)
    return false;

  const bool update_temp_or_pfs_tables = !some_non_temp_table_to_be_updated(thd,lex->query_tables);

  if (create_temp_tables ||
      drop_temp_tables ||
      update_temp_or_pfs_tables)
    return true;
...
/*
 code repeated as func replication_plugin_satisfied_quick_sync()
 but we cant use gloabl var to store it by once
 because these vars changed dynamicly
*/
bool is_replication_bellow_lwm()
{
  int err_flag = g_quick_sync_err_flag.load();
  if(err_flag == GROUP_BELOW_LWM_ALARM_ERR)
    return true;

  return false;
}

// after fust filter in mysql_execute_command
// dont need to check sql_command
// same as check_only
bool send_bellow_lwm_error_during_execute(THD *thd)
{
  if (!is_replication_bellow_lwm())
    return false;
...
bool cleanup_after_dispatch_command(THD *thd)
{
  DBUG_TRACE;
  bool error = false;
  enum_server_command command = thd->get_command();
  ...
  thd_set_quick_sync_error(thd);
  thd->send_statement_status();

  /* do not need execute because COM_CLONE does not support quick sync */
...
int execute_multi_statements_query(THD *thd)
{
    DBUG_TRACE;
    int retval = 0;
    Parser_state *parser_state = thd->m_quick_parser_state;
    ...
    while ((parser_state->m_lip.found_semicolon != NULL)
                && !thd_errored_or_killed(thd))
    {
      thd->reset_quick_sync_for_next_stmt();
      ...
      size_t length= static_cast<size_t>(thd->m_multi_statements_end_pos - beginning_of_next_stmt);
...
/* PSI begin */
      ...
      */
      ...
      check_secondary_engine_statement(thd, parser_state,
                                       thd->m_quick_query_str,
                                       thd->m_quick_query_len);
      thd->set_secondary_engine_optimization(thd->m_quick_saved_secondary_engine);
    }

    /*
    int err = thd->m_error_stack.back();
    thd->m_error_stack.pop_back();
    */
    if (thd_errored_or_killed(thd) ||
        (parser_state->m_lip.found_semicolon == NULL &&  !thd->is_quick_sync()))
    {
        retval = cleanup_after_dispatch_command(thd);
        cleanup_after_do_command(thd);
    }

    return retval;
}

// commit->exec->commit->exec
int commit_group_threads(THD *thd)
{
    int retval = 0;

    if (!thd->m_need_execute)
    {
        if (tc_log)
        {
          retval = tc_log->quick_sync_commit(thd);
        }
    }
    else
    {
        thd->m_need_execute = false; // ensure next step do commit
        retval = execute_multi_statements_query(thd);
    }
...
bool dispatch_command(THD *thd, const COM_DATA *com_data,
  ...
  */
  ...
  thd->set_time();
  thd->sql_dispatch_time = thd->current_utime();
  ...
  */
  ...
  switch (command) {
    ...
    case COM_QUERY: {
      ...
      thd->m_multi_statements_end_pos = packet_end;
      ...
      thd->m_quick_orig_query = thd->query();

      thd->m_quick_parser_state->quick_sync_reset();
      Parser_state *parser_state = thd->m_quick_parser_state;
      ...
      thd->m_quick_saved_secondary_engine = saved_secondary_engine;
      thd->set_secondary_engine_optimization(
      ...
      check_secondary_engine_statement(thd, parser_state, thd->m_quick_orig_query.str,
                                       thd->m_quick_orig_query.length);
...
/* SHOW PROFILE begin */
        ...
        */
        ...
        thd->set_secondary_engine_optimization(
            Secondary_engine_optimization::PRIMARY_TENTATIVELY);
        thd->m_quick_query_str = beginning_of_next_stmt;
        thd->m_quick_query_len = length;
...
bool cleanup_after_execute_command(THD *thd, bool error)
{
    DBUG_ENTER("cleanup_after_execute_command");
    LEX *lex = thd->lex;
    bool gtid_consistency_violation_state = thd->has_gtid_consistency_violation;

    if (Transaction_ctx::SESSION ==  thd->m_trx_type)
    {
        ...
        if (! thd->in_sub_stmt)
        {
          ...
          mysql_audit_notify(thd,
                             thd->m_first_level ? MYSQL_AUDIT_QUERY_STATUS_END :
    ...
    if (!error && !thd->is_error()) {      // if statement succeeded
...
/**
thd error may be set by quick sync, such as slave count is not satistied
*/
bool thd_real_error(THD *thd)
{
  if (!thd->is_error())
    return false;

  if (thd->get_stmt_da()->mysql_errno() == ER_QUICK_SYNC_BELOW_LWM_ALARM ||
      thd->get_stmt_da()->mysql_errno() == ER_QUICK_SYNC_BELOW_HWM_ALARM )
    return false;
...
bool thd_errored_or_killed(THD *thd)
{
    if (thd->is_error() || thd->killed)
        return true;
...
/*
 * This function copy gdb hint info from session to trx
 * for all commands except the two special commands.
 * */
static void set_trx_gdb_hint_info(THD *thd)
{
    if(NULL == thd)
        return;

    if(thd->lex->sql_command == SQLCOM_LOCK_TABLES ||
       thd->lex->sql_command == SQLCOM_UNLOCK_TABLES)
        return;

    thd->copy_gdb_hint_from_stmt_to_trx();
    return;
}

/*
 * This function copy gdb hint info from session to trx
 * for the two special commands.
 * */
static void set_trx_gdb_hint_info_for_special_cmds(THD *thd)
{
    if(NULL == thd)
        return;
...
int mysql_execute_command(THD *thd, bool first_level) {
  ...
  thd->m_first_level = first_level;
  thd->sql_execute_time = thd->current_utime();
  ...
  if (unlikely(thd->slave_thread)) {
    ...
    */
    if (deny_updates_if_read_only_option(thd, all_tables)) {
      // err_readonly(thd);
      err_readonly_or_bellow_lwm(thd);
      return -1;
    }
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
  /* set the trx_gtm_gtid and trx_serial_num. */
  set_trx_gdb_hint_info(thd);
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT
  ...
  */
...
#ifdef HAVE_ZSQL_DISTRIBUTE_MVCC
    // check after update vars, set var do not check again
    if (make_active_gtmgtids(thd) || check_gtmgtids(thd)) {
      goto error;
    }
  ...
  /*execute prepare time*/
  thd->exec_prep_time = thd->current_utime() - thd->sql_execute_time;
  ...
  switch (lex->sql_command) {
...
#ifdef HAVE_ZSQL_COPY_TABLE
    case SQLCOM_COPY_TABLE:
    {
      if (mysql_copy_table(thd, first_table))
        goto error;
      my_ok(thd);
      break;
    }
...
#ifdef HAVE_ZSQL_TSN_AND_LOCK_WAIT
      /* set the trx_gtm_gtid and trx_serial_num. */
      set_trx_gdb_hint_info_for_special_cmds(thd);
#endif // HAVE_ZSQL_TSN_AND_LOCK_WAIT
    ...
    case SQLCOM_COMMIT: {
      ...
      thd->set_trx_chain_and_release(tx_chain, tx_release);
    ...
    case SQLCOM_CALL:
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
    case SQLCOM_DECL_BLOCK:
#endif /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
    ...
    case SQLCOM_SHOW_QUICKSYNC_STATUS:
    {
      if (!thd->security_context()->priv_user().str[0] &&
          check_global_access(thd,PROCESS_ACL))
        break;
      show_quicksync_status(
        thd,
        (thd->security_context()->check_access(PROCESS_ACL) ?
           NullS : thd->security_context()->priv_user().str),
        lex->verbose);
      break;
    }
    case SQLCOM_SHOW_THREADPOOL_STATUS:
    {
      show_threadpool_status(thd);
      break;
    }
    case SQLCOM_SHOW_SLAVE_SYNCREPLAY_STATUS:
    {
      /* Accept one of two privileges */
      if (check_global_access(thd, SUPER_ACL | REPL_CLIENT_ACL))
        goto error;
      res= show_slave_sync_replay_status_cmd(thd);
      break;
    }
    default:
...
void cleanup_after_mysql_parse(THD *thd)
{
    DBUG_ENTER("cleanup_after_mysql_parse");

    if (thd->m_quick_mgr_switched)
      thd->m_quick_mgr_ptr->restore_original_resource_group(thd, thd->m_quick_mgr_src_res_grp,
                                               thd->m_quick_mgr_dest_res_grp);
    thd->resource_group_ctx()->m_switch_resource_group_str[0] = '\0';
    if (thd->m_quick_mgr_ticket != nullptr)
      thd->m_quick_mgr_ptr->release_shared_mdl_for_resource_group(thd, thd->m_quick_mgr_ticket);
    if (thd->m_quick_mgr_cur_ticket != nullptr)
      thd->m_quick_mgr_ptr->release_shared_mdl_for_resource_group(thd, thd->m_quick_mgr_cur_ticket);
    ...
    // another storage engine.
    Parser_state *parser_state = thd->m_quick_parser_state;
    if(thd->m_quick_orig_query.length && NULL == thd->m_quick_query_str)
    {
      check_secondary_engine_statement(thd, parser_state, thd->m_quick_orig_query.str,
                                       thd->m_quick_orig_query.length);
      thd->set_secondary_engine_optimization(thd->m_quick_saved_secondary_engine);
    ...
    {
      check_secondary_engine_statement(thd, parser_state, thd->m_quick_query_str,
                                       thd->m_quick_query_len);
      thd->set_secondary_engine_optimization(thd->m_quick_saved_secondary_engine);
    }
...
void mysql_parse(THD *thd, Parser_state *parser_state) {
  ...
  thd->sql_parse_time = thd->current_utime();
  mysql_reset_thd_for_next_command(thd);
  lex_start(thd);
  ...
  if (!err) {
    ...
    if (found_semicolon && 0 == thd->m_multi_statements )
    {
      thd->m_multi_statements = SERVER_MULTI_STATEMENTS_FIRST;
    }
    if (found_semicolon && thd->m_multi_statements)
      thd->m_multi_statements = SERVER_MULTI_STATEMENTS_MIDDLE;

    if (thd->m_multi_statements && !found_semicolon)
    {
      thd->m_multi_statements = SERVER_MULTI_STATEMENTS_END;
    }
  }
      ...
      if (!thd->is_error()) {
        ...
        */
        ...
        if (unlikely(thd->security_context()->password_expired() &&
          ...
          thd->m_quick_mgr_switched = switched;
          thd->m_quick_mgr_ticket = ticket;
          thd->m_quick_mgr_cur_ticket = cur_ticket;
          thd->m_quick_mgr_ptr = mgr_ptr;
          thd->m_quick_mgr_src_res_grp = src_res_grp;
          thd->m_quick_mgr_dest_res_grp = dest_res_grp;

          error = mysql_execute_command(thd, true);

          if (thd->is_quick_sync() && 0==error)
            return;
...
bool Alter_info::add_field(
    ...
    uint connect_flag, bool is_connect,
  ...
  if(true == is_connect)
  {
    LEX_STRING name = to_lex_string(connect_flag_str);
    new (thd->mem_root)
            engine_option_value(name, connect_flag, &new_field->option_list,
                                &thd->lex->option_list_last, thd->mem_root);
    new_field->connect_flag = connect_flag;
    new_field->flags |= FIELD_HAS_CONNECT_FLAG;
  }
#endif // HAVE_ZSQL_CONNECT_ENGINE
...
TABLE_LIST *SELECT_LEX::add_table_to_list(
  ...
  if (lock_type != TL_IGNORE) {
    ...
    if (alias_duplicate_with_full_join_inner_tables(ptr)) {
