...
#include "sql/sql_partition.h"  // expr_to_string, expr_convert_str
...
bool fill_dd_columns_from_create_fields(THD *thd, dd::Abstract_table *tab_obj,
  ...
  for (const Create_field &field : create_fields) {
    ...
    */
    ...
    // Store connect engine flag
    if (field.flags & FIELD_HAS_CONNECT_FLAG) {
      col_options->set("FLAG", field.connect_flag);
    }
        ...
        if (part_info->part_type == partition_type::RANGE) {
          ...
          String output_str;
            ...
            if (part_elem->max_value) {
              ...
              part_options->set("is_range_str", "true");
              int res = expr_convert_str(part_elem, part_info, create_info,
                                         &create_fields, &output_str);
              if (res == 1 || output_str.length() == 0) return true;

                val_obj->set_value_utf8(
                String_type(output_str.ptr(), output_str.length()));
              } else {
            ...
            if (part_info->is_range_str) { /* #ifdef HAVE_ZSQL_RANGE_STR*/
              if (part_elem->max_value) {
                part_obj->set_description_utf8(partition_keywords[PKW_MAXVALUE].str);
              } else {
                part_obj->set_description_utf8(
                String_type(output_str.ptr(), output_str.length()));
              }
...
static bool fill_dd_table_from_create_info(
  ...
  */
  ...
  // for connect engine
#ifdef HAVE_ZSQL_CONNECT_ENGINE
  if(create_info->used_fields & HA_CREATE_WITH_ODBC_OPTIONS)
  {
    // tabname
    if (create_info->tabname_value.length != 0)
      table_options->set("tabname",
                         make_string_type(create_info->tabname_value));
    // option_list
    if (create_info->option_list_value.length != 0)
      table_options->set("option_list",
                         make_string_type(create_info->option_list_value));
    // table_type
    if (create_info->table_type_value.length != 0)
      table_options->set("table_type",
                         make_string_type(create_info->table_type_value));
  }
#endif // HAVE_ZSQL_CONNECT_ENGINE
