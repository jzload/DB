...
class sp_head {
  ...
  */
  ...
  uint m_tmp_ip1; /// m_tmp_ip1 is num of instrs after m_tmp_ip1
  uint m_tmp_ip2; /// m_tmp_ip2 is num of instrs after sp_proc_stmt
  /* #endif */
 ...
 public:
...
#ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY
  bool check_params(THD* thd, List<Item> *args);
  bool check_maria_mode_params(List<Item> *args);
  bool check_oracle_mode_params(List<Item> *args);
  bool check_oralce_normal_params(List<Item> *args);
  bool check_oralce_notation_params(List<Item> *args);
  bool init_notation_resource(List<Item> *args);
  bool is_param_instantiated_multiply();
  bool is_in_params_without_value();
  bool check_and_set_notaion_flag(List<Item> *args);
  bool check_params_count(uint input_count, uint def_count);
  bool check_default_params_count(uint input_count);
  uint get_valid_defalut_param_count();
  bool set_params_with_default_value(THD *thd,
                                     sp_rcontext *nctx,
                                     uint start_idx);
  bool is_notation_params_contain_out_varaible();
  void reset_notation_resource(THD* thd);

  bool set_sp_params_value(THD *thd, sp_rcontext *nctx, List<Item> *args);

  bool set_sp_params_value_without_notation(THD *thd,
                                            sp_rcontext *nctx,
                                            List<Item> *args);

  bool set_sp_params_value_with_notation(THD *thd,
                                         sp_rcontext *nctx,
                                         List<Item> *args);

  bool fill_variable_info(THD *thd,
                          sp_rcontext *nctx,
                          sp_variable *spvar,
                          Item *var_value,
                          uint  var_idx);

  bool set_out_params_values(THD *thd,
                             sp_rcontext *octx,
                             sp_rcontext *nctx,
                             List<Item> *args);

  bool set_out_params_value_when_exists_notaion(THD *thd,
                                                sp_rcontext *octx,
                                                sp_rcontext *nctx,
                                                List<Item> *args);

  bool set_out_params_value_when_no_notaion(THD *thd,
                                            sp_rcontext *octx,
                                            sp_rcontext *nctx,
                                            List<Item> *args);

  bool set_position_out_params_value(THD *thd,
                                     sp_rcontext *octx,
                                     sp_rcontext *nctx,
                                     List<Item> *args,
                                     uint params_count);

  bool set_notation_out_params_value(THD *thd,
                                     sp_rcontext *octx,
                                     sp_rcontext *nctx);

  bool set_out_variable_value(THD *thd,
                              sp_rcontext *octx,
                              sp_rcontext *nctx,
                              sp_variable *spvar,
                              Item *var_value,
  ...
  int backpatch_sphead();

  int cpoy_instrs(uint start_idx, uint end_idx,
                  Mem_root_array<sp_instr *> *to_array);
  int cpoy_instrs2(Mem_root_array<sp_instr *> *from_array);

  void set_instr_offset(Mem_root_array<sp_instr *> *array,
 ...
 private:
  ...
  /* #ifdef HAVE_ZSQL_ORACLE_COMPATIBILITY */
  Mem_root_array<sp_variable *> m_not_mark_params;
  Mem_root_array<sp_variable *> m_mark_params;

  Mem_root_array<sp_instr *> m_block_instr;
  Mem_root_array<sp_instr *> m_exception_instr;

  bool m_with_notaion; // compatible with Oracle: call sp_name(x=>1);
  List<Item> m_not_mark_values;
  List<Item> m_mark_values;
  /* #endif */
