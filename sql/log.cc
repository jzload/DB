...
uint64_t 
get_total_time(THD *thd, uint64_t current_utime, uint64_t query_utime)
{
  uint64_t total_time = 0;

  if(thd->req_wait_time != 0) /*if equal 0, it is not pushback queue*/
  {
    total_time = current_utime - thd->req_wait_time;
  }
  else if(thd->sql_parse_time != 0)/*some time is 0,like administrator command*/
  {
    total_time = current_utime - thd->sql_parse_time;
  }
  else
  {
    total_time = query_utime;
  }
...
class File_query_log {
  ...
  */
  ...
  bool write_the_fourth_row(THD *thd, uint64_t current_utime);
  bool write_the_fifth_row(THD *thd);
  bool write_the_sixth_row(THD *thd);
...
bool File_query_log::write_slow(THD *thd, ulonglong current_utime,
  ...
  /* 1th row: #Time: 2019-06-04T18:37:39.490415+08:00 */
  /* 2th row: User@Host: root[root] @ localhost []  Id:     3 */  
  ...
  /* For slow query log */
  total_time  = get_total_time(thd, ulonglong2uint64_t(current_utime), ulonglong2uint64_t(query_utime));
  snprintf(query_time_buff, sizeof(query_time_buff), "%.6f", uint64_t2double(total_time)/1000000.0);
  ...
  */
  if (!query_start) {
    /* 3th row: #Query_time  Lock_time  Rows_sent Rows_examined */
    ...
    /* 4th row: #Req_wait_time Pre_dispatch_time Parse_time Execute_time */
    if (write_the_fourth_row(thd, ulonglong2uint64_t(current_utime)))
      goto err;

    /* 5th row: #Exec_prep_time  open_table_time  Mdl_req_time  Innodblock_req_time */
    if (write_the_fifth_row(thd))
      goto err;

    /* 6th row: #Order_commit_time:  Flush_time:  sync_time:  commit_time:  */
    if (write_the_sixth_row(thd))  
     goto err;
...
/* slow log 4th row: #Req_wait_time Pre_dispatch_time Parse_time Execute_time */
bool 
File_query_log::write_the_fourth_row(THD *thd, uint64_t current_utime)
{
  char time_buff_1[30], time_buff_2[30], time_buff_3[30], time_buff_4[30];
  uint64_t req_wait_time=0, pre_dispatch_time=0, parse_time=0, execute_time=0;

  /*one thread per connection or not push queue, the time maybe is zero*/
  if(thd->req_wait_time != 0)
    req_wait_time = thd->pre_dispatch_time - thd->req_wait_time;
  
  /*one thread per connection, the time maybe is zero*/
  if((thd->pre_dispatch_time != 0) && (thd->sql_dispatch_time != 0))
      pre_dispatch_time = thd->sql_dispatch_time - thd->pre_dispatch_time;

  /*some sql cmd does not execute, like 'use database'*/
  if(thd->sql_execute_time != 0)
  {
    if(thd->sql_parse_time != 0)
        parse_time = thd->sql_execute_time - thd->sql_parse_time;
    execute_time = current_utime - thd->sql_execute_time;
  }

  snprintf(time_buff_1, sizeof(time_buff_1), "%.6f", uint64_t2double(req_wait_time)/1000000.0);
  snprintf(time_buff_2, sizeof(time_buff_2), "%.6f", uint64_t2double(pre_dispatch_time)/1000000.0);
  snprintf(time_buff_3, sizeof(time_buff_3), "%.6f", uint64_t2double(parse_time)/1000000.0);
  snprintf(time_buff_4, sizeof(time_buff_4), "%.6f", uint64_t2double(execute_time)/1000000.0);

  if (my_b_printf(&log_file,
                    "# Req_wait_time: %s  Pre_dispatch_time: %s"
                    "  Parse_time: %s  Execute_time: %s\n",
                    time_buff_1, time_buff_2,
                    time_buff_3,time_buff_4) == (uint) -1)
    return true;

  return false;
}

/* slow log 5th row: #Exec_prep_time  open_table_time  Mdl_req_time  Innodblock_req_time */
bool 
File_query_log::write_the_fifth_row(THD *thd)
{
  char time_buff_1[30], time_buff_2[30], time_buff_3[30], time_buff_4[30];
  
  snprintf(time_buff_1, sizeof(time_buff_1), "%.6f", uint64_t2double(thd->exec_prep_time)/1000000.0);
  snprintf(time_buff_2, sizeof(time_buff_2), "%.6f", uint64_t2double(thd->open_table_time)/1000000.0);
  snprintf(time_buff_3, sizeof(time_buff_3), "%.6f", uint64_t2double(thd->mdl_req_time)/1000000.0);
  snprintf(time_buff_4, sizeof(time_buff_4), "%.6f", uint64_t2double(thd->innodblock_req_time)/1000000.0);
  if (my_b_printf(&log_file,
                  "# Exec_prep_time: %s  Open_table_time: %s  "
                  "Mdl_req_time: %s  Innodblock_req_time: %s\n",
                  time_buff_1, time_buff_2,
                  time_buff_3, time_buff_4) == (uint) -1)
    return true;
...
/* slow log 6th row: #Order_commit_time:  Flush_time:  sync_time:  commit_time:  */
bool 
File_query_log::write_the_sixth_row(THD *thd)
{
  char time_buff_1[30], time_buff_2[30], time_buff_3[30], time_buff_4[30],
       time_buff_5[30], time_buff_6[30];

  snprintf(time_buff_1,  sizeof(time_buff_1), "%.6f", uint64_t2double(thd->order_commit_time)/100000.0);
  snprintf(time_buff_2,  sizeof(time_buff_2), "%.6f", uint64_t2double(thd->flush_time)/1000000.0);
  snprintf(time_buff_3,  sizeof(time_buff_3), "%.6f", uint64_t2double(thd->sync_time)/1000000.0);  
  snprintf(time_buff_4,  sizeof(time_buff_4), "%.6f", uint64_t2double(thd->commit_time)/1000000.0);
  snprintf(time_buff_5,  sizeof(time_buff_5), "%.6f", uint64_t2double(thd->ack_wait_time)/1000000.0);
  snprintf(time_buff_6,  sizeof(time_buff_6), "%.6f", uint64_t2double(thd->commit_queue_wait_time)/1000000.0);

  if (my_b_printf(&log_file,
                  "# Order_commit_time: %s  Flush_time: %s  "
                  "Sync_time: %s  Commit_time: %s Ack_wait_time: %s  "
                  "Commit_queue_wait_time: %s\n",
                  time_buff_1, time_buff_2,time_buff_3, time_buff_4,
                  time_buff_5, time_buff_6) == (uint) -1)
    return true;    
...
void log_slow_statement(THD *thd,
  ...
  /*initial when its end*/
  thd->init_slow_log_time();
}
