...
bool Query_result_union::send_data(THD *thd, List<Item> &values) {
  ...
  if (m_is_minus_op) {
    return delete_row_for_minus(table, &m_rows_in_table);
  }
  ...
  if (!error) {
    ...
    if (m_tmp_table_on_disk && thd &&
        thd->variables.tmp_table_max_rows &&
        m_rows_in_table > thd->variables.tmp_table_max_rows)
    {
...
bool Query_result_union_direct::send_result_set_metadata(THD *thd, List<Item> &,
   ...
   */
  offset = unit->global_parameters()->get_offset(thd);
  limit = unit->global_parameters()->get_limit(thd);
...
bool Query_result_union_direct::send_eof(THD *thd) {
  ...
  */
  ulonglong offset = thd->lex->current_select()->get_offset(thd);
  current_found_rows +=
      thd->current_found_rows > offset ? thd->current_found_rows - offset : 0;

  if (thd->lex->current_select() == last_select_lex) {
    /*
    ...
    */
    if (!(unit->first_select()->active_options() & OPTION_FOUND_ROWS)) {
      ha_rows global_limit = unit->global_parameters()->get_limit(thd);
      ha_rows global_offset = unit->global_parameters()->get_offset(thd);
...
bool SELECT_LEX_UNIT::prepare(THD *thd, Query_result *sel_result,
  ...
  for (SELECT_LEX *sl = first_select(); sl; sl = sl->next_select()) {
    ...
    */
    ...
    /* FULL JOIN */
    /*
     * We must preserve the full join child table infos to the derived table
     */
    if (setup_full_join_fields_list(thd, sl))
        goto err;
    else
  ...
  */
  if (!simple_query_expression) {
    ...
    if (union_result->postponed_prepare(thd, types))
      goto err;
  }
  ...
  if (!simple_query_expression) {
    ...
    */
    ...
    const char *linkage_oper_lower = "union";
    const char *linkage_oper_upper = "UNION";

#ifdef HAVE_ZSQL_MINUS
    Query_Blocks_Linkage_Type qb_type =
      get_query_blocks_linkage_type(first_select());

    if (qb_type == Query_Blocks_Linkage_Type::ALL_ARE_MINUS) {
      linkage_oper_lower = "minus";
      linkage_oper_upper = "MINUS";
    } else if (qb_type == Query_Blocks_Linkage_Type::ALL_ARE_MISC) {
      linkage_oper_lower = "union/minus";
      linkage_oper_upper = "UNION/MINUS";
    }
#endif // HAVE_ZSQL_MINUS
    ...
    while ((type = tp++)) {
      if (type->result_type() == STRING_RESULT &&
        ...
        my_error(ER_CANT_AGGREGATE_NCOLLATIONS, MYF(0), linkage_oper_upper);
        goto err;
      }
    }
    ...
    result_table_list.table_name = result_table_list.alias = linkage_oper_lower;
...
void SELECT_LEX_UNIT::create_iterators(THD *thd) {
  ...
  Query_Blocks_Linkage_Type qb_type =
    get_query_blocks_linkage_type(first_select());
  if (qb_type == Query_Blocks_Linkage_Type::ALL_ARE_MINUS) {
    tmp_table->alias = "<minus temporary>";
  } else if (qb_type == Query_Blocks_Linkage_Type::ALL_ARE_MISC) {
    tmp_table->alias = "<union/minus temporary>";
  }
#endif // HAVE_ZSQL_MINUS
