...
char *int10_to_str_ff6(unsigned long val, char *dst) {
  char buffer[65];
  char *p;
  ...
  p = &buffer[sizeof(buffer) - 1];
  *p = '\0';
  new_val = (unsigned long)(uval / 10);
  *--p = '0' + static_cast<char>(uval - new_val * 10);
  val = new_val;

  while (val != 0) {
    new_val = val / 10;
    *--p = '0' + static_cast<char>(val - new_val * 10);
    val = new_val;
  }

  new_val = uval * 10;
  while ((new_val > 0) && (new_val < 999999)) {
    *--p = '0';
    new_val = new_val * 10;
  }
